...
export class Sortable extends Base<HTMLElement> implements INotifyPropertyChanged {
    ...
    private onDragStop: Function = (e: { target: HTMLElement, event: MouseEvent & TouchEvent, helper: Element }) => {
        ...
        const sortableTargetElement: HTMLElement = this.getSortableElement(e.target, dropInst) || (e.target as HTMLElement).querySelector('.e-grabbed');
        const isDroppedOnContainer: boolean = dropInst.element === e.target && !sortableTargetElement;
        if (this.curTarget === this.target && sortableTargetElement === this.curTarget) {
            curIdx = prevIdx;
        }
        if (isDroppedOnContainer || (!isPlaceHolderPresent && this.curTarget === this.target)) {
