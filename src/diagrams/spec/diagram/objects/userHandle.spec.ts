...
describe('user handle Template save and load', () => {
    ...
    it('Checking Userhanlde after serialization ', function (done) {
        ...
        expect(hanldeBeforeSerialization !== null && hanldeAfterSerialization !== null).toBe(true);
       done();
    });
});
...
describe('Check whether the user handle is visible while dragging the node', () => {
    ...
    beforeAll((): void => {
        ele = createElement('div', { id: 'diagram' });
        document.body.appendChild(ele);
        let node1: NodeModel = { id: 'node', offsetX: 100, offsetY: 100, width: 50, height: 50 };
        let node2: NodeModel = { id: 'node2', offsetX: 300, offsetY: 250, width: 400, height: 100, shape: { type: 'SwimLane', lanes: [{}] } };
        let connector: ConnectorModel= {id: 'connector1', type: 'Straight', sourcePoint: { x: 100, y: 400 }, targetPoint: { x: 200, y: 500 },};
        var handle: UserHandleModel = {
            name: 'handle1', pathData: 'M22.0542,27.332C20.4002,27.332,19.0562,25.987,19.0562,24.333C19.0562,22.678,20.4002,21.333,22.0542,21.333C23.7082,21.333,25.0562,22.678,25.0562,24.333C25.0562,25.987,23.7082,27.332,22.0542,27.332 M30.6282,22.889L28.3522,22.889C28.1912,22.183,27.9142,21.516,27.5272,20.905L29.1392,19.293C29.3062,19.126,29.3062,18.853,29.1392,18.687L27.7032,17.251C27.6232,17.173,27.5152,17.125,27.3982,17.125C27.2862,17.125,27.1782,17.173,27.0952,17.251L25.4872,18.863C24.8732,18.476,24.2082,18.201,23.5002,18.038L23.5002,15.762C23.5002,15.525,23.3092,15.333,23.0732,15.333L21.0422,15.333C20.8062,15.333,20.6122,15.525,20.6122,15.762L20.6122,18.038C19.9072,18.201,19.2412,18.476,18.6292,18.863L17.0192,17.252C16.9342,17.168,16.8242,17.128,16.7162,17.128C16.6052,17.128,16.4972,17.168,16.4112,17.252L14.9752,18.687C14.8952,18.768,14.8492,18.878,14.8492,18.99C14.8492,19.104,14.8952,19.216,14.9752,19.293L16.5872,20.905C16.2002,21.516,15.9242,22.183,15.7642,22.889L13.4852,22.889C13.2502,22.889,13.0572,23.08,13.0572,23.316L13.0572,25.35C13.0572,25.584,13.2502,25.777,13.4852,25.777L15.7612,25.777C15.9242,26.486,16.2002,27.15,16.5872,27.764L14.9752,29.374C14.8092,29.538,14.8092,29.813,14.9752,29.979L16.4112,31.416C16.4912,31.494,16.6022,31.541,16.7162,31.541C16.8272,31.541,16.9382,31.494,17.0192,31.416L18.6252,29.805C19.2412,30.191,19.9072,30.467,20.6122,30.63L20.6122,32.906C20.6122,33.141,20.8062,33.333,21.0422,33.333L23.0732,33.333C23.3092,33.333,23.5002,33.141,23.5002,32.906L23.5002,30.63C24.2082,30.467,24.8732,30.191,25.4872,29.805L27.0952,31.416C27.1812,31.499,27.2892,31.541,27.3982,31.541C27.5102,31.541,27.6202,31.499,27.7032,31.416L29.1392,29.979C29.2202,29.899,29.2662,29.791,29.2662,29.677C29.2662,29.563,29.2202,29.453,29.1392,29.374L27.5312,27.764C27.9142,27.149,28.1912,26.486,28.3522,25.777L30.6282,25.777C30.8652,25.777,31.0552,25.584,31.0552,25.35L31.0552,23.316C31.0552,23.08,30.8652,22.889,30.6282,22.889'
            , side: "Top", horizontalAlignment: "Center", verticalAlignment: 'Center', margin: { top: 0, bottom: 0, left: 0, right: 0 }, offset: 0
            , backgroundColor: 'black'
        ...
        diagram = new Diagram({
            width: 600, height: 1100, nodes: [node1, node2], connectors: [connector], selectedItems: { userHandles: [handle] }
        });
        diagram.appendTo('#diagram');
    });

    it('User Handle render or not while dragging node', (done: Function) => {
        let diagramCanvas: HTMLElement = document.getElementById(diagram.element.id + 'content');
        mouseEvents.clickEvent(diagramCanvas, 106, 111);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        mouseEvents.mouseDownEvent(diagramCanvas, 106, 111);
        mouseEvents.mouseMoveEvent(diagramCanvas, 75, 50);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 0).toBe(true);
        mouseEvents.mouseUpEvent(diagramCanvas, 75, 50);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        done();
    });

    it('User Handle render or not while dragging connector', (done: Function) => {
        let diagramCanvas: HTMLElement = document.getElementById(diagram.element.id + 'content');
        mouseEvents.clickEvent(diagramCanvas, 148, 449);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        mouseEvents.mouseDownEvent(diagramCanvas, 148, 449);
        mouseEvents.mouseMoveEvent(diagramCanvas, 248, 450);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 0).toBe(true);
        mouseEvents.mouseUpEvent(diagramCanvas, 248, 450);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        done();
    });
});
...
describe('Check whether the user handle is visible while dragging the group node', () => {
    ...
    beforeAll((): void => {
        ele = createElement('div', { id: 'diagram' });
        document.body.appendChild(ele);
        let node3: NodeModel = { id: 'node3', offsetX: 100, offsetY: 100, width: 50, height: 50 };
        let node4: NodeModel = { id: 'node4', offsetX: 160, offsetY: 160, width: 50, height: 50 };
        let node5: NodeModel = { id: 'group', children: ['node3', 'node4'] };
        var handle: UserHandleModel = {
            name: 'handle1', pathData: 'M22.0542,27.332C20.4002,27.332,19.0562,25.987,19.0562,24.333C19.0562,22.678,20.4002,21.333,22.0542,21.333C23.7082,21.333,25.0562,22.678,25.0562,24.333C25.0562,25.987,23.7082,27.332,22.0542,27.332 M30.6282,22.889L28.3522,22.889C28.1912,22.183,27.9142,21.516,27.5272,20.905L29.1392,19.293C29.3062,19.126,29.3062,18.853,29.1392,18.687L27.7032,17.251C27.6232,17.173,27.5152,17.125,27.3982,17.125C27.2862,17.125,27.1782,17.173,27.0952,17.251L25.4872,18.863C24.8732,18.476,24.2082,18.201,23.5002,18.038L23.5002,15.762C23.5002,15.525,23.3092,15.333,23.0732,15.333L21.0422,15.333C20.8062,15.333,20.6122,15.525,20.6122,15.762L20.6122,18.038C19.9072,18.201,19.2412,18.476,18.6292,18.863L17.0192,17.252C16.9342,17.168,16.8242,17.128,16.7162,17.128C16.6052,17.128,16.4972,17.168,16.4112,17.252L14.9752,18.687C14.8952,18.768,14.8492,18.878,14.8492,18.99C14.8492,19.104,14.8952,19.216,14.9752,19.293L16.5872,20.905C16.2002,21.516,15.9242,22.183,15.7642,22.889L13.4852,22.889C13.2502,22.889,13.0572,23.08,13.0572,23.316L13.0572,25.35C13.0572,25.584,13.2502,25.777,13.4852,25.777L15.7612,25.777C15.9242,26.486,16.2002,27.15,16.5872,27.764L14.9752,29.374C14.8092,29.538,14.8092,29.813,14.9752,29.979L16.4112,31.416C16.4912,31.494,16.6022,31.541,16.7162,31.541C16.8272,31.541,16.9382,31.494,17.0192,31.416L18.6252,29.805C19.2412,30.191,19.9072,30.467,20.6122,30.63L20.6122,32.906C20.6122,33.141,20.8062,33.333,21.0422,33.333L23.0732,33.333C23.3092,33.333,23.5002,33.141,23.5002,32.906L23.5002,30.63C24.2082,30.467,24.8732,30.191,25.4872,29.805L27.0952,31.416C27.1812,31.499,27.2892,31.541,27.3982,31.541C27.5102,31.541,27.6202,31.499,27.7032,31.416L29.1392,29.979C29.2202,29.899,29.2662,29.791,29.2662,29.677C29.2662,29.563,29.2202,29.453,29.1392,29.374L27.5312,27.764C27.9142,27.149,28.1912,26.486,28.3522,25.777L30.6282,25.777C30.8652,25.777,31.0552,25.584,31.0552,25.35L31.0552,23.316C31.0552,23.08,30.8652,22.889,30.6282,22.889'
            , side: "Top", horizontalAlignment: "Center", verticalAlignment: 'Center', margin: { top: 0, bottom: 0, left: 0, right: 0 }, offset: 0
            , backgroundColor: 'black'
        ...
        diagram = new Diagram({
            width: 600, height: 1100, nodes: [node3, node4, node5], selectedItems: { userHandles: [handle] }
        });
        diagram.appendTo('#diagram');
    });

    it('User Handle render or not while dragging group node', (done: Function) => {
        let diagramCanvas: HTMLElement = document.getElementById(diagram.element.id + 'content');
        mouseEvents.clickEvent(diagramCanvas, 110, 110);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        mouseEvents.mouseDownEvent(diagramCanvas, 110, 110);
        mouseEvents.mouseMoveEvent(diagramCanvas, 210, 120);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 0).toBe(true);
        mouseEvents.mouseUpEvent(diagramCanvas, 210, 120);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        done();
    });
});
...
describe('Check whether the user handle is visible while dragging the shapes', () => {
    ...
    beforeAll((): void => {
        ele = createElement('div', { id: 'diagram' });
        document.body.appendChild(ele);
        let node7: NodeModel = { id: 'node7', offsetX: 100, offsetY: 100, width: 50, height: 50,shape: { type: 'Basic', shape: 'Ellipse' } };
        let node8: NodeModel = {
            id: 'node8', width: 50, height: 50, offsetX: 100, offsetY: 200,
            style: { fill: 'red', strokeColor: 'blue', strokeWidth: 5, },
            shape: { type: 'Bpmn', shape: 'DataSource' } 
        };
        let node9: NodeModel = {
            id: 'node9', width: 50, height: 50, offsetX: 100, offsetY: 300,
        ...
        diagram = new Diagram({
            width: 600, height: 1100, nodes: [node7, node8, node9], selectedItems: { userHandles: [handle] }
        });
        diagram.appendTo('#diagram');
    });

    it('User Handle render or not while dragging Basic shapes', (done: Function) => {
        let diagramCanvas: HTMLElement = document.getElementById(diagram.element.id + 'content');
        mouseEvents.clickEvent(diagramCanvas, 107, 106);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        mouseEvents.mouseDownEvent(diagramCanvas, 107, 106);
        mouseEvents.mouseMoveEvent(diagramCanvas, 250, 106);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 0).toBe(true);
        mouseEvents.mouseUpEvent(diagramCanvas, 250, 106);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        done();
    });
    it('User Handle render or not while dragging BPMN shapes', (done: Function) => {
        let diagramCanvas: HTMLElement = document.getElementById(diagram.element.id + 'content');
        mouseEvents.clickEvent(diagramCanvas, 106, 212);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        mouseEvents.mouseDownEvent(diagramCanvas, 106, 212);
        mouseEvents.mouseMoveEvent(diagramCanvas, 250, 212);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 0).toBe(true);
        mouseEvents.mouseUpEvent(diagramCanvas, 250, 212);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        done();
    });
    it('User Handle render or not while dragging Flow shapes', (done: Function) => {
        let diagramCanvas: HTMLElement = document.getElementById(diagram.element.id + 'content');
        mouseEvents.clickEvent(diagramCanvas, 109, 310);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 1).toBe(true);
        mouseEvents.mouseDownEvent(diagramCanvas, 109, 310);
        mouseEvents.mouseMoveEvent(diagramCanvas, 250, 310);
        expect(document.getElementsByClassName('e-diagram-userhandle-path').length === 0).toBe(true);
