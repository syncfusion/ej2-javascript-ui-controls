...
export class XmlWriter {
    ...
    // Controls whether to automatically emit the XML declaration
    // private addXmlDeclaration: boolean;
    //namespace fields
     ...
     *
     * @returns {string} The current XML content accumulated in memory.
     */
    ...
    constructor() {
        ...
        // this.addXmlDeclaration = !!(options && options.addXmlDeclaration);
        this.namespaceStack = [];
    ...
    public writeStartElement(prefix: string, localName: string, namespace: string): void {
        ...
        // if (this.currentState === 'Initial' && this.addXmlDeclaration) {
    ...
    private writeStartAttributeInternal(prefix: string, localName: string, _namespaceName: string): void {
        this.bufferText += ' ';
        if (prefix !== undefined && prefix !== null && prefix.length > 0) {
            this.rawText(prefix);
            this.bufferText += ':';
