...
export class SheetRender implements IRenderer {
    ...
    private updateLeftColGroup(width?: number, rowHdr?: HTMLElement): void {
        ...
        const frag: DocumentFragment = document.createDocumentFragment(); // Using a fragment for batch updates
        let colGrp: Element;
        if (width) {
    ...
    public setPanelWidth(sheet: SheetModel, rowHdr: HTMLElement, isRtlChange?: boolean): void {
        ...
        const width: number = this.getRowHeaderWidth(sheet);
        const isRtl: boolean = this.parent.enableRtl;
        const offset: string = isRtl ? 'right' : 'left';
        let rtlOffset: string;
        const colHeaderPanel: HTMLElement = this.getColHeaderPanel();
        const contentPanel: HTMLElement = this.getContentPanel();
        if (isRtlChange) {
        ...
        const widthCalc: string = `calc(100% - ${width}px)`;
    ...
    private setPanelHeight(sheet: SheetModel): void {
        const scrollSize: number = this.getScrollSize(true);
        const contentPanel: HTMLElement = this.contentPanel;
        const headerPanel: HTMLElement = this.headerPanel;
        if (sheet.frozenRows) {
    ...
    public renderPanel(): void {
        ...
        const rowHeader: HTMLElement = this.parent.createElement('div', { className: 'e-row-header', id: `${id}_row_header` });
        ...
        const content: HTMLElement = this.parent.createElement('div', { className: 'e-sheet-content', id: `${id}_main_content` });
    ...
    private initHeaderPanel(): void {
        ...
        const selectAllContainer: HTMLElement = this.parent.createElement('div', { className: 'e-selectall-container', id: `${id}_selectall` });
        frag.appendChild(selectAllContainer);
        const columnHeader: HTMLElement = this.parent.createElement('div', { className: 'e-column-header', id: `${id}_col_header` });
    ...
    private updateTable(tagName: string[], name: string, appendTo: Element): void {
        const frag: DocumentFragment = document.createDocumentFragment();
        const table: HTMLElement = this.parent.createElement('table', {
            className: `e-table e-${name}-table`, attrs: { 'role': 'grid' }
        });
        tagName.forEach((tag: string): void => { frag.appendChild(this.parent.createElement(tag)); });
    ...
    private clearCFResult(sheet: SheetModel): void {
        if (sheet.conditionalFormats && sheet.conditionalFormats.length) {
            sheet.conditionalFormats.forEach((cfRule: ConditionalFormat) => {
    ...
    public refreshColumnContent(args: SheetRenderArgs): void {
        ...
        let tHead: Element = this.parent.element.querySelector('.e-column-header thead');
        ...
        (args.cells as Map<string, CellModel>).forEach((value: CellModel, key: string): void => {
            const indexes: number[] = getRangeIndexes(key);
            if (indexes[0] === args.indexes[0]) {
                const col: Element = this.updateCol(sheet, indexes[1], hColGrp); colGrp.appendChild(col.cloneNode());
                this.cellRenderer.renderColHeader(indexes[1], hRow);
            }
            if (indexes[1] - frozenCol === args.indexes[1]) {
                row = (indexes[0] < frozenRow) ? hTBody.children[count as number] : tBody.children[count as number];
                if (row) {
        ...
        const appendColumns: Function = (): void => {
            let table: Element = this.getColHeaderTable(); removeAllChildren(table);
    ...
    public refreshRowContent(args: SheetRenderArgs): void {
        ...
        const hTBody: Element = tBody.cloneNode() as Element;
        ...
        const rowHeaderTbody: Element = this.contentPanel.querySelector('.e-row-header tbody');
        if (rowHeaderTbody) {
            detach(rowHeaderTbody);
            this.getRowHeaderTable().appendChild(hFrag);
        }
        const sheetContentTbody: Element = this.contentPanel.querySelector('.e-sheet-content tbody');
    ...
    public updateColContent(args: SheetRenderArgs): void {
        getUpdateUsingRaf((): void => {
            ...
            const colRefChild: Element = colGrp.firstElementChild;
            ...
            (args.cells as Map<string, CellModel>).forEach((value: CellModel, key: string): void => {
                ...
                if (args.direction === 'first' && indexes[1] === args.indexes[1]) {
                    this.checkColMerge(
                        ...
                        getCell(indexes[0], this.parent.viewport.leftIndex + frozenCol, sheet) || {}, sheet);
                }
                ...
                if (args.direction === 'last') {
                    ...
                    cell = this.cellRenderer.render(cellArgs);
                    this.checkColMerge(
                        indexes, args.indexes, cell, value, sheet, ((indexes[0] < frozenRow ? hTBody : tBody).rows[rowCount - 1] ||
                        { cells: [] }).cells[1]);
                } else {
                    cell = this.cellRenderer.render(cellArgs);
                }
        ...
        (args.cells as Map<string, CellModel>).forEach((value: CellModel, cKey: string): void => {
            ...
            if (args.direction ===  'first' && indexes[0] === args.indexes[0]) {
                ...
                this.checkRowMerge(
                    ...
                    getCell(this.parent.viewport.topIndex + frozenRow, indexes[1], sheet) || {}, sheet);
            }
            ...
            if (args.direction === 'last' && tBody.rows.length) {
                this.checkRowMerge(
                    ...
                    (indexes[1] < frozenCol ? rTBody : tBody).rows[0].cells[indexes[1] < frozenCol ? count + 1 : count]);
            }
            count++;
    ...
    private checkRowMerge(indexes: number[], range: number[], cell: Element, model: CellModel,
                          sheet: SheetModel, firstcell?: Element): void {
    ...
    private checkColMerge(indexes: number[], range: number[], cell: Element, model: CellModel,
                          sheet: SheetModel, firstcell?: Element): void {
