...
export class Overlay {
     ...
     * @hidden
     */
    public insertOverlayElement(id: string, range: string, sheetIndex: number):
    { element: HTMLElement, top: number, dprTop: number, left: number } {
        ...
        const pos: { top: number, dprTop: number, left: number } = getCellPosition(
    ...
    private overlayMouseUpHandler(e: MouseEvent & TouchEvent, isMouseUp?: boolean): void {
        ...
        if (this.isOverlayClicked || isMouseUp) {
            ...
            if (isFreezeAvailable) {
                ...
                prevRowIdx.isImage = false; prevRowIdx.isCheckDPR = false; prevColIdx.isImage = false;
                ...
                currRowIdx = { clientY: eventArgs.currentTop, isImage: true, isCheckDPR: prevRowIdx.isCheckDPR };
