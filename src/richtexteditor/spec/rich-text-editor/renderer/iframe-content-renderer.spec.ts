...
describe('Iframe Content renderer module', () => {
    ...
    describe('920102 - Cursor remains in Font name toolbar items after changing font name in selected list items - Iframe sample.', () => {
        let editor: RichTextEditor;
        beforeAll(() => {
            editor = renderRTE({
                toolbarSettings: {
        ...
        it('Should have the proper focus and then active element after the font size action.', (done: DoneFn) => {
            const range: Range = new Range();
            range.setStart(editor.inputElement.firstElementChild.firstChild, 0);
            range.setEnd(editor.inputElement.firstElementChild.firstChild, editor.inputElement.firstElementChild.firstChild.textContent.length);
            ...
            const toolbarItem: HTMLElement = editor.element.querySelector('.e-toolbar-item');
            toolbarItem.querySelector('button').click();
            setTimeout(() => {
                const fontSizeItem: HTMLElement = document.body.querySelector('[aria-label="12 pt"]');
                fontSizeItem.click();
                setTimeout(() => {
                    const expectedElem: HTMLElement = editor.inputElement;
                    expect(editor.inputElement.ownerDocument.activeElement  === expectedElem);
                    done();
                }, 100);
            }, 100);
        });
