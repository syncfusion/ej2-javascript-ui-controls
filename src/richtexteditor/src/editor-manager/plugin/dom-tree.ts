        ...
        if (blockNodeWalker.currentNode && blockCollection.indexOf(blockNodeWalker.currentNode as HTMLElement) === -1) {
            if (currentNode.nodeName === 'LI') {
                ...
                if (isDirectChild) {
                    ...
                    const parentLI: HTMLLIElement = currentNode.parentElement.closest('li');
                    const isNestedLI: boolean = currentNode.nodeName === 'LI' && !!parentLI;
                    const ancestorElement: Node = commonAncestor.nodeType === 3 ? commonAncestor.parentElement : commonAncestor;
                    const isListAncestor: boolean = !!(ancestorElement as HTMLElement).closest('ul,ol');
