    ...
    private imageConversion(elm: HTMLElement, rtfData: string): void {
        ...
        if (imgElem.length > 0) {
            for (let i: number = 0; i < imgElem.length; i++) {
                ...
                if (!isNOU(imgSrc) && imgSrc[i as number] !== '') {
                    const imgType: string =  imgSrc[i as number].split('.').pop().toLowerCase();
                    imgElem[i as number].setAttribute('data-image-type', imgType);
                }
            }
