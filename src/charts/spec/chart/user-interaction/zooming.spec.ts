...
describe('Checking Accessibility in Zooming Toolkit', () => {
    let elem: HTMLElement = createElement('div', { id: 'container' });
    beforeAll(() => {
        document.body.appendChild(elem);
        chartObj = new Chart(
            {
                ...
                title: 'Chart TS Title',
                width: '800',
                zoomSettings: { enablePinchZooming: true, enableSelectionZooming: true, enableMouseWheelZooming: true }
            });
        chartObj.appendTo('#container');
    });
    afterAll((): void => {
    ...
    it('Apply accessibility to Zoom button', (done: Function) => {
        loaded = (args: ILoadedEventArgs): void => {
            trigger.draganddropEvent(elem, 100, 100, 400, 400);
            let element: HTMLElement = document.getElementById('container_Zooming_Zoom');
        ...
        chartObj.zoomSettings.toolbarItems = ['Zoom'];
        chartObj.loaded = loaded;
        chartObj.zoomSettings.accessibility.accessibilityRole = 'button';
        chartObj.zoomSettings.accessibility.focusable = false;
        chartObj.zoomSettings.accessibility.accessibilityDescription = 'Zoom button';
        chartObj.refresh();
    });

    it('Apply accessibility to ZoomIn button', (done: Function) => {
        loaded = (args: ILoadedEventArgs): void => {
            trigger.draganddropEvent(elem, 100, 100, 400, 400);
            let element: HTMLElement = document.getElementById('container_Zooming_ZoomIn');
            expect(element !== null).toBe(true);
            expect(element!.getAttribute('role')).toBe('button');
            expect(element!.getAttribute('tabindex')).toBe('-1');
            done();
        };
        chartObj.zoomSettings.toolbarItems = ['ZoomIn'];
        chartObj.loaded = loaded;
        chartObj.zoomSettings.accessibility.accessibilityRole = 'button';
        chartObj.zoomSettings.accessibility.focusable = false;
        chartObj.zoomSettings.accessibility.accessibilityDescription = 'Zoom In button';
        chartObj.refresh();
    });

    it('Apply accessibility to ZoomOut button', (done: Function) => {
        loaded = (args: ILoadedEventArgs): void => {
            trigger.draganddropEvent(elem, 100, 100, 400, 400);
            let element: HTMLElement = document.getElementById('container_Zooming_ZoomOut');
            expect(element !== null).toBe(true);
            expect(element!.getAttribute('role')).toBe('button');
            expect(element!.getAttribute('tabindex')).toBe('-1');
            done();
        };
        chartObj.zoomSettings.toolbarItems = ['ZoomOut'];
        chartObj.loaded = loaded;
        chartObj.zoomSettings.accessibility.accessibilityRole = 'button';
        chartObj.zoomSettings.accessibility.accessibilityDescription = 'Zoom Out button';
        chartObj.zoomSettings.accessibility.focusable = false;
        chartObj.refresh();
    });

    it('Apply accessibility to Pan button', (done: Function) => {
        loaded = (args: ILoadedEventArgs): void => {
            trigger.draganddropEvent(elem, 100, 100, 400, 400);
            let element: HTMLElement = document.getElementById('container_Zooming_Pan');
            expect(element !== null).toBe(true);
            expect(element!.getAttribute('role')).toBe('button');
            expect(element!.getAttribute('tabindex')).toBe('-1');
            done();
        };
        chartObj.zoomSettings.toolbarItems = ['Pan'];
        chartObj.loaded = loaded;
        chartObj.zoomSettings.accessibility.accessibilityRole = 'button';
        chartObj.zoomSettings.accessibility.accessibilityDescription = 'Zoom Pan button';
        chartObj.zoomSettings.accessibility.focusable = false;
        chartObj.refresh();
    });

    it('Apply accessibility to Reset button', (done: Function) => {
        loaded = (args: ILoadedEventArgs): void => {
            trigger.draganddropEvent(elem, 100, 100, 400, 400);
            let element: HTMLElement = document.getElementById('container_Zooming_Reset');
            expect(element !== null).toBe(true);
            expect(element!.getAttribute('role')).toBe('button');
            expect(element!.getAttribute('tabindex')).toBe('-1');
            done();
        ...
        chartObj.loaded = loaded;
        chartObj.zoomSettings.accessibility.accessibilityRole = 'button';
        chartObj.zoomSettings.accessibility.accessibilityDescription = 'Zoom Reset button';
        chartObj.zoomSettings.accessibility.focusable = false;
        chartObj.refresh();
    });
});
...
describe('Checking Zooming Toolkit with fluent2 high contrast theme', () => {
    ...
    it('Checking tooltip div hovering pan icon in tailwind3', function (done: Function) {
        loaded = (args: Object): void => {
            chartObj.loaded = null;
            trigger.draganddropEvent(elem, 100, 100, 400, 400);
            let targetElement1 = document.getElementById('chartContainer_Zooming_KitCollection');
            trigger.mousemoveEvent(targetElement1, 0, 0, 5, 5);
            let pan = document.getElementById('chartContainer_Zooming_Pan');
            trigger.mousedownEvent(pan, 0,0,5,5);
            trigger.mouseupEvent(pan, 0,0,5,5);
            let targetElement2 = document.getElementById('chartContainer_Zooming_ZoomIn');
            trigger.mouseoverEvent(targetElement2);
            expect(document.getElementById('EJ2_Chart_ZoomTip') != null);
            trigger.mouseoutEvent(targetElement2);
    ...
    it('Checking Zoomkit collection position', function (done: Function) {
        loaded = (args: Object): void => {
            chartObj.loaded = null;
            let Element1: HTMLElement = document.getElementById('chartContainer_Zooming_KitCollection');
            expect(Element1 !== null).toBe(true);
            expect(Element1.getAttribute('transform') === 'translate(630,84.25)' || Element1.getAttribute('transform') === 'translate(630,81.25)').toBe(true); 
            done();
        };
        chartObj.loaded = loaded;
        chartObj.zoomSettings.enableSelectionZooming = true;
        chartObj.zoomSettings.toolbarPosition.x = 30;
        chartObj.zoomSettings.toolbarPosition.y = 30;
        chartObj.refresh();
    });
    it('Checking Zoomkit collection position as top, right', function (done: Function) {
        chartObj.zoomSettings.toolbarPosition.x = 0;
        chartObj.zoomSettings.toolbarPosition.y = 0;
        chartObj.refresh();
        loaded = (args: Object): void => {
            chartObj.loaded = null;
            let Element1: HTMLElement = document.getElementById('chartContainer_Zooming_KitCollection');
            expect(Element1 !== null).toBe(true);
            expect(Element1.getAttribute('transform') === 'translate(600,54.25)' || Element1.getAttribute('transform') === 'translate(600,51.25)').toBe(true);
            done();
        };
        chartObj.loaded = loaded;
        chartObj.zoomSettings.enableSelectionZooming = true;
        chartObj.zoomSettings.toolbarPosition.horizontalAlignment = 'Far';
        chartObj.zoomSettings.toolbarPosition.verticalAlignment = 'Top';        
        chartObj.refresh();
    });
    it('Checking Zoomkit collection position as bottom, left', function (done: Function) {
        loaded = (args: Object): void => {
            chartObj.loaded = null;
            let Element1: HTMLElement = document.getElementById('chartContainer_Zooming_KitCollection');
            expect(Element1 !== null).toBe(true);
        ...
        chartObj.loaded = loaded;
        chartObj.zoomSettings.enableSelectionZooming = true;
        chartObj.zoomSettings.toolbarPosition.horizontalAlignment = 'Near';
        chartObj.zoomSettings.toolbarPosition.verticalAlignment = 'Bottom';
        chartObj.refresh();
    });
    it('Checking Zoomkit collection position as center', function (done: Function) {
        loaded = (args: Object): void => {
            chartObj.loaded = null;
            let Element1: HTMLElement = document.getElementById('chartContainer_Zooming_KitCollection');
            expect(Element1 !== null).toBe(true);
        ...
        chartObj.loaded = loaded;
        chartObj.zoomSettings.enableSelectionZooming = true;
        chartObj.zoomSettings.toolbarPosition.horizontalAlignment = 'Center';
        chartObj.zoomSettings.toolbarPosition.verticalAlignment = 'Middle';
        chartObj.refresh();
    });

    it('Checking toolkit dragAndDrop', function (done) {
        loaded = function (args) {
        const targetElement = document.getElementById('chartContainer_Zooming_Rect');
        const mousedownEvent = new MouseEvent('mousedown', { bubbles: true, clientX: 0, clientY: 0 });
        targetElement.dispatchEvent(mousedownEvent);
        const mousemoveEvent = new MouseEvent('mousemove', { bubbles: true, clientX: 200, clientY: 200 });
        targetElement.dispatchEvent(mousemoveEvent);
        const mouseupEvent = new MouseEvent('mouseup', { bubbles: true, clientX: 200, clientY: 200 });
        targetElement.dispatchEvent(mouseupEvent);
        expect(targetElement !== null).toBe(true);
        expect(document.getElementById('chartContainer_Zooming_KitCollection').getAttribute('transform') === 'translate(102,173)').toBe(true);
        done();
    };
    chartObj.zoomSettings.toolbarPosition.draggable = true;
    chartObj.loaded = loaded;
    chartObj.refresh();
    });
