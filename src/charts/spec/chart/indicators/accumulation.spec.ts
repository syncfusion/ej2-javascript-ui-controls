    ...
    describe('AD Technical Indicators', () => {
        ...
        it('AD Technical indicator for a series with secondary y axis', (done: Function) => {
            loaded = (args: Object): void => {
                ...
                expect(svg.getAttribute('d').split(' ')[1] == '766.5' || svg.getAttribute('d').split(' ')[1] == '761.5' || svg.getAttribute('d').split(' ')[1] == '763.5').toBe(true);
                expect(svg.getAttribute('d').split(' ')[2] == '42.25' || svg.getAttribute('d').split(' ')[2] == '45.25').toBe(true);
                expect(svg.getAttribute('d').split(' ')[4] == '766.5' || svg.getAttribute('d').split(' ')[4] == '761.5' || svg.getAttribute('d').split(' ')[4] == '763.5').toBe(true);
        ...
        it('AD Technical indicator with opposed position', (done: Function) => {
            loaded = (args: Object): void => {
                ...
                expect(svg.getAttribute('d').split(' ')[4] == '766.5' || svg.getAttribute('d').split(' ')[4] == '761.5'
                    || svg.getAttribute('d').split(' ')[4] == '763.5').toBe(true);
