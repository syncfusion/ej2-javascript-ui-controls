...
describe("EJ2-33526 XSS attack prevent with enableHtmlSanitizer as true", () => {
    ...
    describe("Toast Null or undefined value testing", () => { 
        let toastObj: Toast;
        let ele: HTMLElement = document.createElement("div");
        ele.id = "toast";
        beforeEach((): void => {
        ...
        it('buttons', () => {
            toastObj = new Toast( { buttons: null },ele);
            expect(JSON.stringify(toastObj.buttons)).toBe('[]');
            toastObj.destroy();
            toastObj = new Toast( { buttons: undefined },ele);
            expect(JSON.stringify(toastObj.buttons)).toBe('[]');
            toastObj.destroy();
        });
        it('content', () => {
            toastObj = new Toast( { content: null },ele);
            expect(toastObj.content).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { content: undefined },ele);
            expect(toastObj.content).toBe(null);
            toastObj.destroy();
        });
        it('cssClass', () => {
            toastObj = new Toast( { cssClass: null },ele);
            expect(toastObj.cssClass).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { cssClass: undefined },ele);
            expect(toastObj.cssClass).toBe(null);
            toastObj.destroy();
        });
        it('enableHtmlSanitizer', () => {
            toastObj = new Toast( { enableHtmlSanitizer: null },ele);
            expect(toastObj.enableHtmlSanitizer).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { enableHtmlSanitizer: undefined },ele);
            expect(toastObj.enableHtmlSanitizer).toBe(true);
            toastObj.destroy();
        });
        it('enablePersistence', () => {
            toastObj = new Toast( { enablePersistence: null },ele);
            expect(toastObj.enablePersistence).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { enablePersistence: undefined },ele);
            expect(toastObj.enablePersistence).toBe(false);
            toastObj.destroy();
        });
        it('enableRtl', () => {
            toastObj = new Toast( { enableRtl: false },ele);
            expect(toastObj.enableRtl).toBe(false);
            toastObj.destroy();
            toastObj = new Toast( { enableRtl: undefined },ele);
            expect(toastObj.enableRtl).toBe(false);
            toastObj.destroy();
        });
        it('extendedTimeout', () => {
            toastObj = new Toast( { extendedTimeout: null },ele);
            expect(toastObj.extendedTimeout).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { extendedTimeout: undefined },ele);
            expect(toastObj.extendedTimeout).toBe(1000);
            toastObj.destroy();
        });
        it('height', () => {
            toastObj = new Toast( { height: null },ele);
            expect(toastObj.height).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { height: undefined },ele);
            expect(toastObj.height).toBe('auto');
            toastObj.destroy();
        });
        it('icon', () => {
            toastObj = new Toast( { icon: null },ele);
            expect(toastObj.icon).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { icon: undefined },ele);
            expect(toastObj.icon).toBe(null);
            toastObj.destroy();
        });
        it('newestOnTop', () => {
            toastObj = new Toast( { newestOnTop: null },ele);
            expect(toastObj.newestOnTop).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { newestOnTop: undefined },ele);
            expect(toastObj.newestOnTop).toBe(true);
            toastObj.destroy();
        });
        it('progressDirection', () => {
            toastObj = new Toast( { progressDirection: null },ele);
            expect(toastObj.progressDirection).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { progressDirection: undefined },ele);
            expect(toastObj.progressDirection).toBe('Rtl');
            toastObj.destroy();
        }); 
        it('showCloseButton', () => {
            toastObj = new Toast( { showCloseButton: null },ele);
            expect(toastObj.showCloseButton).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { showCloseButton: undefined },ele);
            expect(toastObj.showCloseButton).toBe(false);
            toastObj.destroy();
        });
        it('showProgressBar', () => {
            toastObj = new Toast( { showProgressBar: null },ele);
            expect(toastObj.showProgressBar).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { showProgressBar: undefined },ele);
            expect(toastObj.showProgressBar).toBe(false);
            toastObj.destroy();
        }); 
        it('target', () => {
            toastObj = new Toast( { target: null },ele);
            expect(JSON.stringify(toastObj.target)).toBe('{}');
            toastObj.destroy();
            toastObj = new Toast( { target: undefined },ele);
            expect(JSON.stringify(toastObj.target)).toBe('{}');
            toastObj.destroy();
        }); 
        it('template', () => {
            toastObj = new Toast( { template: null },ele);
            expect(toastObj.template).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { template: undefined },ele);
            expect(toastObj.template).toBe(null);
            toastObj.destroy();
        });
        it('timeOut', () => {
            toastObj = new Toast( { timeOut: null },ele);
            expect(toastObj.timeOut).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { timeOut: undefined },ele);
            expect(toastObj.timeOut).toBe(5000);
            toastObj.destroy();
        });
        it('title', () => {
            toastObj = new Toast( { title: null },ele);
            expect(toastObj.title).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { title: undefined },ele);
            expect(toastObj.title).toBe(null);
            toastObj.destroy();
        });
        it('width', () => {
            toastObj = new Toast( { width: null },ele);
            expect(toastObj.width).toBe(null);
            toastObj.destroy();
            toastObj = new Toast( { width: undefined },ele);
            expect(toastObj.width).toBe('300px');
            toastObj.destroy();
        });   
    });
