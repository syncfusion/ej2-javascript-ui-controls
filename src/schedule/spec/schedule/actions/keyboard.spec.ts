...
describe('Keyboard interaction', () => {
    ...
    describe('Appointment Navigation through tab key in timeline vertical year view', () => {
        ...
        it('Tab key', function () {
            schObj.element.focus();
            const workCells: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-work-cells')) as HTMLElement[];
            const workCell = workCells[0];
            workCell.click();
            keyModule.keyActionHandler({ action: 'escape' });
            const eventCell: HTMLElement = schObj.element.querySelector('.e-event-table') as HTMLElement;
            ...
            const target: HTMLElement = schObj.element.querySelector('.e-appointment-border');
    ...
    describe('Copy and paste feature', () => {
        let schObj: Schedule;
        let keyModule: any;
        beforeAll((done: DoneFn) => {
            const elem: HTMLElement = createElement('div', { id: 'Schedule', attrs: { tabIndex: '1' } });
        ...
        it('single event copy', (done: DoneFn) => {
        ...
        it('Multiple events copy', (done: DoneFn) => {
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            const newAppointmentsArray: HTMLElement[] = [appointments[2], appointments[3]];
            schObj.copy(newAppointmentsArray);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
            const parsedContent: any = JSON.parse(clipboardContent);
            expect(parsedContent[0].Id).toBe(2);
            expect(parsedContent[0].Subject).toBe('Meeting - 1');
            expect(parsedContent[0].StartTime).toContain('2017-10-30');
            expect(parsedContent[0].EndTime).toContain('2017-10-30');
            expect(parsedContent[0].IsAllDay).toBe(false);
            expect(parsedContent[1].Id).toBe(3);
            expect(parsedContent[1].Subject).toBe('Meeting - 2');
            expect(parsedContent[1].StartTime).toContain('2017-10-30');
            expect(parsedContent[1].EndTime).toContain('2017-10-30');
            expect(parsedContent[1].IsAllDay).toBe(false);
            const workCell: HTMLElement = schObj.element.querySelector('.e-work-cells') as HTMLElement;
            workCell.click();
            schObj.paste(workCell);
            clipboardElement.value = '';
        ...
        it('Month view event copy', (done: DoneFn) => {
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            const newAppointmentsArray: HTMLElement[] = [appointments[0], appointments[1]];
            schObj.copy(newAppointmentsArray);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
            const parsedContent: any = JSON.parse(clipboardContent);
            ...
            expect(parsedContent[0].EndTime).toContain('2017-10-29');
            expect(parsedContent[0].IsAllDay).toBe(false);
            expect(parsedContent[1].Id).toBe(2);
            expect(parsedContent[1].Subject).toBe('Meeting - 1');
            expect(parsedContent[1].StartTime).toContain('2017-10-30');
            expect(parsedContent[1].EndTime).toContain('2017-10-30');
            expect(parsedContent[1].IsAllDay).toBe(false);
            const workCell: HTMLElement = schObj.element.querySelector('.e-work-cells') as HTMLElement;
        ...
        it('TimelineDay view event copy', (done: DoneFn) => {
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            const newAppointmentsArray: HTMLElement[] = [appointments[0]];
            schObj.copy(newAppointmentsArray);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
            const parsedContent: any = JSON.parse(clipboardContent);
            expect(parsedContent[0].Id).toBe(9);
            expect(parsedContent[0].Subject).toBe('Vacation');
            expect(parsedContent[0].StartTime).toContain('2017-11-02');
            expect(parsedContent[0].EndTime).toContain('2017-11-02');
            expect(parsedContent[0].IsAllDay).toBe(false);
            const workCell: HTMLElement = schObj.element.querySelector('.e-work-cells') as HTMLElement;
            workCell.click();
            schObj.paste(workCell);
        ...
        it('TimelineWeek view event copy', (done: DoneFn) => {
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            const newAppointmentsArray: HTMLElement[] = [appointments[0]];
            schObj.copy(newAppointmentsArray);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
        ...
        it('tab focus and copy', (done: DoneFn) => {
            schObj.element.focus();
            const workCells: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-work-cells'));
            const workCell: HTMLElement = workCells[0];
            workCell.click();
            keyModule.keyActionHandler({ action: 'escape' });
            const eventCell: HTMLElement = schObj.element.querySelector('.e-schedule-table');
            keyModule.keyActionHandler({
                action: 'tab', target: eventCell, shiftKey: false, preventDefault: function () { }
            });
            expect(schObj.element.querySelectorAll('.e-appointment-border').length).toBe(1);
            ...
            expect(parsedContent[0].IsAllDay).toBe(false);
            workCell.click();
            keyModule.processPaste(parsedContent);
            clipboardElement.value = '';
            schObj.currentView = 'Week';
            done();
        });
        it('all day event copy paste to normal cell', (done: DoneFn) => {
            schObj.dataBound = () => {
                const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
                const targetAppointment: HTMLElement = appointments[5];
                const targetEvent: Record<string, any> = schObj.getEventDetails(targetAppointment);
                expect(targetEvent.StartTime.toISOString()).toContain('2017-10-30T11:00:00.000Z');
                expect(targetEvent.IsAllDay).toBe(false);
                expect(targetEvent.EndTime.toISOString()).toContain('2017-10-30T14:30:00.000Z');
                done();
            };
            const workCells: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-work-cells'));
            const workCell: HTMLElement = workCells[0];
            ...
            const clipboardContent: string = clipboardElement.value;
            const parsedContent: any = JSON.parse(clipboardContent);
            expect(parsedContent[0].Id).toBe(4);
            expect(parsedContent[0].StartTime).toContain('2017-10-31T00:00:00.000Z');
            expect(parsedContent[0].EndTime).toContain('2017-11-01T00:00:00.000Z');
            expect(parsedContent[0].IsAllDay).toBe(true);
            keyModule.processPaste(parsedContent, workCell);
            clipboardElement.value = '';
        });
        it('normal event copy paste to allday cell', (done: DoneFn) => {
            schObj.dataBound = () => {
                const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
                const targetAppointment: HTMLElement = appointments[0];
                const targetEvent: Record<string, any> = schObj.getEventDetails(targetAppointment);
                expect(targetEvent.Subject).toBe('Paris');
                expect(targetEvent.StartTime.toISOString()).toContain('2017-10-29T00:00:00.000Z');
                expect(targetEvent.IsAllDay).toBe(true);
                expect(targetEvent.EndTime.toISOString()).toContain('2017-10-30T00:00:00.000Z');
                schObj.currentView = 'Month';
                schObj.dataBind();
                done();
            };
            const workCells: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-all-day-cells'));
            const workCell: HTMLElement = workCells[1];
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            ...
            const clipboardContent: string = clipboardElement.value;
            const parsedContent: any = JSON.parse(clipboardContent);
            expect(parsedContent[0].Id).toBe(44);
            expect(parsedContent[0].Subject).toBe('Paris');
            expect(parsedContent[0].IsAllDay).toBe(false);
            workCell.click();
            keyModule.processPaste(parsedContent, workCell);
        ...
        it('Month view all day event copy paste', (done: DoneFn) => {
            schObj.dataBound = () => {
                const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
                const targetAppointment: HTMLElement = appointments[appointments.length - 1];
                const targetEvent: Record<string, any> = schObj.getEventDetails(targetAppointment);
                expect(targetEvent.Subject).toBe('Paris');
                expect(targetEvent.StartTime.toISOString()).toContain('2017-11-23');
                expect(targetEvent.IsAllDay).toBe(true);
                expect(targetEvent.EndTime.toISOString()).toContain('2017-11-24');
                done();
            };
            const workCell: HTMLElement = schObj.element.querySelector('[data-date="1511395200000"]');
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            const elementToCopy: HTMLElement = appointments[0] as HTMLElement;
            elementToCopy.click();
            keyModule.keyActionHandler({ action: 'copy', ctrlKey: true, shiftKey: false, target: elementToCopy });
            ...
            const clipboardContent: string = clipboardElement.value;
            const parsedContent: any = JSON.parse(clipboardContent);
            expect(parsedContent[0].Id).toBe(46);
            expect(parsedContent[0].Subject).toBe('Paris');
            expect(parsedContent[0].IsAllDay).toBe(true);
            keyModule.processPaste(parsedContent, workCell);
            clipboardElement.value = '';
    ...
    describe('Copy and paste feature', () => {
        ...
        },
        {
            ...
            StartTime: new Date(2018, 3, 2, 17, 0),
        ...
        },
        {
            ...
            StartTime: new Date(2018, 3, 2, 14, 0),
        ...
        beforeAll((done: DoneFn) => {
            ...
            schObj = util.createSchedule(schOptions, scheduleData, done, elem);
            keyModule = schObj.keyboardInteractionModule;
        });
        afterAll(() => {
            util.destroy(schObj);
        });
        it('Recurring event copy', (done: DoneFn) => {
            const target: NodeListOf<Element> = schObj.element.querySelectorAll('.e-appointment');
            const elementToCopy: HTMLElement = target[0] as HTMLElement;
            elementToCopy.click();
            schObj.copy([elementToCopy]);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
            const parsedContent: any = JSON.parse(clipboardContent);
            expect(parsedContent[0].Id).toBe(1);
            expect(parsedContent[0].Subject).toBe('rec_appointment');
            expect(parsedContent[0].StartTime).toContain('2018-04-02');
            expect(parsedContent[0].EndTime).toContain('2018-04-02');
            expect(parsedContent[0].IsAllDay).toBe(false);
            expect(parsedContent[0].RecurrenceRule).toBe('FREQ=DAILY;INTERVAL=1;COUNT=5');
            const workCell: HTMLElement = schObj.element.querySelector('.e-work-cells') as HTMLElement;
            workCell.click();
            schObj.paste(workCell);
            clipboardElement.value = '';
        ...
        it('Block event copy', (done: DoneFn) => {
            const elementToCopy: HTMLElement = schObj.element.querySelector('.e-block-appointment');
            elementToCopy.click();
            schObj.copy([elementToCopy]);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
        ...
        it('Readonly event copy', (done: DoneFn) => {
            const target: NodeListOf<Element> = schObj.element.querySelectorAll('.e-appointment');
            const elementToCopy: HTMLElement = target[2] as HTMLElement;
            elementToCopy.click();
            schObj.copy([elementToCopy]);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
        ...
        it('Scheduler set to readonly', (done: DoneFn) => {
            schObj.dataBound = () => {
                const target: NodeListOf<Element> = schObj.element.querySelectorAll('.e-appointment');
                const elementToCopy: HTMLElement = target[1] as HTMLElement;
                elementToCopy.click();
                ...
                const parsedContent: any = JSON.parse(clipboardContent);
                expect(parsedContent[0].Id).toBe(4);
                expect(parsedContent[0].Subject).toBe('Normal Event');
                expect(parsedContent[0].StartTime).toContain('2018-04-02');
                expect(parsedContent[0].EndTime).toContain('2018-04-02');
                const workCell: HTMLElement = schObj.element.querySelector('.e-work-cells') as HTMLElement;
                workCell.click();
        ...
        it('allowClipboard property set to false', (done: DoneFn) => {
            schObj.allowClipboard = false;
            schObj.dataBind();
            const target: NodeListOf<Element> = schObj.element.querySelectorAll('.e-appointment');
            const elementToCopy: HTMLElement = target[1] as HTMLElement;
            elementToCopy.click();
    ...
    describe('Cut and paste feature', () => {
        let schObj: Schedule;
        let keyModule: any;
        beforeAll((done: DoneFn) => {
            const elem: HTMLElement = createElement('div', { id: 'Schedule', attrs: { tabIndex: '1' } });
        ...
        it('single event cut', (done: DoneFn) => {
            const target: NodeListOf<Element> = schObj.element.querySelectorAll('.e-appointment');
        ...
        it('Multiple events cut', (done: DoneFn) => {
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            const newAppointmentsArray: HTMLElement[] = [appointments[2], appointments[3]];
            schObj.cut(newAppointmentsArray);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
            const parsedContent: any = JSON.parse(clipboardContent);
            expect(parsedContent[0].Id).toBe(2);
        ...
        it('Month view event cut', (done: DoneFn) => {
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            const newAppointmentsArray: HTMLElement[] = [appointments[0], appointments[1]];
            schObj.cut(newAppointmentsArray);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
        ...
        it('TimelineDay view event cut', (done: DoneFn) => {
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            const newAppointmentsArray: HTMLElement[] = [appointments[0]];
            schObj.cut(newAppointmentsArray);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
        ...
        it('TimelineWeek view event cut', (done: DoneFn) => {
            const appointments: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-appointment')) as HTMLElement[];
            const newAppointmentsArray: HTMLElement[] = [appointments[0]];
            schObj.cut(newAppointmentsArray);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
        ...
        it('tab focus and cut', (done: DoneFn) => {
            schObj.element.focus();
            const workCells: HTMLElement[] = [].slice.call(schObj.element.querySelectorAll('.e-work-cells'));
            const workCell: HTMLElement = workCells[0];
            workCell.click();
    ...
    describe('Cut and paste feature', () => {
        ...
        it('Recurring event cut', (done: DoneFn) => {
            const target: NodeListOf<Element> = schObj.element.querySelectorAll('.e-appointment');
            const elementToCut: HTMLElement = target[0] as HTMLElement;
            elementToCut.click();
            schObj.copy([elementToCut]);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
            const parsedContent: any = JSON.parse(clipboardContent);
            expect(parsedContent[0].Id).toBe(1);
        ...
        it('Block event cut', (done: DoneFn) => {
            const elementToCut: HTMLElement = schObj.element.querySelector('.e-block-appointment');
            elementToCut.click();
            schObj.cut([elementToCut]);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
            expect(clipboardContent).toBe('');
            const workCell: HTMLElement = schObj.element.querySelector('.e-work-cells') as HTMLElement;
        ...
        it('Readonly event cut', (done: DoneFn) => {
            const target: NodeListOf<Element> = schObj.element.querySelectorAll('.e-appointment');
            const elementToCut: HTMLElement = target[2] as HTMLElement;
            elementToCut.click();
            schObj.cut([elementToCut]);
            const clipboardElement: HTMLInputElement = schObj.element.querySelector('.e-clipboard');
            const clipboardContent: string = clipboardElement.value;
