...
describe('Schedule Timeline Week view', () => {
    ...
    describe('Default schedule block events', () => {
        ...
        it('block event initial rendering testing', () => {
            ...
            expect(blockEvent.offsetWidth).toEqual(149);
            expect(blockEvent.offsetLeft).toEqual(3401);
        });
    ...
    describe('ES-907744 - FirstDayOfWeek property', () => {
        let schObj: Schedule;
        beforeAll((done: DoneFn) => {
            const options: ScheduleModel = {
                height: '600px', width: '1000px',
                views: [{ option: 'TimelineWeek', firstDayOfWeek: 1 }],
                headerRows: [{ option: 'Year' }, { option: 'Month' }, { option: 'Week' }, { option: 'Date' }],
                selectedDate: new Date(2018, 4, 1)
            };
            schObj = util.createSchedule(options, timelineData, done);
        ...
        it('Checking the firstDayOfWeek property in the timeline week for view-specific', () => {
            expect(schObj.element.querySelector('.e-timeline-view')).toBeTruthy();
            expect(schObj.element.querySelector('.e-date-range .e-tbar-btn-text').innerHTML).toEqual('Apr 30 - May 06, 2018');
            expect(schObj.activeViewOptions.firstDayOfWeek).toEqual(1);
            expect(schObj.element.querySelectorAll('.e-header-cells')[0].innerHTML).toEqual('<span class="e-header-date e-navigate">Apr 30, Monday</span>');
        });
        it('Checking the firstDayOfWeek property in the timeline week for component specific', () => {
            schObj.views = [ { option: 'TimelineWeek' }];
            schObj.firstDayOfWeek = 2;
            schObj.refresh();
            expect(schObj.element.querySelector('.e-date-range .e-tbar-btn-text').innerHTML).toEqual('May 01 - 07, 2018');
            expect(schObj.activeViewOptions.firstDayOfWeek).toEqual(2);
            expect(schObj.element.querySelectorAll('.e-header-cells')[0].innerHTML).toEqual('<span class="e-header-date e-navigate">May 1, Tuesday</span>');
        });
    });
