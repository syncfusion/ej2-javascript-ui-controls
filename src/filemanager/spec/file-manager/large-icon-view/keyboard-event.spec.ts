...
describe('FileManager control LargeIcons view', () => {
    ...
    describe('for LargeIcons View', () => {
        ...
        describe('focus regression tests - LargeIcons view', () => {
            it('context menu closed with Esc should return focus to item', (done: Function) => {
                const items: any = document.getElementById('file_largeicons').querySelectorAll('li.e-list-item');
                const target: HTMLElement = items[1] as HTMLElement;
                ...
                setTimeout(function () {
                    ...
                    setTimeout(function () {
                        expect(target.classList.contains('e-active') ||
                            target.classList.contains('e-focus') ||
                            target.getAttribute('aria-selected') === 'true').toBe(true);
                        done();
                    }, 100);
                }, 100);
            });
            ...
            it('context menu Details → close dialog should return focus', (done: Function) => {
                const items: any = document.getElementById('file_largeicons').querySelectorAll('li.e-list-item');
                const target: HTMLElement = items[2] as HTMLElement;
                const contextEvent = new MouseEvent('contextmenu', { bubbles: true, cancelable: true });
                target.dispatchEvent(contextEvent);
                setTimeout(function () {
            ...
            it('open folder via keyboard should focus first item inside', (done: Function) => {
                const container = document.getElementById('file_largeicons');
                const items: NodeListOf<Element> = container.querySelectorAll('li.e-list-item');
                const folderItem: HTMLElement = items[0] as HTMLElement;
                const oldHtml = container.innerHTML;
                keyboardEventArgs.action = 'enter';
                keyboardEventArgs.target = folderItem;
                feObj.largeiconsviewModule.keyActionHandler(keyboardEventArgs);
                ...
                setTimeout(() => {
                    const newItems: NodeListOf<Element> = container.querySelectorAll('li.e-list-item');
                    if (!newItems.length || container.innerHTML === oldHtml) {
                        done();
                        return;
                    }
                    const firstItem: HTMLElement = newItems[0] as HTMLElement;
                    expect(
                        firstItem.classList.contains('e-active') ||
                        firstItem.classList.contains('e-focus') ||
                        firstItem.getAttribute('aria-selected') === 'true'
                    ).toBe(true);
                    done();
                }, 500);
            });

            it('after delete, focus should move to a remaining item', (done: Function) => {
                const container = document.getElementById('file_largeicons');
                const itemsBefore: NodeListOf<Element> = container.querySelectorAll('li.e-list-item');
                const beforeLen: number = itemsBefore.length;
                const idx = beforeLen > 1 ? 1 : 0;
                const target: HTMLElement = itemsBefore[idx] as HTMLElement;
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        const itemsAfter: NodeListOf<Element> = container.querySelectorAll('li.e-list-item');
                        if (!itemsAfter.length || itemsAfter.length === beforeLen) {
                            done();
                            return;
                        }
                        const hasFocus = Array.from(itemsAfter).some((item: Element) =>
                            item.classList.contains('e-active') || item.classList.contains('e-focus') || item.getAttribute('aria-selected') === 'true');
