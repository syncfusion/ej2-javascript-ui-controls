...
export class LargeIconsView {
    ...
    private nextFocusIndex: number | null = null;
    private onDeleteInit(): void {
        if (this.parent.activeModule === 'largeiconsview') {
            const activeItem: Element = this.getFocusedItem() || this.getActiveItem();
            if (activeItem) {
                const index: number = this.itemList.indexOf(<HTMLElement>activeItem);
                if (index !== -1) {
    ...
    private onDeleteEnd(args: ReadArgs): void {
        ...
        if (typeof this.nextFocusIndex === 'number') {
            const items: NodeListOf<Element> = this.element.querySelectorAll('li.e-list-item');
            if (items.length > 0) {
                const focusIndex: number = this.nextFocusIndex >= items.length
    ...
    private onPathChanged(args: ReadArgs): void {
        ...
        if (this.parent.view === 'LargeIcons') {
            ...
            let mappedItem: Element = null;
            if ((this.parent as any)._lastFocused && this.parent.pathId && this.parent.pathId.length && this.items && this.itemList) {
                const key: string = this.parent.pathId[this.parent.pathId.length - 1];
                // eslint-disable-next-line security/detect-object-injection
                const mappedId: string = (this.parent as any)._lastFocused[key];
                if (mappedId) {
                    ...
                    // eslint-disable-next-line security/detect-object-injection
                    if (foundIndex > -1 && this.itemList[foundIndex]) { mappedItem = this.itemList[foundIndex]; }
                }
            }
            const targetItem: Element = this.getFocusedItem() || mappedItem || this.parent.visitedItem || this.getFirstItem();
            if (targetItem) { this.addFocus(targetItem as Element); }
        }
    }
                    ...
                    if (!this.parent.isFile) {
                        ...
                        if (val === '' && !this.parent.isFiltered) {
                            if (this.parent.pathId && this.parent.pathId.length) {
                                (this.parent as any)._lastFocused = Object.assign((this.parent as any)._lastFocused || {}, { [this.parent.pathId[this.parent.pathId.length - 1]]: getValue('_fm_id', details) || getValue('id', details) || text });
                            }
     ...
     * @hidden
     */
    public addFocus(item: Element): void {
        this.element.setAttribute('tabindex', '-1');
        const fItem: Element = this.getFocusedItem();
        if (fItem) {
