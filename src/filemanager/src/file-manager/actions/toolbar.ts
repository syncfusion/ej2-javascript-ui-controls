...
export class Toolbar {
    ...
    private toolbarItemData(data: string[]): ItemModel[] {
        ...
        for (const item of items as ItemModel[]) {
            if (typeof item === 'object' && item !== null && typeof item.template === 'string') {
                const template: string = item.template;
                item.template = template.replace(
                    />((?:Upload|SortBy|View|Refresh|NewFolder|Cut|Copy|Paste|Delete|Rename|Download|SelectAll|Details))</g,
                    (_match: string, key: string): string => '>' + getLocaleText(this.parent, key) + '<'
                );
            }
        }
        return items;
