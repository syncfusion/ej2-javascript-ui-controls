...
export class Keyboard {
    ...
    private keyActionHandler(e: KeyboardEventArgs): void {
        const selectedCard: Element = this.parent.element.querySelectorAll(`.${cls.CARD_CLASS}.${cls.CARD_SELECTION_CLASS}`).item(0);
        if (!selectedCard && !closest(document.activeElement, `.${cls.ROOT_CLASS}`)) {
            return;
        }
        let contentCell: Element;
        let selectedCards: HTMLElement[];
        const selectedCardsData: Record<string, any>[] = [];
        switch (e.action) {
        case 'upArrow':
        case 'downArrow':
        ...
        case 'escape':
            if (document.activeElement.classList.contains(cls.CARD_CLASS) ||
                ...
                if (document.activeElement.classList.contains(cls.CARD_SELECTION_CLASS)) {
                    ...
                    const ele: Element = closest(document.activeElement, '.' + cls.CONTENT_CELLS_CLASS);
                    const cards: HTMLElement[] = [].slice.call(ele.querySelectorAll('.' + cls.CARD_CLASS));
    ...
    private processCardSelection(action: string, selectedCard: Element): void {
        ...
        this.cardTabIndexRemove();
        const cards: HTMLElement[] = [].slice.call(this.parent.element.querySelectorAll('.' + cls.CARD_CLASS));
        const element: Element = action === 'firstCardSelection' ? cards[0] : cards[cards.length - 1];
        this.parent.actionModule.cardSelection(element, false, false);
        this.addRemoveTabIndex('Remove');
        (element as HTMLElement).focus();
        const card: HTMLElement[] = [].slice.call(closest(element, '.' + cls.CONTENT_CELLS_CLASS).querySelectorAll('.' + cls.CARD_CLASS));
    ...
    private processUpDownArrow(action: string, selectedCard: Element): void {
        if (action === 'upArrow' && document.activeElement) {
            if (document.activeElement.classList.contains(cls.CARD_CLASS) && document.activeElement.previousElementSibling) {
                ...
                const cell: Element = closest(document.activeElement, '.' + cls.CONTENT_CELLS_CLASS);
            ...
            if (document.activeElement.nextElementSibling) {
                ...
                const ele: Element = closest(document.activeElement, '.' + cls.CARD_WRAPPER_CLASS).nextElementSibling;
    ...
    public cardTabIndexRemove(): void {
        ...
        const addButton: HTMLElement[] = [].slice.call(this.parent.element.querySelectorAll('.' + cls.SHOW_ADD_BUTTON));
    ...
    private processEnter(e: KeyboardEventArgs, selectedCard: Element): void {
        ...
        if (element.classList.contains(cls.CONTENT_CELLS_CLASS)) {
            const cards: HTMLElement[] = [].slice.call(element.querySelectorAll('.' + cls.CARD_CLASS));
            this.addRemoveTabIndex('Remove');
            if (cards.length > 0) {
    ...
    public addRemoveTabIndex(action: string): void {
        const attribute: string = action === 'Add' ? '0' : '-1';
        const headerIcon: HTMLElement[] = [].slice.call(this.parent.element.querySelectorAll('.' + cls.HEADER_ICON_CLASS));
        if (headerIcon.length > 0) {
            headerIcon.forEach((element: HTMLElement) => { element.setAttribute('tabindex', attribute); });
        }
        const swimlaneIcon: HTMLElement[] = [].slice.call(this.parent.element.querySelectorAll('.' + cls.SWIMLANE_ROW_EXPAND_CLASS));
        if (swimlaneIcon.length > 0) {
            swimlaneIcon.forEach((element: HTMLElement) => { element.setAttribute('tabindex', attribute); });
        }
        const className: string = '.' + cls.CONTENT_ROW_CLASS + ':not(.' + cls.SWIMLANE_ROW_CLASS + ') .' + cls.CONTENT_CELLS_CLASS;
        const contentCell: HTMLElement[] = [].slice.call(this.parent.element.querySelectorAll(className));
