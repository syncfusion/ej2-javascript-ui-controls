...
export class PdfGraphics {
    ...
    _applyStringSettings(font: PdfFont, pen: PdfPen, brush: PdfBrush, format: PdfStringFormat): void {
        ...
        if (font && font instanceof PdfTrueTypeFont && (font.isUnicode || (font._style & PdfFontStyle.bold) !== 0)) {
            const fontName: string = font._fontInternal._metrics._postScriptName;
            let isBoldFont: boolean = false;
...
export class _PdfTransformationMatrix {
    ...
    _skew(angleX: number, angleY: number): void {
        const tanA: number = Math.tan(this._degreeToRadians(angleX));
        const tanB: number = Math.tan(this._degreeToRadians(angleY));
        const skew: _Matrix = new _Matrix(1, tanA, tanB, 1, 0, 0);
        this._matrix._multiply(skew);
    }
    _degreeToRadians(degreesX: number): number {
        const degreeRadFactor: number = Math.PI / 180;
