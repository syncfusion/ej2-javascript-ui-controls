...
/**
 * Provides bidirectional visual reordering support for text, including
 * level-based runs reordering and mirrored glyph shaping for RTL contexts.
 *
 * @private
 */
export class _Bidirectional {
    /**
     * Stores the reordering indexes computed by the bidi algorithm.
     *
     * @private
     */
    _indexes: number[] = [];
    /**
     * Stores the resolved embedding levels for each code point.
     *
     * @private
     */
    _indexLevels: number[] = [];
    /**
     * Maps code points to their mirrored counterparts for RTL shaping.
     *
     * @private
     */
    ...
    /**
     * Applies Unicode symmetric mirroring to characters whose resolved embedding level
     * is odd, using an internal mirroring map.
     *
     * @private
     * @param {string} text The source text to mirror where required.
     * @returns {string} The string with mirrored characters applied at odd levels.
     */
    ...
    /**
     * Computes visual order from logical input using BiDi levels and mirroring, and
     * returns the visually ordered string.
     *
     * @private
     * @param {string} inputText The original logical-order string.
     * @param {boolean} isRtl Whether the paragraph base direction is RTL.
     * @returns {string} The visually ordered (reordered) string.
     */
    ...
    /**
     * Initializes the index map to identity based on the
     * current `_indexLevels` length.
     *
     * @private
     * @returns {void}
     */
    ...
    /**
     * Reorders indices in the range `[sIndex, eIndex]` according to the BiDi algorithm
     * by processing embedding levels from maximum to minimum odd levels.
     *
     * @private
     * @param {number} sIndex The start index (inclusive).
     * @param {number} eIndex The end index (inclusive).
     * @returns {void}
     */
    ...
    /**
     * Reverses the `_indexes` segment in the range.
     *
     * @private
     * @param {number} i The inclusive starting index of the segment.
     * @param {number} j The exclusive ending index of the segment.
     * @returns {void}
     */
...
/**
 * Implements a lightweight Unicode BiDi classifier and visual ordering engine.
 * It classifies characters into BiDi classes, computes levels, normalizes runs,
 * and produces per-character embedding levels for visual reordering.
 *
 * @private
 */
export class _RtlCharacters {
    /**
     * Bidi character types for the input text sequence.
     *
     * @private
     */
    _type: number[] = [];
    /**
     * Selected text order for shaping (-1 = auto).
     *
     * @private
     */
    _textOrder: number = -1;
    /**
     * Length of the input text sequence.
     *
     * @private
     */
    _length: number;
    /**
     * Resulting visual order index map after processing.
     *
     * @private
     */
    _result: number[];
    /**
     * Resolved embedding levels for each input character.
     *
     * @private
     */
    _levels: number[];
    /**
     * Table that maps BMP code points to RTL character types.
     *
     * @private
     */
    ...
    ON: number = 18;
    /**
     * Compact range table mapping Unicode intervals to bidi types.
     *
     * @private
     */
    _charTypes: number[] = [
    ...
    /**
     * Produces per-character embedding levels for the input by classifying characters,
     * resolving paragraph direction, and executing a simplified BiDi pass.
     *
     * @private
     * @param {string} inputText The logical-order text to analyze.
     * @param {boolean} isRtl When `true`, sets the paragraph base type to LRE; otherwise `L`.
     * @returns {number[]} A shallow copy of the resolved embedding levels for each character.
     */
    ...
    /**
     * Maps each character in `text` to its BiDi class using `_rtlCharacterTypes`.
     *
     * @private
     * @param {string} text The input string.
     * @returns {number[]} The array of BiDi class codes for each character.
     */
    ...
    /**
     * Initializes `_levels` to the paragraph base level for all characters.
     *
     * @private
     * @returns {void}
     */
    ...
    /**
     * Seeds the `_levels` array from default values.
     *
     * @private
     * @returns {void}
     */
    ...
    /**
     * Adjusts levels for the resolved run , incrementing even/odd
     * runs according to BiDi rules for neutral and number classes.
     *
     * @private
     * @param {number} index The start index of the run.
     * @param {number} level The current level of the run.
     * @param {number} length The end index (exclusive) of the run.
     * @returns {void}
     */
    ...
    /**
     * Executes the main BiDi resolution pass:
     * - initializes levels,
     * - removes embeddings/overrides/BN for processing,
     * - resolves runs and types (EN/AN/AL/neutral handling),
     * - reapplies embeddings, and fills sentinel levels.
     *
     * @private
     * @returns {void}
     */
    ...
    /**
     * Compacts `_result` and `_levels` by skipping embedding/override/BN codes
     * and returns the resulting logical length.
     *
     * @private
     * @returns {number} The compacted length excluding embedding/override/BN controls.
     */
    ...
    /**
     * Re-expands the compacted `_result` and `_levels` back to full length, restoring
     * embedding/override/BN positions and carrying over levels as needed.
     *
     * @private
     * @param {number} length The compacted logical length.
     * @returns {void}
     */
    ...
    /**
     * Normalizes combining marks to inherit the preceding strong type and
     * starts class resolution by dispatching to digit/Arabic/neutral handlers.
     *
     * @private
     * @param {number} index Start index of the current run.
     * @param {number} length End index (exclusive) of the current run.
     * @param {number} level Resolved embedding level of the run.
     * @param {number} startType The effective type before the run.
     * @param {number} endType The effective type after the run.
     * @returns {void}
     */
    ...
    /**
     * Resolves European numbers to Arabic numbers when preceded by `AL`,
     * per BiDi rules, within the given run.
     *
     * @private
     * @param {number} index Start index of the run.
     * @param {number} length End index (exclusive) of the run.
     * @param {number} level Embedding level.
     * @param {number} startType Type before the run.
     * @param {number} endType Type after the run.
     * @returns {void}
     */
    ...
    /**
     * Converts Arabic letters (`AL`) to right-to-left (`R`) within the run.
     *
     * @private
     * @param {number} index Start index of the run.
     * @param {number} length End index (exclusive) of the run.
    ...
    /**
     * Resolves European number separators `ES` and common separators `CS`
     * to `EN`/`AN` when flanked by matching number classes on both sides.
     *
     * @private
     * @param {number} index Start index of the run.
     * @param {number} length End index (exclusive) of the run.
    ...
    /**
     * Normalizes number terminators `ET` by collapsing contiguous ranges and
     * resolving their type based on surrounding strong/number classes.
     *
     * @private
     * @param {number} index Start index of the run.
     * @param {number} length End index (exclusive) of the run.
    ...
    /**
     * Converts remaining `ES`/`ET`/`CS` to `ON` for further resolution.
     *
     * @private
     * @param {number} index Start index of the run.
     * @param {number} length End index (exclusive) of the run.
    ...
    /**
     * Resolves European numbers `EN` to `L` when preceded by a left-to-right
     * strong type within the run, per BiDi rules.
     *
     * @private
     * @param {number} index Start index of the run.
     * @param {number} length End index (exclusive) of the run.
    ...
    /**
     * Scans forward from `index` while the current type matches any of `validSet`,
     * returning the first index not in the set or `length` if none.
     *
     * @private
     * @param {number} index The starting index (inclusive).
     * @param {number} length The maximum index (exclusive).
     * @param {number[]} validSet The set of types to consume.
     * @returns {number} The next index after the consumed range.
     */
    ...
    /**
     * Resolves whitespace, neutrals, and separators (`WS`, `ON`, `B`, `S`) by
     * assigning them the surrounding strong type (or the default based on the level),
     * per the BiDi resolution steps.
     *
     * @private
     * @param {number} index Start index of the run.
     * @param {number} length End index (exclusive) of the run.
