...
export class PdfStateItem extends PdfWidgetAnnotation {
    ...
    set exportValue(value: string) {
        ...
        if (this._dictionary.has('AP')) {
            ...
            if (dictionary.has('N') || dictionary.has('D')) {
                const oldValue: string = this._tryGetExportValue(this);
    ...
    _setCheckedStatus(value: boolean): void {
        ...
        let fieldValue: string = this._tryGetExportValue(this);
    ...
    _unCheckOthers(child: PdfStateItem, value: string, isChecked: boolean): void {
        if (!this._field._isUpdating) {
            ...
            for (let i: number = 0; i < count; ++i) {
                ...
                if (item) {
                    if (item !== child) {
                        const exportValue: string = this._tryGetExportValue(item);
    ...
    _tryGetExportValue(item: PdfStateItem): string {
        let itemValue: string = item._exportValue;
        if (item._dictionary && item._dictionary.has('AP')) {
            const apDictionary: _PdfDictionary = item._dictionary.get('AP');
            ...
            const asValue: _PdfName = item._dictionary.get('AS');
