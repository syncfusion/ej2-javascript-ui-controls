...
import { ContextMenu, DocumentHelper, Editor, EditorHistory, WCharacterFormat } from '../../../../../../src/index';
...
console.log('Dialog with bold and italic property apply testing');
        ...
        let event: any;
        event = { key: 'End', code: 'End', preventDefault: function () { }, ctrlKey: false, shiftKey: true };
...
console.log('Underline value changed and apply testing');
        ...
        event = { key: 'End', code: 'End', preventDefault: function () { }, ctrlKey: false, shiftKey: true };
        editor.documentHelper.onKeyDownInternal(event);
        event = { key: 'b', code: 'KeyB', preventDefault: function () { }, ctrlKey: true, shiftKey: false };
        editor.documentHelper.onKeyDownInternal(event);
        expect(editor.selection.characterFormat.bold).toBe(true);
        event = { key: 'i', code: 'KeyI', preventDefault: function () { }, ctrlKey: true, shiftKey: false };
        editor.documentHelper.onKeyDownInternal(event);
        dialog.showFontDialog();
...
console.log('handle context menu validation');
        ...
        let event: any = { key: 'd', code: 'KeyD', preventDefault: function () { }, ctrlKey: true, shiftKey: false };
        editor.documentHelper.onKeyDownInternal(event);
        expect(() => { editor.fontDialogModule.onCancelButtonClick(); }).not.toThrowError();
    });
});
...
console.log('Font Color event change and apply testing');
        ...
        dialog.showFontDialog();
        dialog.loadFontDialog();
        event = { preventDefault: function () { }, target: { value: { rgba: "#0000ff" } }, shiftKey: true };
...
describe('font dialog api validation_1', () => {
    let editor: DocumentEditor;
    let documentHelper: DocumentHelper;
    let dialog: FontDialog;
    beforeEach((): void => {
    ...
    it('Dialog with bold property with selection is empty', () => {
        console.log('Dialog with bold property with selection is empty');
                let event: any;
                event = { key: 'ArrowDown', code: 'ArrowDown', preventDefault: function () { }, ctrlKey: false, shiftKey: false };
                editor.documentHelper.onKeyDownInternal(event);
