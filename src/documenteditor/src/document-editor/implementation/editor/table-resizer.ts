...
export class TableResizer {
    ...
    private getRowReSizerPosition(widget: TableCellWidget, touchPoint: Point): number {
        ...
        if (tableWidget && cellSpacing > 0) {
            ...
            for (let i: number = 0; i < tableWidget.childWidgets.length; i++) {
                ...
                const rowWidget: TableRowWidget = tableWidget.childWidgets[parseInt(i.toString(), 10)] as TableRowWidget;
    ...
    private getTableWidget(cursorPoint: Point): TableWidget {
        ...
        if (!isNullOrUndefined(currentPage)) {
            for (let i: number = 0; i < currentPage.bodyWidgets.length; i++) {
                const bodyWidget: BodyWidget = currentPage.bodyWidgets[parseInt(i.toString(), 10)];
    ...
    private getTableWidgetFromWidget(point: Point, widget: Widget): TableWidget {
        for (let j: number = 0; j < widget.childWidgets.length; j++) {
            if (widget.childWidgets[parseInt(j.toString(), 10)] instanceof TableWidget) {
                const childWidget: TableWidget = widget.childWidgets[parseInt(j.toString(), 10)] as TableWidget;
    ...
    private resizeColumnWithSelection(selection: Selection, table: TableWidget, dragValue: number): boolean {
        ...
        if (cellwidget && (selection.selectedWidgets.containsKey(cellwidget) || ((cellwidget as TableCellWidget).previousWidget
            ...
            if (this.resizerPosition === 0) {
                ...
                dragValue = this.changeWidthOfCells(table, leftColumnCollection, rightColumnCollection, dragValue);
        ...
        if (rowFormat.beforeWidth > 0) {
            ...
            if (newGridBefore > 0) {
                for (let i: number = 0; i < table.childWidgets.length; i++) {
                    const tableRow: TableRowWidget = table.childWidgets[parseInt(i.toString(), 10)] as TableRowWidget;
                    if (selectedCells.indexOf(tableRow.childWidgets[0] as TableCellWidget) !== -1) {
                        const cellWidget: TableCellWidget = tableRow.childWidgets[0] as TableCellWidget;
                        if (!isNullOrUndefined(cellWidget) &&
                            cellWidget.cellFormat.preferredWidth - dragValue <= cellWidget.ownerColumn.minimumWidth) {
                            dragValue = cellWidget.cellFormat.preferredWidth - cellWidget.ownerColumn.minimumWidth;
                        }
                        this.updateGridBefore(tableRow, dragValue);
                        this.increaseOrDecreaseWidth(cellWidget, dragValue, false);
                    }
                }
            } else {
                ...
                if (newGridBefore < leastGridBefore && offset !== newGridBefore) {
                    ...
                    for (let i: number = 0; i < table.childWidgets.length; i++) {
                        const tableRow: TableRowWidget = table.childWidgets[parseInt(i.toString(), 10)] as TableRowWidget;
                        if (selectedCells.indexOf(tableRow.childWidgets[0] as TableCellWidget) !== -1) {
                            this.updateGridBefore(tableRow, -tableRow.rowFormat.beforeWidth);
            ...
            if (dragValue < 0) {
                ...
                if (currentTableIndent === 0) {
                    for (let i: number = 0; i < table.childWidgets.length; i++) {
                        ...
                        if (selectedCells.indexOf(tableRow.childWidgets[0] as TableCellWidget) !== -1) {
                            /* eslint-disable-next-line max-len */
                            const nextCellWidget: TableCellWidget = (tableRow.childWidgets[0] as TableCellWidget).nextWidget as TableCellWidget;
    ...
    private increaseOrDecreaseWidth(cell: TableCellWidget, dragValue: number, isIncrease: boolean, isRightCellResized?: boolean): boolean {
                ...
                if (!isNullOrUndefined(nextCellWidget) &&
                    ...
                    while (isContinue) {
                        ...
                        if (newRightWidth >= nextCellWidget.ownerColumn.minimumWidth) {
                            ...
                            dragValue += newRightWidth - (cell.nextWidget as TableCellWidget).ownerColumn.minimumWidth;
                        }
                    }
                }
                else {
    ...
    /* eslint-disable-next-line max-len */
    private changeWidthOfCells(table: TableWidget, leftColumnCollection: TableCellWidget[], rightColumnCollection: TableCellWidget[], dragValue: number): number {
        let isRightCellWidthUpdated: boolean = false;
        if (leftColumnCollection.length > 0) {
        ...
        if (rightColumnCollection.length > 0 && !isRightCellWidthUpdated) {
            let diff: number = 0;
            for (let i: number = 0; i < rightColumnCollection.length; i++) {
            ...
            if (diff !== 0) {
                ...
                for (let j: number = 0; j < table.childWidgets.length; j++) {
                    const row: TableRowWidget = table.childWidgets[parseInt(j.toString(), 10)] as TableRowWidget;
    ...
    public applyProperties(table: TableWidget, tableHistoryInfo: TableHistoryInfo): void {
        ...
        for (let i: number = 0; i < table.childWidgets.length; i++) {
            const row: TableRowWidget = table.childWidgets[parseInt(i.toString(), 10)] as TableRowWidget;
            const rowFormat: RowFormatHistoryInfo = tableHistoryInfo.rows[parseInt(i.toString(), 10)];
            ...
            for (let j: number = 0; j < row.childWidgets.length; j++) {
                const cell: TableCellWidget = row.childWidgets[parseInt(j.toString(), 10)] as TableCellWidget;
                const cellFormat: CellFormatHistoryInfo = rowFormat.cells[parseInt(j.toString(), 10)];
    ...
    private getActualWidth(row: TableRowWidget): number {
        ...
        if (row.childWidgets.length > 0) {
            for (let i: number = 0; i < row.childWidgets.length; i++) {
                width += (row.childWidgets[parseInt(i.toString(), 10)] as TableCellWidget).cellFormat.cellWidth;
            }
        }
        return width;
    }
    ...
    public setPreferredWidth(table: TableWidget): void {
        for (let i: number = 0; i < table.childWidgets.length; i++) {
            const rw: TableRowWidget = table.childWidgets[parseInt(i.toString(), 10)] as TableRowWidget;
            ...
            for (let j: number = 0; j < rw.childWidgets.length; j++) {
                const cell: TableCellWidget = rw.childWidgets[parseInt(j.toString(), 10)] as TableCellWidget;
