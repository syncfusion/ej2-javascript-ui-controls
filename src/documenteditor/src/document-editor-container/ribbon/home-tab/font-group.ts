...
export class FontGroup extends RibbonGroupBase implements IRibbonGroup {
    ...
    private changeFontFamily(): void {
        const fontFamilyElement: HTMLInputElement = document.getElementById(this.ribbonId + FONT_FAMILY_ID) as HTMLInputElement;
        if (fontFamilyElement) {
            const fontFamily: string = fontFamilyElement.value;
            FontHelper.changeFontFamily(this.documentEditor, fontFamily);
        }
    }

    private changeFontSize(): void {
        const fontSizeElement: HTMLInputElement = document.getElementById(this.ribbonId + FONT_SIZE_ID) as HTMLInputElement;
        if (fontSizeElement) {
            const fontSize: string = fontSizeElement.value;
