...
import { PivotEngine, IAxisSet, IDataOptions, IDataSet, FieldItemInfo, INumberIndex, IValueSortSettings } from '../../base/engine';
...
import { FieldList } from '../../common/actions/field-list';
...
export class PivotView extends Component<HTMLElement> implements INotifyPropertyChanged {
    ...
    /** @hidden */
    public fieldListModule: FieldList;
    /** @hidden */
    ...
    /**
     * Configures paging settings to divide large datasets into smaller, manageable pages for display when `enablePaging` is set to `true`.
     *
     * This property uses the `PageSettings` class to customize page size and current page for both row and column axes:
     * - `columnPageSize`: Number of columns per page. Default is 5. Must be a positive integer.
     * - `rowPageSize`: Number of rows per page. Default is 5. Must be a positive integer.
     * - `currentColumnPage`: The currently displayed column page. Default is 1. Must be a positive integer.
     * - `currentRowPage`: The currently displayed row page. Default is 1. Must be a positive integer.
     *
     * @example
     * ```typescript
     ...
     * });
     *
     * // Append the PivotView to the DOM element with ID 'PivotView'
     * pivotObj.appendTo('#PivotView');
     * ```
     *
     * @see PageSettings
    ...
    private initProperties(): void {
        ...
        if (this.dataType === 'pivot' && this.dataSourceSettings.mode !== 'Server' &&
            !isNullOrUndefined(this.dataSourceSettings.valueSortSettings.headerText)
			&& this.dataSourceSettings.valueSortSettings.headerText !== '') {
            let valueSortSettings: IValueSortSettings;
            if (this.dataSourceSettings.valueAxis === 'column') {
                valueSortSettings = { columnHeaderText: this.dataSourceSettings.valueSortSettings.headerText,
                    columnSortOrder: this.dataSourceSettings.valueSortSettings.sortOrder };
            } else if (this.dataSourceSettings.valueAxis === 'row') {
                valueSortSettings = { rowHeaderText: this.dataSourceSettings.valueSortSettings.headerText,
                    rowSortOrder: this.dataSourceSettings.valueSortSettings.sortOrder };
            }
            this.setProperties({ dataSourceSettings: { valueSortSettings: valueSortSettings } }, true);
        }
    }
    ...
    private onSuccess(excelExportProperties: ExcelExportProperties): void {
        if (this.request.readyState === XMLHttpRequest.DONE) {
            ...
            if (this.currentAction === 'onExcelExport' || this.currentAction === 'onCsvExport' ||
                this.currentAction === 'onPivotExcelExport' || this.currentAction === 'onPivotCsvExport') {
                if (this.request.status === 200) {
                    const buffer: Blob | Promise<{ blobData: Blob; }> = this.request.response;
                    excelExportProperties = isNullOrUndefined(excelExportProperties) ? {} : excelExportProperties;
                    const fileName: string = isNullOrUndefined(excelExportProperties.fileName) ? (this.currentAction === 'onExcelExport' || this.currentAction === 'onPivotExcelExport' ? 'default.xlsx' : 'default.csv') : excelExportProperties.fileName;
                    if (!this.isBlobData) {
                        Save.save(fileName, buffer as Blob);
                    }
                    const exportCompleteEventArgs: ExportCompleteEventArgs = {
                        type: this.currentAction === 'onExcelExport' || this.currentAction === 'onPivotExcelExport' ? 'xlsx' : 'csv',
                ...
                try {
                    ...
                    if (this.currentAction === 'fetchFieldMembers') {
                        ...
                        const parsedObj: { [key: string]: Object }[] = JSON.parse(engine.indexObject);
                        const indexObject: { [key: string]: Object } = {};
                        for (let len: number = 0; len < parsedObj.length; len++) {
                            indexObject[parsedObj[len as number].Key as string] = parsedObj[len as number].Value;
                        }
                        this.drillThroughValue.indexObject = indexObject as INumberIndex;
                        ...
                        const parsedData: IDataOptions = JSON.parse(engine.dataSourceSettings);
                        if (this.dataSourceSettings.groupSettings.length > 0) {
                            this.setProperties({ dataSourceSettings: { rows: parsedData.rows } }, true);
                            this.setProperties({ dataSourceSettings: { columns: parsedData.columns } }, true);
                            this.setProperties({ dataSourceSettings: { formatSettings: parsedData.formatSettings } }, true);
                        }
                        const valueSort: { [key: string]: Object } = parsedData.valueSortSettings as { [key: string]: Object };
    ...
    public getEngine(
        ...
        excelExportProperties?: ExcelExportProperties, isPivotTableExport?: boolean
    ): void {
        this.engineModule.isEmptyData = false;
        if (this.element.querySelector('.e-spin-hide') && !(action === 'onExcelExport' || action === 'onCsvExport' ||
            action === 'onPivotExcelExport' || action === 'onPivotCsvExport')) {
            this.showWaitingPopup();
        }
        ...
        const customProperties: { [key: string]: Object } = {
            ...
            showFieldList: this.showFieldList || this.isStaticFieldList,
            showPivotChart: this.displayOption.view === 'Both' || this.displayOption.view === 'Chart',
            chartType: this.chartSettings && this.chartSettings.chartSeries && this.chartSettings.chartSeries.type,
            showGroupingBar: this.showGroupingBar,
            showTooltip: this.showTooltip,
            chartSettings: isPivotTableExport ? JSON.parse(this.getPersistData()).chartSettings : null,
            gridSettings: isPivotTableExport ? JSON.parse(this.getPersistData()).gridSettings : null,
            enableAutoFit: this.grid && this.grid.isAutoFitColumns
        };
        ...
        if (params.action === 'onExcelExport' || params.action === 'onCsvExport' ||
            params.action === 'onPivotExcelExport' || params.action === 'onPivotCsvExport') {
     ...
     * @hidden
     */
    public getChartSettings(): string {
        const keyEntity: string[] = ['chartSettings'];
        const chartLoadEvent: EmitType<ILoadedEventArgs> = this.chartSettings['load'];
        const chartLoadedEvent: EmitType<ILoadedEventArgs> = this.chartSettings['loaded'];
        const chartTextRenderEvent: EmitType<ITextRenderEventArgs> = this.chartSettings['textRender'];
        const chartPointRenderEvent: EmitType<IPointRenderEventArgs> = this.chartSettings['pointRender'];
        const chartSeriesRenderEvent: EmitType<ISeriesRenderEventArgs> = this.chartSettings['seriesRender'];
        const chartLegendRenderEvent: EmitType<ILegendRenderEventArgs> = this.chartSettings['legendRender'];
        const chartPointClickEvent: EmitType<IPointEventArgs> = this.chartSettings['pointClick'];
        const chartTooltipRenderEvent: EmitType<ITooltipRenderEventArgs> = this.chartSettings['tooltipRender'];
        const chartLegendClickEvent: EmitType<ILegendClickEventArgs> = this.chartSettings['legendClick'];
        const chartMultiLevelLabelRenderEvent: EmitType<MultiLevelLabelRenderEventArgs> = this.chartSettings['multiLevelLabelRender'];
        const chartBeforePrintEvent: EmitType<IPrintEventArgs> = this.chartSettings['beforePrint'];
        const chartAnimationCompleteEvent: EmitType<IAnimationCompleteEventArgs> = this.chartSettings['animationComplete'];
        const chartMouseMoveEvent: EmitType<IMouseEventArgs> = this.chartSettings['chartMouseMove'];
        const chartMouseClickEvent: EmitType<IMouseEventArgs> = this.chartSettings['chartMouseClick'];
        const chartPointMoveEvent: EmitType<IPointEventArgs> = this.chartSettings['pointMove'];
        const chartMouseLeaveEvent: EmitType<IMouseEventArgs> = this.chartSettings['chartMouseLeave'];
        const chartMouseDownEvent: EmitType<IMouseEventArgs> = this.chartSettings['chartMouseDown'];
        const chartMouseUpEvent: EmitType<IMouseEventArgs> = this.chartSettings['chartMouseUp'];
        const chartDragCompleteEvent: EmitType<IDragCompleteEventArgs> = this.chartSettings['dragComplete'];
        const chartZoomCompleteEvent: EmitType<IZoomCompleteEventArgs> = this.chartSettings['zoomComplete'];
        const chartScrollStartEvent: EmitType<IScrollEventArgs> = this.chartSettings['scrollStart'];
        const chartScrollEndEvent: EmitType<IScrollEventArgs> = this.chartSettings['scrollEnd'];
        const chartScrollChangedEvent: EmitType<IScrollEventArgs> = this.chartSettings['scrollChanged'];
    ...
    private initialLoad(): void {
        try {
            ...
            if (this.spinnerTemplate) {
                createSpinner({ target: this.element, template: this.spinnerTemplate as string, cssClass: this.cssClass
    ...
    public getPersistData(isRemoveDatasource?: boolean): string {
        ...
        const columnRender: EmitType<ColumnRenderEventArgs> = this.gridSettings['columnRender'];
        const excelQueryCellInfo: EmitType<ExcelQueryCellInfoEventArgs> = this.gridSettings['excelQueryCellInfo'];
        const excelHeaderQueryCellInfo: EmitType<ExcelHeaderQueryCellInfoEventArgs> = this.gridSettings['excelHeaderQueryCellInfo'];
        const pdfQueryCellInfo: EmitType<PdfQueryCellInfoEventArgs> = this.gridSettings['pdfQueryCellInfo'];
        const pdfHeaderQueryCellInfo: EmitType<PdfHeaderQueryCellInfoEventArgs> = this.gridSettings['pdfHeaderQueryCellInfo'];
        const chartLoadEvent: EmitType<ILoadedEventArgs> = this.chartSettings['load'];
        const chartLoadedEvent: EmitType<ILoadedEventArgs> = this.chartSettings['loaded'];
        const chartTextRenderEvent: EmitType<ITextRenderEventArgs> = this.chartSettings['textRender'];
        const chartPointRenderEvent: EmitType<IPointRenderEventArgs> = this.chartSettings['pointRender'];
    ...
    /**
     * Exports the Pivot Table data to the specified format, preserving structure when exporting to Excel.
     *
     * This method exports the Pivot Table's data using the provided `ExportType` value:
     * - When using `ExportType.Excel`, the export retains the native Excel Pivot Table structure, enabling features such as filtering, field rearrangement, and drill-down.
     * - When using `ExportType.CSV`, the data is flattened into a simple tabular format without Pivot-specific functionality.
     *
     * > **Note:** This method is only supported when server-side aggregation mode is enabled (`dataSourceSettings.mode = 'Server'`)
     * and the `allowExcelExport` property is set to `true`.
     *
     * @param {ExportType} [exportType=ExportType.Excel] - Specifies the export format:
     * - `ExportType.Excel`: Exports with native Excel Pivot Table features.
     * - `ExportType.CSV`: Exports as a comma-separated values file.
     * If not specified, the default is `ExportType.Excel`.
     *
     * @returns {void}
     *
     * @example
     ...
     * };
     */
    public exportAsPivot(exportType?: PivotExportType): void {
        if (this.dataSourceSettings.mode === 'Server') {
            if (exportType === 'CSV') {
                this.getEngine('onPivotCsvExport', null, null, null, null, null, null, null, null, null, true);
            } else {
                this.getEngine('onPivotExcelExport', null, null, null, null, null, null, null, null, null, true);
            }
        }
    }
    ...
    private gridPdfExport(
        ...
        let pdfDocument: Promise<Object> = null;
    ...
    public chartExport(
        ...
        return pdfDocument as Promise<Object>;
    }
    ...
    public pdfExport(
        ...
        let pdfDocument: Promise<Object> = null;
        return new Promise((resolve: (value: Promise<Object>) => void) => {
            if (exportBothTableAndChart && this.displayOption.view === 'Both') {
                if (this.displayOption.primary === 'Chart') {
                    const chartAndTableExportDocument: Promise<Object> = this.chartExport('PDF', pdfExportProperties, true, pdfDoc, false);
                    ...
                    const tableAndChartExportDocument: Promise<Object> = this.gridPdfExport(pdfExportProperties, true, pdfDoc, false);
                ...
                const tableExportDocumnt: Promise<Object> = this.gridPdfExport(pdfExportProperties, isMultipleExport, pdfDoc, isBlob);
        ...
        if (this.grid) {
            ...
            if (this.enableVirtualization && engine && this.virtualscrollModule) {
                ...
                if (gridContentDiv && !virtualScrollTableDiv) {
                    ...
                    (newContentVirtualDiv).append(...Array.prototype.slice.call(mCnt.childNodes));
                ...
                let translateValue: number = Number(
                    -(mCnt.querySelector('.' + cls.TABLE) as HTMLElement).style.transform.split('(')[1].split(',')[0].split('px')[0]);
    ...
    private setToolTip(args: TooltipEventArgs): void {
        ...
        if (cell && hasField) {
            ...
            if (this.tooltipTemplate && this.getTooltipTemplate() !== undefined) {
                ...
                const contentFunc: Function = function (): string {
                    return contentTemp;
                };
                this.tooltip.content = initializeCSPTemplate(contentFunc) as Function;
            }
        } else {
            args.cancel = true;
        }
    ...
    public getHeaderField(rowIndex: number, colIndex: number, axis: string): string {
        ...
        if (axis === 'row') {
            ...
            for (let j: number = 0; j < len && this.dataSourceSettings.columns.length > 0 &&
                this.dataSourceSettings.columns[j as number]; j++) {
            ...
            if (this.dataSourceSettings.valueAxis === 'row') {
                rowText = (this.dataSourceSettings.rows.length === 0 && this.dataSourceSettings.values.length > 1) ? (this.pivotValues[rowIndex as number][0] as IAxisSet).valueSort.levelName.toString().slice((this.localeObj.getConstant('grandTotal')
                    ...
                    : this.dataSourceSettings.values.length === 1 ? !isNullOrUndefined(this.dataSourceSettings.values[0].caption)
                        ? this.dataSourceSettings.values[this.isTabular ? colIndex : 0].caption : this.dataSourceSettings.values[0].name
                        : (this.pivotValues[rowIndex as number][this.isTabular ? colIndex : 0] as IAxisSet).valueSort.levelName.toString();
                ...
                if (this.isTabular) {
                    ...
                    while (level > 0 || (cell && cell.index === undefined)) {
                        rowIndex--;
    ...
    private mouseClickHandler(e: MouseEvent): void {
        ...
        if (!target.classList.contains(cls.COLLAPSE) && !target.classList.contains(cls.EXPAND) && this.enableValueSorting && !target.classList.contains(cls.RESIZED) && !(target.classList.contains(cls.RHANDLER) && target.classList.contains(cls.RCURSOR)) && !(this.dataType === 'olap' && this.enableVirtualization)) {
            ...
            const isColumnHeaderElement: boolean = ele && !isNullOrUndefined(ele.closest('.' + cls.HEADERCONTENT));
            ...
            try {
                if (ele && ((isColumnHeaderElement || isRowHeaderElement) && this.dataType !== 'olap' &&
                    ...
                    if (this.dataSourceSettings.valueAxis === 'row' && (this.dataSourceSettings.values.length > 1 || this.dataSourceSettings.alwaysShowValueHeader)
                        ...
                        if (this.dataSourceSettings.valueIndex === -1 && isRowHeaderElement && !this.isTabular &&
                            this.dataSourceSettings.subTotalsPosition !== 'Bottom') {
                    ...
                    if (this.dataType === 'pivot' && this.dataSourceSettings.mode !== 'Server' &&
                        (isColumnHeaderElement || isRowHeaderElement)) {
                        const cell: IAxisSet = this.pivotValues[rowIndex as number][colIndex as number];
                        const isRowValueCell: boolean = isRowHeaderElement && this.isTabular &&
                            ((this.dataSourceSettings.valueAxis === 'row' && cell.type === 'value') ||
                                this.dataSourceSettings.valueAxis === 'column');
                        const isRowHeaderValid: boolean = isRowHeaderElement && (!this.isTabular || isRowValueCell);
                        if (isRowHeaderValid || isColumnHeaderElement) {
                            const valueSortSettings: IValueSortSettings = isColumnHeaderElement ? {
                                ...
                                columnSortOrder: this.dataSourceSettings.valueSortSettings.columnSortOrder === 'Descending' ?
                                    'Ascending' : 'Descending'
                            } : {
                                rowIndex: (Number(ele.getAttribute('index')) + Number(ele.getAttribute('aria-rowspan')) - 1),
                                rowSortOrder: this.dataSourceSettings.valueSortSettings.rowSortOrder === 'Descending' ?
                                    'Ascending' : 'Descending',
                                rowHeaderText: (
                                    this.pivotValues[rowIndex as number][colIndex as number] as IAxisSet
                                ).valueSort.levelName as string
                            };
                            valueSortSettings['headerDelimiter'] = this.dataSourceSettings.valueSortSettings.headerDelimiter ?
                                this.dataSourceSettings.valueSortSettings.headerDelimiter : '.';
                            this.setProperties({ dataSourceSettings: { valueSortSettings: valueSortSettings } }, true);
                        }
                    } else if (this.dataType === 'olap' || this.dataSourceSettings.mode === 'Server') {
                        this.setProperties({
                            dataSourceSettings: {
                                valueSortSettings: {
                                    columnIndex: (Number(ele.getAttribute('aria-colindex')) + Number(ele.getAttribute('aria-colspan')) - 1),
    ...
    private framePivotColumns(gridcolumns: ColumnModel[]): void {
        for (const column of gridcolumns) {
            if (column.columns && column.columns.length > 0) {
                ...
                const levelName: string = column.field === '0.formattedText' ? '' : ((column.customAttributes &&
                    column.customAttributes.cell && (column.customAttributes.cell as IAxisSet).valueSort) ?
                    (column.customAttributes.cell as IAxisSet).valueSort.levelName as string : '');
    ...
    public clearSelection(ele: Element, e: MouseEvent | KeyboardEventArgs): void {
    ...
    public applyRowSelection(colIndex: number, rowIndex: number, e: MouseEvent | KeyboardEvent): void {
        ...
        if (!e.ctrlKey && !e.shiftKey && pivotValue && this.selectedRowIndex !== rowIndex) {
            ...
            const _this: PivotView = this as PivotView;
            if (this.isAdaptive) {
    ...
    public applyColumnSelection(
        ...
        if (!target.classList.contains(cls.ROWSHEADER) &&
            ...
            if (e.ctrlKey && this.gridSettings.selectionSettings.mode === 'Both' && type === 'Multiple' && !target.classList.contains(cls.ROWSHEADER)) {
                for (const ele of [].slice.call(this.element.querySelectorAll(`.${cls.ROWSHEADER}.${cls.CELL_SELECTED_BGCOLOR},.${cls.ROWSHEADER}.${cls.SELECTED_BGCOLOR}`))) {
    ...
    private applyHyperlinkSettings(): void {
        if (this.pivotValues) {
            ...
            const colIndex: number[] = [];
            for (let len: number = pivotValues.length, i: number = 0; i < len; i++) {
                if (!isNullOrUndefined(pivotValues[i as number]) && isNullOrUndefined(pivotValues[i as number][0])) {
                    colIndex.push(i);
                }
            }
                        ...
                        if ((pivotValues[i as number][j as number] as IAxisSet).axis === 'value') {
                            ...
                            if (((pivotValues[i as number][0] as IAxisSet).valueSort.levelName as string).indexOf(label) > -1) {
                                ...
                                for (let k: number = 0; k < colIndex.length; k++) {
                                    ...
                                    if ((headerDelimiter + (pivotValues[colIndex[k as number]][j as number]).valueSort
    ...
    private removeButtonFocus(): void {
    ...
    public destroy(): void {
        ...
        if (this.pivotFieldListModule && !this.pivotFieldListModule.isDestroyed) {
            ...
            this.pivotFieldListModule.element.remove();
            this.pivotFieldListModule.element = null;
        }
        if (this.fieldListModule) {
            this.fieldListModule.element = null;
        }
    ...
    private setColumnWidth(column: ColumnModel, width: number): void {
        if (!column.autoFit) {
            if (column.width !== 'auto') {
