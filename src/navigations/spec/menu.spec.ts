...
describe('Menu', () => {
    ...
    const menuItems: MenuItemModel[] = [
        {
            text: 'Book Categories', id: 'books',
            htmlAttributes: {
                'role':'menu-role',
                'tabindex':'1',
                'aria-label':'menu-aria-label',
                'data-value':'menu-data-value',
            },
            items: [
                {
                    text: 'Cookbooks', id:'cbooks',
                    htmlAttributes: {
                        'role':'sub-menu-role',
                        'tabindex':'11',
                        'aria-label':'sub-menu-aria-label',
                        'data-value':'sub-menu-data-value',
                    },
                },
            ]
        },
    ...
    describe('Property', () => {
        ...
        it('menu items htmlAttributes Checking', () => {
            document.body.appendChild(ul);
            menu = new Menu({ items: menuItems }, '#menu');
            const wrap: HTMLElement = menu.getWrapper();
            expect((wrap.childNodes[0].childNodes[0] as Element).getAttribute('role').includes('menu-role')).toEqual(true);
            expect((wrap.childNodes[0].childNodes[0] as Element).getAttribute('aria-label').includes('menu-aria-label')).toEqual(true);
            expect((wrap.childNodes[0].childNodes[0] as Element).getAttribute('tabindex').includes('1')).toEqual(true);
        });
    });
