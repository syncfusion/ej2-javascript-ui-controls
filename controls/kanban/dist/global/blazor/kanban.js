    ...
    Action.prototype.columnToggle = function (target) {
        ...
        if (target.classList.contains(COLLAPSED_CLASS)) {
            ...
            if (this.parent.isAdaptive) {
                colGroup.forEach(function (col) { return col.style.width = sf.base.formatUnit(_this.parent.layoutModule.getWidth()); });
            }
            ...
            if (this.parent.isAdaptive) {
                colGroup.forEach(function (col) { return col.style.width = sf.base.formatUnit(toggleWidth); });
            }
...
var DragAndDrop = /** @class */ (function () {
    ...
    DragAndDrop.prototype.dragHelper = function (e) {
        ...
        var cloneWrapper = sf.base.createElement('div', { innerHTML: this.dragObj.element.outerHTML });
    ...
    DragAndDrop.prototype.drag = function (e) {
        ...
        if (contentCell) {
            ...
            if (keys.length === 1 || isDrag) {
                if (target.classList.contains(CARD_CLASS)) {
                    var insertClone = (sf.base.isNullOrUndefined(target.previousElementSibling) &&
                ...
                else if (target.classList.contains(CARD_WRAPPER_CLASS) && !sf.base.closest(target, '.' + SWIMLANE_ROW_CLASS)
        ...
        if (tColumn.length > 0 && !target.classList.contains(TOGGLE_VISIBLE_CLASS)
            && !sf.base.closest(target, '.' + TOGGLE_VISIBLE_CLASS)) {
...
var KanbanDialog = /** @class */ (function () {
    ...
    KanbanDialog.prototype.renderDialog = function (args, action) {
        this.element = sf.base.createElement('div', { id: this.parent.element.id + '_dialog_wrapper' });
        ...
        this.dialogObj = new sf.popups.Dialog(dialogModel, this.element);
        if (action !== 'Delete') {
    ...
    KanbanDialog.prototype.getDialogContent = function (args, action) {
        if (action === 'Delete') {
            ...
            var container = sf.base.createElement('div', { className: FORM_WRAPPER_CLASS });
            ...
            if (this.parent.dialogSettings.template) {
                if (args) {
                    ...
                    [].slice.call(form.childNodes).forEach(function (node) { return sf.base.remove(node); });
                }
                ...
                for (var _i = 0, dialogFields_1 = dialogFields; _i < dialogFields_1.length; _i++) {
                    ...
                    tr.appendChild(sf.base.createElement('td', { className: 'e-label', innerHTML: field.text ? field.text : field.key }));
    ...
    KanbanDialog.prototype.renderComponents = function (field) {
        var wrapper = sf.base.createElement('div', { className: field.key + '_wrapper' });
        var element = sf.base.createElement('input', { className: FIELD_CLASS, attrs: { 'name': field.key } });
        ...
        switch (field.type) {
            case 'DropDown':
                ...
                break;
            case 'Numeric':
                controlObj = new sf.inputs.NumericTextBox({ value: fieldValue });
                break;
            case 'TextBox':
                controlObj = new sf.inputs.TextBox({ value: fieldValue });
                break;
            case 'Input':
                if (fieldValue) {
                ...
                if (fieldValue && this.parent.cardSettings.headerField === field.key) {
                    sf.inputs.Input.createInput({ element: element, properties: { enabled: false } });
                }
                else {
                    sf.inputs.Input.createInput({ element: element });
                }
                break;
            case 'TextArea':
                sf.base.remove(element);
    ...
    KanbanDialog.prototype.createTooltip = function (element, error, name, display) {
        ...
        var arrow = sf.base.createElement('div', { className: 'e-arrow-tip e-tip-top' });
        arrow.appendChild(sf.base.createElement('div', { className: 'e-arrow-tip-outer e-tip-top' }));
        arrow.appendChild(sf.base.createElement('div', { className: 'e-arrow-tip-inner e-tip-top' }));
    ...
    KanbanDialog.prototype.destroyToolTip = function () {
        if (this.element) {
            this.element.querySelectorAll('.' + ERROR_VALIDATION_CLASS).forEach(function (node) { return sf.base.remove(node); });
        }
...
var Keyboard = /** @class */ (function () {
    ...
    Keyboard.prototype.processUpDownArrow = function (action, selectedCard) {
        ...
        var isShift = false;
        if (selectedCard) {
            var key = sf.base.closest(this.parent.actionModule.lastCardSelection, '.' + CONTENT_CELLS_CLASS).getAttribute('data-key');
            ...
            if (document.activeElement.classList.contains(SWIMLANE_ROW_EXPAND_CLASS)) {
                var parentEle = sf.base.closest(document.activeElement, '.' + SWIMLANE_ROW_CLASS);
    ...
    Keyboard.prototype.processSwimlaneExpandCollapse = function (action) {
        ...
        element.forEach(function (ele) {
            if (ele.classList.contains(CARD_CLASS)) {
                ele = sf.base.closest(ele, '.' + CONTENT_ROW_CLASS).previousElementSibling;
            }
            if (ele.classList.contains(COLLAPSED_CLASS)) {
                sf.base.removeClass([ele, ele.nextElementSibling], COLLAPSED_CLASS);
                sf.base.classList(ele.querySelector('.' + ICON_CLASS), [SWIMLANE_ROW_EXPAND_CLASS], [SWIMLANE_ROW_COLLAPSE_CLASS]);
                ...
                sf.base.addClass([ele, ele.nextElementSibling], COLLAPSED_CLASS);
                sf.base.classList(ele.querySelector('.' + ICON_CLASS), [SWIMLANE_ROW_COLLAPSE_CLASS], [SWIMLANE_ROW_EXPAND_CLASS]);
    ...
    Keyboard.prototype.processTab = function (action, selectedCard) {
        if (selectedCard) {
            var target = sf.base.closest(selectedCard, '.' + CONTENT_ROW_CLASS);
    ...
    Keyboard.prototype.processMoveCards = function (action, card) {
        var nextCell;
        var nextCellCards;
        var curCell = sf.base.closest(card, '.' + CONTENT_CELLS_CLASS);
        var curCellCards = [].slice.call(curCell.querySelectorAll('.' + CARD_CLASS));
        var curRow = sf.base.closest(curCell, '.' + CONTENT_ROW_CLASS);
...
var LayoutRender = /** @class */ (function (_super) {
    ...
    LayoutRender.prototype.renderHeader = function (header) {
        var headerWrap = sf.base.createElement('div', { className: this.parent.swimlaneSettings.keyField ? SWIMLANE_CLASS : '' });
        ...
        var tr = sf.base.createElement('tr', { className: HEADER_ROW_CLASS });
        ...
        var _loop_1 = function (column) {
            if (this_1.isColumnVisible(column)) {
                ...
                var headerWrapper = sf.base.createElement('div', { className: HEADER_WRAP_CLASS });
                th_1.appendChild(headerWrapper);
                var noOfCard = this_1.columnData[column.keyField].length;
                var headerTitle = sf.base.createElement('div', { className: HEADER_TITLE_CLASS });
                ...
                if (column.template) {
                    ...
                    var header_1 = sf.base.createElement('div', { className: HEADER_TEXT_CLASS, innerHTML: column.headerText });
    ...
    LayoutRender.prototype.renderContent = function () {
        var content = sf.base.createElement('div', { className: CONTENT_CLASS });
        this.parent.element.appendChild(content);
        var contentWrap = sf.base.createElement('div', { className: this.parent.swimlaneSettings.keyField ? SWIMLANE_CLASS : '' });
            ...
            for (var _i = 0, _a = this_2.parent.columns; _i < _a.length; _i++) {
                ...
                if (this_2.isColumnVisible(column)) {
                    ...
                    if (column.allowToggle && !column.isExpanded || index !== -1) {
                        ...
                        td.appendChild(sf.base.createElement('div', { className: COLLAPSE_HEADER_TEXT_CLASS, innerHTML: column.headerText }));
                        td.setAttribute('aria-expanded', 'false');
                    }
                    if (column.showAddButton) {
                        var button = sf.base.createElement('div', { className: SHOW_ADD_BUTTON });
                        button.appendChild(sf.base.createElement('div', { className: SHOW_ADD_ICON + ' ' + ICON_CLASS }));
    ...
    LayoutRender.prototype.renderSwimlaneRow = function (tBody, row, isCollapsed) {
        ...
        var swimlaneHeader = sf.base.createElement('div', { className: SWIMLANE_HEADER_CLASS });
        ...
        var headerWrap = sf.base.createElement('div', { className: HEADER_WRAP_CLASS });
        ...
        if (this.parent.swimlaneSettings.template) {
            var templateArgs = sf.base.extend({}, row, { count: cardCount }, true);
    ...
    LayoutRender.prototype.renderCards = function () {
        ...
        cardRows.forEach(function (tr, index) {
            ...
            var _loop_3 = function (column) {
                if (_this.isColumnVisible(column)) {
                    ...
                    var cardWrapper_1 = sf.base.createElement('div', { className: CARD_WRAPPER_CLASS });
                    if (column.showAddButton) {
                    ...
                    var _loop_4 = function (data) {
                        ...
                        if (_this.parent.cardSettings.template) {
                            ...
                            if (_this.parent.cardSettings.showHeader) {
                                var cardHeader = sf.base.createElement('div', { className: CARD_HEADER_CLASS });
                                var cardCaption = sf.base.createElement('div', { className: CARD_HEADER_TEXT_CLASS });
        ...
        if (!this.parent.swimlaneSettings.showEmptyRow) {
            removeTrs.forEach(function (tr) { return sf.base.remove(tr); });
        }
    };
            ...
            if (_this.isColumnVisible(column)) {
                ...
                var col = sf.base.createElement('col', {
                    ...
                    styles: _this.parent.isAdaptive ? 'width: ' +
                        (isToggle ? sf.base.formatUnit(toggleWidth) : sf.base.formatUnit(_this.getWidth())) : ''
                });
    ...
    LayoutRender.prototype.createStackedRow = function (rows) {
        var tr = sf.base.createElement('tr', { className: HEADER_ROW_CLASS + ' ' + STACKED_HEADER_ROW_CLASS });
        var stackedHeaders = [];
        this.parent.columns.forEach(function (column) {
            var headerText = '';
        ...
        for (var h = 0; h < stackedHeaders.length; h++) {
            ...
            var div = sf.base.createElement('div', { className: HEADER_TEXT_CLASS, innerHTML: stackedHeaders[h] });
    ...
    LayoutRender.prototype.renderLimits = function (column, target) {
        var limits = sf.base.createElement('div', { className: LIMITS_CLASS });
        if (column.minCount) {
    ...
    LayoutRender.prototype.refreshHeaders = function () {
        var header = this.parent.element.querySelector('.' + HEADER_CLASS);
        [].slice.call(header.children).forEach(function (child) { return sf.base.remove(child); });
        this.renderHeader(header);
    };
    LayoutRender.prototype.refreshCards = function () {
        var cards = [].slice.call(this.parent.element.querySelectorAll('.' + CARD_WRAPPER_CLASS));
        cards.forEach(function (card) { return sf.base.remove(card); });
        this.renderCards();
    };
...
var Kanban = /** @class */ (function (_super) {
    ...
    Kanban.prototype.render = function () {
        ...
        sf.popups.createSpinner({ target: this.element });
        this.showSpinner();
    ...
    Kanban.prototype.destroy = function () {
        this.destroyModules();
        if (!this.isBlazorRender()) {
            [].slice.call(this.element.childNodes).forEach(function (node) { return sf.base.detach(node); });
        }
...
sfBlazor.loadDependencies(sfBlazor.dependencyJson.kanban, () => {
    sf.kanban = sf.kanban({});
});
