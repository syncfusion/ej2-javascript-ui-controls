...
export class PdfViewerBase {
    ...
    public getTileCount(pageWidth: any): number {
        if (pageWidth && typeof pageWidth === 'number') {
            ...
            if (this.getZoomFactor() > 2 && pageWidth <= 1632) {
                tileCount = 2;
            } else {
    ...
    private createRequestForRender(pageIndex: number): void {
        ...
        // tslint:disable-next-line
        let viewportWidth: any = 1630; // On diving the value greater than 1630 we will get the tile count as 2.
        // tslint:disable-next-line
        ...
        if (canvas) {
            ...
            if (data && data.uniqueId === proxy.documentId) {
                ...
                let zoomFactor: number = this.retrieveCurrentZoomFactor();
                if (zoomFactor > 2 && pageWidth <= 1632) {
                    viewportWidth = 700;
                } else {
                    viewportWidth = 1630;
                }
    ...
    public checkForResizeHandles(
        ...
        if (!isSticky) {
            // tslint:disable-next-line:max-line-length
            if ((isStamp || this.pdfViewer.selectedItems.annotations[0].shapeAnnotationType === 'HandWrittenSignature') || ((element.actualSize.width >= forty && element.actualSize.height >= forty) && isNodeShape && (resizerLocation === 1 || resizerLocation === 3))) {
                if (contains(
    ...
    public diagramMouseDown(evt: MouseEvent | TouchEvent): void {
        ...
        if (this.pdfViewer.annotationModule) {
            ...
            if (freeTextAnnotModule.isNewFreeTextAnnot === true && !(currentObj && currentObj.shapeAnnotationType === 'FreeText')) {
                let canvas: Rect;
                // tslint:disable-next-line:max-line-length
                if (evt.target && ((evt.target as PdfAnnotationBaseModel).id.indexOf('_text') > -1 || (evt.target as HTMLElement).classList.contains('e-pv-hyperlink')) && this.pdfViewer.annotation) {
    ...
    private updateExportItem(): boolean {
        ...
        // tslint:disable-next-line
        let freeTextObject: any = window.sessionStorage.getItem(this.documentId + '_annotations_freetext');
        if (shapeObject || measureObject || stampObject || stickyObject || textMarkupObject || freeTextObject || this.isImportAction || this.isStorageExceed) {
            return true;
        } else {
            return false;
        }
    ...
    public createAnnotationsCollection(): any {
        ...
        for (let i: number = 0; i < this.pageCount; i++) {
            let annotation: IAnnotationCollection = {
                // tslint:disable-next-line:max-line-length
                textMarkupAnnotation: [], shapeAnnotation: [], measureShapeAnnotation: [], stampAnnotations: [], stickyNotesAnnotation: [], freeTextAnnotation: []
            };
