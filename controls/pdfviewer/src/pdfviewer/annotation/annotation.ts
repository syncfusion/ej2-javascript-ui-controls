...
import { DecoratorShapes, PointModel, processPathData, scalePathData, splitArrayCollection } from '@syncfusion/ej2-drawings';
...
export class Annotation {
    ...
    */
     ...
     */
    // eslint-disable-next-line
    public storeAnnotationCollections(annotation: any, pageNumber: number, isNeedToReorderCollection?: boolean, orderNumber?: number): void {
   ...
   /**
            ...
            switch (actionObject.action) {
                ...
                case 'Addition':
                    if (this.pdfViewer.formDesigner && actionObject.annotation.formFieldAnnotationType) {
                        ...
                        let filteredAnnotations: any = this.pdfViewer.annotationCollection.filter(annotation => {
                            let excludeAnnotation: boolean = annotation.annotationId !== actionObject.annotation.annotName;                           
                            if (excludeAnnotation) {
                                let removeDiv: HTMLElement = document.getElementById(annotation.annotationId);
                                if (removeDiv) {
                                    if (removeDiv.parentElement.childElementCount === 1) {
                                        this.stickyNotesAnnotationModule.updateAccordionContainer(removeDiv);
                                    } else {
    ...
    public modifyFontSize(currentValue: number, isInteracted: boolean): void {
        ...
        let canvas: any = document.getElementById(this.pdfViewer.element.id + '_annotationCanvas_' + currentAnnotation.pageIndex);
     ...
     * @private
     */
    public handleFontSizeUpdate(size: any): void {
        let fontSize: any = parseFloat(size);
        let isInteracted: any = true;
    ...
    public getEventPageNumber(event: any): number {
        ...
        if (isNaN(pageString)) {
            event = this.pdfViewerBase.annotationEvent && this.pdfViewerBase.annotationEvent.target.id === eventTarget.id ? this.pdfViewerBase.annotationEvent: event;
    ...
    public renderAnnotations(pageNumber: number, shapeAnnotation: any, measureShapeAnnotation: any, textMarkupAnnotation: any, canvas?: any, isImportAnnotations?: boolean, isAnnotOrderAction?: boolean, freeTextAnnotation?: any): void {
        ...
        if (!this.pdfViewerBase.isInkAdded && this.pdfViewer.tool === 'Ink' && this.pdfViewer.currentPageNumber - 1 === pageNumber) {
            let currentcanvas: any = document.getElementById(this.pdfViewer.element.id + '_annotationCanvas_' + (this.pdfViewer.currentPageNumber - 1));
            let zoom: number = this.pdfViewerBase.getZoomFactor();
            let ratio: number = this.pdfViewerBase.getWindowDevicePixelRatio();
            let context: any = currentcanvas.getContext('2d');
            ...
            let collectionData: any = processPathData(this.pdfViewer.annotationModule.inkAnnotationModule.updateInkDataWithZoom());
            let csData: any = splitArrayCollection(collectionData);
    ...
    private modifyAnnotationProperties(newAnnotation: any, annotation: any, annotationType: string): any {
        ...
        if (!isNullOrUndefined(annotation.annotationSelectorSettings) && (newAnnotation.annotationSelectorSettings !== annotation.annotationSelectorSettings)) {
