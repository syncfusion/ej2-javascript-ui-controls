...
export class StickyNotesAnnotation {
        ...
        if (stickyAnnotations) {
            if (stickyAnnotations.length > 0) {
                for (let i: number = 0; i < stickyAnnotations.length; i++) {
                    ...
                    annot = {
                        // tslint:disable-next-line:max-line-length
                        author: author, modifiedDate: annotationObject.modifiedDate, annotName: annotationObject.annotName, pageIndex: pageNumber,
    ...
    private createTitleContainer(commentsDivElement: HTMLElement, type: string, subType?: string, modifiedDate?: string): any {
        ...
        let annotationAuthor: string = this.pdfViewer.annotationModule.updateAnnotationAuthor(annotationType, subType);
        ...
        if (commentsContainer) {
            commentsContainer.setAttribute('author', annotationAuthor);
        }
            ...
            if (annotationCommentObject[index]) {
                ...
                for (let i: number = 0; i < pageCollections.length; i++) {
                    if (pageCollections[i].annotName === id) {
                        ...
                        if (type === 'textMarkup') {
                            // tslint:disable-next-line:max-line-length
                            this.pdfViewer.annotationModule.textMarkupAnnotationModule.clearAnnotationSelection(pageNumber - 1);
                            // tslint:disable-next-line:max-line-length
