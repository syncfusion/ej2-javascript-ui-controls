 ...
 * @hidden
 */
export interface IMeasureShapeAnnotation extends IAnnotation {
    bounds: IRectangle
    thickness: number
    borderStyle: string
...
export class MeasureAnnotation {
                ...
                if (!measureAnnots || isImportAction || isAnnotOrderAction) {
                    for (let i: number = 0; i < shapeAnnotations.length; i++) {
                        ...
                        if (annotation.ShapeAnnotationType) {
                            ...
                            const nameTableAnnot: any = (this.pdfViewer.nameTable as any)[annot.id];
                            if (nameTableAnnot && nameTableAnnot.wrapper && nameTableAnnot.wrapper.bounds &&
                                (annot.measureType === 'Distance' || annot.measureType === 'Perimeter' ||
                                    annot.measureType === 'Area' || annot.measureType === 'Volume')) {
                                let collectionIndex: number = -1;
                                const collection: any[] = this.pdfViewer.annotationCollection as any[];
                                for (let j: number = 0; j < collection.length; j++) {
                                    if (collection[j as number].uniqueKey === annot.id) {
                                        collectionIndex = j;
                                        break;
                                    }
                                }
                                if (collectionIndex !== -1) {
                                    const existingAnnot: any = collection[collectionIndex as number];
                                    const wrapperBounds: any = nameTableAnnot.wrapper.bounds;
                                    existingAnnot.bounds.left = wrapperBounds.x;
                                    existingAnnot.bounds.top = wrapperBounds.y;
                                    existingAnnot.bounds.width = wrapperBounds.width;
                                    existingAnnot.bounds.height = wrapperBounds.height;
                                    existingAnnot.bounds.right = wrapperBounds.right;
                                    existingAnnot.bounds.bottom = wrapperBounds.bottom;
                                }
                            }
    ...
    public saveMeasureShapeAnnotations(): string {
        const storeObject: IPageAnnotations[] = this.pdfViewer.annotationsCollection.get(this.pdfViewerBase.documentId + '_annotations_shape_measure');
                        ...
                        if (this.pdfViewerBase.isBounds) {
                            ...
                            if (pageAnnotations[parseInt(i.toString(), 10)].shapeAnnotationType === 'Line' || pageAnnotations[parseInt(i.toString(), 10)].shapeAnnotationType === 'Polyline') {
                                ...
                                pageAnnotations[parseInt(i.toString(), 10)].bounds =
                                {
                                    ...
                                    right: annotationBase.wrapper.bounds.right, bottom: annotationBase.wrapper.bounds.bottom
                                };
                                ...
                                {
                                    left: annotationBase.wrapper.bounds.x, top: annotationBase.wrapper.bounds.y,
                                    width: annotationBase.wrapper.bounds.width,
                                    height: annotationBase.wrapper.bounds.height, right: annotationBase.wrapper.bounds.right,
                                    bottom: annotationBase.wrapper.bounds.bottom
                                };
                            } else {
                                pageAnnotations[parseInt(i.toString(), 10)].bounds =
                                {
    ...
    private manageAnnotations(pageAnnotations: IMeasureShapeAnnotation[], pageNumber: number): void {
        ...
        if (storeObject) {
            ...
            this.pdfViewer.annotationsCollection.set(this.pdfViewerBase.documentId + '_annotations_shape_measure', storeObject);
        }
    }
