...
describe('EJ2-54065 - Filtering on Date column is not working properly with few operators', () => {
    let gridObj: Grid;
    let actionComplete: (args: any) => void;
    let data1: object[] =  [{ OrderID: 10248, CustomerID: 'VINET', OrderDate: '2021-10-06T13:04:10.0456283+00:00' },
                                { OrderID: 10249, CustomerID: 'TOMSP', OrderDate: '2021-10-12T09:22:05.8896886+00:00' },
                                { OrderID: 10251, CustomerID: 'VICTE', OrderDate: '2021-09-24T12:52:15.2898138+00:00', }];
    let data2: object[] = DataUtil.parse.parseJson(JSON.stringify(data1));
    beforeAll((done: Function) => {
        gridObj = createGrid(
            {
                dataSource: data2,
                ...
                columns: [
                    ...
                    { field: 'CustomerID', headerText: 'Customer ID', width: 150 },
                    { field: 'OrderDate', headerText: 'Order Date', width: 130, type: 'date', format: { type: 'date',
                            skeleton: 'short' }, textAlign: 'Right' }],
                allowPaging: true,
                actionComplete: actionComplete, 
            }, done);
    });

    it('filter with lessthanorequal operator ', (done: Function) => {
        actionComplete = (args: any) => {
            if (args.requestType == 'filtering') {
                done();
            }
        ...
        gridObj.actionComplete = actionComplete;
        gridObj.filterByColumn('OrderDate', 'lessthanorequal', '10/6/21');
    });

    it('Check the filter records', () => {
        expect(gridObj.currentViewData.length).toBe(2);
    });

    afterAll(() => {
        destroy(gridObj);
        gridObj = data1 = data2 = actionComplete =  null;
    });
});


describe('EJ2-54457 - showFilterBarOperator is not working with filterBarTemplate => ', () => {
    let gridObj: Grid;
    beforeAll((done: Function) => {
        gridObj = createGrid(
            {
                ...
                allowPaging: false,
                pageSettings: { currentPage: 1 },
                filterSettings: { type: 'FilterBar', columns: [], showFilterBarOperator: true },
                columns: [
                    { field: 'OrderID', type: 'number', visible: true },
                    {
                        field: 'EmployeeID', filterBarTemplate: {
