...
describe('Virtual-freeze-renderer --- row virtualization', () => {
    describe('Freeze Column', () => {
        ...
        it('Frozen column testing', () => {
            expect(fCont.querySelector('.e-row').querySelectorAll('.e-leftfreeze').length).toBe(gridObj.frozenColumns);
            let colsLength: number = gridObj.getColumns().length;
            expect(fCont.querySelector('.e-row').querySelectorAll('.e-unfreeze').length).toBe(colsLength - gridObj.frozenColumns);
        });

        it('Ensure tables, virtualtracks and its count', () => {
            expect(gridObj.getContent().querySelectorAll('.e-virtualtable').length).toBe(1);
            // expect(parentsUntil(mCont, 'e-virtualtable')).toBeTruthy();
            expect(gridObj.getHeaderContent().querySelectorAll('.e-virtualtable').length).toBe(0);
    ...
    describe('Freeze Row and Column', () => {
        ...
        it('Change dynamically', () => {
            gridObj.dataBound = function (done: Function) {
                ...
                expect(fcnt.querySelector('.e-row').querySelectorAll('.e-leftfreeze').length).toBe(gridObj.frozenColumns);
                let colsLength: number = gridObj.getColumns().length;
                expect(fcnt.querySelector('.e-row').querySelectorAll('.e-unfreeze').length).toBe(colsLength - gridObj.frozenColumns);
