...
export class StackingBarSeries3D {
    /**
    ...
    private update(series: Chart3DSeries, point: Chart3DPoint, pointIndex: number, chart: Chart3D): void {
        ...
        const tlpoint: Chart3DLocation = chart.svg3DRenderer.transform3DToVisible(series, (point.left > xStart) ? point.left :
            xStart, topValue, chart);
        const rbpoint: Chart3DLocation = chart.svg3DRenderer.transform3DToVisible(series, (xEnd > point.right) ? point.right :
            xEnd, (bottom > point.bottom) ? bottom : point.bottom, chart);
        const tlfVector: Chart3DVector = chart.vector.vector3D(Math.min(tlpoint.x, rbpoint.x),
                                                               Math.min(tlpoint.y, rbpoint.y), point.startDepth);
        const brbVector: Chart3DVector = chart.vector.vector3D(Math.max(tlpoint.x, rbpoint.x),
                                                               Math.max(tlpoint.y, rbpoint.y), point.endDepth);
        ...
        if (series.columnFacet === 'Cylinder') {
            chart.polygon.createCylinder(tlfVector, brbVector, chart, pointIndex, series.type,
                                         '', point.color, null, styleOptions.opacity, name, chart.chart3D);
        } else if (series.columnFacet === 'Rectangle') {
            chart.polygon.createBox(tlfVector, brbVector, chart, pointIndex,
                                    '', point.color, null, styleOptions.opacity, chart.requireInvertedAxis, name, chart.chart3D);
        }
    }
    ...
    public createSegments(series: Chart3DSeries): void {
        ...
        for (let i: number = 0; i < visiblePoints.length; i++) {
            ...
            const y2: number = series.stackedValues.startValues[i as number];
            const y1: number = series.stackedValues.endValues[i as number];
