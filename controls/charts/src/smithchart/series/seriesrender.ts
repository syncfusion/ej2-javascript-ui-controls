...
export class SeriesRender {
    ...
    public draw(smithchart: Smithchart, axisRender: AxisRender, bounds: SmithchartRect): void {
        ...
        for (let j: number = 0; j < smithchart.series.length; j++) {
            if (smithchart.series[j].enableSmartLabels && smithchart.series[j].marker.dataLabel.visible) {
                let gdlcEle: Element = smithchart.renderer.createGroup({
        ...
        for (let j: number = 0; j < smithchart.series.length; j++) {
            ...
            if (smithchart.series[j].marker.dataLabel.visible) {
                let element: Element = document.getElementById(smithchart.element.id + '_svg' + '_seriesCollection' + j);
                let gdEle: Element = smithchart.renderer.createGroup({
                    'id': smithchart.element.id + '_svg'
                ...
                for (let k: number = 0; k < smithchart.series[j].points.length; k++) {
                    ...
                    if (!dataLabel.template && currentPoint.visible) {
                        ...
                        let textRenderEventArgs: ISmithchartTextRenderEventArgs = {
                            text: options['text'],
                            x: options['x'],
                            y: options['y'],
                            seriesIndex: j,
                        ...
                        let textRenderSuccess: Function = (args: ISmithchartTextRenderEventArgs) => {
                            if (!args.cancel) {
    ...
    private drawSeries(smithchart: Smithchart, seriesindex: number, groupElement: Element, bounds: SmithchartRect): void {
        ...
        let gsEle: Element = smithchart.renderer.createGroup({
        ...
        let seriesEventArgs: ISmithchartSeriesRenderEventArgs = {
            text: smithchart.series[seriesindex].name,
            fill: fill,
            name: seriesRender,
            cancel: false
        };
        let seriesRenderSuccess: Function = (args: ISmithchartSeriesRenderEventArgs) => {
            if (!args.cancel) {
                let options: PathOption = new PathOption(
                    smithchart.element.id + '_series' + seriesindex + '_points', 'none', smithchart.series[seriesindex].width,
                    seriesEventArgs.fill, smithchart.series[seriesindex].opacity, 'none', path
                );
