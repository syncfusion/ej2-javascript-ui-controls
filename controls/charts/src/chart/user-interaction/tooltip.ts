...
export class Tooltip extends BaseTooltip {
        ...
        if (!chart.tooltip.showNearestPoint) {
            ...
            for (const data of dataCollection) {
                if (data.point.symbolLocations[0].x === lastData.point.symbolLocations[0].x || ((
                    data.series.type.indexOf('Column') !== -1 ||
                    lastData.series.type.indexOf('Column') !== -1 ||
                    data.series.type.indexOf('Bar') !== -1 ||
                    lastData.series.type.indexOf('Bar') !== -1
                ) && (data.point.xValue === lastData.point.xValue))) {
    ...
    private formatPointValue(point: Points, axis: Axis, dataValue: string, isXPoint: boolean, isYPoint: boolean): string {
        ...
        if (axis.valueType !== 'Category' && isXPoint) {
            ...
            const formattedValue: string | number | Object = axis.valueType === 'Double' ? +point[dataValue as string] : point[dataValue as string];
