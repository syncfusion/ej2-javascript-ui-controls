...
import { ToolbarPositionModel } from '../model/chart-base-model';
...
export class Zoom {
    ...
    public doPan(chart: Chart, axes: AxisModel[], xDifference: number = 0, yDifference: number = 0): void {
        ...
        if (chart.tooltipModule) {
            const tooltipElement: Element = getElement(chart.element.id + '_tooltip');
            if (tooltipElement) {
    ...
    private showZoomingToolkit(chart: Chart): boolean {
        ...
        const toolbarPosition: ToolbarPositionModel = this.zooming.toolbarPosition;
        ...
        transX = this.toolkit.dragHorizontalRatio != null ? Math.min(
            Math.max(this.chart.border.width + toolkitShadowPadding, this.toolkit.dragHorizontalRatio * this.chart.availableSize.width),
            this.chart.availableSize.width - width - this.chart.border.width - toolkitShadowPadding
        ) : transX;
        transY = this.toolkit.dragVerticalRatio != null ? Math.min(
            Math.max(this.chart.border.width + toolkitShadowPadding, this.toolkit.dragVerticalRatio * this.chart.availableSize.height),
            this.chart.availableSize.height - height - this.chart.border.width - toolkitShadowPadding
        ) : transY;
        ...
        const zoomFillColor: string = this.chart.theme === 'Tailwind3' ? '#F9FAFB' : this.chart.theme === 'Fluent' ? '#F3F2F1' :
