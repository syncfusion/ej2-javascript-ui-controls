        ...
        for (const point of series.points) {
            if (point.visible && point.symbolLocations && point.symbolLocations.length) {
                point.symbolLocations.map((location: ChartLocation, index: number) => {
                    if (series.chart.chartAreaType === 'Cartesian' && (!series.isRectSeries || series.type.indexOf('Column') > -1) && series.type.indexOf('100') === -1 && series.type.indexOf('Range') === -1) {
                        const isVisibleOnAnyYAxis: boolean = series.chart.axisCollections
                            .filter((axis: Axis) => series.chart.isTransposed ? axis.orientation === 'Horizontal' : axis.orientation === 'Vertical')
                            .some((axis: Axis) => {
                                const range: VisibleRangeModel = axis.actualRange;
                                if (axis.valueType === 'Logarithmic') {
                                    const logValue: number = logBase(point.yValue, axis.logBase);
                                    return isFinite(logValue) && logValue >= range.min && logValue <= range.max;
                                }
                                return point.yValue >= range.min && point.yValue <= range.max;
                            });
                        if (series.marker.shape !== 'None' && isVisibleOnAnyYAxis) {
