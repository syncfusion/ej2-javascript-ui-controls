...
{ x: 7000, y: 40 }, { x: 8000, y: 70 }];
    ...
    describe('Column Series - Marker', () => {
        ...
        it('Data label renders multiline when text contains \n', function (done) {
            loaded = function (args) {
                var label = document.getElementById('container_Series_0_Point_0_Text_0');
                console.log(label, 'label');
                expect(label != null).toBe(true);
                var tspans = label.getElementsByTagName('tspan');
                expect(tspans.length > 1).toBe(true);
                done();
            };
            chartObj.loaded = loaded;
            chartObj.series[0].dataSource = [{ x: new Date(2000, 6, 11), y: 10, text1: '0/0\r\n1%\r\n0/0\r\n1%' }];
            chartObj.series[0].xName = 'x';
            chartObj.series[0].yName = 'y';
            chartObj.series[0].marker.dataLabel.name = 'text1';
            chartObj.series[0].marker.dataLabel.visible = true;
            chartObj.refresh();
        });
