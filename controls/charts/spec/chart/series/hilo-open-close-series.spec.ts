...
describe('Chart Control Series', () => {
    ...
    describe('HiloOpenClose Series', () => {
        ...
        it('Apply accessibilityDescriptionFormat to HiloOpenClose series', (done: Function) => {
            loaded = (args: Object) => {
                let element: HTMLElement = document.getElementById('container_Series_0_Point_0');
                expect(element !== null).toBe(true);
                expect(element!.getAttribute('aria-label')).toBe('HiloOpenClose');
                done();
            };
            chartObj.loaded = loaded;
            chartObj.series[0].accessibility.accessibilityDescriptionFormat = '${series.name}';
            chartObj.refresh();
        });
