...
class LegendColorCollection {
    constructor(value, color, label, isHidden) {
        this.value = value;
        this.color = color;
        this.label = label;
...
class CellColor {
    ...
    convertToRGB(value, colorMapping) {
        let previousOffset = colorMapping[0].value;
        let nextOffset = 0;
        for (let i = 1; i < colorMapping.length; i++) {
        ...
        let previousColor = this.getEqualColor(colorMapping, previousOffset);
        let nextColor = this.getEqualColor(colorMapping, nextOffset);
...
class Series {
    ...
    setTextAndColor(dataXIndex, dataYIndex) {
        ...
        if (this.heatMap.bubbleSizeWithColor) {
            ...
            let tempBubbleCollection = new BubbleTooltipData(adaptData.isJsonData && adaptData.adaptorType === 'Cell' ? this.heatMap.dataSource.bubbleDataMapping.size : null, this.text, 'Size');
...
class TwoDimensional {
    ...
    processDataSource(dataSource) {
        ...
        for (let z = axis[1].valueType === 'Category' ? axis[1].min : 0; z < yLength; z++) {
...
class Legend {
            ...
            if (heatMap.paletteSettings.type === 'Gradient') {
                ...
                if (heatMap.enableCanvasRendering) {
                    ...
                    for (let i = 0; i < heatMap.legendColorCollection.length; i++) {
                        let value = ((heatMap.legendColorCollection[i].value - this.legendMinValue) /
    ...
    calculateLegendSize(rect, legendTop) {
        ...
        let width = stringToNumber(this.legendWidth, rect.width);
    ...
    calculateColorAxisGrid(legendRect) {
        ...
        if (heatMap.paletteSettings.type === 'Gradient') {
            for (let index = 0; index < colorCollection.length; index++) {
                let value = ((colorCollection[index].value - this.legendMinValue) /
                    (this.legendMaxValue - this.legendMinValue)) * 100;
                value = isNaN(value) ? 0 : value;
                if (!heatMap.horizontalGradient) {
                    legendPart = rect.height / 100;
...
let HeatMap = class HeatMap extends Component {
     ...
     */
    highlightSelectedCells(previousRect, currentRect, pageX, pageY) {
        ...
        let rect = new Rect(x - this.selectedCellsRect.x, y - this.selectedCellsRect.y, Math.abs(pageX - this.initialCellX), Math.abs(pageY - this.initialCellY));
    ...
    removeSelectedCellsBorder() {
        if (!this.enableCanvasRendering) {
            ...
            let rect = this.previousSelectedCellsRect[0];
...
class Adaptor {
     ...
     * @private
     */
    processJsonTableData(adaptordata) {
        // tslint:disable-next-line:no-any 
        let tempDataCollection = adaptordata.data;
        let currentDataXIndex = 0;
        let currentDataYIndex = 0;
