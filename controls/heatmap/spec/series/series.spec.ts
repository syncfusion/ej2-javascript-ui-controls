...
describe('Heatmap Control', () => {
    ...
    describe('Heatmap label when textOverflow is set to Wrap', () => {
        let heatmap: HeatMap;
        let ele: HTMLElement;
        let tempElement: HTMLElement;
        let created: EmitType<Object>;
        ...
        it('Checking heatmap creation', (done: Function) => {
            created = (args: object): void => {
                expect(heatmap != null).toBe(true);
                done();
            };
        ...
        it('Checking heatMap Label when textOverflow is as None', (done: Function) => {
            heatmap.refresh();
            tempElement = document.getElementById('sampleTest_HeatMapRectLabels_0');
            expect(tempElement).toBe(null);
            heatmap.appendTo('#sampleTest');
            setTimeout(done, 1600);
        });
        it('Checking heatMap Label when textOverflow is set as Wrap', (done: Function) => {
            heatmap.cellSettings.textStyle.textOverflow = 'Wrap';
            heatmap.refresh();
            tempElement = document.getElementById('sampleTest_HeatMapRectLabels_0');
            expect(tempElement.textContent).toBe('Ine...');
            heatmap.appendTo('#sampleTest');
            setTimeout(done, 1600);
        });
