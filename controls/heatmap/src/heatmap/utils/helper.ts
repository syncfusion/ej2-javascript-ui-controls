import { remove, createElement, isNullOrUndefined, compile as templateComplier, SanitizeHtmlHelper} from '@syncfusion/ej2-base';
 ...
 * @private
 */
export function getSanitizedTexts(texts: string[], enableHtmlSanitizer: boolean): string[] {
    if (enableHtmlSanitizer) {
        for (let i: number = 0; i < texts.length; i++) {
            texts[i as number] = SanitizeHtmlHelper.sanitize(texts[i as number]);
        }
    }
    return texts;
}
 ...
 * @private
 */
export function textWrap(currentLabel: string, maximumWidth: number, font: FontModel, isCellLabel?: boolean): string[] {
