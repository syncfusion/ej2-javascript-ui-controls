...
describe('preventFocus in open event - else block behavior', () => {
    let ele: HTMLElement;
    let dialog: Dialog;

    beforeEach(() => {
        ele = createElement('div', { id: 'dialogctrl-prevent-focus' });
        document.body.appendChild(ele);

        dialog = new Dialog({
            header: 'Dialog with PreventFocus',
            content: 'Dialog content with event binding',
            animationSettings: { effect: 'None' },
            visible: false,
            closeOnEscape: true
        }, '#dialogctrl-prevent-focus');
    });
    ...
    it('When preventFocus is false, focusContent should be triggered', (done: Function) => {
        let preventFocusCalled: boolean = false;

        dialog.beforeOpen = function (args: any) {
            preventFocusCalled = true;
        ...
        setTimeout(() => {
            expect(preventFocusCalled).toEqual(true);
            expect(dialog.element.classList.contains('e-popup-open')).toEqual(true);
            done();
        }, 500);
    });

    it('Dialog should maintain event bindings when preventFocus is true in else block', (done: Function) => {
        // Create dialog with keyboard event listeners
        ...
        setTimeout(() => {
            // Verify the dialog is visible and properly initialized
            expect(dialogElement.classList.contains('e-popup-open')).toEqual(true);
            // Verify keyboard event handlers are attached (by checking if keydown event can be triggered)
    ...
    it('When preventFocus is true, manually focus in dialog and press esc key should close dialog', (done: Function) => {
        // Create a dialog element with an input field to manually focus on
        const dialogElement = dialog.element;
        const inputElement = createElement('input', { id: 'dialog-input' });
        (inputElement as HTMLInputElement).type = 'text';
        dialogElement.querySelector('.e-dlg-content').appendChild(inputElement);
        ...
        setTimeout(() => {
            ...
            // Manually focus on the input element inside the dialog
            inputElement.focus();
            expect(document.activeElement).toEqual(inputElement);

            // Simulate ESC key press by dispatching keydown event on the dialog element
            ...
            // Set keyCode property since it's read-only but needed by the dialog handler
            Object.defineProperty(escapeKeyEvent, 'keyCode', { value: 27, enumerable: true });
            dialogElement.dispatchEvent(escapeKeyEvent);
