...
describe('coverage for render file ', () => {
  let TreeGridObj: TreeGrid;
  beforeAll((done: Function) => {
    TreeGridObj = createGrid(
      {
  ...
  it('check drag and drop icon is not displayed', () => {
    let rows = TreeGridObj.getRows();
    let row: any;
    rows.forEach(element => {
      if (element.classList.contains('e-summaryrow')) {
          row = element;
      }
    });
    let cell = row!.cells[0];
    expect(cell.classList.contains('e-summarycell')).toBe(true);
  });
  afterAll(() => {
    destroy(TreeGridObj);
  });
