...
describe('PdfStringLayouter.ts', () => {
    describe('Constructor initializing',()=> {
        ...
        it('-text == empty string', () => {
            expect(function (): void {t2 = t1.layout('', font, format, new SizeF(20, 20), false, new SizeF(0, 0))}).toThrowError();
        })
        it('-text == testing with size(0,0)', () => {
            expect(function (): void {t2 = t1.layout('testing', font, format, new SizeF(0,0), false, new SizeF(0, 0))}).toThrowError();
        })
        it('-text == size.height < font.size', () => {
            expect(function (): void {t2 = t1.layout('testing', font, format, new RectangleF(40, 600, 100, 40), false, new SizeF(0, 0))}).toThrowError();
        })
        it('-text == size.height < font.size', () => {
            let format2 : PdfStringFormat = new PdfStringFormat('alignment');
            format2.wordWrap = PdfWordWrapType.None;
            expect(function (): void {t2 = t1.layout('testing With Testing With Testing, testing With Testing With Testing', font, format2, new RectangleF(40, 600, 100, 40), false, new SizeF(0, 0))}).toThrowError();
        })
    })
})