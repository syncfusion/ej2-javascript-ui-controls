/**
 * dictionaryProperties.ts class for EJ2-PDF
 * PDF dictionary properties.
 * @private
 */
export class DictionaryProperties {
    /**
     * Specifies the value of `Pages`.
     * @private
     */
    public readonly pages : string = 'Pages';
    /**
     * Specifies the value of `Kids`.
     * @private
     */
    public readonly kids : string = 'Kids';
    /**
     * Specifies the value of `Count`.
     * @private
     */
    public readonly count : string = 'Count';
    /**
     * Specifies the value of `Resources`.
     * @private
     */
    public readonly resources : string = 'Resources';
    /**
     * Specifies the value of `Type`.
     * @private
     */
    public readonly type : string = 'Type';
    /**
     * Specifies the value of `Size`.
     * @private
     */
    public readonly size : string = 'Size';
    /**
     * Specifies the value of `MediaBox`.
     * @private
     */
    public readonly mediaBox : string = 'MediaBox';
    /**
     * Specifies the value of `Parent`.
     * @private
     */
    public readonly parent : string = 'Parent';
    /**
     * Specifies the value of `Root`.
     * @private
     */
    public readonly root : string = 'Root';
    /**
     * Specifies the value of `DecodeParms`.
     * @private
     */
    public readonly decodeParms : string = 'DecodeParms';
    /**
     * Specifies the value of `Filter`.
     * @private
     */
    public readonly filter : string = 'Filter';
    /**
     * Specifies the value of `Font`.
     * @private
     */
    public readonly font : string = 'Font';
    /**
     * Specifies the value of `Type1`.
     * @private
     */
    public readonly type1 : string = 'Type1';
    /**
     * Specifies the value of `BaseFont`.
     * @private
     */
    public readonly baseFont : string = 'BaseFont';
    /**
     * Specifies the value of `Encoding`.
     * @private
     */
    public readonly encoding : string = 'Encoding';
    /**
     * Specifies the value of `Subtype`.
     * @private
     */
    public readonly subtype : string = 'Subtype';
    /**
     * Specifies the value of `Contents`.
     * @private
     */
    public readonly contents : string = 'Contents';
    /**
     * Specifies the value of `ProcSet`.
     * @private
     */
    public readonly procset : string = 'ProcSet';
    /**
     * Specifies the value of `ColorSpace`.
     * @private
     */
    public readonly colorSpace : string = 'ColorSpace';
    /**
     * Specifies the value of `ExtGState`.
     * @private
     */
    public readonly extGState : string = 'ExtGState';
    /**
     * Specifies the value of `Pattern`.
     * @private
     */
    public readonly pattern : string = 'Pattern';
    /**
     * Specifies the value of `XObject`.
     * @private
     */
    public readonly xObject : string = 'XObject';
    /**
     * Specifies the value of `Length`.
     * @private
     */
    public readonly length : string = 'Length';
    /**
     * Specifies the value of `Width`.
     * @private
     */
    public readonly width : string  = 'Width';
    /**
     * Specifies the value of `Height`.
     * @private
     */
    public readonly height : string  = 'Height';
    /**
     * Specifies the value of `BitsPerComponent`.
     * @private
     */
    public readonly bitsPerComponent : string  = 'BitsPerComponent';
    /**
     * Specifies the value of `Image`.
     * @private
     */
    public readonly image : string  = 'Image';
    /**
     * Specifies the value of `dctdecode`.
     * @private
     */
    public readonly dctdecode : string  = 'DCTDecode';
    /**
     * Specifies the value of `Columns`.
     * @private
     */
    public readonly columns : string  = 'Columns';
    /**
     * Specifies the value of `BlackIs1`.
     * @private
     */
    public readonly blackIs1 : string  = 'BlackIs1';
    /**
     * Specifies the value of `K`.
     * @private
     */
    public readonly k : string  = 'K';
    /**
     * Specifies the value of `S`.
     * @private
     */
    public readonly s : string  = 'S';
    /**
     * Specifies the value of `Predictor`.
     * @private
     */
    public readonly predictor : string  = 'Predictor';
    /**
     * Specifies the value of `DeviceRGB`.
     * @private
     */
    public readonly deviceRgb : string  = 'DeviceRGB';
    /**
     * Specifies the value of `Next`.
     * @private
     */
    public readonly next : string  = 'Next';
    /**
     * Specifies the value of `Action`.
     * @private
     */
    public readonly action : string  = 'Action';
    /**
     * Specifies the value of `Link`.
     * @private
     */
    public readonly link : string  = 'Link';
    /**
     *
     * Specifies the value of `A`.
     * @private
     */
    public readonly a : string  = 'A';
    /**
     * Specifies the value of `Annot`.
     * @private
     */
    public readonly annot : string  = 'Annot';
    /**
     * Specifies the value of `P`.
     * @private
     */
    public readonly p : string  = 'P';
    /**
     * Specifies the value of `C`.
     * @private
     */
    public readonly c : string  = 'C';
    /**
     * Specifies the value of `Rect`.
     * @private
     */
    public readonly rect : string  = 'Rect';
    /**
     * Specifies the value of `URI`.
     * @private
     */
    public readonly uri : string = 'URI';
    /**
     * Specifies the value of `Annots`.
     * @private
     */
    public readonly annots : string = 'Annots';
    /**
     * Specifies the value of `ca`.
     * @private
     */
    public readonly ca : string = 'ca';
    /**
     * Specifies the value of `CA`.
     * @private
     */
    public readonly CA : string = 'CA';
    /**
     * Specifies the value of `XYZ`.
     * @private
     */
    public readonly xyz : string = 'XYZ';
    /**
     * Specifies the value of `Fit`.
     * @private
     */
    public readonly fit : string = 'Fit';
    /**
     * Specifies the value of `Dest`.
     * @private
     */
    public readonly dest : string = 'Dest';
    /**
     * Specifies the value of `BM`.
     * @private
     */
    public readonly BM : string = 'BM';
    /**
     * Specifies the value of `flatedecode`.
     * @private
     */
    public readonly flatedecode : string = 'FlateDecode';
    /**
     * Specifies the value of `Rotate`.
     * @private
     */
    public readonly rotate : string = 'Rotate';
    /**
     * Specifies the value of 'bBox'.
     * @private
     */
    public readonly bBox : string = 'BBox';
    /**
     * Specifies the value of 'form'.
     * @private
     */
    public readonly form : string = 'Form';
    /**
     * Specifies the value of 'w'.
     * @private
     */
    public readonly w : string = 'W';
    /**
     * Specifies the value of 'cIDFontType2'.
     * @private
     */
    public readonly cIDFontType2 : string = 'CIDFontType2';
    /**
     * Specifies the value of 'cIDToGIDMap'.
     * @private
     */
    public readonly cIDToGIDMap : string = 'CIDToGIDMap';
    /**
     * Specifies the value of 'identity'.
     * @private
     */
    public readonly identity : string = 'Identity';
    /**
     * Specifies the value of 'dw'.
     * @private
     */
    public readonly dw : string = 'DW';
    /**
     * Specifies the value of 'fontDescriptor'.
     * @private
     */
    public readonly fontDescriptor : string = 'FontDescriptor';
    /**
     * Specifies the value of 'cIDSystemInfo'.
     * @private
     */
    public readonly cIDSystemInfo : string = 'CIDSystemInfo';
    /**
     * Specifies the value of 'fontName'.
     * @private
     */
    public readonly fontName : string = 'FontName';
    /**
     * Specifies the value of 'flags'.
     * @private
     */
    public readonly flags : string = 'Flags';
    /**
     * Specifies the value of 'fontBBox'.
     * @private
     */
    public readonly fontBBox : string = 'FontBBox';
    /**
     * Specifies the value of 'missingWidth'.
     * @private
     */
    public readonly missingWidth : string = 'MissingWidth';
    /**
     * Specifies the value of 'stemV'.
     * @private
     */
    public readonly stemV : string = 'StemV';
    /**
     * Specifies the value of 'italicAngle'.
     * @private
     */
    public readonly italicAngle : string = 'ItalicAngle';
    /**
     * Specifies the value of 'capHeight'.
     * @private
     */
    public readonly capHeight : string = 'CapHeight';
    /**
     * Specifies the value of 'ascent'.
     * @private
     */
    public readonly ascent : string = 'Ascent';
    /**
     * Specifies the value of 'descent'.
     * @private
     */
    public readonly descent : string = 'Descent';
    /**
     * Specifies the value of 'leading'.
     * @private
     */
    public readonly leading : string = 'Leading';
    /**
     * Specifies the value of 'avgWidth'.
     * @private
     */
    public readonly avgWidth : string = 'AvgWidth';
    /**
     * Specifies the value of 'fontFile2'.
     * @private
     */
    public readonly fontFile2 : string = 'FontFile2';
    /**
     * Specifies the value of 'maxWidth'.
     * @private
     */
    public readonly maxWidth : string = 'MaxWidth';
    /**
     * Specifies the value of 'xHeight'.
     * @private
     */
    public readonly xHeight : string = 'XHeight';
    /**
     * Specifies the value of 'stemH'.
     * @private
     */
    public readonly stemH : string = 'StemH';
    /**
     * Specifies the value of 'registry'.
     * @private
     */
    public readonly registry : string = 'Registry';
    /**
     * Specifies the value of 'ordering'.
     * @private
     */
    public readonly ordering : string = 'Ordering';
    /**
     * Specifies the value of 'supplement'.
     * @private
     */
    public readonly supplement : string = 'Supplement';
    /**
     * Specifies the value of 'type0'.
     * @private
     */
    public readonly type0 : string = 'Type0';
    /**
     * Specifies the value of 'identityH'.
     * @private
     */
    public readonly identityH : string = 'Identity-H';
    /**
     * Specifies the value of 'toUnicode'.
     * @private
     */
    public readonly toUnicode : string = 'ToUnicode';
    /**
     * Specifies the value of 'descendantFonts'.
     * @private
     */
    public readonly descendantFonts : string = 'DescendantFonts';
    /**
     * Specifies the value of 'background'.
     * @private
     */
    public readonly background : string = 'Background';
    /**
     * Specifies the value of 'shading'.
     * @private
     */
    public readonly shading : string = 'Shading';
    /**
     * Specifies the value of 'matrix'.
     * @private
     */
    public readonly matrix : string = 'Matrix';
    /**
     * Specifies the value of 'antiAlias'.
     * @private
     */
    public readonly antiAlias : string = 'AntiAlias';
    /**
     * Specifies the value of 'function'.
     * @private
     */
    public readonly function : string = 'Function';
    /**
     * Specifies the value of 'extend'.
     * @private
     */
    public readonly extend : string = 'Extend';
    /**
     * Specifies the value of 'shadingType'.
     * @private
     */
    public readonly shadingType : string = 'ShadingType';
    /**
     * Specifies the value of 'coords'.
     * @private
     */
    public readonly coords : string = 'Coords';
    /**
     * Specifies the value of 'domain'.
     * @private
     */
    public readonly domain : string = 'Domain';
    /**
     * Specifies the value of 'range'.
     * @private
     */
    public readonly range : string = 'Range';
    /**
     * Specifies the value of 'functionType'.
     * @private
     */
    public readonly functionType : string = 'FunctionType';
    /**
     * Specifies the value of 'bitsPerSample'.
     * @private
     */
    public readonly bitsPerSample : string = 'BitsPerSample';
    /**
     * Specifies the value of 'patternType'.
     * @private
     */
    public readonly patternType : string = 'PatternType';
    /**
     * Specifies the value of 'paintType'.
     * @private
     */
    public readonly paintType : string = 'PaintType';
    /**
     * Specifies the value of 'tilingType'.
     * @private
     */
    public readonly tilingType : string = 'TilingType';
    /**
     * Specifies the value of 'xStep'.
     * @private
     */
    public readonly xStep : string = 'XStep';
    /**
     * Specifies the value of 'yStep'.
     * @private
     */
    public readonly yStep : string = 'YStep';
    /**
     * Specifies the value of viewer preferences.
     * @private
     */
    public readonly viewerPreferences : string = 'ViewerPreferences';
    /**
     * Specifies the value of center window.
     * @private
     */
    public readonly centerWindow : string = 'CenterWindow';
    /**
     * Specifies the value of display title.
     * @private
     */
    public readonly displayTitle : string = 'DisplayTitle';
    
    /**
     * Specifies the value of fit window.
     * @private
     */
    public readonly fitWindow : string = 'FitWindow';
    /**
     * Specifies the value of hide menu bar.
     * @private
     */
    public readonly hideMenuBar : string = 'HideMenubar';
    /**
     * Specifies the value of hide tool bar.
     * @private
     */
    public readonly hideToolBar : string = 'HideToolbar';
    /**
     * Specifies the value of hide window UI.
     * @private
     */
    public readonly hideWindowUI : string = 'HideWindowUI';
    /**
     * Specifies the value of page mode.
     * @private
     */
    public readonly pageMode : string = 'PageMode';
    /**
     * Specifies the value of page layout.
     * @private
     */
    public readonly pageLayout : string = 'PageLayout';
    /**
     * Specifies the value of duplex.
     * @private
     */
    public readonly duplex : string = 'Duplex';
    /**
     * Specifies the value of print scaling.
     * @private
     */
    public readonly printScaling : string = 'PrintScaling';
    /**
     * Initialize an instance for `PdfDictionaryProperties` class.
     * @private
     */
    public constructor() {
        //
    }
}