...
export class PdfSection implements IPdfWrapper {
    ...
    public containsTemplates(document : PdfDocument, page : PdfPage, foreground : boolean) : boolean {
        let documentHeaders : PdfPageTemplateElement[] = this.getDocumentTemplates(document, page, true, foreground);
        let documentTemplates : PdfPageTemplateElement[] = this.getDocumentTemplates(document, page, false, foreground);
        let contains : boolean = (documentHeaders.length > 0 || documentTemplates.length > 0);
        return contains;
    }
    /**
     ...
     */
    /* tslint:disable */
    private getDocumentTemplates(document : PdfDocument, page : PdfPage, headers : boolean, foreground : boolean) : PdfPageTemplateElement[] {
