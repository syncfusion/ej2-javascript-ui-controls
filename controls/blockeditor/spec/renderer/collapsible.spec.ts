...
import { BlockEditor, BlockModel, BlockType, CollapsibleProps, ContentType, getBlockContentElement, setSelectionRange } from "../../src/index";
...
describe('Toggle Blocks', () => {
    ...
    describe('Testing collapsible block', () => {
            ...
            const blocks: BlockModel[] = [
                {
                    ...
                    props: {
                        children: [
                            {
                                type: BlockType.Heading, props: { level: 1 }, 
        ...
        it('should render other collapsible headings correctly', () => {
            ...
            let editor3: BlockEditor = createEditor({
                blocks: [
                    ...
                    {
                        type: BlockType.CollapsibleHeading, props: { level: 1 },
                        content: [{ type: ContentType.Text, content: 'Toggle Heading 1' }]
                    },
                    {
                        type: BlockType.CollapsibleHeading, props: { level: 2 },
                        content: [{ type: ContentType.Text, content: 'Toggle Heading 2' }]
                    },
                    {
                        type: BlockType.CollapsibleHeading, props: { level: 3 },
                        content: [{ type: ContentType.Text, content: 'Toggle Heading 3' }]
                    },
                    {
                        type: BlockType.CollapsibleHeading, props: { level: 4 },
        ...
        it('should collapse/expand block correctly', () => {
            ...
            expect(toggleContent.style.display).toBe('block');
            expect((editor.blocks[0].props as CollapsibleProps).isExpanded).toBe(true);
            
            // Now collapse it
            toggleIcon.click();
            expect(toggleBlock.getAttribute('data-collapsed')).toBe('true');
            expect(toggleContent.style.display).toBe('none');
            expect((editor.blocks[0].props as CollapsibleProps).isExpanded).toBe(false);
            
            // Test direct method call with false state
            const collapsibleRenderer = editor.blockRendererManager.collapsibleRenderer;
