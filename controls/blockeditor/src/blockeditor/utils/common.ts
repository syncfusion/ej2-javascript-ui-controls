...
export function normalizeRange(range: Range): Range {
    ...
    if (range.startContainer !== range.endContainer &&
        range.startOffset === (range.startContainer as any).length) {

        // Adjust to select only the end container
        newRange.setStart(range.endContainer, 0);
        newRange.setEnd(range.endContainer, (range.endContainer as any).length || 0);
    }
    // Case 2: Partial selection across element boundaries
        ...
        const endTextSelected: boolean = range.endOffset < (range.endContainer as any).length;
 ...
 *
 * @param {string} bgColor - The background color in any CSS-supported format (hex, rgb, etc.)
 * @returns {string} - '#000000' or '#ffffff'
 */
export function getAccessibleTextColor(bgColor: string): string {
    const rgb: { r: number; g: number; b: number } = parseColor(bgColor);
