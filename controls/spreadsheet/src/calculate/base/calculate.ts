...
export class Calculate extends Base<HTMLElement> implements INotifyPropertyChanged {
    ...
    public findWildCardValue(lookVal: string, cellValue: string): string {
        ...
        if (lookVal.indexOf('?') > -1) {
            ...
            if (leftVal > -1 || rightVal > -1) {
                ...
                if (lookVal.startsWith('*') && lookVal.endsWith('*')) {
                    const trimmedVal: string = lookVal.slice(1, -1);
                    if (trimmedVal !== '' && cellValue.includes(trimmedVal) && this.isNaN(this.parseFloat(trimmedVal))) {
                        cellValue = lookVal;
                    }
                }
                finalText = cellValue;
    ...
    public isDate(date: any): Date {
        if (typeof date === 'object' || Date.parse(date) !== null) {
            ...
            if (typeof date === 'string') {
                dateVal = this.checkDateFormat(date, false, true);
            } else {
    ...
    public checkDateFormat(date: string, pvtParse?: boolean, fromIsDate?: boolean): Date {
        if ((this.parentObject as { getModuleName: Function }).getModuleName() === 'spreadsheet') {
            const dateEventArgs: { value: string, cell: { value: string }, isDate?: boolean, isTime?: boolean;
                dateObj?: Date } = { value: date, cell: { value: date } };
            ...
            } else if (fromIsDate && !dateEventArgs.isTime) {
                return new Date(NaN);
            }
        }
    ...
    /**
     * Registers a custom formula function to use in the spreadsheet.
     *
     * @param {string} formulaName - Specifies the name to display for the custom formula.
     * @param {string | Function} functionName - Specifies the function or its name to use as the custom formula logic.
     * @param {string} [formulaDescription] - Specifies the description of the custom formula in the Insert function dialog for help.
     * @param {string} [category] - Specifies the category to group the formula in the Insert Function dialog.
     * {% codeBlock src='spreadsheet/addCustomFunction/index.md' %}{% endcodeBlock %}
     * @returns {void} - Registers the custom formula in spreadsheet.
     */
    public defineFunction(formulaName: string, functionName: string | Function, formulaDescription: string, category: string): void {
        if (typeof functionName === 'string') {
            functionName = getValue(functionName, window);
        }
        formulaName = formulaName.toUpperCase();
        this.libraryFormulas.set(formulaName, {
            handler: functionName as Function, isCustom: true,
            description: formulaDescription, category: category
        });
    }
