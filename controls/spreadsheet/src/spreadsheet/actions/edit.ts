...
export class Edit {
    ...
    private updateEditedValue(tdRefresh: boolean, value: string, e: MouseEvent & TouchEvent | KeyboardEventArgs, isPublic: boolean): void {
        ...
        if (this.parent.isEdit && this.editCellData.value && !isNumber(this.editCellData.value)) {
            const firstChar: string = this.editCellData.value[0];
            if (firstChar === '+' || firstChar === '-') {
                const valuePart: string = this.editCellData.value.substring(1).trim();
                if (valuePart) {
                    const parseCell: CellModel = { value: this.editCellData.value };
                    getAutoDetectFormatParser(this.parent)(parseCell);
                    if (!isNumber(parseCell.value)) {
                        this.editCellData.value = '=' + firstChar + valuePart;
                    }
                }
            }
        }
    ...
    private removeRefIndicator(): void {
        const formulaRefIndicator: HTMLElement = this.parent.element.querySelector('.e-formularef-indicator');
        if (formulaRefIndicator) {
            formulaRefIndicator.parentElement.removeChild(formulaRefIndicator);
        }
