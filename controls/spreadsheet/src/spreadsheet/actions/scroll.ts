...
export class Scroll {
    ...
    private getRowOffset(scrollTop: number, scrollDown: boolean): IOffset {
        ...
        while (i < count) {
            if (scrollDown) {
                const rowHeight: number = getRowHeight(sheet, i - 1 + frozenRow, true);
                temp += rowHeight;
                ...
                const rowHeight: number = getRowHeight(sheet, i + frozenRow, true);
                temp -= rowHeight;
                if (temp <= 0) {
                    return { idx: 0, size: 0 };
                }
    ...
    private getColOffset(scrollLeft: number, increase: boolean, skipHidden: boolean): IOffset {
        ...
        while (i < count) {
            if (increase) {
                const colWidth: number = getColumnWidth(sheet, i - 1 + frozenCol, skipHidden, true);
                temp += colWidth;
                ...
                const colWidth: number = getColumnWidth(sheet, i + frozenCol, skipHidden, true);
                temp -= colWidth;
                if (temp <= 0) {
                    return { idx: 0, size: 0 };
                }
