...
import { applyNumberFormatting, getFormattedCellObject, refreshCellElement, checkDateFormat, getFormattedBarText, applyCF, ApplyCFArgs } from '../common/index';
...
export class WorkbookNumberFormat {
    ...
    private processCustomDateTime(args: NumberFormatArgs, cell: CellModel, isDate: boolean, formatSections: string[]): string {
        ...
        const checkCustomDate: Function = (): boolean => {
            ...
            const formats: { months?: { [key: string]: { abbreviated: { [key: string]: string } } } } =
                IntlBase.getDependables(cldrData, this.parent.locale, null).dateObject;
            const months: { [key: string]: string } = formats.months['stand-alone'] && formats.months['stand-alone'].abbreviated;
        ...
        let type: string;
        let custFormat: string = formatSections[0];
        const intl: Internationalization = new Internationalization();
                                ...
                                if (spaceIdx <= 0) {
                                    ...
                                    if (dateFormat && timeFormat) {
                                        ...
                                        result = intl.formatDate(dateArgs.dateObj, { type: 'date', format: dateFormat }) + ' ' +
                                            intl.formatDate(intToDate(parseFloat(
                                                valArr[0] + 1 + '.' + valArr[1])), { type: 'time', format: timeFormat });
                                    } else {
                                        result = intl.formatDate(dateArgs.dateObj, { type: type, format: custFormat });
                                    }
                                }
        ...
        if (cell.format.indexOf('m') > -1) {
            if (cell.format.indexOf('s') > -1 || cell.format.indexOf('h') > -1) {
                ...
                if (custFormat.includes('MMMMM')) {
                    const prevFormat: string = custFormat;
                    custFormat = 'MMMM';
    ...
    private processCustomAccounting(cell: CellModel, args: NumberFormatArgs, formats: string[], format?: string): boolean {
        const cellVal: number = parseFloat(cell.value);
        if (cellVal < 0) {
    ...
    private processCustomText(cell: CellModel, args: NumberFormatArgs, formatSections?: string[]): string {
        const cellVal: string = cell.value || <unknown>cell.value ? cell.value.toString() : '';
        let format: string;
        if (formatSections) {
            if (formatSections[3]) {
    ...
    private processFormats(
        ...
        if (fResult !== '') {
            ...
            case 'Percentage':
                ...
                break;
            case 'Accounting':
                fResult = this.accountingFormat(args, fResult, cell);
    ...
    private accountingFormat(args: NumberFormatArgs, fResult: string, cell: CellModel): string {
    ...
    private checkCustomTimeFormat(val: string, cell: CellModel): { val: string, format: string } {
        ...
        if (timeArr.length <= 3) {
            ...
            timeArr.forEach((timeVal: string, index: number) => {
                ...
                timeArr[index as number] = timeVal;
    ...
    private checkCustomDateFormat(val: string, cell: CellModel, isEdit: boolean): { val: string, format: string, isDateTime?: boolean } {
        ...
        if (val.includes(this.localeObj.dateSeparator) && ((!val.includes(` ${this.localeObj.am}`) &&
            ...
            const intl: Internationalization = new Internationalization(this.parent.locale);
            const parsedDate: Date = intl.parseDate(val, { skeleton: 'full' }) || intl.parseDate(val, { skeleton: 'long' });
            ...
            } else if (val.indexOf(' ') > 0 && this.parent.locale === 'en-US') {
                {
                    if (val.trim().startsWith('-')) {
                        return { val: 'Invalid', format: '' };
                    }
                    const dateParts: string[] = val.trim().split(' ');
                    const dateLength: number = dateParts.length;
                    if (dateLength < 2 || dateLength > 3) {
                        return { val: 'Invalid', format: '' };
                    }
                    separator = ' ';
                }
        ...
        if (val.includes(this.localeObj.timeSeparator) && val.includes(' ') && separator !== ' ') {
