...
describe('Hide & Show ->', () => {
    ...
    describe('public method ->', () => {
        ...
        it('EJ2-881093--Undo not working as expected when columns were merged', (done: Function) => {
            helper.invoke('selectRange', ['G1:J100']);
            helper.setAnimationToNone('#' + helper.id + '_contextmenu');
            helper.openAndClickCMenuItem(0, 6, [8], false, true);
            const columns: RowModel[] = helper.getInstance().sheets[0].columns;
            expect(columns[6].hidden).toBeTruthy();
            expect(columns[7].hidden).toBeTruthy();
            expect(columns[8].hidden).toBeTruthy();
            expect(columns[9].hidden).toBeTruthy();
            helper.invoke('selectRange', ['F1:M100']);
            helper.openAndClickCMenuItem(0, 5, [8], false, true);
            expect(columns[5].hidden).toBeTruthy();
            expect(columns[10].hidden).toBeTruthy();
            expect(columns[11].hidden).toBeTruthy();
            expect(columns[12].hidden).toBeTruthy();
            helper.click('#spreadsheet_undo');
            expect(columns[5].hidden).toBeFalsy();
            expect(columns[10].hidden).toBeFalsy();
            expect(columns[11].hidden).toBeFalsy();
            expect(columns[12].hidden).toBeFalsy();
            helper.click('#spreadsheet_undo');
            expect(columns[6].hidden).toBeFalsy();
            expect(columns[7].hidden).toBeFalsy();
            expect(columns[8].hidden).toBeFalsy();
            expect(columns[9].hidden).toBeFalsy();
