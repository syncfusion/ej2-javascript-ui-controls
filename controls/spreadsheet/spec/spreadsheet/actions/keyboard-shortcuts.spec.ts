...
describe('Keyboard shortcuts module ->', () => {
    ...
    describe('UI interaction checking ->', () => {
        ...
        it('EJ2-881334 -> Issue in hiding the rows when protect sheet is enabled', (done: Function) => {
            ...
            helper.triggerKeyNativeEvent(57, true, false, null, 'keydown', false, null);
            expect(helper.getInstance().sheets[0].rows[4].hidden).toBeFalsy();
            helper.invoke('protectSheet', ['Sheet1', { selectCells: true, selectUnLockedCells: true, formatRows: true }]);
            expect(helper.getInstance().sheets[0].isProtected).toBeTruthy();
            helper.triggerKeyNativeEvent(57, true, false, null, 'keydown', false, null);
            expect(helper.getInstance().sheets[0].rows[4].hidden).toBeTruthy();
            done();
        });
        it('EJ2-881338 -> Issue in hiding the columns when protect sheet is enabled', (done: Function) => {
            helper.invoke('protectSheet', ['Sheet1', { selectCells: true, selectUnLockedCells: true }]);
            expect(helper.getInstance().sheets[0].isProtected).toBeTruthy();
            helper.invoke('selectRange', ['C1']);
            helper.triggerKeyNativeEvent(48, true, false, null, 'keydown', false, null);
            expect(helper.getInstance().sheets[0].columns[2].hidden).toBeFalsy();
            helper.invoke('protectSheet', ['Sheet1', { selectCells: true, selectUnLockedCells: true, formatColumns: true }]);
            expect(helper.getInstance().sheets[0].isProtected).toBeTruthy();
            helper.triggerKeyNativeEvent(48, true, false, null, 'keydown', false, null);
    ...
    describe('Testing Keyboard shortcuts in Ribbon after enabling sheet protection', () => {
        let spreadsheet: any;
        beforeAll((done: Function) => {
            helper.initializeSpreadsheet({
                sheets: [{ ranges: [{ dataSource: defaultData }] }],
    ...
    describe('Key board Shortcuts->', () => {
        ...
        it('Undo the last keyboard shortcut action->', (done: Function) => {
            helper.invoke('selectRange', ['B6']);
        ...
        it('Adding notes through keyboard shortcut after enabling sheet protection ->', (done: Function) => {
            helper.invoke('selectRange', ['D4']);
            const spreadsheet: Spreadsheet = helper.getInstance();
            spreadsheet.protectSheet('Sheet2', { selectCells: true, formatRows: true, formatColumns: true, formatCells: true });
            helper.triggerKeyNativeEvent(113, false, true);
            setTimeout(() => {
