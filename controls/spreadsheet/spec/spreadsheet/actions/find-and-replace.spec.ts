...
import { Spreadsheet, SpreadsheetModel, SheetModel, DialogBeforeOpenEventArgs, HyperlinkModel, FindOptions } from '../../../src/index';
...
describe('Find & Replace ->', () => {
    ...
    describe('UI Interaction ->', () => {
        ...
        it('Checking GoTo dialog box when click keyboardshortcut key twice', function (done) {
            helper.triggerKeyNativeEvent(71, true);
            setTimeout(function () {
                helper.setAnimationToNone('.e-goto-dlg.e-dialog');
                helper.triggerKeyNativeEvent(71, true);
    ...
    describe('EJ2-878074 ->', () => {
        beforeEach((done: Function) => {
            helper.initializeSpreadsheet({ allowScrolling: false, sheets: [{ ranges: [{ dataSource: defaultData }] }, {}] }, done);
        });
        afterEach(() => {
            helper.invoke('destroy');
        });
        it('Script error occurs while while using `Find` method in sample', (done: Function) => {
            const spreadsheet: Spreadsheet = helper.getInstance();
            expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].activeCell).toEqual('A1');
            const findOption: FindOptions = {
                value: "Sneakers", sheetIndex: 0, findOpt: "next", mode: "Sheet", isCSen: false,
                isEMatch: false, searchBy: "By Row"
            };
            helper.invoke('find', [findOption]);
            expect(spreadsheet.activeSheetIndex).toEqual(0);
            expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].activeCell).toEqual('A7');
            done();
        });
    });
