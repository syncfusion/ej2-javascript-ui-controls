...
describe('Spreadsheet sorting module ->', () => {
    ...
    describe('EJ2-914385 -> Providing support for updating cell reference while using column validation', () => {
        beforeAll((done: Function) => {
            helper.initializeSpreadsheet({ sheets: [{ ranges: [{ dataSource: defaultData }] }] }, done);
        });
        afterAll(() => {
        ...
        it('Cell validation checking with Sorting - ', (done: Function) => {
            const spreadsheet: Spreadsheet = helper.getInstance();
            helper.getInstance().addDataValidation({ type: 'Custom', value1: '=D2', ignoreBlank: true }, 'E2:E10');
            expect(spreadsheet.sheets[0].rows[1].cells[4].validation.value1).toBe('=D2');
            ...
            expect(spreadsheet.sheets[0].rows[2].cells[4].value.toString()).toBe('30');
            expect(spreadsheet.sheets[0].rows[3].cells[4].validation.value1).toBe('=D4');
            expect(spreadsheet.sheets[0].rows[3].cells[4].value.toString()).toBe('15');
            helper.invoke('selectRange', ['E:E']);
            helper.invoke('sort', [{ sortDescriptors: { order: 'Descending' } }]).then((args: SortEventArgs) => {
                expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[1].cells[4].validation.value1).toBe('=D2');
                expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[1].cells[4].value.toString()).toBe('30');
                expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[2].cells[4].validation.value1).toBe('=D3');
                expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[2].cells[4].value.toString()).toBe('30');
                expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[3].cells[4].validation.value1).toBe('=D4');
                expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[3].cells[4].value.toString()).toBe('20');
                helper.invoke('selectRange', ['E:E']);
                helper.invoke('sort', [{ sortDescriptors: { order: 'Ascending' } }]).then((args: SortEventArgs) => {
                    expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[1].cells[4].validation.value1).toBe('=D2');
                    expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[1].cells[4].value.toString()).toBe('10');
                    expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[2].cells[4].validation.value1).toBe('=D3');
                    expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[2].cells[4].value.toString()).toBe('10');
                    expect(spreadsheet.sheets[spreadsheet.activeSheetIndex].rows[3].cells[4].validation.value1).toBe('=D4');
