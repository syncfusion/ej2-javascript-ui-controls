...
diagram = new Diagram({
    width: '850px', height: '1000px', nodes: [node, node2, node3, {
        id: 'nativenode', width: 150, height: 100, offsetX: 400, offsetY: 300, style: { fill: 'none' },
        shape: {
...
document.getElementById('print').onclick = () => {
    ...
    if (options.region === 'CustomBounds') {
        options.bounds = new Rect(100, 100, 200, 200);
    }
    ...
    let callback: Ajax = new Ajax(
        'http://localhost:65181/Diagram/DiagramFeatures.aspx/GenerateDocument', 'POST', false, 'application/json; charset=utf-8'
    );
    let contentData: string = JSON.stringify(jsonResult);
    callback.data = contentData;
    let tempObject = diagram;
    callback.onSuccess = (imageData: string): void => {
        let content: { d: string } = JSON.parse(imageData);
        diagram.printImage((content).d, options);
    };
...
document.getElementById('exportTypes').onchange = () => {
    ...
    callback.onSuccess = (imageData: string): void => {
        let content: { d: string } = JSON.parse(imageData);
        diagram.exportImage((content).d, options);
    };
    callback.send(contentData).then();
};
