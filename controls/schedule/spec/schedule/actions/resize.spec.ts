...
xdescribe('Timeline view events resizing', () => {
    ...
    describe('Resize does not work properly in TimelineMonth', () => {
        let schObj: Schedule;
        const appointmentData = [{
            Id: 1,
            Subject: 'Conference',
            StartTime: new Date(2018, 6, 14, 0, 0),
            EndTime: new Date(2018, 6, 15, 0, 0),
            IsAllDay: true
        }];
        beforeAll((done: DoneFn) => {
            const schOptions: ScheduleModel = {
        ...
        it('should resize appointment properly', (done: DoneFn) => {
            schObj.dataBound = () => {
                const resizeElement: HTMLElement = schObj.element.querySelector('[data-id="Appointment_1"]') as HTMLElement;
                expect(resizeElement.offsetWidth).toEqual(138);
                done();
            };
            const resizeElement: HTMLElement = schObj.element.querySelector('[data-id="Appointment_1"]') as HTMLElement;
            expect(resizeElement.offsetWidth).toEqual(68);
            ...
            const cloneElement: HTMLElement = schObj.element.querySelector('.e-resize-clone') as HTMLElement;
            expect(cloneElement).toBeTruthy();
            expect(cloneElement.offsetWidth).toEqual(138);
            triggerMouseEvent(resizeHandler, 'mouseup');
            done();
        });
    });
    describe('Ensure Resize in TimelineDay', () => {
        let schObj: Schedule;
        const appointmentData = [{
            Id: 1,
            Subject: 'Conference',
            StartTime: new Date(2018, 6, 1, 0, 0),
            EndTime: new Date(2018, 6, 1, 1, 0),
            IsAllDay: false
        }];
        beforeAll((done: DoneFn) => {
            const schOptions: ScheduleModel = {
        ...
        it('should resize appointment properly', (done: DoneFn) => {
            ...
            const cloneElement: HTMLElement = schObj.element.querySelector('.e-resize-clone') as HTMLElement;
            expect(cloneElement).toBeTruthy();
            expect(cloneElement.offsetWidth).toEqual(150);
            triggerMouseEvent(resizeHandler, 'mouseup');
            done();
        });
    });
