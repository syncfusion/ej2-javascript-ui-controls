...
export class KeyboardInteraction {
    ...
    public onCellMouseDown(e: Event & MouseEvent): void {
        ...
        if (this.parent.activeViewOptions.readonly || this.parent.options.currentView === 'MonthAgenda' || !this.initialTarget) { return; }
        if ((e.target as HTMLElement).classList.contains(cls.WORK_CELLS_CLASS) && e.which !== 3) {
        ...
        if ((e.target as HTMLElement).classList.contains(cls.ALLDAY_CELLS_CLASS) && e.which !== 3) {
            this.parent.removeSelectedClass();
    ...
    private processViewNavigation(e: KeyboardEventArgs): void {
        ...
        let views: HTMLElement[] = [].slice.call(this.parent.element.querySelectorAll('.e-schedule-toolbar .e-toolbar-item.e-views'));
