...
import { HeaderRowsModel } from '../base/interface';
...
export class DragAndDrop extends ActionBase {
    ...
    public wireDragEvent(element: HTMLElement): void {
        if ((element as EJ2Instance).ej2_instances) {
            let dragInstance: Draggable[] = [].slice.call((element as EJ2Instance).ej2_instances);
            dragInstance.forEach((instance: Draggable) => instance.destroy());
        }
        let dragObj: Draggable = new Draggable(element, {
            ...
            dragArea: (this.parent.options.eventDragArea) ?
                document.querySelector(this.parent.options.eventDragArea) as HTMLElement :
    ...
    private dragStart(e: MouseEvent & TouchEvent & BlazorDragEventArgs & DragEventArgs): void {
        ...
        let args: string = JSON.stringify(dragArgs);
        // tslint:disable-next-line:no-any
        (this.parent as any).dotNetRef.invokeMethodAsync('OnDragStart', args, eventGuid).then((dragData: { [key: string]: Object }) => {
            let dragEventArgs: DragEventArgs = dragData.dragEventArgs;
            this.actionObj.isAllDay = dragData.isAllDay as boolean;
            this.actionObj.event = extend({}, dragData, null, true) as { [key: string]: Object };
            delete (this.actionObj.event.dragEventArgs);
            ...
            if ((this.parent.isTimelineView() || !this.parent.activeViewOptions.timeScale.enable)) {
                ...
                this.isTimelineDayProcess = !this.parent.activeViewOptions.timeScale.enable || this.isHeaderRows ||
                    this.parent.options.currentView === 'TimelineMonth' || (rows.length > 0 && rows[rows.length - 1].option === 'Date');
    ...
    private drag(e: MouseEvent & TouchEvent): void {
        ...
        this.isAllDayDrag = (this.parent.activeViewOptions.timeScale.enable) ?
            this.actionObj.clone.classList.contains(cls.ALLDAY_APPOINTMENT_CLASS) :
            this.actionObj.event.isAllDay as boolean;
        ...
        if ((this.parent.options.currentView === 'Month' || this.isAllDayDrag) && this.daysVariation < 0) {
            ...
            if (!isNullOrUndefined(date)) {
                ...
                let startDate: Date = util.resetTime(eventObj.startTime as Date);
    ...
    private calculateMinutesDiff(eventObj: { [key: string]: Object }): void {
        ...
        let startDate: Date = eventObj.startTime as Date;
    ...
    private morePopupEventDragging(e: MouseEvent & TouchEvent): void {
        ...
        let eventObj: { [key: string]: Object } = extend({}, this.actionObj.event, null, true) as { [key: string]: Object };
        let eventDuration: number = (<Date>eventObj.endTime).getTime() -
            (<Date>eventObj.startTime).getTime();
    ...
    private calculateVerticalTime(e: MouseEvent & TouchEvent): void {
        ...
        let dragArea: HTMLElement = this.parent.element.querySelector('.' + cls.CONTENT_WRAP_CLASS) as HTMLElement;
        let eventObj: { [key: string]: Object } = extend({}, this.actionObj.event, null, true) as { [key: string]: Object };
        let eventStart: Date = eventObj.startTime as Date;
        let eventEnd: Date = eventObj.endTime as Date;
    ...
    private updateAllDayEvents(startDate: Date, endDate: Date, colIndex: number): void {
        ...
        let query: string = '.e-all-day-cells[data-date="' +
            this.parent.getMsFromDate(<Date>events[0].startTime) + '"]';
        if (this.parent.activeViewOptions.group.resources.length > 0) {
    ...
    private calculateVerticalDate(e: MouseEvent & TouchEvent): void {
        ...
        if (!isNullOrUndefined(td)) {
            ...
            if (!isNullOrUndefined(currentDate)) {
                ...
                let dragEnd: Date = new Date(dragStart.getTime());
                let startTimeDiff: number = (eventObj.startTime as Date).getTime() -
                    (util.resetTime(eventObj.startTime as Date)).getTime();
    ...
    private calculateTimelineTime(e: MouseEvent & TouchEvent): void {
        ...
        let eventDuration: number = (<Date>eventObj.endTime).getTime() -
            (<Date>eventObj.startTime).getTime();
        let offsetLeft: number = this.parent.options.enableRtl ?
        ...
        if (this.isTimelineDayProcess) {
            let eventSrt: Date = eventObj.startTime as Date;
        ...
        let startTime: Date = this.getStartTime(events[0], eventData);
        let endTime: Date = this.getEndTime(events[0], eventData);
        let width: number = this.timelineEventModule.
            getEventWidth(startTime, endTime, eventObj.isAllDay as boolean, eventData.count as number);
        let day: number = this.parent.getIndexOfDate(this.timelineEventModule.dateRender, util.resetTime(new Date(startTime.getTime())));
        day = day < 0 ? 0 : day;
        let left: number =
            this.timelineEventModule.getPosition(startTime, endTime, eventObj.isAllDay as boolean, day);
        if (this.parent.options.enableRtl) {
    ...
    public getStartTime(event: { [key: string]: Object }, eventData: { [key: string]: Object }): Date {
        ...
        let schedule: { [key: string]: Date } = util.
    ...
    private getNextDay(startTime: Date, eventData: { [key: string]: Object }): Date {
        let startDate: Date;
        for (let i: number = 1; i <= this.timelineEventModule.dateRender.length; i++) {
    ...
    private calculateResourceGroupingPosition(e: MouseEvent & TouchEvent): void {
        ...
        let rowHeight: number = (this.parent.options.rowAutoHeight) ?
