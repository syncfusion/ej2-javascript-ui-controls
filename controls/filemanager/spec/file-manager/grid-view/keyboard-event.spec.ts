...
describe('FileManager control Grid view', () => {
    ...
    describe('for details View', () => {
        ...
        describe('focus regression tests - Details view', () => {
            it('Context menu closed with Esc should return focus to row', (done: Function) => {
                const rows: any = document.getElementById('file_grid').querySelectorAll('tr.e-row');
                const idx = rows.length > 1 ? 1 : 0;
                const target: HTMLElement = rows[idx] as HTMLElement;
                const contextEvent = new MouseEvent('contextmenu', { bubbles: true, cancelable: true });
                if (target) target.dispatchEvent(contextEvent);
                setTimeout(function () {
                    const escEvent = new KeyboardEvent('keydown', { key: 'Escape', bubbles: true, cancelable: true });
                    document.dispatchEvent(escEvent);
                    setTimeout(function () {
                        const hasFocus = target && (target.classList.contains('e-selectionbackground') ||
                            target.classList.contains('e-focused') ||
            ...
            it('context menu Details → close dialog should return focus', (done: Function) => {
                const rows: any = document.getElementById('file_grid').querySelectorAll('tr.e-row');
                const idx = rows.length > 2 ? 2 : 0;
                const target: HTMLElement = rows[idx] as HTMLElement;
                const contextEvent = new MouseEvent('contextmenu', { bubbles: true, cancelable: true });
                if (target) target.dispatchEvent(contextEvent);
                setTimeout(function () {
                    const menu = document.getElementById(feObj.element.id + '_contextmenu');
                    const detailsItem = menu && (menu.querySelector('[data-uid*="details"],li[data-uid*="Details"]') as HTMLElement);
                    if (detailsItem) detailsItem.click();
                    setTimeout(function () {
                        const closeBtn = document.querySelector('.e-dlg-closeicon-btn') as HTMLElement;
            ...
            it('open particular folder via keyboard then PageDown should keep first item focused', (done: Function) => {
                const rows: any = document.getElementById('file_grid').querySelectorAll('tr.e-row');
                const folderRow: HTMLElement = rows[0] as HTMLElement;
                keyboardEventArgs.action = 'enter';
                (feObj.detailsviewModule.gridObj as any).selectRow(0);
                keyboardEventArgs.target = folderRow;
                (feObj.detailsviewModule as any).keyupHandler(keyboardEventArgs);
                (this as any).request = jasmine.Ajax.requests.mostRecent();
                (this as any).request.respondWith({ status: 200, responseText: JSON.stringify(data1) });
                setTimeout(function () {
                    const newRows: any = document.getElementById('file_grid').querySelectorAll('tr.e-row');
                    const firstRow: HTMLElement = newRows[0] as HTMLElement;
                    (feObj.detailsviewModule as any).addFocus(0);
                    const pd = new KeyboardEvent('keydown', { key: 'PageDown', bubbles: true, cancelable: true });
                    document.dispatchEvent(pd);
                    setTimeout(function () {
                        const hasFocus = firstRow && (firstRow.classList.contains('e-selectionbackground') ||
                            firstRow.classList.contains('e-focused') ||
                            firstRow.getAttribute('aria-selected') === 'true');
                        expect(hasFocus).toBe(true);
                        done();
                    }, 200);
                }, 400);
            ...
            it('after delete, focus should move to a remaining row', (done: Function) => {
                const rowsBefore: any = document.getElementById('file_grid').querySelectorAll('tr.e-row');
                const idx = rowsBefore.length > 1 ? 1 : 0;
                const target: HTMLElement = rowsBefore[idx] as HTMLElement;
                keyboardEventArgs.action = 'del';
                (feObj.detailsviewModule.gridObj as any).selectRow(idx);
                keyboardEventArgs.target = target;
                (feObj.detailsviewModule as any).keyupHandler(keyboardEventArgs);
                setTimeout(function () {
                    const okBtn = document.querySelector('#file_dialog .e-footer-content .e-primary') as HTMLElement;
                    if (okBtn) okBtn.click();
                    (this as any).request = jasmine.Ajax.requests.mostRecent();
                    (this as any).request.respondWith({ status: 200, responseText: JSON.stringify(data12) });
                    setTimeout(function () {
                        const rowsAfter: any = document.getElementById('file_grid').querySelectorAll('tr.e-row');
                        const hasFocus = Array.from(rowsAfter).some((row: Element) =>
                            row.classList.contains('e-selectionbackground') || row.classList.contains('e-focused') || row.getAttribute('aria-selected') === 'true');
                        expect(hasFocus).toBe(true);
                        done();
                    }, 400);
                }, 200);
