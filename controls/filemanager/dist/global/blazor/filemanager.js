...
function updatePath(node, data, instance) {
    var text = sf.base.getValue('name', data);
    var id = node.getAttribute('data-id');
    var newText = sf.base.isNullOrUndefined(id) ? text : id;
...
function generatePath(parent) {
    ...
    for (i; i < parent.pathId.length; i++) {
        var data = sf.base.getValue(parent.pathId[i], parent.feParent);
        newPath += sf.base.getValue(key, data) + '/';
    }
...
function activeElement(action, parent) {
    ...
    while (i < parent.activeRecords.length) {
        ...
        parent.selectedNodes.push(sf.base.getValue('name', parent.activeRecords[i]));
        i++;
    }
...
function getPathObject(parent) {
    return sf.base.getValue(parent.pathId[parent.pathId.length - 1], parent.feParent);
}
// Copy files
...
function fileType(file) {
    var isFile$$1 = sf.base.getValue('isFile', file);
    if (!isFile$$1) {
        return FOLDER;
    }
    ...
    var filetype = sf.base.getValue('type', file);
...
function getImageUrl(parent, item) {
    ...
    var imgUrl;
    var fileName = sf.base.getValue('name', item);
    var fPath = sf.base.getValue('filterPath', item);
    if (parent.hasId) {
        var imgId = sf.base.getValue('id', item);
        imgUrl = baseUrl + '?path=' + parent.path + '&id=' + imgId;
    }
    else if (!sf.base.isNullOrUndefined(fPath)) {
...
function getFullPath(parent, data, path) {
    var filePath = sf.base.getValue(parent.hasId ? 'id' : 'name', data) + '/';
    var fPath = sf.base.getValue(parent.hasId ? 'filterId' : 'filterPath', data);
    if (!sf.base.isNullOrUndefined(fPath)) {
...
function getName(parent, data) {
    var name = sf.base.getValue('name', data);
    var fPath = sf.base.getValue('filterPath', data);
    if ((parent.breadcrumbbarModule.searchObj.element.value !== '' || parent.isFiltered) && !sf.base.isNullOrUndefined(fPath)) {
...
function getObject(parent, key, value) {
    var currFiles = sf.base.getValue(parent.pathId[parent.pathId.length - 1], parent.feFiles);
    var query = new sf.data.Query().where(key, 'equal', value);
    var lists = new sf.data.DataManager(currFiles).executeLocal(query);
    return lists[0];
}
function createEmptyElement(parent, element, args) {
    var top;
    var layoutElement = sf.base.select('#' + parent.element.id + LAYOUT_ID, parent.element);
    var addressBarHeight = sf.base.select('#' + parent.element.id + BREADCRUMBBAR_ID, layoutElement).offsetHeight;
    top = layoutElement.offsetHeight - addressBarHeight;
    if (parent.view === 'Details') {
        top = top - sf.base.select('.' + GRID_HEADER, layoutElement).offsetHeight;
    }
    if (sf.base.isNullOrUndefined(element.querySelector('.' + EMPTY))) {
        var emptyDiv = sf.base.createElement('div', { className: EMPTY });
        var emptyFolder = sf.base.createElement('div', { className: LARGE_EMPTY_FOLDER });
        var emptyEle = sf.base.createElement('div', { className: EMPTY_CONTENT });
        var dragFile = sf.base.createElement('div', { className: EMPTY_INNER_CONTENT });
        if (parent.view === 'Details') {
    ...
    if (element.querySelector('.' + EMPTY)) {
        if (!sf.base.isNullOrUndefined(args.error)) {
    ...
    var eDiv = sf.base.select('.' + EMPTY, element);
    top = (top - eDiv.offsetHeight) / 2;
    eDiv.style.marginTop = top + 'px';
}
function getDirectories(files) {
    return new sf.data.DataManager(files).executeLocal(new sf.data.Query().where(isFile, 'equal', false, false));
}
function setNodeId(result, rootId) {
    var dirs = getDirectories(result.files);
    for (var i = 0, len = dirs.length; i < len; i++) {
        sf.base.setValue('_fm_id', rootId + '_' + i, dirs[i]);
    }
}
function setDateObject(args) {
    for (var i = 0; i < args.length; i++) {
        sf.base.setValue('_fm_created', new Date(sf.base.getValue('dateCreated', args[i])), args[i]);
        sf.base.setValue('_fm_modified', new Date(sf.base.getValue('dateModified', args[i])), args[i]);
    }
}
...
function getCssClass(parent, css) {
    ...
    cssClass = (sf.base.isNullOrUndefined(cssClass) || cssClass === '') ? css : (cssClass + ' ' + css);
    return cssClass;
}
...
function setNextPath(parent, path) {
    ...
    var root = sf.base.getValue(parent.pathId[0], parent.feParent);
    var key = sf.base.isNullOrUndefined(sf.base.getValue('id', root)) ? 'name' : 'id';
    for (var i = currfolders.length - 1, len = folders.length - 1; i < len; i++) {
        ...
        var id = sf.base.getValue('_fm_id', data);
        parent.setProperties({ path: newPath }, true);
        parent.pathId.push(id);
        parent.itemData = [data];
        parent.pathNames.push(sf.base.getValue('name', data));
...
function getDirectoryPath(parent, args) {
    var filePath = sf.base.getValue(parent.hasId ? 'id' : 'name', args.cwd) + '/';
    var fPath = sf.base.getValue(parent.hasId ? 'filterId' : 'filterPath', args.cwd);
    if (!sf.base.isNullOrUndefined(fPath)) {
        if (fPath === '') {
...
function readDropPath(parent) {
    var pathId = sf.base.getValue('_fm_id', parent.dropData);
    ...
    if (parent.isPathDrag) {
        ...
        if (parent.navigationpaneModule) {
            var node = sf.base.select('[data-uid="' + pathId + '"]', parent.navigationpaneModule.treeObj.element);
...
function getDuplicateData(parent, name) {
    ...
    for (var i = 0; i < records.length; i++) {
        if (sf.base.getValue('name', records[i]) === name) {
            data = records[i];
            break;
        }
    }
...
function dragStopHandler(parent, args) {
    ...
    removeBlur(parent);
    parent.uploadObj.dropArea = sf.base.select('#' + parent.element.id + CONTENT_ID, parent.element);
    var virtualEle = sf.base.select('.' + CLONE, parent.element);
    if (virtualEle) {
    ...
    parent.trigger('fileDragStop', dragArgs, function (dragArgs) {
        if (!dragArgs.cancel && !sf.base.isNullOrUndefined(parent.targetModule) && parent.targetModule !== '') {
            dropHandler(parent);
        }
    });
}
function dragStartHandler(parent, args, dragObj) {
    ...
    if (!parent.allowDragAndDrop || ((parent.activeModule === 'navigationpane') &&
        (sf.base.closest(args.element, 'li').getAttribute('data-uid') === parent.pathId[0]))) {
        dragArgs.cancel = true;
    }
    if ((parent.activeModule === 'navigationpane') &&
        (parent.pathId.indexOf(sf.base.closest(args.element, 'li').getAttribute('data-uid')) !== -1)) {
...
function objectToString(data) {
    ...
    for (var i = 0; i < keys.length; i++) {
        if (keys[i] !== 'message') {
            str += (i === 0 ? '' : ', ') + keys[i] + ': ' + sf.base.getValue(keys[i], data);
        }
    }
    return str;
}
function getItemName(parent, data) {
    if (parent.hasId) {
        return sf.base.getValue('id', data);
    }
    return getName(parent, data);
}
function updateRenamingData(parent, data) {
    parent.itemData = [data];
    parent.currentItemText = sf.base.getValue('name', data);
    parent.isFile = sf.base.getValue('isFile', data);
    parent.filterPath = sf.base.getValue('filterPath', data);
}
...
function createDeniedDialog(parent, data, action) {
    ...
    message = (message === '') ? '"' + sf.base.getValue('name', data) + '" is not accessible. you need permission to perform the ' +
...
function hasReadAccess(data) {
    var permission = sf.base.getValue('permission', data);
    return (permission && !sf.base.getValue('read', permission)) ? false : true;
}
function hasEditAccess(data) {
    var permission = sf.base.getValue('permission', data);
    return permission ? ((sf.base.getValue('read', permission) && sf.base.getValue('write', permission))) : true;
}
function hasContentAccess(data) {
    var permission = sf.base.getValue('permission', data);
    return permission ? ((sf.base.getValue('read', permission) && sf.base.getValue('writeContents', permission))) : true;
}
function hasUploadAccess(data) {
    var permission = sf.base.getValue('permission', data);
    return permission ? ((sf.base.getValue('read', permission) && sf.base.getValue('upload', permission))) : true;
}
function hasDownloadAccess(data) {
    var permission = sf.base.getValue('permission', data);
    return permission ? ((sf.base.getValue('read', permission) && sf.base.getValue('download', permission))) : true;
}
...
function read(parent, event, path) {
    ...
    for (var i = 0; i < itemData.length; i++) {
        if (sf.base.isNullOrUndefined(sf.base.getValue('hasChild', itemData[i]))) {
            sf.base.setValue('hasChild', false, itemData[i]);
        }
    }
...
function filter(parent, event) {
    ...
    var filterData;
    filterData = parent.filterData ? sf.base.extend(filterData, data, parent.filterData) : data;
    createAjax(parent, filterData, filterSuccess, event, sf.base.getValue('action', filterData));
}
/**
...
function createAjax(parent, data, fn, event, operation, targetPath) {
    ...
    var eventArgs = { action: sf.base.getValue('action', data), ajaxSettings: ajaxSettings, cancel: false };
    parent.trigger('beforeSend', eventArgs, function (beforeSendArgs) {
        if (!beforeSendArgs.cancel) {
            ...
            var ajax = new sf.base.Ajax({
                url: sf.base.getValue('url', beforeSendArgs.ajaxSettings),
                type: sf.base.getValue('type', beforeSendArgs.ajaxSettings),
                mode: sf.base.getValue('mode', beforeSendArgs.ajaxSettings),
                dataType: sf.base.getValue('dataType', beforeSendArgs.ajaxSettings),
                contentType: sf.base.getValue('contentType', beforeSendArgs.ajaxSettings),
                data: sf.base.getValue('data', beforeSendArgs.ajaxSettings),
                beforeSend: sf.base.getValue('beforeSend', beforeSendArgs.ajaxSettings),
                onSuccess: function (result) {
                    ...
                    if (!sf.base.isNullOrUndefined(result.cwd) && (sf.base.getValue('action', data) === 'read')) {
                        ...
                        if (!sf.base.isNullOrUndefined(result.files) || result.error.code === '401') {
                            if ((event === 'finalize-end' || event === 'initial-end') && parent.pathNames.length === 0) {
                                var root = sf.base.getValue(parent.pathId[0], parent.feParent);
                                parent.pathNames[0] = sf.base.getValue('name', root);
                                parent.hasId = !sf.base.isNullOrUndefined(sf.base.getValue('id', root));
                            }
                    ...
                    if (!sf.base.isNullOrUndefined(result.files)) {
                        // tslint:disable-next-line
                        setDateObject(result.files);
                        for (var i = 0, len = result.files.length; i < len; i++) {
                            var item = result.files[i];
                            sf.base.setValue('_fm_iconClass', fileType(item), item);
                        }
                        if (sf.base.getValue('action', data) === 'read') {
                            setNodeId(result, id);
                            sf.base.setValue(id, result.files, parent.feFiles);
                        }
                    }
                    if (!sf.base.isNullOrUndefined(result.details) && !sf.base.isNullOrUndefined(parent.rootAliasName)) {
                        var rootName = parent.rootAliasName || sf.base.getValue('name', result.details);
                        var location_1 = sf.base.getValue('location', result.details).replace(new RegExp('/', 'g'), '\\');
                        if ((sf.base.getValue('path', data) === '/') || (parent.hasId && sf.base.getValue('path', data).match(/[/]/g).length === 1)) {
                            if (sf.base.getValue('names', data).length === 0) {
                                sf.base.setValue('name', rootName, result.details);
                                location_1 = rootName;
                            }
                            else {
                        ...
                        sf.base.setValue('location', location_1, result.details);
                    }
                    fn(parent, result, event, operation, targetPath);
                    if (!sf.base.isNullOrUndefined(result.files) && (event === 'path-changed' || event === 'finalize-end' || event === 'open-end')) {
                        parent.notify(searchTextChange, result);
                    }
                    if (typeof sf.base.getValue('onSuccess', beforeSendArgs.ajaxSettings) === 'function') {
                        sf.base.getValue('onSuccess', beforeSendArgs.ajaxSettings)();
                    }
                },
                onFailure: function () {
                    var result = {
...
function triggerAjaxFailure(parent, beforeSendArgs, fn, result, event, operation, targetPath) {
    ...
    fn(parent, result, event, operation, targetPath);
    if (typeof sf.base.getValue('onFailure', beforeSendArgs.ajaxSettings) === 'function') {
        sf.base.getValue('onFailure', beforeSendArgs.ajaxSettings)();
    }
}
...
function createSuccess(parent, result, itemName) {
    if (!sf.base.isNullOrUndefined(result.files)) {
        ...
        if (result.error.code === '400') {
            if (parent.dialogObj && parent.dialogObj.visible) {
                var ele = sf.base.select('#newname', parent.dialogObj.element);
...
function renameSuccess(parent, result, path) {
    if (!sf.base.isNullOrUndefined(result.files)) {
        if (!sf.base.isNullOrUndefined(parent.dialogObj)) {
            parent.dialogObj.hide();
        }
        ...
        if (parent.activeModule === 'navigationpane') {
            parent.pathId.pop();
            parent.itemData = [sf.base.getValue(parent.pathId[parent.pathId.length - 1], parent.feParent)];
        ...
        if (result.error.code === '400' && parent.dialogObj && parent.dialogObj.visible) {
            var ele = sf.base.select('#rename', parent.dialogObj.element);
...
parent, result, path, operation) {
    ...
    else if (!result.error && !sf.base.isNullOrUndefined(result.files)) {
...
// tslint:disable-next-line
parent, result, path, operation) {
    if (!sf.base.isNullOrUndefined(result.details)) {
...
function createDialog(parent, text, e, details, replaceItems) {
    ...
    if (sf.base.isNullOrUndefined(parent.dialogObj)) {
        parent.dialogObj = new sf.popups.Dialog({
...
function createExtDialog(parent, text, replaceItems, newPath) {
    ...
    if (sf.base.isNullOrUndefined(parent.extDialogObj)) {
        parent.extDialogObj = new sf.popups.Dialog({
...
function retryDlgClose(parent) {
    ...
    var ele = sf.base.select('.e-dlg-checkbox', parent.extDialogObj.element);
    if (ele) {
        sf.base.remove(ele);
    }
    if (flag) {
...
function onRetryOpen(parent, args) {
    ...
    var dialogEle = sf.base.getValue('element', args);
    var container = sf.base.select('.e-dlg-content', dialogEle);
    ...
    var checkBoxObj = new sf.buttons.CheckBox({
...
function onRenameDialogOpen(parent) {
    var inputEle = sf.base.select('#rename', parent.dialogObj.element);
...
function getOptions(parent, text, e, details, replaceItems) {
    ...
    switch (text) {
        ...
        case 'details':
            ...
            var intl = new sf.base.Internationalization(parent.locale);
            var formattedString = intl.formatDate(new Date(details.modified), { format: 'MMMM dd, yyyy HH:mm:ss' });
            var permission = '';
            if (!sf.base.isNullOrUndefined(details.permission)) {
...
function keydownAction(parent, dialogName, args) {
    var btnElement = sf.base.selectAll('.e-btn', parent.dialogObj.element);
...
function beforeExtOpen(parent, dlgName, args) {
    var btnElement = sf.base.selectAll('.e-btn', parent.extDialogObj.element);
    preventKeydown(btnElement);
...
function onReSubmit(parent) {
    ...
    parent.renamedId = sf.base.getValue('id', parent.itemData[0]);
    if (parent.isFile) {
...
function createImageDialog(parent, header, imageUrl) {
    var content = sf.base.createElement('div', { className: 'e-image-wrap' });
    var image = sf.base.createElement('img', { className: 'e-image', attrs: { src: imageUrl, alt: header } });
    content.appendChild(image);
    if (sf.base.isNullOrUndefined(parent.viewerObj)) {
        parent.viewerObj = new sf.popups.Dialog({
            header: header,
            content: content,
            animationSettings: { effect: 'None' },
            showCloseIcon: true,
...
function updateImage(parent) {
    var content = sf.base.select('.e-dlg-content', parent.viewerObj.element);
    var imgWrap = sf.base.select('.e-image-wrap', parent.viewerObj.element);
...
var LargeIconsView = /** @class */ (function () {
    ...
    function LargeIconsView(parent) {
        ...
        this.parent = parent;
        this.element = sf.base.select('#' + this.parent.element.id + LARGEICON_ID, this.parent.element);
        sf.base.addClass([this.element], LARGE_ICONS);
    ...
    LargeIconsView.prototype.render = function (args) {
        ...
        if (this.parent.view === 'LargeIcons') {
            ...
            var iconsView = sf.base.select('#' + this.parent.element.id + LARGEICON_ID, this.parent.element);
            var ul = sf.base.select('ul', iconsView);
            if (ul) {
                sf.base.remove(ul);
            }
            this.listObj = {
            ...
            this.listElements = sf.lists.ListBase.createListFromJson(sf.base.createElement, this.items, this.listObj);
            this.itemList = Array.prototype.slice.call(sf.base.selectAll('.' + LIST_ITEM, this.listElements));
            ...
            this.wireEvents();
            this.isRendered = true;
            sf.popups.hideSpinner(this.parent.element);
    ...
    LargeIconsView.prototype.createDragObj = function () {
        ...
        if (!this.parent.isMobile && this.listObj) {
            if (this.parent.allowDragAndDrop) {
                ...
                this.dragObj = new sf.base.Draggable(this.listElements, {
                    enableTailMode: true,
    ...
    LargeIconsView.prototype.dragHelper = function (args) {
        ...
        var dragLi = sf.base.closest(dragTarget, '.e-list-item');
        if (!dragLi) {
            return null;
        }
    ...
    LargeIconsView.prototype.onDropInit = function (args) {
        if (this.parent.targetModule === this.getModuleName()) {
            var dropLi = sf.base.closest(args.target, '.e-list-item');
            var cwdData = sf.base.getValue(this.parent.pathId[this.parent.pathId.length - 1], this.parent.feParent);
            if (dropLi) {
    ...
    LargeIconsView.prototype.adjustHeight = function () {
        var pane = sf.base.select('#' + this.parent.element.id + CONTENT_ID, this.parent.element);
        var bar = sf.base.select('#' + this.parent.element.id + BREADCRUMBBAR_ID, this.parent.element);
    ...
    LargeIconsView.prototype.onItemCreated = function (args) {
        ...
        if (!this.parent.showFileExtension && sf.base.getValue('isFile', args.curData)) {
            var textEle = args.item.querySelector('.' + LIST_TEXT);
            var txt = sf.base.getValue('name', args.curData);
            var type = sf.base.getValue('type', args.curData);
    ...
    LargeIconsView.prototype.renderCheckbox = function (args) {
        ...
        var checkElement;
        checkElement = sf.buttons.createCheckBox(sf.base.createElement, false, {
            checked: false,
            cssClass: 'e-small'
        });
    ...
    LargeIconsView.prototype.onLayoutChange = function (args) {
        if (this.parent.view === 'LargeIcons') {
            ...
            this.render(args);
            /* istanbul ignore next */
            if (sf.base.getValue('name', args) === 'layout-change' && this.parent.fileAction === 'move' &&
    ...
    LargeIconsView.prototype.renderList = function (args) {
        ...
        while (i < items.length) {
            ...
            var name_1 = sf.base.getValue('name', items[i]);
            ...
            if (icon === ICON_IMAGE && this.parent.showThumbnail && hasReadAccess(items[i])) {
                ...
                sf.base.setValue('_fm_imageUrl', imgUrl, items[i]);
                sf.base.setValue('_fm_imageAttr', { alt: name_1 }, items[i]);
            }
            else {
                sf.base.setValue('_fm_icon', icon, items[i]);
            }
            sf.base.setValue('_fm_htmlAttr', { class: className, title: name_1 }, items[i]);
            i++;
        }
        return items;
    };
    ...
    LargeIconsView.prototype.onCreateEnd = function (args) {
        ...
        this.clearSelect();
        this.selectItems([sf.base.getValue(this.parent.hasId ? 'id' : 'name', this.parent.createdItem)]);
    ...
    LargeIconsView.prototype.onPathColumn = function () {
        if (this.parent.view === 'LargeIcons' && !sf.base.isNullOrUndefined(this.listObj) &&
    ...
    LargeIconsView.prototype.onMenuItemData = function (args) {
        if (this.parent.activeModule === this.getModuleName()) {
            var ele = sf.base.closest(args.target, 'li');
    ...
    LargeIconsView.prototype.onDetailsInit = function () {
        if (this.parent.activeModule === this.getModuleName()) {
            if (this.parent.selectedItems.length !== 0) {
                ...
                this.parent.itemData = [sf.base.getValue(this.parent.pathId[this.parent.pathId.length - 1], this.parent.feParent)];
            }
        }
    };
    LargeIconsView.prototype.onpasteInit = function () {
        if (this.parent.activeModule === this.getModuleName()) {
            this.parent.itemData = (this.parent.folderPath === '') ? [getPathObject(this.parent)] :
                [this.getItemObject(sf.base.select('.e-active', this.element))];
        }
    };
        ...
        for (var _i = 0, _a = Object.keys(e.newProp); _i < _a.length; _i++) {
            ...
            switch (prop) {
                ...
                case 'selectedItems':
                    ...
                    var currentSelected = sf.base.isNullOrUndefined(this.parent.selectedItems) ? [] : this.parent.selectedItems.slice(0);
    ...
    LargeIconsView.prototype.wireEvents = function () {
        ...
        this.keyboardDownModule = new sf.base.KeyboardEvents(this.element, {
    ...
    LargeIconsView.prototype.onMouseOver = function (e) {
        var targetEle = sf.base.closest(e.target, '.e-list-item');
    ...
    LargeIconsView.prototype.wireClickEvent = function (toBind) {
        if (toBind) {
            var proxy_1 = this;
            this.clickObj = new sf.base.Touch(this.element, {
                tap: function (eve) {
                ...
                tapHold: function (e) {
                    if (proxy_1.parent.isDevice) {
                        ...
                        if (proxy_1.parent.allowMultiSelection) {
                            sf.base.addClass([proxy_1.parent.element], MULTI_SELECT);
                        }
    ...
    LargeIconsView.prototype.doTapAction = function (eve) {
        ...
        var item = sf.base.closest(target, '.' + LIST_ITEM);
        if (this.multiSelect || target.classList.contains(LIST_PARENT) || sf.base.isNullOrUndefined(item)) {
    ...
    LargeIconsView.prototype.doSelection = function (target, e) {
        ...
        this.parent.isFile = false;
        var action = 'select';
        if (e.which === 3 && !sf.base.isNullOrUndefined(item) && item.classList.contains(ACTIVE)) {
            this.updateType(item);
            return;
        }
        else if (!sf.base.isNullOrUndefined(item)) {
    ...
    LargeIconsView.prototype.resetMultiSelect = function () {
        ...
        sf.base.removeClass([this.parent.element], MULTI_SELECT);
    };
        ...
        if (!sf.base.isNullOrUndefined(item)) {
            ...
            this.parent.trigger('fileOpen', eventArgs, function (fileOpenArgs) {
                if (!fileOpenArgs.cancel) {
                    var text = sf.base.getValue('name', details_1);
                    if (!_this.parent.isFile) {
                        var val = _this.parent.breadcrumbbarModule.searchObj.element.value;
                        if (val === '' && !_this.parent.isFiltered) {
                            var id = sf.base.getValue('id', details_1);
                            var newPath = _this.parent.path + (sf.base.isNullOrUndefined(id) ? text : id) + '/';
                            _this.parent.setProperties({ path: newPath }, true);
                            _this.parent.pathNames.push(text);
                            _this.parent.pathId.push(sf.base.getValue('_fm_id', details_1));
    ...
    LargeIconsView.prototype.updateType = function (item) {
        var folder = sf.base.select('.' + FOLDER, item);
        this.parent.isFile = sf.base.isNullOrUndefined(folder) ? true : false;
    };
    /* istanbul ignore next */
    // tslint:disable-next-line:max-func-body-length
    ...
    LargeIconsView.prototype.keyActionHandler = function (e) {
        ...
        switch (e.action) {
            ...
            case 'home':
                ...
                break;
            case 'tab':
                if (!sf.base.isNullOrUndefined(fItem)) {
                    this.addFocus(fItem);
                }
                else if (!sf.base.isNullOrUndefined(firstItem)) {
    ...
    LargeIconsView.prototype.updateRenameData = function () {
        var item = sf.base.select('.' + LIST_ITEM + '.' + ACTIVE, this.element);
    ...
    LargeIconsView.prototype.getFocusedItem = function () {
        return sf.base.select('.' + LIST_ITEM + '.' + FOCUS, this.element);
    };
    LargeIconsView.prototype.getActiveItem = function () {
        return sf.base.select('.' + LIST_ITEM + '.' + ACTIVE, this.element);
    };
    ...
    LargeIconsView.prototype.getNextItem = function (li, isTowards, perRow) {
        ...
        } while (!sf.base.isVisible(nextItem));
        return nextItem;
    };
    ...
    LargeIconsView.prototype.spaceKey = function (fItem) {
        if (!sf.base.isNullOrUndefined(fItem) && !fItem.classList.contains(ACTIVE)) {
    ...
    LargeIconsView.prototype.ctrlAKey = function (firstItem, lastItem) {
        if (this.parent.allowMultiSelection && !sf.base.isNullOrUndefined(firstItem)) {
    ...
    LargeIconsView.prototype.csEndKey = function (lastItem, e) {
        ...
        else if (!sf.base.isNullOrUndefined(lastItem)) {
    ...
    LargeIconsView.prototype.csDownKey = function (fItem, e) {
        if (!this.parent.allowMultiSelection) {
            ...
            if (!sf.base.isNullOrUndefined(dItem)) {
    ...
    LargeIconsView.prototype.csLeftKey = function (fItem, e) {
        if (!this.parent.allowMultiSelection) {
            ...
            if (!sf.base.isNullOrUndefined(lItem)) {
    ...
    LargeIconsView.prototype.csRightKey = function (fItem, e) {
        if (!this.parent.allowMultiSelection) {
            ...
            if (!sf.base.isNullOrUndefined(rItem)) {
    ...
    LargeIconsView.prototype.csUpKey = function (fItem, e) {
        if (!this.parent.allowMultiSelection) {
            ...
            if (!sf.base.isNullOrUndefined(uItem)) {
    ...
    LargeIconsView.prototype.addActive = function (nextItem) {
        if (!sf.base.isNullOrUndefined(nextItem)) {
            if (!nextItem.classList.contains(ACTIVE)) {
                ...
                sf.base.addClass([nextItem], [ACTIVE]);
    ...
    LargeIconsView.prototype.removeActive = function (preItem) {
        if (!sf.base.isNullOrUndefined(preItem)) {
            sf.base.removeClass([preItem], [ACTIVE]);
            if (this.parent.allowMultiSelection) {
    ...
    LargeIconsView.prototype.addFocus = function (item) {
        ...
        if (fItem) {
            ...
            sf.base.removeClass([fItem], [FOCUS]);
        }
        sf.base.addClass([item], [FOCUS]);
    ...
    LargeIconsView.prototype.checkState = function (item, toCheck) {
        ...
        var checkEle = sf.base.select('.' + FRAME, item);
        if (sf.base.isNullOrUndefined(checkEle)) {
            return;
        }
        if (toCheck) {
            if (!checkEle.classList.contains(CHECK)) {
                sf.base.addClass([checkEle], CHECK);
                sf.base.closest(checkEle, '.' + CB_WRAP).setAttribute('aria-checked', 'true');
            }
        }
        else {
            if (checkEle.classList.contains(CHECK)) {
                sf.base.removeClass([checkEle], CHECK);
                sf.base.closest(checkEle, '.' + CB_WRAP).setAttribute('aria-checked', 'false');
            }
        }
    };
    LargeIconsView.prototype.clearSelect = function () {
        var eles = Array.prototype.slice.call(sf.base.selectAll('.' + ACTIVE, this.listElements));
    ...
    LargeIconsView.prototype.getIndexes = function (items, byId) {
        ...
        for (var i = 0, len = this.items.length; i < len; i++) {
            if (items.indexOf(sf.base.getValue(filter$$1, this.items[i])) !== -1) {
                indexes.push(i);
            }
        }
        return indexes;
    ...
    LargeIconsView.prototype.addSelection = function (data) {
        ...
        if (this.parent.hasId) {
            resultData = new sf.data.DataManager(this.items).
                executeLocal(new sf.data.Query().where('id', 'equal', this.parent.renamedId, false));
        }
        else {
            var newData = new sf.data.DataManager(this.items).
                executeLocal(new sf.data.Query().where('name', 'equal', sf.base.getValue('name', data), false));
            if (newData.length > 0) {
                resultData = new sf.data.DataManager(newData).
                    executeLocal(new sf.data.Query().where('filterPath', 'equal', this.parent.filterPath, false));
            }
        }
    ...
    LargeIconsView.prototype.updateSelectedData = function () {
        var data = [];
        var items = sf.base.selectAll('.' + LIST_ITEM + '.' + ACTIVE, this.element);
        for (var i = 0; i < items.length; i++) {
    ...
    LargeIconsView.prototype.onMethodCall = function (args) {
        ...
        var action = sf.base.getValue('action', args);
        switch (action) {
            case 'deleteFiles':
                this.deleteFiles(sf.base.getValue('ids', args));
                break;
            case 'downloadFiles':
                this.downloadFiles(sf.base.getValue('ids', args));
                break;
            case 'openFile':
                this.openFile(sf.base.getValue('id', args));
                break;
            case 'renameFile':
                this.isInteraction = false;
                this.isInteracted = false;
                this.renameFile(sf.base.getValue('id', args), sf.base.getValue('newName', args));
                break;
            case 'createFolder':
                this.isInteraction = false;
                this.isInteracted = false;
    ...
    LargeIconsView.prototype.getItemsIndex = function (items) {
        ...
        if (this.parent.hasId || !isFilter) {
            for (var i = 0, len = this.items.length; i < len; i++) {
                if (items.indexOf(sf.base.getValue(filterName, this.items[i])) !== -1) {
                    indexes.push(i);
                }
            }
        }
            ...
            for (var i = 0, len = this.items.length; i < len; i++) {
                var name_2 = sf.base.getValue('filterPath', this.items[i]) + sf.base.getValue('name', this.items[i]);
    ...
    LargeIconsView.prototype.deleteFiles = function (ids) {
        this.parent.activeModule = 'largeiconsview';
        if (sf.base.isNullOrUndefined(ids)) {
            this.performDelete();
            return;
        }
    ...
    LargeIconsView.prototype.renameFile = function (id, name) {
        ...
        if (indexes.length > 0) {
            ...
            if (sf.base.isNullOrUndefined(name)) {
                doRename(this.parent);
            }
            else {
...
var BreadCrumbBar = /** @class */ (function () {
        ...
        for (var _i = 0, _a = Object.keys(e.newProp); _i < _a.length; _i++) {
            ...
            switch (prop) {
                case 'searchSettings':
                    ...
                    if (!sf.base.isNullOrUndefined(value.allowSearchOnTyping)) {
                    ...
                    if (this.parent.breadcrumbbarModule.searchObj.value && this.parent.breadcrumbbarModule.searchObj.value !== '' &&
                        !(!sf.base.isNullOrUndefined(value.allowSearchOnTyping) && sf.base.isNullOrUndefined(value.filterType) &&
                            sf.base.isNullOrUndefined(value.ignoreCase))) {
    ...
    BreadCrumbBar.prototype.onPathChange = function () {
        ...
        if (pathNames.length > 0) {
            ...
            for (var i = 0; i < pathNamesLen; i++) {
                ...
                if (i !== 0) {
                    var icon = sf.base.createElement('span', { className: ICONS });
                    addressbarLI.appendChild(icon);
                }
                if (pathNamesLen - i !== 1) {
                    addressATag = sf.base.createElement('a', { className: LIST_TEXT });
                    addressbarLI.setAttribute('tabindex', '0');
                }
                else {
                    addressATag = sf.base.createElement('span', { className: LIST_TEXT });
                }
                id = '';
            ...
            if (!sf.base.isNullOrUndefined(ulElement)) {
                if (!sf.base.isNullOrUndefined(this.subMenuObj)) {
                    this.subMenuObj.destroy();
                }
                sf.base.remove(ulElement);
            }
    ...
    BreadCrumbBar.prototype.updateBreadCrumbBar = function (addresBarUL) {
        ...
        var searchInput = sf.base.createElement('input', { id: id,
        ...
        if (sf.base.isNullOrUndefined(searchEle)) {
            this.parent.breadCrumbBarNavigation.appendChild(searchContainer);
            var span = sf.base.createElement('span', { className: 'e-icons e-fe-search' });
            sf.base.EventHandler.add(span, 'click', this.onShowInput, this);
            searchInput.parentElement.insertBefore(span, searchInput);
            this.searchObj = new sf.inputs.TextBox({
                value: '',
            ...
            sf.base.EventHandler.add(search$$1, 'mousedown', this.searchChangeHandler.bind(this), this);
            sf.base.EventHandler.add(this.searchObj.element, 'keyup', this.onKeyUp.bind(this), this);
        }
        var searchWrap = this.parent.breadCrumbBarNavigation.querySelector('.e-search-wrap');
        ...
        if (liElementsWidth > breadCrumbBarWidth) {
            ...
            while (i--) {
                ...
                if (diff > 40) {
                    ...
                    this.subMenuObj = new sf.splitbuttons.DropDownButton({
                        items: items,
    ...
    BreadCrumbBar.prototype.onFocus = function () {
        var wrap = sf.base.closest(this.searchObj.element, '.e-search-wrap');
    ...
    BreadCrumbBar.prototype.addSubMenuAttributes = function (args) {
        args.element.setAttribute('data-utext', sf.base.getValue('utext', args.item));
    ...
    BreadCrumbBar.prototype.onShowInput = function () {
        if (this.parent.isMobile) {
            if (this.parent.element.classList.contains(FILTER)) {
                sf.base.removeClass([this.parent.element], FILTER);
                this.searchWrapWidth = null;
            }
            else {
                var searchWrap = this.parent.breadCrumbBarNavigation.querySelector('.e-search-wrap');
                this.searchWrapWidth = searchWrap.offsetWidth;
                sf.base.addClass([this.parent.element], FILTER);
                this.searchObj.element.focus();
            }
        }
    };
    BreadCrumbBar.prototype.updatePath = function (list) {
        var li = sf.base.closest(list, 'li');
    ...
    BreadCrumbBar.prototype.addEventListener = function () {
        this.keyboardModule = new sf.base.KeyboardEvents(this.parent.breadCrumbBarNavigation, {
            keyAction: this.keyActionHandler.bind(this),
            keyConfigs: this.keyConfigs,
            eventName: 'keydown',
        });
        this.parent.on(modelChanged, this.onPropertyChanged, this);
        sf.base.EventHandler.add(this.parent.breadCrumbBarNavigation, 'click', this.addressPathClickHandler, this);
    ...
    BreadCrumbBar.prototype.onDropInit = function (args) {
        if (this.parent.targetModule === this.getModuleName()) {
            ...
            this.parent.dropData = sf.base.getValue(this.parent.pathId[this.parent.pathId.length - 1], this.parent.feParent);
    ...
    BreadCrumbBar.prototype.destroy = function () {
        ...
        if (!sf.base.isNullOrUndefined(this.searchObj)) {
 ...
 * ContextMenu module
 */
var ContextMenu$2 = /** @class */ (function () {
    /**
    ...
    ContextMenu$$1.prototype.render = function () {
        ...
        this.contextMenu = new sf.navigations.ContextMenu({
    ...
    ContextMenu$$1.prototype.onBeforeItemRender = function (args) {
        if (args.item.id === this.getMenuId('largeiconsview')) {
            var iconSpan = sf.base.createElement('span');
    ...
    ContextMenu$$1.prototype.onBeforeOpen = function (args) {
        ...
        this.targetElement = this.parent.view === 'Details' ? sf.base.closest(target, 'tr') : target;
        var view = this.getTargetView(target);
        this.updateActiveModule();
        /* istanbul ignore next */
        if (target.classList.contains(TREE_VIEW) || sf.base.closest(target, 'th') ||
            (sf.base.closest(target, '#' + this.parent.element.id + BREADCRUMBBAR_ID)) ||
            (sf.base.closest(target, '#' + this.parent.element.id + TOOLBAR_ID))) {
            args.cancel = true;
            // tslint:disable-next-line
        }
        ...
        else if (sf.base.closest(target, '.' + EMPTY)) {
            this.setLayoutItem(target);
            // tslint:disable-next-line
        }
            ...
            /* istanbul ignore next */
            // tslint:disable-next-line
            if (this.parent.view === 'LargeIcons' && !sf.base.isNullOrUndefined(sf.base.closest(target, 'li')) && !sf.base.closest(target, '#' + this.parent.element.id + TREE_ID)) {
                var eveArgs = { ctrlKey: true, shiftKey: true };
                if (!sf.base.closest(target, 'li').classList.contains('e-active')) {
                    this.parent.largeiconsviewModule.doSelection(target, eveArgs);
                }
                data = this.parent.visitedData;
                selected = true;
            }
            else if (!sf.base.isNullOrUndefined(sf.base.closest(target, 'tr'))) {
                uid = this.targetElement.getAttribute('data-uid');
                data = this.parent.detailsviewModule.gridObj.getRowObjectFromUID(uid).data;
                if (sf.base.isNullOrUndefined(this.targetElement.getAttribute('aria-selected'))) {
                    /* istanbul ignore next */
                    // tslint:disable-next-line
            ...
            else if (sf.base.closest(target, '#' + this.parent.element.id + TREE_ID)) {
                uid = sf.base.closest(target, 'li').getAttribute('data-uid');
                treeFolder = true;
            }
            /* istanbul ignore next */
            if (selected) {
                if (sf.base.getValue('isFile', data) === true) {
                    this.setFileItem();
                }
                else {
        ...
        var pasteEle = sf.base.select('#' + this.getMenuId('Paste'), this.contextMenu.element);
    ...
    ContextMenu$$1.prototype.updateActiveModule = function () {
        this.parent.activeModule = sf.base.closest(this.menuTarget, '#' + this.parent.element.id + TREE_ID) ?
            'navigationpane' : sf.base.closest(this.menuTarget, '#' + this.parent.element.id + GRID_ID) ?
            'detailsview' : sf.base.closest(this.menuTarget, '#' + this.parent.element.id + LARGEICON_ID) ?
    ...
    ContextMenu$$1.prototype.setLayoutItem = function (target) {
        ...
        if (!this.parent.allowMultiSelection || ((this.parent.view === 'LargeIcons' &&
            (sf.base.closest(target, '#' + this.parent.element.id + LARGEICON_ID).getElementsByClassName(EMPTY).length !== 0))
            || (this.parent.view === 'Details' &&
                (sf.base.closest(target, '#' + this.parent.element.id + GRID_ID).getElementsByClassName(EMPTY).length !== 0)))) {
    ...
    ContextMenu$$1.prototype.checkValidItem = function (nameEle) {
        if (!sf.base.isNullOrUndefined(sf.base.select('#' + this.getMenuId(nameEle), this.currentElement))) {
            return 1;
        }
        else if (!sf.base.isNullOrUndefined(sf.base.select('#' + nameEle, this.currentElement))) {
            return 2;
        }
        else {
            return -1;
    ...
    ContextMenu$$1.prototype.onSelect = function (args) {
        var _this = this;
        if (sf.base.isNullOrUndefined(args.item) || !args.item.id) {
            return;
        }
    ...
    ContextMenu$$1.prototype.addEventListener = function () {
        this.parent.on(destroy, this.destroy, this);
        this.parent.on(modelChanged, this.onPropertyChanged, this);
        this.keyboardModule = new sf.base.KeyboardEvents(this.contextMenu.element, {
            keyAction: this.keyActionHandler.bind(this),
            keyConfigs: this.keyConfigs,
            eventName: 'keydown',
        });
...
var FileManager = /** @class */ (function (_super) {
    ...
    FileManager.prototype.preRender = function () {
        ...
        var ele = sf.base.closest(this.element, '.e-bigger');
        this.isBigger = ele ? true : false;
        this.activeModule = (this.view === 'LargeIcons') ? 'largeiconsview' : 'detailsview';
        sf.popups.createSpinner({ target: this.element }, sf.base.createElement);
        this.addWrapper();
        this.keyConfigs = {
            altN: 'alt+n',
            f5: 'f5',
    ...
    FileManager.prototype.render = function () {
        this.initialize();
        var slItems = sf.base.isNullOrUndefined(this.selectedItems) ? [] :
        ...
        this.adjustHeight();
        if (sf.base.isNullOrUndefined(this.navigationpaneModule)) {
            this.splitterObj.collapse(this.enableRtl ? 1 : 0);
            var bar = sf.base.select('.' + SPLIT_BAR, this.element);
    ...
    FileManager.prototype.ensurePath = function () {
        ...
        if (sf.base.isNullOrUndefined(currentPath)) {
            currentPath = '/';
        }
    ...
    FileManager.prototype.initialize = function () {
        if (this.isMobile) {
            sf.base.addClass([this.element], MOBILE);
        }
        if (this.allowMultiSelection) {
            sf.base.addClass([this.element], CHECK_SELECT);
        }
    ...
    FileManager.prototype.addWrapper = function () {
        ...
        this.splitterObj = new sf.layouts.Splitter({
    ...
    FileManager.prototype.addCssClass = function (oldOne, newOne) {
        if (!sf.base.isNullOrUndefined(oldOne) && oldOne !== '') {
            sf.base.removeClass([this.element], oldOne.split(' '));
        }
        if (!sf.base.isNullOrUndefined(newOne) && newOne !== '') {
            sf.base.addClass([this.element], newOne.split(' '));
        }
    };
    ...
    FileManager.prototype.checkMobile = function () {
        return (/iphone|ipod|android|blackberry|opera|mini|windows\sce|palm|smartphone|iemobile/i.test(sf.base.Browser.userAgent.toLowerCase())
            && /mobile/i.test(sf.base.Browser.userAgent.toLowerCase()));
    };
    FileManager.prototype.renderFileUpload = function () {
        ...
        this.uploadDialogObj = new sf.popups.Dialog({
    ...
    FileManager.prototype.renderUploadBox = function () {
        ...
        this.uploadObj = new sf.inputs.Uploader({
            dropArea: sf.base.select('#' + this.element.id + CONTENT_ID, this.element),
            asyncSettings: {
                saveUrl: uploadUrl,
                removeUrl: uploadUrl
            },
    ...
    FileManager.prototype.onUploading = function (args) {
        ...
        var data = JSON.stringify(sf.base.getValue(this.pathId[this.pathId.length - 1], this.feParent));
        ...
        this.trigger('beforeSend', this.uploadEventArgs, function (uploadEventArgs) {
            args.customFormData = JSON.parse(sf.base.getValue('data', uploadEventArgs.ajaxSettings));
            args.cancel = uploadEventArgs.cancel;
            var eventArgs = {
                cancel: false,
                httpRequest: args.currentRequest
            };
            if (typeof sf.base.getValue('beforeSend', uploadEventArgs.ajaxSettings) === 'function') {
                sf.base.getValue('beforeSend', uploadEventArgs.ajaxSettings)(eventArgs);
                if (sf.base.getValue('cancel', eventArgs)) {
                    args.cancel = sf.base.getValue('cancel', eventArgs);
                }
            }
        });
    };
    ...
    FileManager.prototype.onUploadSuccess = function (files) {
        ...
        this.itemData = [sf.base.getValue(this.pathId[this.pathId.length - 1], this.feParent)];
        read(this, pathChanged, this.path);
        this.onFileUploadSuccess({ count: 1 });
        if (typeof sf.base.getValue('onSuccess', this.uploadEventArgs.ajaxSettings) === 'function') {
            sf.base.getValue('onSuccess', this.uploadEventArgs.ajaxSettings)();
        }
    };
    /* istanbul ignore next */
    FileManager.prototype.onUploadFailure = function (files) {
        var response = sf.base.getValue('response', files);
        var statusText = sf.base.getValue('statusText', response);
        if (statusText !== '') {
            sf.base.setValue('statusText', statusText, files);
        }
        var args = { action: 'Upload', error: files };
        this.trigger('failure', args);
        if (sf.base.getValue('statusCode', response) === 400) {
            this.retryFiles.push(sf.base.getValue('file', files));
            if (!this.isRetryOpened) {
                createExtDialog(this, 'UploadRetry');
            }
        }
        if (typeof sf.base.getValue('onFailure', this.uploadEventArgs.ajaxSettings) === 'function') {
            sf.base.getValue('onFailure', this.uploadEventArgs.ajaxSettings)();
        }
    };
    ...
    FileManager.prototype.addEventListeners = function () {
        ...
        sf.base.EventHandler.add(this.element, 'contextmenu', this.onContextMenu, this);
    };
    FileManager.prototype.removeEventListeners = function () {
        ...
        sf.base.EventHandler.remove(this.element, 'contextmenu', this.onContextMenu);
    };
    FileManager.prototype.onDetailsInit = function () {
        if (sf.base.isNullOrUndefined(this.activeModule)) {
            this.itemData = [sf.base.getValue(this.pathId[this.pathId.length - 1], this.feParent)];
        }
    };
    ...
    FileManager.prototype.keyActionHandler = function (e) {
        switch (e.action) {
            ...
            case 'ctrlU':
                e.preventDefault();
                var uploadEle = sf.base.select('#' + this.element.id + UPLOAD_ID, this.element);
                uploadEle.click();
                break;
        }
    };
            ...
            switch (prop) {
                ...
                case 'allowMultiSelection':
                    if (this.allowMultiSelection) {
                        ...
                        sf.base.removeClass([this.element], CHECK_SELECT);
                    }
                    this.notify(modelChanged, { module: 'common', newProp: newProp, oldProp: oldProp });
                    break;
                case 'cssClass':
                ...
                case 'rootAliasName':
                    ...
                    break;
                case 'height':
                    var height = !sf.base.isNullOrUndefined(newProp.height) ? sf.base.formatUnit(newProp.height) : newProp.height;
                    sf.base.setStyleAttribute(this.element, { 'height': height });
                    this.adjustHeight();
                    this.notify(modelChanged, { module: 'common', newProp: newProp, oldProp: oldProp });
                    break;
                case 'locale':
                    if (!sf.base.isNullOrUndefined(newProp.enableRtl)) {
                ...
                case 'path':
                    ...
                    break;
                case 'searchSettings':
                    if (!sf.base.isNullOrUndefined(newProp.searchSettings.allowSearchOnTyping)) {
                        this.setProperties({ searchSettings: { allowSearchOnTyping: newProp.searchSettings.allowSearchOnTyping } }, true);
                    }
                    if (sf.base.isNullOrUndefined(newProp.searchSettings.ignoreCase)) {
                        this.setProperties({ searchSettings: { ignoreCase: newProp.searchSettings.ignoreCase } }, true);
                    }
                    if (sf.base.isNullOrUndefined(newProp.searchSettings.filterType)) {
                ...
                case 'view':
                    ...
                    break;
                case 'width':
                    var width = !sf.base.isNullOrUndefined(newProp.width) ? sf.base.formatUnit(newProp.width) : newProp.width;
    ...
    FileManager.prototype.ajaxSettingSetModel = function (newProp) {
        if (!sf.base.isNullOrUndefined(newProp.ajaxSettings.url)) {
            this.setProperties({ ajaxSettings: { url: newProp.ajaxSettings.url } }, true);
        }
        if (!sf.base.isNullOrUndefined(newProp.ajaxSettings.uploadUrl)) {
            this.setProperties({ ajaxSettings: { uploadUrl: newProp.ajaxSettings.uploadUrl } }, true);
        }
        if (!sf.base.isNullOrUndefined(newProp.ajaxSettings.downloadUrl)) {
            this.setProperties({ ajaxSettings: { downloadUrl: newProp.ajaxSettings.downloadUrl } }, true);
        }
        if (!sf.base.isNullOrUndefined(newProp.ajaxSettings.getImageUrl)) {
    ...
    FileManager.prototype.destroy = function () {
        ...
        sf.base.removeClass([this.element], [RTL, MOBILE, CHECK_SELECT]);
    ...
    FileManager.prototype.disableToolbarItems = function (items) {
        if (!sf.base.isNullOrUndefined(items)) {
    ...
    FileManager.prototype.disableMenuItems = function (items) {
        if (!sf.base.isNullOrUndefined(items) && !sf.base.isNullOrUndefined(this.contextmenuModule.contextMenu)) {
    ...
    __decorate$8([
        sf.base.Complex({}, UploadSettings)
...
var Toolbar$1 = /** @class */ (function () {
    ...
    Toolbar$$1.prototype.render = function () {
        ...
        this.parent.trigger('toolbarCreate', eventArgs, function (toolbarCreateArgs) {
            ...
            _this.toolbarObj = new sf.navigations.Toolbar({
                items: _this.items,
    ...
    Toolbar$$1.prototype.onClicked = function (args) {
        ...
        this.parent.trigger('toolbarClick', eventArgs, function (toolbarClickArgs) {
            if (!toolbarClickArgs.cancel) {
                switch (tool) {
                    case 'sortby':
                        var target = sf.base.closest(args.originalEvent.target, '.' + TB_ITEM);
    ...
    Toolbar$$1.prototype.toolbarCreateHandler = function () {
        if (!sf.base.isNullOrUndefined(sf.base.select('#' + this.getId('SortBy'), this.parent.element))) {
            var items = [
        ...
        if (!sf.base.isNullOrUndefined(sf.base.select('#' + this.getId('View'), this.parent.element))) {
            ...
            this.layoutBtnObj = new sf.splitbuttons.DropDownButton({
        ...
        this.hideStatus();
        var btnElement = sf.base.selectAll('.e-btn', this.toolbarObj.element);
        var _loop_1 = function (btnCount) {
            /* istanbul ignore next */
            btnElement[btnCount].onkeydown = function (e) {
    ...
    Toolbar$$1.prototype.onSelectionChanged = function () {
        ...
        var ele = sf.base.select('#' + this.getId('Selection'), this.toolbarObj.element);
        if (this.parent.selectedItems.length > 0 && ele) {
            ...
            sf.base.select('.e-tbar-btn-text', ele).textContent = txt;
            this.toolbarObj.hideItem(ele.parentElement, false);
        }
    };
    Toolbar$$1.prototype.hideItems = function (tools, toHide) {
        for (var i = 0; i < tools.length; i++) {
            var ele = sf.base.select('#' + this.getId(tools[i]), this.parent.element);
            if (ele) {
    ...
    Toolbar$$1.prototype.enableItems = function (items, isEnable) {
        for (var i = 0; i < items.length; i++) {
            var ele = sf.base.select('#' + this.getId(items[i]), this.parent.element);
            if (ele) {
...
var NavigationPane = /** @class */ (function () {
    ...
    NavigationPane.prototype.onInit = function () {
        if (!sf.base.isNullOrUndefined(this.treeObj)) {
            return;
        }
        var rootData = sf.base.getValue(this.parent.pathId[0], this.parent.feParent);
        sf.base.setValue('_fm_icon', 'e-fe-folder', rootData);
        var attr = {};
        var id = sf.base.getValue('id', rootData);
        if (!sf.base.isNullOrUndefined(id)) {
            sf.base.setValue('data-id', id, attr);
        }
        if (!hasEditAccess(rootData)) {
            sf.base.setValue('class', getAccessClass(rootData), attr);
        }
        if (!sf.base.isNullOrUndefined(attr)) {
            sf.base.setValue('_fm_htmlAttr', attr, rootData);
        }
    ...
    NavigationPane.prototype.addDragDrop = function () {
        ...
        if (!this.parent.isMobile && this.treeObj) {
            if (this.parent.allowDragAndDrop) {
                ...
                this.dragObj = new sf.base.Draggable(this.treeObj.element, {
                    cursorAt: this.parent.dragCursorPosition,
                    dragTarget: '.' + FULLROW,
                    dragArea: this.parent.element,
                    drag: draggingHandler.bind(this, this.parent),
    ...
    NavigationPane.prototype.dragHelper = function (args) {
        ...
        var dragLi = sf.base.closest(dragTarget, 'li');
    ...
    NavigationPane.prototype.addChild = function (files, target, prevent) {
        ...
        if (directories.length > 0) {
            ...
            while (length_1 < directories.length) {
                ...
                var attr = {};
                var id = sf.base.getValue('id', folders[length_1]);
                if (!sf.base.isNullOrUndefined(id)) {
                    sf.base.setValue('data-id', id, attr);
                }
                if (!hasEditAccess(folders[length_1])) {
                    sf.base.setValue('class', getAccessClass(folders[length_1]), attr);
                }
                if (!sf.base.isNullOrUndefined(attr)) {
                    sf.base.setValue('_fm_htmlAttr', attr, folders[length_1]);
                }
                length_1++;
            }
    ...
    NavigationPane.prototype.onNodeSelected = function (args) {
        ...
        var nodeData = this.getTreeData(sf.base.getValue('id', args.nodeData));
    ...
    NavigationPane.prototype.onNodeExpand = function (args) {
        ...
        if (args.node.querySelector('.' + LIST_ITEM) === null) {
            var text = sf.base.getValue('text', args.nodeData);
            var id = args.node.getAttribute('data-id');
            var isId = sf.base.isNullOrUndefined(id) ? false : true;
            var newText = sf.base.isNullOrUndefined(id) ? text : id;
            var path = getPath(args.node, newText, isId);
            this.expandNodeTarget = args.node.getAttribute('data-uid');
            this.parent.expandedId = this.expandNodeTarget;
            this.parent.itemData = this.getTreeData(sf.base.getValue('id', args.nodeData));
    ...
    NavigationPane.prototype.onNodeEditing = function (args) {
        if (!sf.base.isNullOrUndefined(args.innerHtml)) {
            args.cancel = true;
        }
    };
    NavigationPane.prototype.onPathChanged = function (args) {
        this.parent.isCut = false;
        var currFiles = sf.base.getValue(this.parent.pathId[this.parent.pathId.length - 1], this.parent.feFiles);
        if (this.expandNodeTarget === 'add') {
            var sNode = sf.base.select('[data-uid="' + this.treeObj.selectedNodes[0] + '"]', this.treeObj.element);
            var ul = sf.base.select('.' + LIST_PARENT, sNode);
            if (sf.base.isNullOrUndefined(ul)) {
                this.addChild(args.files, this.treeObj.selectedNodes[0], !this.expandTree);
            }
            this.expandNodeTarget = '';
        }
        this.expandTree = false;
        if (sf.base.isNullOrUndefined(currFiles)) {
            sf.base.setValue(this.parent.pathId[this.parent.pathId.length - 1], args.files, this.parent.feFiles);
        }
    };
    ...
    NavigationPane.prototype.removeChildNodes = function (id) {
        var sNode = sf.base.select('[data-uid="' + id + '"]', this.treeObj.element);
        var parent = sf.base.select('.' + LIST_PARENT, sNode);
    ...
    NavigationPane.prototype.onInitialEnd = function (args) {
        this.onInit();
        this.addChild(args.files, sf.base.getValue('_fm_id', args.cwd), false);
    };
    NavigationPane.prototype.onFinalizeEnd = function (args) {
        this.onInit();
        var id = sf.base.getValue('_fm_id', args.cwd);
        this.removeChildNodes(id);
    ...
    NavigationPane.prototype.onDeleteInit = function () {
        if (this.parent.activeModule === 'navigationpane') {
            ...
            var name_1 = sf.base.getValue('name', this.parent.itemData[0]);
    ...
    NavigationPane.prototype.onDeleteEnd = function (args) {
        if (this.parent.activeModule === 'navigationpane') {
            ...
            var selcetedEle = sf.base.select('[data-uid="' + selectedNode + '"]', this.treeObj.element);
            var selectedNodeEle = sf.base.closest(selcetedEle, '.' + LIST_PARENT).parentElement;
    ...
    NavigationPane.prototype.onRenameEndParent = function (args) {
        ...
        if (this.renameParent) {
            ...
            var resultData = [];
            if (this.parent.hasId) {
                resultData = new sf.data.DataManager(this.treeObj.getTreeData()).
                    executeLocal(new sf.data.Query().where('id', 'equal', this.parent.renamedId, false));
            }
            else {
                var nData = new sf.data.DataManager(this.treeObj.getTreeData()).
                    executeLocal(new sf.data.Query().where(this.treeObj.fields.text, 'equal', this.parent.renameText, false));
                if (nData.length > 0) {
                    resultData = new sf.data.DataManager(nData).
                        executeLocal(new sf.data.Query().where('_fm_pId', 'equal', id, false));
                }
            }
            if (resultData.length > 0) {
                this.isRenameParent = true;
                var id_1 = sf.base.getValue(this.treeObj.fields.id, resultData[0]);
    ...
    NavigationPane.prototype.onRenameEnd = function (args) {
        if (this.parent.breadcrumbbarModule.searchObj.element.value === '' && !this.parent.isFiltered) {
            ...
            var resultData = [];
            if (this.parent.hasId) {
                resultData = new sf.data.DataManager(data).
                    executeLocal(new sf.data.Query().where('id', 'equal', this.parent.renamedId, false));
            }
            else {
                var nData = new sf.data.DataManager(data).
                    executeLocal(new sf.data.Query().where(this.treeObj.fields.text, 'equal', this.parent.currentItemText, false));
                if (nData.length > 0) {
                    resultData = new sf.data.DataManager(nData).
                        executeLocal(new sf.data.Query().where('filterPath', 'equal', this.parent.filterPath, false));
                }
            ...
            if (resultData.length > 0) {
                this.renameParent = sf.base.getValue(this.treeObj.fields.parentID, resultData[0]);
        ...
        for (var i = 0; i < files.length; i++) {
            if (!files[i].isFile) {
                if (!this.parent.hasId) {
                    ...
                    var resultData = new sf.data.DataManager(this.treeObj.getTreeData()).
                        executeLocal(new sf.data.Query().where(this.treeObj.fields.text, 'equal', name_2, false));
                    for (var j = 0; j < resultData.length; j++) {
                        var fPath = sf.base.getValue('filterPath', resultData[j]);
                        fPath = fPath.replace(/\\/g, '/');
                        if (fPath === path) {
                            moveNames.push(sf.base.getValue(this.treeObj.fields.id, resultData[j]));
                            break;
                        }
                    }
                }
    ...
    NavigationPane.prototype.selectResultNode = function (resultObj) {
        if (!this.parent.hasId) {
            var path = sf.base.getValue('filterPath', resultObj);
            var itemname = sf.base.getValue('name', resultObj);
            var data = new sf.data.DataManager(this.treeObj.getTreeData()).
                executeLocal(new sf.data.Query().where(this.treeObj.fields.text, 'equal', itemname, false));
            if (data.length > 0) {
                var resultData = new sf.data.DataManager(data).
                    executeLocal(new sf.data.Query().where('filterPath', 'equal', path, false));
                if (resultData.length > 0) {
                    var id = sf.base.getValue(this.treeObj.fields.id, resultData[0]);
            ...
            this.treeObj.selectedNodes = [sf.base.getValue('_fm_id', resultObj)];
            this.treeObj.dataBind();
        }
    };
    ...
    NavigationPane.prototype.onpasteEnd = function (args) {
        ...
        if (this.parent.hasId) {
            resultData = new sf.data.DataManager(this.treeObj.getTreeData()).
                executeLocal(new sf.data.Query().where('id', 'equal', sf.base.getValue('id', args.cwd), false));
        }
        else {
            var nData = new sf.data.DataManager(this.treeObj.getTreeData()).
                executeLocal(new sf.data.Query().where(this.treeObj.fields.text, 'equal', sf.base.getValue('name', args.cwd), false));
            if (nData.length > 0) {
                resultData = new sf.data.DataManager(nData).
                    executeLocal(new sf.data.Query().where('filterPath', 'equal', sf.base.getValue('filterPath', args.cwd), false));
            }
        }
        if (resultData.length > 0) {
            var id = sf.base.getValue(this.treeObj.fields.id, resultData[0]);
    ...
    NavigationPane.prototype.onpasteInit = function () {
        ...
        for (var i = 0; i < obj.length; i++) {
            if (sf.base.getValue('isFile', obj[i]) === false) {
                this.moveNames.push(sf.base.getValue('_fm_id', obj[i]));
            }
        }
    };
    ...
    NavigationPane.prototype.onMenuItemData = function (args) {
        if (this.parent.activeModule === this.getModuleName()) {
            var liEle = sf.base.closest(args.target, 'li');
    ...
    NavigationPane.prototype.onDropInit = function (args) {
        if (this.parent.targetModule === this.getModuleName()) {
            var dropLi = sf.base.closest(args.target, 'li');
            this.parent.dropData = this.getTreeData(dropLi)[0];
            this.parent.dropPath = this.getDropPath(dropLi, sf.base.getValue('name', this.parent.dropData));
        }
    };
    /**
     * For internal use only - Get the module name.
    ...
    NavigationPane.prototype.wireEvents = function () {
        this.keyboardModule = new sf.base.KeyboardEvents(this.treeObj.element, {
    ...
    NavigationPane.prototype.getTreeData = function (args) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (sf.base.isNullOrUndefined(sf.base.getValue('hasChild', data[i]))) {
                sf.base.setValue('hasChild', false, data[i]);
            }
        }
        return data;
    };
    NavigationPane.prototype.updateRenameData = function () {
        this.updateItemData();
        this.parent.currentItemText = sf.base.getValue('name', this.parent.itemData[0]);
    };
    ...
    NavigationPane.prototype.doDownload = function () {
        ...
        var name = (itemId === this.parent.pathId[0]) ? '' : sf.base.getValue('name', this.parent.itemData[0]);
...
var DetailsView = /** @class */ (function () {
    ...
    function DetailsView(parent) {
        ...
        this.uploadOperation = false;
        sf.grids.Grid.Inject(sf.grids.Resize, sf.grids.ContextMenu, sf.grids.Sort, sf.grids.VirtualScroll);
        this.parent = parent;
        this.element = sf.base.select('#' + this.parent.element.id + GRID_ID, this.parent.element);
        this.addEventListener();
        this.keyConfigs = {
            altEnter: 'alt+enter',
            esc: 'escape',
    ...
    DetailsView.prototype.checkNameWidth = function () {
        ...
        for (var i = 0; i < initialColumn.length; i++) {
            if (initialColumn[i].field === 'name') {
                this.isNameWidth = !sf.base.isNullOrUndefined(initialColumn[i].width);
                return;
            }
        }
    };
    ...
    DetailsView.prototype.adjustHeight = function () {
        ...
        var pane = sf.base.select('#' + this.parent.element.id + CONTENT_ID, this.parent.element);
        var bar = sf.base.select('#' + this.parent.element.id + BREADCRUMBBAR_ID, this.parent.element);
        var gridHeader = sf.base.select('.' + GRID_HEADER, this.parent.element);
    ...
    DetailsView.prototype.onRowDataBound = function (args) {
        var td = sf.base.select('.e-fe-grid-name', args.row);
        if (!td) {
        ...
        if (td) {
            td.setAttribute('title', sf.base.getValue('name', args.data));
        }
        if (this.parent.isLayoutChange && this.parent.isCut && this.parent.fileAction === 'move' &&
            ...
            if (this.parent.selectedNodes.indexOf(sf.base.getValue('name', args.data)) !== -1) {
                addBlur(args.row);
            }
        }
        if (!this.parent.showFileExtension && sf.base.getValue('isFile', args.data)) {
            var textEle = args.row.querySelector('.e-fe-text');
            if (textEle) {
                var name_1 = sf.base.getValue('name', args.data);
                var type = sf.base.getValue('type', args.data);
                textEle.innerHTML = name_1.substr(0, name_1.length - type.length);
            }
        }
        if (sf.base.getValue('size', args.data) !== undefined && args.row.querySelector('.e-fe-size')) {
            var sizeEle = args.row.querySelector('.e-fe-size');
            var modifiedSize = void 0;
            if (!sf.base.getValue('isFile', args.data)) {
                modifiedSize = '';
            }
            else {
                var sizeValue = sf.base.getValue('size', args.data);
                var intl = new sf.base.Internationalization(this.parent.locale);
        ...
        if (this.parent.isMobile) {
            if (sf.base.getValue('_fm_modified', args.data) !== undefined && args.row.querySelector('.e-fe-date')) {
                ...
                var formattedString = intl.formatDate(new Date(sf.base.getValue('_fm_modified', args.data)), format);
        ...
        if (args.requestType === 'sorting') {
            ...
            if (this.parent.selectedItems.length !== 0) {
                ...
                while (len > 0) {
                    ...
                    this.sortSelectedNodes.push(sf.base.getValue(this.parent.hasId ? 'id' : 'name', data));
                    len--;
                }
            }
            this.parent.notify(sortByChange, {});
    ...
    DetailsView.prototype.onDataBound = function () {
        ...
        if (this.parent.createdItem) {
            this.selectRecords([sf.base.getValue(this.parent.hasId ? 'id' : 'name', this.parent.createdItem)]);
            this.parent.createdItem = null;
        }
        ...
        if (this.gridObj.currentViewData.length * this.gridObj.getRowHeight() < this.gridObj.height) {
            ...
            var hdContent = sf.base.select('.e-headercontent', hdTable);
    ...
    DetailsView.prototype.selectRecords = function (nodes) {
        ...
        for (var i = 0, len = gridRecords.length; i < len; i++) {
            var node = this.parent.hasId ? sf.base.getValue('id', gridRecords[i]) : getName(this.parent, gridRecords[i]);
    ...
    DetailsView.prototype.addSelection = function (data) {
        ...
        var rData = [];
        if (this.parent.hasId) {
            rData = new sf.data.DataManager(items).
                executeLocal(new sf.data.Query().where('id', 'equal', this.parent.renamedId, false));
        }
        else {
            var nData = new sf.data.DataManager(items).
                executeLocal(new sf.data.Query().where('name', 'equal', sf.base.getValue('name', data), false));
            if (nData.length > 0) {
                rData = new sf.data.DataManager(nData).
                    executeLocal(new sf.data.Query().where('filterPath', 'equal', this.parent.filterPath, false));
            }
        }
        if (rData.length > 0) {
        ...
        for (var _i = 0, _a = Object.keys(e.newProp); _i < _a.length; _i++) {
            ...
            switch (prop) {
                ...
                case 'height':
                    ...
                    break;
                case 'detailsViewSettings':
                    if (!sf.base.isNullOrUndefined(this.gridObj)) {
                        this.checkNameWidth();
                        var columns = this.getColumns();
                ...
                case 'selectedItems':
                    ...
                    else if (!sf.base.isNullOrUndefined(this.gridObj)) {
    ...
    DetailsView.prototype.checkEmptyDiv = function (args) {
        var items = getSortedData(this.parent, args.files);
        if (items.length === 0 && !sf.base.isNullOrUndefined(this.element.querySelector('.' + GRID_VIEW))) {
            createEmptyElement(this.parent, this.element, args);
        }
    ...
    DetailsView.prototype.openContent = function (data) {
        ...
        this.parent.trigger('fileOpen', eventArgs, function (fileOpenArgs) {
            if (!fileOpenArgs.cancel) {
                var name_2 = sf.base.getValue('name', data);
                if (sf.base.getValue('isFile', data)) {
                    ...
                    var val = _this.parent.breadcrumbbarModule.searchObj.element.value;
                    if (val === '' && !_this.parent.isFiltered) {
                        var id = sf.base.getValue('id', data);
                        var newPath = _this.parent.path + (sf.base.isNullOrUndefined(id) ? name_2 : id) + '/';
                        _this.parent.setProperties({ path: newPath }, true);
                        _this.parent.pathNames.push(name_2);
                        _this.parent.pathId.push(sf.base.getValue('_fm_id', data));
    ...
    DetailsView.prototype.onSelectionChanged = function () {
        sf.base.removeClass([this.element], HEADER_CHECK);
        if (this.parent.selectedItems.length > 0) {
            sf.base.addClass([this.element], HEADER_CHECK);
        }
    };
    ...
    DetailsView.prototype.createDragObj = function () {
        ...
        if (!this.parent.isMobile && this.gridObj) {
            if (this.parent.allowDragAndDrop) {
                ...
                this.dragObj = new sf.base.Draggable(this.gridObj.element, {
                    cursorAt: this.parent.dragCursorPosition,
                    enableTailMode: true,
                    dragArea: this.parent.element,
                    dragTarget: '.' + ROW,
    ...
    DetailsView.prototype.onSelected = function (args) {
        ...
        if (!sf.base.isNullOrUndefined(item) && !sf.base.isNullOrUndefined(item.querySelector('.e-checkselect'))) {
            if (this.gridObj.getSelectedRowIndexes().length !== 1) {
        ...
        if (!sf.base.isNullOrUndefined(this.gridObj) && !sf.base.isNullOrUndefined(this.gridObj.element.querySelector('.e-checkselectall'))) {
        ...
        if (len > 0) {
            var data = this.gridObj.getRowsObject()[rows[len - 1]].data;
            this.parent.currentItemText = sf.base.getValue('name', data);
        }
        var indexes = sf.base.getValue('rowIndexes', args);
        var multiSelect = sf.base.getValue('enableSelectMultiTouch', this.gridObj.selectionModule);
        if (this.parent.isDevice && sf.base.isNullOrUndefined(indexes) && args.target && !multiSelect && !args.target.closest('.e-headercell')) {
            this.parent.isFile = sf.base.getValue('isFile', args.data);
            if (!this.parent.isFile) {
                this.openContent(args.data);
            }
        }
        this.parent.visitedItem = args.row;
        if (this.parent.allowMultiSelection && !sf.base.isNullOrUndefined(item) && !sf.base.isNullOrUndefined(item.querySelector('.e-checkselect'))) {
    ...
    DetailsView.prototype.onPathColumn = function () {
        if (this.parent.view === 'Details' && !sf.base.isNullOrUndefined(this.gridObj)) {
            if (this.parent.breadcrumbbarModule.searchObj.element.value === '' && !this.parent.isFiltered) {
                this.removePathColumn(false);
            }
        }
    ...
    DetailsView.prototype.onDeSelection = function (args) {
        /* istanbul ignore next */
        if (!this.parent.allowMultiSelection && sf.base.isNullOrUndefined(args.data)) {
        ...
        this.selectedRecords();
        if (this.parent.selectedItems.length === 0) {
            sf.base.setValue('enableSelectMultiTouch', false, this.gridObj.selectionModule);
            sf.base.removeClass([this.parent.element], MULTI_SELECT);
        }
        this.parent.notify(selectionChanged, {});
    ...
    DetailsView.prototype.wireEvents = function () {
        this.wireClickEvent(true);
        this.keyboardModule = new sf.base.KeyboardEvents(this.gridObj.element, {
        ...
        sf.base.EventHandler.add(this.gridObj.element, 'blur', this.removeFocus, this);
    };
    DetailsView.prototype.unWireEvents = function () {
        this.wireClickEvent(false);
        this.keyboardModule.destroy();
        this.keyboardDownModule.destroy();
        sf.base.EventHandler.remove(this.gridObj.element, 'blur', this.removeFocus);
    };
    DetailsView.prototype.wireClickEvent = function (toBind) {
        if (toBind) {
            var proxy_1 = this;
            var ele = this.gridObj.getContent();
            this.clickObj = new sf.base.Touch(ele, {
                tap: function (eve) {
                ...
                tapHold: function (e) {
                    if (proxy_1.parent.isDevice) {
                        ...
                        if (proxy_1.parent.allowMultiSelection) {
                            sf.base.setValue('enableSelectMultiTouch', proxy_1.parent.allowMultiSelection, proxy_1.gridObj.selectionModule);
                            sf.base.addClass([proxy_1.parent.element], MULTI_SELECT);
                        }
                        var target = e.originalEvent.target;
                        if (target) {
                            var row = sf.base.closest(target, '.' + ROW);
    ...
    DetailsView.prototype.getFocusedItemIndex = function () {
        return (!sf.base.isNullOrUndefined(this.getFocusedItem())) ?
    ...
    DetailsView.prototype.keyupHandler = function (e) {
        ...
        switch (action) {
            ...
            case 'f2':
                ...
                break;
            case 'ctrlA':
                if (!sf.base.isNullOrUndefined(gridItems[0]) && this.parent.allowMultiSelection) {
                    this.gridObj.selectionModule.selectRowsByRange(0, gridItems.length - 1);
                }
                break;
            case 'ctrlHome':
            case 'tab':
                if (!sf.base.isNullOrUndefined(gridItems[0])) {
            ...
            case 'csEnd':
                if (!this.parent.allowMultiSelection) {
                    ...
                    if (!sf.base.isNullOrUndefined(gridItems[0])) {
                        if (!sf.base.isNullOrUndefined(selIndex) && selIndex !== -1) {
            ...
            case 'ctrlDown':
                ...
                break;
            case 'home':
                var firstItem = [sf.base.getValue(this.parent.hasId ? 'id' : 'name', gridItems[0])];
            ...
            case 'moveDown':
                ...
                break;
            case 'end':
                var lastItem = [sf.base.getValue(this.parent.hasId ? 'id' : 'name', gridItems[gridLength - 1])];
    ...
    DetailsView.prototype.moveFunction = function (selectedItems, e, rowIndex) {
        if (!sf.base.isNullOrUndefined(this.getFocusedItem()) && this.parent.allowMultiSelection) {
        ...
        else if (!sf.base.isNullOrUndefined(rowIndex) && rowIndex !== -1) {
            ...
            if (!sf.base.isNullOrUndefined(selectedItems[0])) {
                this.gridObj.selectRow(0);
            }
        }
    };
    DetailsView.prototype.spaceSelection = function (selRowIndeces, focIndex, selIndex, e) {
        ...
        else if (!sf.base.isNullOrUndefined(this.getFocusedItem()) && focIndex !== selIndex) {
    ...
    DetailsView.prototype.ctrlMoveFunction = function (items, e, rowIndex) {
        var nextItem;
        if (!sf.base.isNullOrUndefined(this.getFocusedItem())) {
            ...
            if (!sf.base.isNullOrUndefined(items[0])) {
                nextItem = 0;
            }
        }
        this.addFocus(nextItem);
    ...
    DetailsView.prototype.checkRowsKey = function (items, indexValue, focIndex, e) {
        if (this.gridObj.checkAllRows === 'Uncheck' || this.gridObj.checkAllRows === 'Intermediate') {
            if (e.action !== 'csHome' && e.action !== 'csEnd') {
                if (sf.base.isNullOrUndefined(this.startIndex) && this.firstItemIndex !== indexValue) {
                ...
                if (e.action === 'csEnd') {
                    this.gridObj.
                        selectRows(this.InnerItems(sf.base.isNullOrUndefined(indexValue) ? 0 : indexValue, sf.base.isNullOrUndefined(focIndex) ? items.length - 1 : focIndex, e));
                }
                else {
                    ((sf.base.isNullOrUndefined(indexValue)) ? this.gridObj.selectRow(0) :
                        this.gridObj.selectRows(this.InnerItems(sf.base.isNullOrUndefined(focIndex) ? 0 : focIndex, indexValue, e)));
                }
            }
        }
        else {
    ...
    DetailsView.prototype.shiftSelectFocusItem = function (selIndex, fIndex, selRowIndexes, e) {
        ...
        fItem = sf.base.isNullOrUndefined(this.startIndex) ? selIndex : selRowIndexes[0];
        if (fItem === lItem) {
    ...
    DetailsView.prototype.addFocus = function (item) {
        ...
        if (fItem) {
            fItem.removeAttribute('tabindex');
            sf.base.removeClass([fItem], [FOCUS, FOCUSED]);
        }
    ...
    DetailsView.prototype.getFocusedItem = function () {
        return sf.base.select('.' + FOCUSED, this.element);
    };
    ...
    DetailsView.prototype.shiftSelectedItem = function (selIndex, selRowIndexes, gridItems, e) {
        if (selIndex === -1) {
            ...
            if (sf.base.isNullOrUndefined(this.startIndex) && e.shiftKey) {
                this.startIndex = this.gridObj.selectedRowIndex;
    ...
    DetailsView.prototype.onMethodCall = function (e) {
        ...
        var action = sf.base.getValue('action', e);
        switch (action) {
            case 'deleteFiles':
                this.deleteFiles(sf.base.getValue('ids', e));
                break;
            case 'downloadFiles':
                this.downloadFiles(sf.base.getValue('ids', e));
                break;
            case 'openFile':
                this.openFile(sf.base.getValue('id', e));
                break;
            case 'createFolder':
                this.interaction = false;
                break;
            case 'renameFile':
                this.interaction = false;
                this.renameFile(sf.base.getValue('id', e), sf.base.getValue('newName', e));
                break;
            case 'selectAll':
                this.interaction = false;
                this.onSelectAllInit();
    ...
    DetailsView.prototype.getRecords = function (nodes) {
        ...
        if (this.parent.hasId || !hasFilter) {
            for (var i = 0, len = gridRecords.length; i < len; i++) {
                if (nodes.indexOf(sf.base.getValue(filter$$1, gridRecords[i])) !== -1) {
            ...
            for (var i = 0, len = gridRecords.length; i < len; i++) {
                var name_4 = sf.base.getValue('filterPath', gridRecords[i]) + sf.base.getValue('name', gridRecords[i]);
...
exports.DetailsViewSettings = DetailsViewSettings;
exports.fileItems = fileItems;
exports.folderItems = folderItems;
exports.layoutItems = layoutItems;
...
exports.TOOLBAR_ID = TOOLBAR_ID;
exports.LAYOUT_ID = LAYOUT_ID;
exports.NAVIGATION_ID = NAVIGATION_ID;
exports.TREE_ID = TREE_ID;
exports.GRID_ID = GRID_ID;
exports.LARGEICON_ID = LARGEICON_ID;
exports.DIALOG_ID = DIALOG_ID;
exports.ALT_DIALOG_ID = ALT_DIALOG_ID;
exports.IMG_DIALOG_ID = IMG_DIALOG_ID;
exports.EXTN_DIALOG_ID = EXTN_DIALOG_ID;
exports.UPLOAD_DIALOG_ID = UPLOAD_DIALOG_ID;
exports.RETRY_DIALOG_ID = RETRY_DIALOG_ID;
exports.CONTEXT_MENU_ID = CONTEXT_MENU_ID;
exports.SORTBY_ID = SORTBY_ID;
exports.VIEW_ID = VIEW_ID;
exports.SPLITTER_ID = SPLITTER_ID;
exports.CONTENT_ID = CONTENT_ID;
exports.BREADCRUMBBAR_ID = BREADCRUMBBAR_ID;
exports.UPLOAD_ID = UPLOAD_ID;
exports.RETRY_ID = RETRY_ID;
exports.SEARCH_ID = SEARCH_ID;
exports.ROOT = ROOT;
...
exports.LAYOUT_CONTENT = LAYOUT_CONTENT;
exports.LARGE_ICONS = LARGE_ICONS;
exports.TB_ITEM = TB_ITEM;
exports.LIST_ITEM = LIST_ITEM;
exports.LIST_TEXT = LIST_TEXT;
exports.LIST_PARENT = LIST_PARENT;
exports.TB_OPTION_TICK = TB_OPTION_TICK;
exports.TB_OPTION_DOT = TB_OPTION_DOT;
exports.BLUR = BLUR;
...
exports.CHECK = CHECK;
exports.FRAME = FRAME;
exports.CB_WRAP = CB_WRAP;
exports.ROW = ROW;
exports.ROWCELL = ROWCELL;
exports.EMPTY = EMPTY;
exports.EMPTY_CONTENT = EMPTY_CONTENT;
exports.EMPTY_INNER_CONTENT = EMPTY_INNER_CONTENT;
exports.CLONE = CLONE;
exports.DROP_FOLDER = DROP_FOLDER;
exports.DROP_FILE = DROP_FILE;
exports.FOLDER = FOLDER;
exports.ICON_IMAGE = ICON_IMAGE;
exports.ICON_MUSIC = ICON_MUSIC;
exports.ICON_VIDEO = ICON_VIDEO;
exports.LARGE_ICON = LARGE_ICON;
exports.LARGE_EMPTY_FOLDER = LARGE_EMPTY_FOLDER;
exports.LARGE_EMPTY_FOLDER_TWO = LARGE_EMPTY_FOLDER_TWO;
exports.LARGE_ICON_FOLDER = LARGE_ICON_FOLDER;
exports.SELECTED_ITEMS = SELECTED_ITEMS;
exports.TEXT_CONTENT = TEXT_CONTENT;
exports.GRID_HEADER = GRID_HEADER;
exports.TEMPLATE_CELL = TEMPLATE_CELL;
exports.TREE_VIEW = TREE_VIEW;
exports.MENU_ITEM = MENU_ITEM;
exports.MENU_ICON = MENU_ICON;
exports.SUBMENU_ICON = SUBMENU_ICON;
exports.GRID_VIEW = GRID_VIEW;
exports.ICON_VIEW = ICON_VIEW;
exports.ICON_OPEN = ICON_OPEN;
exports.ICON_UPLOAD = ICON_UPLOAD;
exports.ICON_CUT = ICON_CUT;
exports.ICON_COPY = ICON_COPY;
exports.ICON_PASTE = ICON_PASTE;
exports.ICON_DELETE = ICON_DELETE;
exports.ICON_RENAME = ICON_RENAME;
exports.ICON_NEWFOLDER = ICON_NEWFOLDER;
exports.ICON_DETAILS = ICON_DETAILS;
exports.ICON_SHORTBY = ICON_SHORTBY;
exports.ICON_REFRESH = ICON_REFRESH;
exports.ICON_SELECTALL = ICON_SELECTALL;
exports.ICON_DOWNLOAD = ICON_DOWNLOAD;
exports.ICON_OPTIONS = ICON_OPTIONS;
exports.ICON_GRID = ICON_GRID;
exports.ICON_LARGE = ICON_LARGE;
exports.ICON_BREADCRUMB = ICON_BREADCRUMB;
exports.ICON_CLEAR = ICON_CLEAR;
exports.ICON_DROP_IN = ICON_DROP_IN;
exports.ICON_DROP_OUT = ICON_DROP_OUT;
exports.ICON_NO_DROP = ICON_NO_DROP;
exports.ICONS = ICONS;
exports.DETAILS_LABEL = DETAILS_LABEL;
exports.ERROR_CONTENT = ERROR_CONTENT;
exports.STATUS = STATUS;
exports.BREADCRUMBS = BREADCRUMBS;
exports.RTL = RTL;
exports.DISPLAY_NONE = DISPLAY_NONE;
exports.COLLAPSED = COLLAPSED;
exports.FULLROW = FULLROW;
exports.ICON_COLLAPSIBLE = ICON_COLLAPSIBLE;
exports.SPLIT_BAR = SPLIT_BAR;
exports.HEADER_CHECK = HEADER_CHECK;
exports.OVERLAY = OVERLAY;
exports.VALUE = VALUE;
exports.isFile = isFile;
exports.modelChanged = modelChanged;
exports.initialEnd = initialEnd;
exports.finalizeEnd = finalizeEnd;
exports.createEnd = createEnd;
exports.filterEnd = filterEnd;
exports.beforeDelete = beforeDelete;
exports.pathDrag = pathDrag;
exports.deleteInit = deleteInit;
exports.deleteEnd = deleteEnd;
exports.refreshEnd = refreshEnd;
exports.resizeEnd = resizeEnd;
exports.splitterResize = splitterResize;
exports.pathChanged = pathChanged;
...
exports.layoutRefresh = layoutRefresh;
exports.actionFailure = actionFailure;
exports.search = search;
exports.openInit = openInit;
exports.openEnd = openEnd;
exports.selectionChanged = selectionChanged;
exports.selectAllInit = selectAllInit;
exports.clearAllInit = clearAllInit;
exports.clearPathInit = clearPathInit;
exports.layoutChange = layoutChange;
exports.sortByChange = sortByChange;
exports.nodeExpand = nodeExpand;
exports.detailsInit = detailsInit;
exports.menuItemData = menuItemData;
exports.renameInit = renameInit;
exports.renameEndParent = renameEndParent;
exports.renameEnd = renameEnd;
exports.showPaste = showPaste;
exports.hidePaste = hidePaste;
exports.selectedData = selectedData;
exports.cutCopyInit = cutCopyInit;
exports.pasteInit = pasteInit;
exports.pasteEnd = pasteEnd;
exports.cutEnd = cutEnd;
exports.hideLayout = hideLayout;
exports.updateTreeSelection = updateTreeSelection;
exports.treeSelect = treeSelect;
exports.sortColumn = sortColumn;
exports.pathColumn = pathColumn;
exports.searchTextChange = searchTextChange;
exports.beforeDownload = beforeDownload;
exports.downloadInit = downloadInit;
exports.dropInit = dropInit;
exports.dragEnd = dragEnd;
exports.dropPath = dropPath;
exports.dragHelper = dragHelper;
exports.dragging = dragging;
exports.updateSelectionData = updateSelectionData;
exports.methodCall = methodCall;
exports.permissionRead = permissionRead;
exports.permissionEdit = permissionEdit;
exports.permissionEditContents = permissionEditContents;
exports.permissionCopy = permissionCopy;
exports.permissionUpload = permissionUpload;
exports.permissionDownload = permissionDownload;
...
exports.ContextMenu = ContextMenu$2;
