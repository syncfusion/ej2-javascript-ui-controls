...
export class ContextMenu {
        ...
        if (target.classList.contains(CLS.TREE_VIEW) || closest(target, 'th') ||
         ...
         !target.classList.contains(CLS.MENU_ICON) && !target.classList.contains(CLS.SUBMENU_ICON)) {
            ...
            if (this.parent.view === 'LargeIcons' && !isNOU(closest(target, 'li')) && !closest(target, '#' + this.parent.element.id + CLS.TREE_ID)) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const gridObj: any = this.parent.detailsviewModule && this.parent.detailsviewModule.gridObj;
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const rowObj: any = (!isNOU(uid) && !isNOU(gridObj)) ? gridObj.getRowObjectFromUID(uid) : null;
                if (!isNOU(rowObj)) {
                    data = rowObj.data as { [key: string]: Object };
    ...
    private setLayoutItem(target: Element): void {
        ...
        this.contextMenu.dataBind();
        const largeIconsEle: Element = closest(target, '#' + this.parent.element.id + CLS.LARGEICON_ID);
        const detailsViewEle: Element = closest(target, '#' + this.parent.element.id + CLS.GRID_ID);
