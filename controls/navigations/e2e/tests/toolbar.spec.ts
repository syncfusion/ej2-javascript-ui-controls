...
import { browser, element, By, ElementArrayFinder, Key, WebElement } from "@syncfusion/ej2-base/e2e/index";
...
function tbarFoucsPopup_Interaction(theme: String) {
    ...
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbarPop_item_focus_3rdItem_' + theme);
    eleLoc.get(2).sendKeys(Key.TAB);
    load_delay(300, 'sleep');
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_Popup_navi_focus_' + theme);
    ...
    load_delay(400, 'sleep');
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_popup_1stitem_down_focus_' + theme);
    if (browser.browserName === 'internet explorer') {
        browser.actions().mouseDown(navEle).mouseUp(navEle).perform();
        //var EC = protractor.ExpectedConditions;
        browser.wait(EC.visibilityOf(PopEleLoc.get(1)), 5000); }
    PopEleLoc.get(0).sendKeys(Key.DOWN)
    load_delay(400, 'sleep');
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_popup_2nditem_down_focus_' + theme);
    if (browser.browserName === 'internet explorer') {
        browser.actions().mouseDown(navEle).mouseUp(navEle).perform();
        //var EC = protractor.ExpectedConditions;
        browser.wait(EC.visibilityOf(PopEleLoc.get(1)), 5000); }
...
function tbarHoverActivePopup_Interaction(theme: String) {
    ...
    browser.actions().mouseMove(eleLoc.get(0)).perform();
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_popup_1stitem_hover_' + theme);
    browser.actions().mouseMove(navEle).perform();
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_popup_nav_hover_' + theme);
    browser.actions().mouseDown(eleLoc.get(0)).perform();
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_popup_1stitem_active_' + theme);
    element(By.tagName('body')).click();
    browser.actions().mouseDown(navEle).perform()
    if (browser.browserName === 'MicrosoftEdge') {
        load_delay(300, 'sleep') }
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_popup_nav_active_' + theme);
    element(By.tagName('body')).click();
    browser.executeScript(pop_Nav);
    load_delay(400, 'sleep');
    browser.actions().mouseMove(PopEleLoc.get(0)).perform()
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_popupOpen_1stItem_hover_' + theme);
    browser.actions().mouseDown(PopEleLoc.get(1)).perform();
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_popupOpen_2ndItem_active_' + theme);
}

function tbarFocusInteraction(theme: String) {
    let eleLoc: ElementArrayFinder = element.all(By.css('.e-toolbar-items .e-tbar-btn'));
    let navEle: ElementArrayFinder = element.all(By.css('.e-toolbar-items .e-scroll-nav'));
    //let scroll_Nav: string = "document.getElementsByClassName('e-hscroll_22_nav e-scroll-nav e-scroll-right-nav e-control e-touch')[0].click();";
    ...
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_item_focus_2ndItem_' + theme);
    eleLoc.get(1).sendKeys(Key.TAB);
    load_delay(300, 'sleep');
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_right_navi_focus_' + theme);
    navEle.get(1).sendKeys(Key.chord(Key.SHIFT, Key.TAB));
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_item_focus_2ndItem_' + theme);
    //browser.executeScript(scroll_Nav);
    browser.actions().mouseDown(navEle.get(1)).mouseUp(navEle.get(1)).perform();
    load_delay(700, 'sleep');
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_left_navi_focus_' + theme);
}
function tbarHoverActiveInteraction(theme: String) {
    ...
    load_delay(400, 'sleep');
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_nrml_hover_' + theme);
    element(By.tagName('body')).click();
    browser.actions().mouseMove(navEle.get(1)).perform();
    load_delay(400, 'sleep');
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_right_navi_hover_' + theme);
    browser.actions().mouseMove(eleLoc.get(21)).perform()
    browser.actions().mouseMove(navEle.get(0)).perform();
    load_delay(400, 'sleep');
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_left_navi_hover_' + theme);
    browser.actions().mouseDown(eleLoc.get(3)).perform();
    load_delay(400, 'sleep');
    browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_item_active_4rtItem_' + theme);
    element(By.tagName('body')).click();
...
describe('Default-Toolbar Theme Wise UI Testing', function () {
    ...
    it("UI Interactions Focus Testing " +themes[i]+"  theme ", function () {
        ...
        browser.load('/demos/Toolbar/default-scroll.html');
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
            let fileName: string = '../../../demos/Toolbar/theme-files/'+ theme.toLowerCase() +'.css';
            let path: string = "((document.getElementsByTagName('head')[0]).querySelector('link')).setAttribute('href','"+ fileName+"')";
            let refresh: string = "document.querySelector('#ej2Toolbar').ej2_instances[0].refresh();";
            let popup_refresh: string = "document.querySelector('#ej2Toolbar_pop').ej2_instances[0].refresh();";
            browser.executeScript(path);
            browser.executeScript(refresh);
            load_delay(1000, 'sleep');
            tbarUIScrollInteraction(theme);
        browser.load('/demos/Toolbar/popup.html');
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
            fileName = '../../../demos/Toolbar/theme-files/'+ theme.toLowerCase() +'.css';
            path = "((document.getElementsByTagName('head')[0]).querySelector('link')).setAttribute('href','"+ fileName+"')";
            browser.executeScript(path);
            browser.executeScript(popup_refresh);
            load_delay(1000, 'sleep');
            if (browser.browserName === 'MicrosoftEdge') {
                load_delay(300, 'sleep') }
...
describe('Default-Toolbar', function () {
    it('default rendering only icons and separator', function () {
        ...
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar');
        browser.executeScript("document.getElementById('btn_mouse').click();")
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_nrml');
    },120000);
    it('default rendering only icons without separator', function () {
        browser.load('/demos/Toolbar/default-no-separator.html');
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_no_separator');
        browser.executeScript("document.getElementById('btn_mouse').click();")
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_no_separator_nrml');
    },120000);
    it('default rendering only icons with text', function () {
        browser.load('/demos/Toolbar/default-with-text.html');
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_icontext');
        browser.executeScript("document.getElementById('btn_mouse').click();")
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_icontext_nrml');
    },120000);
    it('default rendering only icons and iocn with text', function () {
        ...
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_icontext_noSep');
        browser.executeScript("document.getElementById('btn_mouse').click();")
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_icontext_noSep_nrml');
    },120000);
    it('default rendering only icons and iocn with text', function () {
        browser.load('/demos/Toolbar/default-with-text-icon.html');
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_icon_icontext');
        browser.executeScript("document.getElementById('btn_mouse').click();")
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_icon_icontext_nrml');
    },120000);
});
...
describe('Toolbar- alignment Feature', function () {
    it('default alignemnt in toolbar', function () {
        ...
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_align');
    },120000);
    it('default alignemnt in toolbar with center', function () {
        browser.load('/demos/Toolbar/default-align-center.html');
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_align_center');
    },120000);
});
...
describe('Toolbar-popup', function () {
    it('default rendering', function () {
        browser.load('/demos/Toolbar/popup.html');
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup');
        browser.executeScript("document.getElementById('btn_mouse').click();")
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_nrml');
        ...
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_open_nrml');
    },120000);
    it('Toolbar popup open and close', function () {
        ...
        browser.executeScript(pop_Nav)
        load_delay(2000, 'visible', element(By.id('ej2Toolbar_pop_popup')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_open');
        browser.executeScript(pop_Nav)
        if (browser.browserName === 'firefox') {
            load_delay(300, 'sleep') }
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_close');
    },120000);
    it('Toolbar popup open and close with textshown', function () {
        ...
        browser.executeScript(pop_Nav)
        load_delay(2000, 'visible', element(By.id('ej2Toolbar_pop_popup')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_textShown_open');
        browser.executeScript(pop_Nav)
        if (browser.browserName === 'firefox') {
            load_delay(300, 'sleep') }
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_textShown_close');
    },120000);
    it('Toolbar popup open and close with priority', function () {
        ...
        browser.executeScript(pop_Nav)
        load_delay(2000, 'visible', element(By.id('ej2Toolbar_pop_popup')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_priority_open');
        browser.executeScript(pop_Nav)
        if (browser.browserName === 'firefox') {
            load_delay(300, 'sleep') }
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_priority_close');
    },120000);
    it('Toolbar popup open and close with priority textShown', function () {
        ...
        browser.executeScript(pop_Nav)
        load_delay(2000, 'visible', element(By.id('ej2Toolbar_pop_popup')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_priority_textShown_open');
        browser.executeScript(pop_Nav)
        if (browser.browserName === 'firefox') {
            load_delay(300, 'sleep') }
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_priority_textShown_close');
    },120000);
    it('Toolbar popup open and close with alignment and priority', function () {
        ...
        browser.executeScript(pop_Nav)
        load_delay(2000, 'visible', element(By.id('ej2Toolbar_pop_popup')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_priority_align_open');
        browser.executeScript(pop_Nav)
        if (browser.browserName === 'firefox') {
            load_delay(300, 'sleep') }
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_dflt_popup_priority_align_close');
    },120000);
});
...
describe('Toolbar-Scrollable', function () {
    it('default scrolling action', function () {
        browser.load('/demos/Toolbar/default-scroll.html');
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_scroll_left_disable');
        let rightArrow: string = "document.querySelector('.e-nav-right-arrow').click();";
        browser.executeScript(rightArrow)
        load_delay(300, 'sleep')
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_scroll_left-active');
        browser.executeScript(rightArrow); browser.executeScript(rightArrow);
        browser.executeScript(rightArrow); browser.executeScript(rightArrow);
        browser.executeScript(rightArrow); browser.executeScript(rightArrow);
        browser.executeScript(rightArrow)
        load_delay(300, 'sleep')
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_scroll_right-disable');
        let leftArrow: string = "document.querySelector('.e-nav-left-arrow').click();";
        browser.executeScript(leftArrow)
        load_delay(300, 'sleep')
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_scroll_right-active');
...
describe(' Toolbar RTL mode Testing', function () {
    it('default RTL mode Testing', function () {
        ...
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_scrollable_rtl');
    },120000);
    it('Toolbar RTL popup mode', function () {
        ...
        let rightArrow: string = "document.querySelector('.e-nav-right-arrow').click();";
        let leftArrow: string = "document.querySelector('.e-nav-left-arrow').click();";
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_scroll_rtl_left_disable');
        browser.executeScript(rightArrow);
        load_delay(1000, 'sleep');
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_scroll_rtl_right-disable');
        browser.executeScript(leftArrow);
        load_delay(1000, 'sleep');
        browser.compareScreen(element(By.id('Toolbar')), 'default_tbar_scroll_rtl_left_disable');
        browser.executeScript(rtlSwitch)
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_popup_rtl');
        browser.executeScript(pop_Nav)
        load_delay(2000, 'visible', element(By.id('ej2Toolbar_Rtl_popup')));
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_rtl_popup_open');
        browser.executeScript(pop_Nav)
        if (browser.browserName === 'firefox') {
            load_delay(300, 'sleep') }
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_rtl_popup_close');
        browser.executeScript(rtlSwitch)
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_scrollable_rtl');
    },120000);
});
...
describe('Toolbar- public Method testing', function () {
    it('Add item public method', function () {
        ...
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_add_api');
    },120000);
    it('Add multi item public method with multiple element', function () {
        ...
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_multiAdd_api');
    },120000);
    it('Add multi item public method remove element', function () {
        browser.load('/demos/Toolbar/api.html');
        load_delay(2000, 'presence',element(By.css('.e-toolbar.e-keyboard.e-control')));
        browser.executeScript("document.getElementById('btnRemove').click();")
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_remove_api');
    },120000);
    it('Add multi item public method Disable/Enable element', function () {
        ...
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_Disable_api');
        browser.executeScript(enable)
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_Enable_api');
    },120000);
    it('Add multi item and remove public method Combination', function () {
        ...
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_add_api');
        browser.executeScript("document.getElementById('btnMultiAdd').click();")
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_multiAddCom_api');
        browser.executeScript("document.getElementById('btnRemove').click();")
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_removeCom_api');
    },120000);
    it('Add multi item public method with alignment feature', function () {
        ...
        browser.executeScript(addItem);
        browser.executeScript(addItem)
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_addAlign_api');
        browser.executeScript(removeItems)
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_removeAlign_api');
        browser.executeScript(removeItems)
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_removeAlign1_api');
        browser.executeScript(removeItems)
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_removeAlign2_api');
        browser.executeScript(removeItems)
        browser.compareScreen(element(By.id('Toolbar')), 'tbar_removeAlign3_api');
    },120000);
});
