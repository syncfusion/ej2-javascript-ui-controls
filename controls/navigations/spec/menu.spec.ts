...
describe('Menu', () => {
    ...
    describe('CR issues', () => {
        ...
        it('EJ2-58256 - Script error throws when we remove items with hamburger mode in mobile view', () => {
            document.body.appendChild(ul);
            let menuItems: MenuItemModel[] = [];
            menu = new Menu({ items: menuItems , hamburgerMode: true, showItemOnClick: true}, '#menu');
            menu.items = [
            ...
            expect(wrap.lastElementChild.getAttribute('role')).toEqual('menubar');
            expect(wrap.lastElementChild.childElementCount).toEqual(5);
            menu.removeItems(["Help"]);
            expect(wrap.lastElementChild.childElementCount).toEqual(4);
         });
    });
});
