        ...
        if (tableResize.resizeNode === 1) {
            if (!isNullOrUndefined(this.currentBaseHistoryInfo)) {
                ...
                if (rowHistoryFormat.startingPoint.y === point.y) {
                    ...
                    // Capture final height and heightType AFTER resizing
                    const row: TableRowWidget = tableResize.currentResizingTable.childWidgets[tableResize.resizerPosition] as
                    TableRowWidget;
                    if (row) {
                        rowHistoryFormat.finalHeight = row.rowFormat.height;
                        rowHistoryFormat.rowHeightType = row.rowFormat.heightType;
                    }
