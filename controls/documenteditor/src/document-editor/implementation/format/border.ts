...
export class WBorder {
    ...
    private getPropertyValue(property: string): Object {
        ...
        if (hasValue) {
            let propertyType: number = WUniqueFormat.getPropertyType(WBorder.uniqueFormatType, property);
    ...
    private initializeUniqueBorder(property: string, propValue: Object): void {
        let uniqueBorderFormatTemp: Dictionary<number, object> = new Dictionary<number, object>();
    ...
    public getBorderWeight(): number {
        let weight: number = 0;
        let numberOfLines: number = this.getNumberOfLines();
        let borderNumber: number = this.getBorderNumber();
        switch (this.lineStyle) {
            case 'Single':
            case 'DashSmallGap':
            case 'DashDot':
