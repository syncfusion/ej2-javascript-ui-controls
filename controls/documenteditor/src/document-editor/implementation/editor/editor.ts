...
import {
    ...
    TextFormFieldInfo, CheckBoxFormFieldInfo, DropDownFormFieldInfo, RevisionMatchedInfo, FootNoteWidgetsInfo, SelectedCommentInfo, FieldInfo, ListSearchResultInfo
} from './editor-helper';
...
export class Editor {
    ...
    private insertCommentInternal(text: string): void {
        ...
        const commentAdv: CommentElementBox = new CommentElementBox(HelperMethods.getUtcDate());
    ...
    private checkToMapRevisionWithNextNode(inline: ElementBox, tempSpan: ElementBox, isBidi: boolean, revisionType: RevisionType, inlinePara?: ParagraphWidget): boolean {
        if (!isNullOrUndefined(inlinePara) && inlinePara.characterFormat.revisions.length == 1) {
            let nextElementPara: WCharacterFormat = inlinePara.characterFormat;
    ...
    private checkToMapRevisionWithInlineText(inline: ElementBox, indexInInline: number, newElement: ElementBox, isBidi: boolean, revisionType: RevisionType, inlinePara?: WCharacterFormat): boolean {
     ...
     * @returns {BodyWidget}
     */
    public insertSection(selection: Selection, selectFirstBlock: boolean, isUndoing?: boolean, sectionBreakContinuous?: boolean): BlockWidget {
        ...
        //Split body widget
        firstBlock = this.splitBodyWidget(lastBlock.bodyWidget, newSectionFormat, lastBlock, sectionBreakContinuous).firstChild as BlockWidget;
        if (isUndoing) {
            this.layoutWholeDocument(true);
        }
     ...
     * @private
     */
    public splitBodyWidget(bodyWidget: BodyWidget, sectionFormat: WSectionFormat, startBlock: BlockWidget, sectionBreakContinuous?: boolean): BodyWidget {
     ...
     * @private
     */
    public applyPasteOptions(options: PasteOptions, isPasteOptionTextOnly?: boolean): void {
                ...
                if (j === widgets.length - 1 && widget instanceof ParagraphWidget) {
                    ...
                    if (newParagraph.childWidgets.length > 0
                        ...
                        if (this.currentPasteOptions === 'KeepTextOnly') {
                            let paraFormat: WParagraphFormat = new WParagraphFormat(this.selection.start.paragraph);
    ...
    private checkToMatchEmptyParaMarkBack(paraWidget: ParagraphWidget): boolean {
        let prevPara: ParagraphWidget = paraWidget.nextRenderedWidget as ParagraphWidget;
        let lineWid: LineWidget = prevPara.childWidgets[0] as LineWidget;
        let textWid: TextElementBox = lineWid.children[0] as TextElementBox;
        if (!isNullOrUndefined(prevPara) && prevPara instanceof ParagraphWidget && prevPara.characterFormat.revisions.length > 0) {
                ...
                for (let i: number = this.documentHelper.pages.indexOf(block.containerWidget.page); i < this.documentHelper.pages.length; i++) {
                    ...
                    for (let j: number = 0; j < page.bodyWidgets.length; j++) {
                        if (page.bodyWidgets[j].indexInOwner === fromSectionIndex) {
                            ...
                            nextIndex = 0;
                            if (!isNullOrUndefined(page.bodyWidgets[j].nextRenderedWidget) && (page.bodyWidgets[j].nextRenderedWidget as BodyWidget).sectionFormat.breakCode !== 'NoBreak') {
                                fromSectionIndex++;
                            }
                        }
                    }
    ...
    private getSelectedComments(): SelectedCommentInfo {
                        ...
                        for (let i: number = 0; i < paragraph.childWidgets.length; i++) {
                            ...
                            for (let j: number = 0; j < lineWidget.children.length; j++) {
                                if (lineWidget.children[j] instanceof FootnoteElementBox) {
                                    let index: number = this.documentHelper.footnoteCollection.indexOf(lineWidget.children[j] as FootnoteElementBox);
                                    if (index === -1) {
                                        (lineWidget.children[j] as FootnoteElementBox).isLayout = false;
                                        let indexcolle: number = parseInt((lineWidget.children[j] as FootnoteElementBox).text);
                                        this.documentHelper.footnoteCollection.splice(indexcolle - 1, 0, lineWidget.children[j] as FootnoteElementBox);
     ...
     * @returns {void}
     */
    public insertPicture(base64String: string, width: number, height: number, isUiInteracted: boolean): void {
    ...
    private generateFallBackImage(base64String: string, width: number, height: number, imageElementBox: ImageElementBox): void {
    ...
    private insertPictureInternal(imageElementBox: ImageElementBox, isUiInteracted: boolean): void {
    ...
    private updateFootNoteIndex(): void {
        ...
        for (let i: number = 0; i < footNoteCollec.length; i++) {
            ...
            if (!isNullOrUndefined(((footNoteCollec[i].bodyWidget.childWidgets[0] as BlockWidget).childWidgets[0] as LineWidget).children[0])) {
                (((footNoteCollec[i].bodyWidget.childWidgets[0] as BlockWidget).childWidgets[0] as LineWidget).children[0] as TextElementBox).text = (i + 1).toString();
            }
        }
    }
