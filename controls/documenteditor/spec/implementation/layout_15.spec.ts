...
describe('Postioning in spiltted table', () => {
   let editor: DocumentEditor = undefined;
   beforeAll(() => {
      let ele: HTMLElement = createElement('div', { id: 'container' });
      document.body.appendChild(ele);
   ...
   it('width check', () => {
      editor.open(JSON.stringify(doc4));
      let y: TableWidget = editor.documentHelper.pages[0].bodyWidgets[0].floatingElements[0] as TableWidget;
      let x: number = y.tableHolder.columns[0].preferredWidth;
      expect(Math.round(x)).toEqual(93.5);
   });
});