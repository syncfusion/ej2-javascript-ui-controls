...
describe("Footnote delete validation", () => {
	...
	beforeAll(() => {
		...
		(editor.documentHelper.render as any).selectionCanvasIn = TestHelper.pageSelectionCanvas;
		editor.appendTo('#container');
		editor.open(JSON.stringify(footnote));
	});
	afterAll((done) => {
		editor.destroy();
		document.body.removeChild(document.getElementById('container'));
	...
	it('delete footnote script error - validation', () => {
		console.log('delete footnote script error - validation')
		editor.openBlank();
        editor.open(footnote);
        editor.selection.select('0;FN;0;0;0;0','0;FN;0;0;0;7');
