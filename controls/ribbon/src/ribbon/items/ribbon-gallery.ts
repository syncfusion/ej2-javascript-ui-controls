...
export class RibbonGallery {
    ...
    private alignGalleryPopupLeft(popup: HTMLElement, buttonEle: HTMLElement): void {
        const popupEl: HTMLElement = popup.parentElement;
        if (!popupEl) { return; }
        const currentLeft: number = parseFloat(popupEl.style.left);
        const buttonRect: ClientRect = buttonEle.getBoundingClientRect();
        const popupWidth: number = popupEl.offsetWidth;
        let targetLeft: number = buttonRect.left - popupWidth - 1;
        if (targetLeft < 0) {
            targetLeft = 1;
        }
