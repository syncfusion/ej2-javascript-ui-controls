...
/**
 * SHA-256 hash implementation used internally for PDF cryptography.
 *
 * @private
 */
export class _Sha256 {
     ...
     *
     * @private
     * @param {number} x - The 32 bit value to rotate.
     * @param {number} n - The number of bits to rotate.
     * @returns {number} The rotated 32-bit value.
     */
    _rotateRight(x: number, n: number): number {
        return (x >>> n) | (x << (32 - n));
    }
    /**
     * Upper case sigma function used in SHA-256.
     *
     * @private
     * @param {number} x - The input 32-bit word.
     * @returns {number} The transformed 32-bit word.
     */
    _sigma(x: number): number {
        return this._rotateRight(x, 2) ^ this._rotateRight(x, 13) ^ this._rotateRight(x, 22);
    }
    /**
     * Upper-case sigma prime function used in SHA-256.
     *
     * @private
     * @param {number} x - The input 32-bit word.
     * @returns {number} The transformed 32-bit word.
    ...
    /**
     * Lower-case sigma function used in the message schedule.
     *
     * @private
     * @param {number} x - The input 32-bit word.
     * @returns {number} The transformed 32-bit word.
    ...
    /**
     * Lower-case sigma prime function used in the message schedule.
     *
     * @private
     * @param {number} x The input 32-bit word.
     * @returns {number} The transformed 32-bit word.
    ...
    /**
     * Compute the SHA-256 hash for a slice of bytes.
     *
     * @private
     * @param {Uint8Array} data bytes.
     * @param {number}  offset into `data`.
     * @param {number} length of bytes to hash.
     * @returns {Uint8Array} The 32-byte SHA-256 digest as `Uint8Array`.
     */
