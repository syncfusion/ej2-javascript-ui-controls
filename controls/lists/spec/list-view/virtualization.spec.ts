...
describe('UI virtualization', () => {
    ...
    describe('UI virtualization for flat List', () => {
        ...
        describe('UI virtualization with icon css', () => {
            ...
            let initialData: { [key: string]: Object }[] = [
                { text: 'Item 1', id: '1' },
                { text: 'Item 2', id: '2' },
                { text: 'Item 3', id: '3' }
            ];
            let updatedData: { [key: string]: Object }[] = [];
            for (let i = 0; i < 100; i++) {
                updatedData.push({ text: `Item ${i + 1}`, id: `${i + 1}` });
            }
            ...
            it('should initially render with empty space and then fill space after data update', function () {
                listObj.destroy();
                listObj.dataSource = initialData;
                listObj.dataBind();
                const initialItemCount: number = listObj.element.querySelectorAll('.e-list-item').length;
                expect(initialItemCount).toBe(3);
                const ulScrolltHeight: number = listObj.element.querySelector('ul').clientHeight;
                expect(ulScrolltHeight < 500).toBe(true);
                listObj.dataSource = updatedData;
                listObj.dataBind();
                let updatedItemCount: number = listObj.element.querySelectorAll('.e-list-item').length;
                expect(updatedItemCount).toBeGreaterThan(initialItemCount);
                let updatedulScrolltHeight: number = listObj.element.querySelector('ul').clientHeight;
                expect(updatedulScrolltHeight > 500).toBe(true);
                listObj.dataSource = initialData;
                listObj.dataBind();
                updatedItemCount = listObj.element.querySelectorAll('.e-list-item').length;
                expect(updatedItemCount).toBe(3);
                updatedulScrolltHeight = listObj.element.querySelector('ul').clientHeight;
                expect(updatedulScrolltHeight < 500).toBe(true);
                listObj.dataSource = updatedData;
                listObj.dataBind();
                updatedItemCount = listObj.element.querySelectorAll('.e-list-item').length;
                expect(updatedItemCount).toBeGreaterThan(3);
                simulateScrollEvent(listObj.element, 200);
                const topItemBeforeDelete: HTMLElement = listObj.element.querySelector('.e-list-item');
                const topItemTextBeforeDelete: string = topItemBeforeDelete.textContent;
                updatedData.splice(10, 1);
                listObj.dataSource = updatedData;
                listObj.dataBind();
                expect(listObj.element.scrollTop).toBe(200);
                const topItemAfterDelete: HTMLElement = listObj.element.querySelector('.e-list-item');
                const topItemTextAfterDelete: string = topItemAfterDelete.textContent;
                expect(topItemTextAfterDelete).toBe(topItemTextBeforeDelete);
            });
            afterAll(() => {
                ele.remove();
            });
