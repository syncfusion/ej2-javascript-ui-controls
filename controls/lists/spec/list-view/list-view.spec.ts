...
describe('ListView', () => {
    ...
    describe('enableHtmlSanitizer testing with simple data', () => {
        ...
        beforeAll(() => {
            document.body.appendChild(ele);
            treeObj = new ListView({ dataSource: XSSData , enableHtmlSanitizer: false, disableHtmlEncode: false});
            treeObj.appendTo(ele);
        });
    ...
    describe('enableHtmlSanitizer testing with nested data', () => {
        ...
        beforeAll(() => {
            document.body.appendChild(ele);
            treeObj = new ListView({ dataSource: XSSnestedData, headerTitle: '<style>body{background:rgb(0, 0, 255)}</style>Folders', showHeader: true, enableHtmlSanitizer: false, disableHtmlEncode: false });
            treeObj.appendTo(ele);
        });
    ...
    describe('property change', () => {
        ...
        beforeAll(() => {
            document.body.appendChild(ele);
            treeObj = new ListView({ dataSource: NestedData, enableHtmlSanitizer: false, disableHtmlEncode: false });
            treeObj.appendTo(ele);
        });
    ...
    describe('disableHtmlEncode preserves literal < in text', () => {
        let listObj: any;
        let ele: HTMLElement = createElement('div', { id: 'ListView-disable' });
        beforeAll(() => {
            document.body.appendChild(ele);
            listObj = new ListView({ dataSource: ['new<york'], enableHtmlSanitizer: false, disableHtmlEncode: true });
            listObj.appendTo(ele);
        });
        it('should render the full text including <', () => {
            const li: HTMLElement = ele.querySelector('.e-list-item') as HTMLElement;
            expect(li.innerText.trim()).toBe('new<york');
        });
        afterAll(() => { ele.remove(); });
    });
});
