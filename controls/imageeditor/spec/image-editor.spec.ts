 ...
 */
import { CurrentObject, ImageEditor, Point, SelectionPoint, StrokeSettings, TextSettings } from '../src/image-editor/index';
   ...
   describe('Rotate With Annotations', () => {
       ...
       it('90 Degree Combination', (done) => {
           ...
           setTimeout(() => {
               ...
               imageEditor.notify('shape', { prop: 'applyActObj', value: {isMouseDown: null }});
               expect(imageEditor.objColl.length).toEqual(1);
               imageEditor.drawText(200, 200, 'Syncfusion', 'Arial', 60, true, true, 'red');
               expect(imageEditor.objColl[imageEditor.objColl.length - 1].shape).toEqual('text');
               imageEditor.notify('shape', { prop: 'applyActObj', value: {isMouseDown: null }});
   ...
   describe('Rotate With Crop', () => {
       ...
       it('90 Degree Rotate With Crop', (done) => {
           ...
           setTimeout(() => {
               ...
               expect(imageEditor.objColl[0].activePoint.startY).toEqual(242.72967189728953);
               expect(imageEditor.objColl[0].activePoint.endX).toEqual(494.9096189117587);
               expect(imageEditor.objColl[0].activePoint.endY).toEqual(350.371815773385);
               done();
           }, 100);
       });
       it('180 Degree Rotate With Crop', (done) => {
           ...
           setTimeout(() => {
               ...
               expect(imageEditor.activeObj.shape).toEqual('crop-square');
               imageEditor.crop();
               expect(imageEditor.img.destLeft).toEqual(244.49999999999997);
               expect(imageEditor.img.destTop).toEqual(14.5);
               expect(imageEditor.img.destWidth).toEqual(278.00000000000006);
               expect(imageEditor.img.destHeight).toEqual(278);
               ...
               expect(imageEditor.objColl[0].activePoint.endY).toEqual(262.903041825095);
               done();
           }, 100);
       });
       it('270 Degree Rotate With Crop', (done) => {
           ...
           setTimeout(() => {
               ...
               expect(imageEditor.img.destWidth).toEqual(278);
               expect(imageEditor.img.destHeight).toEqual(278);
               expect(imageEditor.objColl[0].activePoint.startX).toEqual(272.0903810882413);
               expect(imageEditor.objColl[0].activePoint.startY).toEqual(-43.37181577338498);
               expect(imageEditor.objColl[0].activePoint.endX).toEqual(352.8219889953129);
               expect(imageEditor.objColl[0].activePoint.endY).toEqual(64.27032810271044);
               done();
           }, 100);
       });
   ...
   describe('Combination testing', () => {
       ...
       it('Text size improper', (done) => {
           ...
           setTimeout(() => {
                ...
                imageEditor.rotate(-90);
                expect(imageEditor.transform.degree).toEqual(-180);
                expect(imageEditor.objColl[imageEditor.objColl.length - 1].textSettings.fontSize).toEqual(24.402925277305695);
                done();
           }, 100);
       });
       ...
       it('Render Image with cancel combination', (done) => {
           ...
           setTimeout(() => {
                ...
                imageEditor.notify('draw', {prop: 'performCancel', value: {isContextualToolbar: null }});
                imageEditor.drawEllipse(0, 0, 300, 200, 15, 'red', 'green');
                imageEditor.flip('Horizontal');
