...
export class ToolbarModule {
    ...
    private updatePrivateVariables(): void {
        ...
        if (!isNullOrUndefined(this.inMemoryCanvas)) {this.inMemoryContext = this.inMemoryCanvas.getContext('2d'); }
    }

    private reset(): void {
        this.defToolbarItems = []; this.toolbarHeight = 46; this.parent.prevCurrSelectionPoint = null;
        this.zoomBtnHold = null; this.currToolbar = '';
        this.currentToolbar = 'main'; this.selFhdColor = '#42a5f5'; this.parent.currentFilter = '';
    ...
    private destroyTopToolbar(): void {
        ...
        (!isNullOrUndefined((getComponent(document.getElementById(parent.element.id + '_toolbar'), 'toolbar') as Toolbar)))) {
    ...
    private destroyBottomToolbar(): void {
        ...
        (!isNullOrUndefined((getComponent(document.getElementById(parent.element.id + '_bottomToolbar'), 'toolbar') as Toolbar)))) {
            ...
            for (let i: number = 0, len: number = parent.toolbar.length; i < len; i++) {
                if (typeof(parent.toolbar[i as number]) === 'object') {
                    if (isNullOrUndefined((parent.toolbar[i as number] as ItemModel).align)) {
                        if (position === 'left') {
                            toolbarItems.push(this.parent.toolbar[i as number] as ItemModel);
                        }
    ...
    private renderAnnotationBtn(isContextualToolbar?: boolean): void {
        ...
        const drpDownBtn: DropDownButton = new DropDownButton({ items: items, iconCss: 'e-icons ' + iconCss,
            ...
            open: (args: OpenCloseMenuEventArgs) => {
                ...
                if (!isNullOrUndefined(this.parent.activeObj.shape)) {
                    document.getElementById(this.parent.activeObj.shape).classList.add('e-selected');
                } else if (this.parent.togglePen) {
    ...
    private renderCropBtn(): void {
        ...
        const drpDownBtn: DropDownButton = new DropDownButton({
            open: (args: OpenCloseMenuEventArgs) => {
                ...
                if (!isNullOrUndefined(this.parent.activeObj.shape) && this.parent.activeObj.shape.split('-').length > 1) {
                    document.getElementById(this.parent.activeObj.shape.split('-')[1]).classList.add('e-selected');
                }
    ...
    private renderSaveBtn(): void {
        const imageEditorObj: ImageEditor = this.parent;
        const saveItems: DropDownButtonItemModel[] = [
            { text: 'JPEG', id: 'jpeg' },
            { text: 'PNG', id: 'png' },
            { text: 'SVG', id: 'svg' }
        ];
        const ddbElem: Element = document.getElementById(this.parent.element.id + '_saveBtn');
        if (ddbElem) {
    ...
    private initShapesToolbarItem(items: (string | ItemModel)[]): void {
        ...
        const toolbar: Toolbar = new Toolbar({
            ...
            created: () => {
                ...
                if (this.parent.activeObj.shape === 'arrow') {
                    this.createStartBtn();
                    this.createEndBtn();
                }
                this.wireZoomBtnEvents();
    ...
    private createShapeColor(items: (string | ItemModel)[]): void {
        ...
        if (items.indexOf('fillColor') > -1) {
            ...
            const fillColor: ColorPicker = new ColorPicker({
                ...
                change: (args: ColorPickerEventArgs): void => {
                    this.parent.updateFillColor(args.currentValue.hex)
        ...
        if (items.indexOf('strokeColor') > -1) {
            ...
            const strokeColor: ColorPicker = new ColorPicker({
                ...
                change: (args: ColorPickerEventArgs): void => {
                    this.parent.updateStrokeColor(args.currentValue.hex);
            ...
            const strokeDDB: DropDownButton = new DropDownButton({
                open: (args: OpenCloseMenuEventArgs) => {
                    if (Browser.isDevice) {
                        ...
                        args.element.parentElement.offsetHeight + 'px';
                        args.element.parentElement.style.left = this.parent.element.offsetLeft + 'px';
                    }
                },
        ...
        if (items.indexOf('strokeWidth') > -1) {
            ...
            const drpDownBtn: DropDownButton = new DropDownButton({ items: strokeWidthItems,
                ...
                select: (args: MenuEventArgs) => {
                    ...
                    this.parent.updateStrokeWidth(args.item.id);
                    if (Browser.isDevice) {
                        if (!isNullOrUndefined(document.getElementById(this.parent.element.id + '_bottomToolbar'))) {
                            /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
                            const toolbar: any = getComponent(this.parent.element.id + '_bottomToolbar', 'toolbar') as Toolbar;
                            toolbar.refreshOverflow();
                        }
                    } else {
                        if (!isNullOrUndefined(document.getElementById(this.parent.element.id + '_toolbar'))) {
                            /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
                            const toolbar: any = getComponent(this.parent.element.id + '_toolbar', 'toolbar') as Toolbar;
                            toolbar.refreshOverflow();
                        }
                    }
                }
        ...
        const toolbar: Toolbar = new Toolbar({
            ...
            created: () => {
                ...
                if (Browser.isDevice) {
                    ...
                    this.createLeftToolbarControls();
                    if (this.defToolbarItems.length > 0 && (!isNullOrUndefined(document.getElementById(parent.element.id + '_toolbar')))) {
                        /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
                        (toolbar as any).refreshOverflow();
                    }
                }
    ...
    private createTextColor(items: (string | ItemModel)[]): void {
        ...
        if (items.indexOf('fontColor') > -1 && !isNullOrUndefined(parent.element.querySelector('.e-template.e-text-font-color'))) {
            ...
            const fontColor: ColorPicker = new ColorPicker({
                ...
                change: (args: ColorPickerEventArgs): void => {
                    this.parent.updateFontColor(args.currentValue.hex);
                    (strokeDDB.element.children[0] as HTMLElement).style.backgroundColor = args.currentValue.rgba;
                    strokeDDB.toggle();
                }
    ...
    private refreshToolbar(type: string, isApplyBtn?: boolean, isCropping?: boolean, isZooming?: boolean, cType?: string): void {
        ...
        if (type !== 'filter' && type !== 'color') {
            if (document.getElementById(this.parent.element.id + '_toolbar') && this.defToolbarItems.length > 0) {
                (getComponent(document.getElementById(this.parent.element.id + '_toolbar'), 'toolbar') as Toolbar).destroy();
            ...
            if (document.getElementById(this.parent.element.id + '_bottomToolbar') && this.defToolbarItems.length > 0) {
                if (document.getElementById(this.parent.element.id + '_bottomToolbar').className.indexOf('e-control') > -1) {
                    (getComponent(document.getElementById(this.parent.element.id + '_bottomToolbar'), 'toolbar') as Toolbar).destroy();
    ...
    private initPenToolbarItem(items: (string | ItemModel)[]): void {
        ...
        const toolbar: Toolbar = new Toolbar({
            ...
            created: () => {
                ...
                this.parent.trigger('toolbarCreated', {toolbarType: 'pen'});
                if (Browser.isDevice) {
                    if (this.defToolbarItems.length > 0 && (!isNullOrUndefined(document.getElementById(this.parent.element.id + '_toolbar')))) {
                        /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
    ...
    private createPenColor(items: (string | ItemModel)[]): void {
        ...
        if (items.indexOf('strokeColor') > -1) {
            ...
            const penColor: ColorPicker = new ColorPicker({
                ...
                change: (args: ColorPickerEventArgs): void => {
                    this.parent.updatePenStrokeColor(args.currentValue.hex);
            ...
            if (!isNullOrUndefined(indexObj['freehandSelectedIndex']) && indexObj['freehandSelectedIndex'] > -1) {
                (this.parent.element.querySelector('.e-pen-stroke-color.e-template .e-dropdownbtn-preview') as HTMLElement).style.background
    ...
    private createPenBtn(items: (string | ItemModel)[]): void {
        ...
        if (items.indexOf('strokeWidth') > -1) {
            ...
            const drpDownBtn: DropDownButton = new DropDownButton({ items: strokeWidthItems,
                ...
                select: (args: MenuEventArgs) => {
                    spanElem.textContent = args.item.text;
                    this.parent.updatePenStrokeWidth(args.item.id);
                    if (Browser.isDevice) {
                        if (!isNullOrUndefined(document.getElementById(parent.element.id + '_bottomToolbar'))) {
                            /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
                            const toolbar: any = getComponent(parent.element.id + '_bottomToolbar', 'toolbar') as Toolbar;
    ...
    private initFilterToolbarItem(): void {
        ...
        if (document.querySelector('#' + this.parent.element.id + '_contextualToolbar').classList.contains('e-control')) {
            (getComponent(document.getElementById(this.parent.element.id + '_contextualToolbar'), 'toolbar') as Toolbar).destroy();
        }
        const toolbar: Toolbar = new Toolbar({
            width: '100%',
            items: mainItem,
            ...
            created: () => {
                ...
                if (parent.currentFilter === '') {
                    parent.currentFilter = this.parent.element.id + '_default';
                }
    ...
    private updateFilterCanvas(selector: string, type: string): void {
        const filter: HTMLCanvasElement = this.parent.element.querySelector('#' + this.parent.element.id + selector);
    ...
    private renderQAT(isPenEdit?: boolean): void {
        if (!isNullOrUndefined(this.parent.activeObj) && this.parent.showQuickAccessToolbar) {
            const qtArea: HTMLElement = document.getElementById(this.parent.element.id + '_quickAccessToolbarArea');
            ...
            if (isNullOrUndefined(isPenEdit)) {
                ...
                let x: number = this.parent.activeObj.activePoint.startX < this.parent.activeObj.activePoint.endX ?
                    this.parent.activeObj.activePoint.startX : this.parent.activeObj.activePoint.endX;
                let y: number = this.parent.activeObj.activePoint.startY < this.parent.activeObj.activePoint.endY ?
                    this.parent.activeObj.activePoint.startY : this.parent.activeObj.activePoint.endY;
                let width: number = this.parent.activeObj.activePoint.width;
                if (this.parent.activeObj.rotatedAngle !== 0 && this.parent.activeObj.shape !== 'arrow') {
                    const point: ActivePoint = this.parent.getSquarePointForRotatedShape(this.parent.activeObj);
                    x = point.startX; y = point.startY; width = point.width;
                } else if (this.parent.activeObj.shape === 'path') {
                    const path: ActivePoint = this.parent.getSquarePointForPath(this.parent.activeObj);
    ...
    private refreshDropDownBtn(isDisabled: boolean): void {
        ...
        const annotation: HTMLElement = document.querySelector('#' + this.parent.element.id + '_annotationBtn');
        ...
        const transform: HTMLElement = document.querySelector('#' + this.parent.element.id + '_transformBtn');
        ...
        const adjustment: HTMLElement = document.querySelector('#' + this.parent.element.id + '_adjustment');
        ...
        const filter: HTMLElement = document.querySelector('#' + this.parent.element.id + '_filter');
        if (!isNullOrUndefined(filter)) {
            if (isDisabled) {
        ...
        if (!parent.disabled) {
            ...
            case 'crop':
                if (!isNullOrUndefined(parent.currSelectionPoint)) {
                    ...
                    (getComponent(parent.element.querySelector('#' + this.parent.element.id + '_cropBtn') as HTMLElement, 'dropdown-btn') as DropDownButton).toggle();
                    if (!isNullOrUndefined(parent.activeObj.shape)) {
    ...
    private destroyQuickAccessToolbar(): void {
        ...
        if (!isNullOrUndefined(document.getElementById(parent.element.id + '_quickAccessToolbar')) &&
            !isNullOrUndefined((getComponent(document.getElementById(parent.element.id + '_quickAccessToolbar'), 'toolbar') as Toolbar))) {
            (getComponent(document.getElementById(parent.element.id + '_quickAccessToolbar'), 'toolbar') as Toolbar).destroy();
        }
    ...
    private renderSlider(type: string): void {
        ...
        const value: number = this.parent.getCurrAdjustmentValue(type);
    ...
    private createSlider(min: number, max: number, value: number, type: string): Slider {
        ...
        return new Slider({
            ...
            change: (args: SliderChangeEventArgs): void => {
                this.parent.setCurrAdjustmentValue(type, args.value as number);
                this.enableDisableTbrBtn();
            }
        });
    }
    ...
    private updateToolbarItems(): void {
        const selFillElem: HTMLElement = this.parent.element.querySelector('.e-fill.e-template .e-dropdownbtn-preview') as HTMLElement;
        const selStrokeElem: HTMLElement = this.parent.element.querySelector('.e-stroke.e-template .e-dropdownbtn-preview') as HTMLElement;
        const selTextStrokeElem: HTMLElement = this.parent.element.querySelector('.e-text-font-color.e-template .e-dropdownbtn-preview') as HTMLElement;
        const selPenStrokeElem: HTMLElement = this.parent.element.querySelector('.e-pen-stroke-color.e-template .e-dropdownbtn-preview') as HTMLElement;
        const strokeWidthElem: HTMLElement = this.parent.element.querySelector('.e-shape-stroke-width') as HTMLElement;
        const fontFamilyElem: HTMLElement = this.parent.element.querySelector('.e-text-font-family') as HTMLElement;
        const fontSizeElem: HTMLElement = this.parent.element.querySelector('.e-text-font-size') as HTMLElement;
        const boldBtn: HTMLElement = this.parent.element.querySelector('#' + this.parent.element.id + '_bold') as HTMLElement;
        const italicBtn: HTMLElement = this.parent.element.querySelector('#' + this.parent.element.id + '_italic') as HTMLElement;
        ...
        if (selFillElem) {
            ...
            (getComponent(this.parent.element.id + '_shape_fill', 'colorpicker') as ColorPicker).value
        ...
        if (selStrokeElem) {
            ...
            (getComponent(this.parent.element.id + '_shape_stroke', 'colorpicker') as ColorPicker).value
        ...
        if (selTextStrokeElem) {
            ...
            (getComponent(this.parent.element.id + '_text_font', 'colorpicker') as ColorPicker).value
            = this.parent.activeObj.strokeSettings.strokeColor + 'ff';
        }
        if (selPenStrokeElem) {
            selPenStrokeElem.style.background = this.parent.activeObj.strokeSettings.strokeColor;
            (getComponent(this.parent.element.id + '_pen_stroke', 'colorpicker') as ColorPicker).value
            = this.parent.activeObj.strokeSettings.strokeColor + 'ff';
        }
        if (fontFamilyElem) {
            if (Browser.isDevice) {
        ...
        if (fontSizeElem) {
            for (let i: number = 0; i < this.parent.fontSizeColl.length; i++) {
                if (parseInt(this.parent.fontSizeColl[i as number].text, 10) >= Math.round(this.parent.activeObj.textSettings.fontSize)) {
        ...
        if (strokeWidthElem) {
            const strokeWidth: string = Math.round((this.parent.activeObj.strokeSettings.strokeWidth)).toString();
    ...
    private destroySubComponents(): void {
        const inputElement: NodeListOf<HTMLElement> = this.parent.element.querySelectorAll('input.e-control') as NodeListOf<HTMLElement>;
        const btnElement: NodeListOf<HTMLElement> = this.parent.element.querySelectorAll('button.e-control');
        for (let i: number = 0, len: number = inputElement.length; i < len; i++) {
            if (inputElement[i as number].classList.contains('e-color-picker')) {
                ...
                detach(select('input#' + inputElement[i as number].id, this.parent.element));
            }
        }
        for (let i: number = 0, len: number = btnElement.length; i < len; i++) {
            if (btnElement[i as number].classList.contains('e-dropdown-btn')) {
                ...
                detach(select('button#' + btnElement[i as number].id, this.parent.element));
