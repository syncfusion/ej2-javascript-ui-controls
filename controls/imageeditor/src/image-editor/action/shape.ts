...
import { CurrentObject, ImageEditor, Point, SelectionPoint, CropSelectionSettings, ShapeChangeEventArgs, ShapeSettings, ShapeType, StrokeSettings, TextSettings, ArrowheadType } from '../index';
...
export class Shape {
    ...
    private shape(args?: { onPropertyChange: boolean, prop: string, value?: object }): void {
        ...
        case 'drawRectangle':
            this.drawRectangle(args.value['x'], args.value['y'], args.value['width'], args.value['height'],
                               args.value['strokeWidt'], args.value['strokeColor'], args.value['fillColor']);
            break;
        case 'drawText':
    ...
    private drawShape(type: string, strokeWidth?: number, strokeColor?: string, fillColor?: string, start?: Point, width?: number,
                      height?: number, pointColl?: Point[], arrowStart?: ArrowheadType, arrowEnd?: ArrowheadType): void {
        ...
        if (!parent.disabled && parent.isImageLoaded) {
            ...
            if (id.split('_')[0] === 'shape') {
                ...
                if (obj['isIndex']) {
                    ...
                    if (!isBlazor()) {parent.notify('toolbar', { prop: 'update-toolbar-items', onPropertyChange: false}); }    
                } else {
                    isSelected = false;
                }
            }
