...
export class Draw {
    ...
    private reset(): void {
        ...
        this.isShapeTextInserted = false; this.isImageApply = false; this.isNullExtension = true;
    ...
    private restoreOldImage(): void {
        ...
        const extension: string[] = parent.getExtensionArray();
        const openURLType: string = typeof this.openURL;
        ...
        if (((this.openURL as string).indexOf('data:image/') > -1 && (this.openURL as string).indexOf('base64') > -1) ||
            ...
            this.getImageSizeFromURL(this.openURL.toString(), (imageSizeMB: number | null) => {
                const isInvalidSize: boolean = (parent.uploadSettings.minFileSize && imageSizeMB < parent.uploadSettings.minFileSize) ||
    ...
    private handleFileSize(isError: boolean, dropArea: HTMLElement, fileTypeError: boolean): void {
        const parent: ImageEditor = this.parent;
        if (isError) {
            this.errorLoading();
    ...
    private openImageData(dropArea: HTMLElement, isBase64?: boolean): void {
        const parent: ImageEditor = this.parent;
        // eslint-disable-next-line @typescript-eslint/no-this-alias
        const proxy: Draw = this;
        const canvas: HTMLCanvasElement = parent.createElement('canvas');
        ...
        if (isBase64) {
            ...
            img.src = this.openURL as string;
            img.onload = () => {
            ...
            canvas.width = (this.openURL as ImageData).width;
            canvas.height = (this.openURL as ImageData).height;
            ctx.putImageData((this.openURL as ImageData), 0, 0);
            this.getImageSize(canvas, dropArea);
        }
    }

    private getImageSize(canvas: HTMLCanvasElement, dropArea: HTMLElement): void {
        const parent: ImageEditor = this.parent;
        /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
        canvas.toBlob((function (blob: any): void {
    ...
    private fileSelect(inputElement: HTMLInputElement, args: Event): void {
        ...
        if (!parent.disabled) {
            ...
            const extension: string[] = parent.getExtensionArray();
            const isJPG: boolean = ((fileExtension === 'jpg' || fileExtension === 'jpeg') && (parent.uploadSettings.allowedExtensions.indexOf('jpg') > -1 || parent.uploadSettings.allowedExtensions.indexOf('jpeg') > -1));
