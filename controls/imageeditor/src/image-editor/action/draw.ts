...
export class Draw {
    ...
    private adjToCenter(): void {
        const parent: ImageEditor = this.parent;
        const diffX: number = ((parent.lowerCanvas.width) / 2) - (parent.activeObj.activePoint.endX - parent.activeObj.activePoint.width / 2);
        const diffY: number = ((parent.lowerCanvas.height) / 2) - (parent.activeObj.activePoint.endY - parent.activeObj.activePoint.height / 2);
    ...
    private cancelZoom(): void {
        const parent: ImageEditor = this.parent;
        const length: number = parent.transform.cropZoomFactor - this.tempZoomFactor;
        ...
        if (isNullOrUndefined(parent.cropObj.activeObj.shape)) {
            ...
            || (parent.transform.degree !== 0 && parent.panPoint.totalPannedInternalPoint.x === 0 &&
                ...
                parent.transform.cropZoomFactor = this.tempZoomFactor; parent.currObjType.isCustomCrop = false;
                parent.upperCanvas.style.cursor = parent.cursor = 'default'; parent.currObjType.isCustomCrop = false;
                this.tempStrokeSettings = {strokeColor: '#fff', fillColor: '', strokeWidth: null};
                const eventargs: object = { type: 'main', isApplyBtn: null, isCropping: false, isZooming: null};
                if (!isBlazor()) {parent.notify('toolbar', { prop: 'refresh-toolbar', onPropertyChange: false, value: eventargs}); }
        ...
        if (isNullOrUndefined(parent.cropObj.activeObj.shape)) {
            if (parent.transform.degree === 0) {
                const activeObj: SelectionPoint = extend({}, parent.activeObj, {}) as SelectionPoint;
            ...
            this.isCancelAction = true; parent.objColl = []; parent.pointColl = [];
            const freehandCounter: number = parent.freehandCounter; parent.freehandCounter = 0;
            ...
            const cropObj: CurrentObject = extend({}, parent.cropObj, {}) as CurrentObject;
            ...
            parent.cropObj = cropObj; parent.afterCropActions = afterCropActions;
            parent.objColl = extend([], this.cancelObjColl, [], true) as SelectionPoint[];
            parent.pointColl = extend([], this.cancelPointColl, [], true) as Point[]; parent.freehandCounter = freehandCounter;
            parent.notify('freehand-draw', { prop: 'setSelPointColl', onPropertyChange: false,
