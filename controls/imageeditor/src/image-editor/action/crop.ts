...
export class Crop {
    ...
    private reset(): void {
        ...
        this.tempFlipPanPoint = {x: 0, y: 0}; this.isPreventScaling = false;
    }
    ...
    private rotateCrop(): void {
        ...
        this.lowerContext.filter = temp;
        let length: number = 0;
        if (tempDegree === 90 || tempDegree === -270) {length = 3; }
        else if (tempDegree === 180 || tempDegree === -180) {length = 2; }
        else if (tempDegree === 270 || tempDegree === -90) {length = 1; }
        ...
        if (parent.transform.currFlipState !== '') {
            ...
            const flipState: string = this.getCurrCropState('initial');
            parent.transform.degree = 0;
        ...
        length = 0;
        if (tempDegree === 90 || tempDegree === -270) {length = 1; }
        else if (tempDegree === 180 || tempDegree === -180) {length = 2; }
        else if (tempDegree === 270 || tempDegree === -90) {length = 3; }
        for (let i: number = 0; i < length; i++) {
            parent.notify('shape', { prop: 'rotateObjColl', onPropertyChange: false});
            parent.notify('freehand-draw', { prop: 'rotateFhdColl', onPropertyChange: false});
        }
        if (this.getCurrFlipState() !== '') {
            ...
            const flipState: string = this.getCurrCropState('reverse');
            parent.notify('shape', { prop: 'redrawObj', onPropertyChange: false, value: {degree: flipState}});
            parent.notify('freehand-draw', { prop: 'flipFHDColl', onPropertyChange: false,
                value: {value: flipState}});
        }
    ...
    private setCurrSelPoints(isSetDimension?: boolean): void {
        ...
        if (parent.cropObj.activeObj.shape) {
            ...
            const cropObjColl: SelectionPoint[] = extend([], parent.objColl, null, true) as SelectionPoint[];
            const cropPointColl: Point[] = extend([], parent.pointColl, null, true) as Point[];
    ...
    private cropCircle(context: CanvasRenderingContext2D, isSave?: boolean, isFlip?: boolean): void {
        ...
        context.beginPath();
        const centerX: number = isNullOrUndefined(isSave) ? parent.img.destLeft + (parent.img.destWidth / 2) : parent.img.destWidth / 2;
        const centerY: number = isNullOrUndefined(isSave) ? parent.img.destTop + (parent.img.destHeight / 2) : parent.img.destHeight / 2;
        const radius: number = parent.img.destWidth / 2;
    ...
    private cropEvent(transitionArgs: CropEventArgs, obj: Object, object: Object): void {
        ...
        if (!transitionArgs.cancel) {
            ...
            if (!parent.disabled && parent.activeObj.horTopLine && (parent.currObjType.isCustomCrop || (splitWords.length > 0 &&
                ...
                this.cropImg();
                parent.transform.zoomFactor = 0; parent.zoomSettings.zoomFactor = 1;
    ...
    private calcRatio(obj?: Object): Dimension {
        const parent: ImageEditor = this.parent;
