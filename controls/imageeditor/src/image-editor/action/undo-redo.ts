...
export class UndoRedo {
    ...
    private initializeUrPvtProp(): void {
        if (!isNullOrUndefined(this.parent.lowerCanvas)) {this.lowerContext = this.parent.lowerCanvas.getContext('2d'); }
        if (!isNullOrUndefined(this.parent.upperCanvas)) {this.upperContext = this.parent.upperCanvas.getContext('2d'); }
    }
    ...
    private undoDefault(obj: Transition): void {
        this.lowerContext.filter = obj.previousObj.filter;
        let parent: ImageEditor = this.parent;
        parent.objColl = []; parent.pointColl = []; parent.freehandCounter = 0;
        parent.notify('freehand-draw', { prop: 'setSelPointColl', onPropertyChange: false,
            value: {obj: {selPointColl: [] } }});
