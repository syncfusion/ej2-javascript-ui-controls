...
export class Selection {
    ...
    private updatePrivateVariables(): void {
        ...
        if (!isNullOrUndefined(parent.lowerCanvas)) {this.lowerContext = parent.lowerCanvas.getContext('2d'); }
        if (!isNullOrUndefined(parent.upperCanvas)) {this.upperContext = parent.upperCanvas.getContext('2d'); }
    }
    ...
    private updateActivePoint(x: number, y: number, isCropSelection: boolean):  void {
        ...
        if (isNullOrUndefined(this.shapeResizingArgs)) {this.shapeResizingArgs = shapeResizingArgs; }
        if (isNullOrUndefined(this.shapeMovingArgs)) {this.shapeMovingArgs = shapeMovingArgs; }
    ...
    private triggerShapeChange(shapeResizingArgs: ShapeChangeEventArgs, shapeMovingArgs: ShapeChangeEventArgs, type: string): void {
        ...
        if (type === 'resize') {
            ...
            if (!this.isCropSelection && this.parent.eventType !== 'resize' && isBlazor() && !isNullOrUndefined(parent.events) && this.parent.events.onShapeResizeStart.hasDelegate === true) {
                ...
                if (isNullOrUndefined(this.selectionResizingArgs)) {this.selectionResizingArgs = selectionResizingArgs; }
    ...
    private clickEvent(imageEditorClickEventArgs: ImageEditorClickEventArgs, e: MouseEvent & TouchEvent): void {
        ...
        const cursor: string = !isNullOrUndefined(parent.activeObj.shape) && parent.activeObj.shape === 'text' ?
