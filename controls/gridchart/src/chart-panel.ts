...
export class ChartPanel {
    ...
    private defaultMenuItems: string[] = [
    ...
    private getCustomMenuItems(contextMenuItems: MenuItemModel[]): MenuItemModel[] {
        let chartItem: MenuItemModel[] = [];
        for (const item of contextMenuItems) {
            if (item.items && item.items.length) {
                chartItem = [...chartItem, ...this.getCustomMenuItems(item.items)];
            } else if (item && item.id) {
                const text = item.id.replace(this.parent.element.id + '_cmenu_', '');
    ...
    private getOrderedChartItems(menuItems: MenuItemModel[]): MenuItemModel[] {
        let chartItem: MenuItemModel[] = [];
        let groupedItems: { [key: string]: MenuItemModel[] } = {
            LineChart: [],
            AreaChart: [],
            ColumnChart: [],
            BarChart: [],
        ...
        for (let i: number = 0; i < menuItems.length; i++) {
            const item: MenuItemModel = menuItems[i];
            const chartType: string = this.chartTypeMap[item.id.replace(this.parent.element.id + '_cmenu_', '')];
            if (chartType) {
                groupedItems[chartType].push(item);
            }
        }
        for (const chartType in groupedItems) {
    ...
    private chartTab(): void {
        ...
        if (this.gridChart.chartType) {
            if (this.chartListAccordion) {
                ...
                const availableChartItems = this.getCustomMenuItems(contextMenuModule.contextMenu.items);
                const chartItem = this.getOrderedChartItems(availableChartItems);
                ...
                this.chartListAccordion = new Accordion({
                    expanding: (args: ExpandEventArgs) => {
                        ...
                        if (!content.querySelector('.e-grid-chart-list')) {
                            ...
                            content.innerHTML = '';
                            const chartInfo: MenuItemModel = chartItem
