...
function createSvg(maps) {
    maps.renderer = new sf.svgbase.SvgRenderer(maps.element.id);
    calculateSize(maps);
...
function convertGeoToPoint(latitude, longitude, factor, layer, mapModel) {
    ...
    var size = (mapModel.isTileMap) ? Math.pow(2, 1) * 256 : (sf.base.isNullOrUndefined(factor)) ? Math.min(mapSize.width, mapSize.height) :
        (Math.min(mapSize.width, mapSize.height) * factor);
    if (layer.geometryType === 'Normal') {
        x = sf.base.isNullOrUndefined(factor) ? longitude : Math.abs((longitude - longitudeMinMax.min) * factor);
        y = sf.base.isNullOrUndefined(factor) ? latitude : Math.abs((latitudeMinMax.max - latitude) * factor);
    }
...
function xToCoordinate(mapObject, val) {
    ...
    var totalSize = sf.base.isNullOrUndefined(mapObject.baseSize) ? mapObject.mapAreaRect.width : mapObject.mapAreaRect.width +
...
function measureText(text, font) {
    ...
    if (measureObject === null) {
        measureObject = sf.base.createElement('text', { id: 'mapsmeasuretext' });
...
function convertElementFromLabel(element, labelId, data, index, mapObj) {
    var labelEle = sf.base.isNullOrUndefined(element.childElementCount) ? element[0] : element;
...
function drawSymbols(shape, imageUrl, location, markerID, shapeCustom, markerCollection, maps) {
    ...
    if (shape === 'Circle') {
        ...
        sf.base.merge(pathOptions, { 'href': imageUrl, 'height': size.height, 'width': size.width, x: x, y: y });
...
function getValueFromObject(data, value) {
    if (!sf.base.isNullOrUndefined(data) && !sf.base.isNullOrUndefined(value)) {
        var splits = value.replace(/\[/g, '.').replace(/\]/g, '').split('.');
        if (splits.length === 1) {
            data = data[splits[0]];
        }
        else {
            for (var i = 0; i < splits.length && !sf.base.isNullOrUndefined(data); i++) {
                data = data[splits[i]];
            }
        }
    }
...
function markerColorChoose(eventArgs, data) {
    var color = (!sf.base.isNullOrUndefined(eventArgs.colorValuePath)) ? ((eventArgs.colorValuePath.indexOf('.') > -1) ? (getValueFromObject(data, eventArgs.colorValuePath)).toString() :
        data[eventArgs.colorValuePath]) : data[eventArgs.colorValuePath];
    eventArgs.fill = (!sf.base.isNullOrUndefined(eventArgs.colorValuePath) &&
        !sf.base.isNullOrUndefined(color)) ?
...
function markerShapeChoose(eventArgs, data) {
    if (!sf.base.isNullOrUndefined(eventArgs.shapeValuePath) && !sf.base.isNullOrUndefined(data[eventArgs.shapeValuePath])) {
        ...
        if (data[eventArgs.shapeValuePath] == 'Image') {
            eventArgs.imageUrl = (!sf.base.isNullOrUndefined(eventArgs.imageUrlValuePath) &&
                !sf.base.isNullOrUndefined(data[eventArgs.imageUrlValuePath])) ?
        ...
        var shapes = (!sf.base.isNullOrUndefined(eventArgs.shapeValuePath)) ? ((eventArgs.shapeValuePath.indexOf('.') > -1) ? getValueFromObject(data, eventArgs.shapeValuePath).toString() : eventArgs.shape) : eventArgs.shape;
        eventArgs.shape = shapes;
        var shapeImage = (!sf.base.isNullOrUndefined(eventArgs.imageUrlValuePath)) ? ((eventArgs.imageUrlValuePath.indexOf('.') > -1) ? (getValueFromObject(data, eventArgs.imageUrlValuePath)).toString() : eventArgs.imageUrl) : eventArgs.imageUrl;
    ...
    maps.trigger('markerClusterRendering', eventArg, function (clusterargs) {
        for (var o_1 = 0; o_1 < markerTemplate.childElementCount; o_1++) {
            ...
            if (markerTemplate.childNodes[o_1]['style']['visibility'] !== 'hidden') {
                ...
                if (!sf.base.isNullOrUndefined(bounds1)) {
                    for (var p_1 = o_1 + 1; p_1 < markerTemplate.childElementCount; p_1++) {
                        if (markerTemplate.childNodes[p_1]['style']['visibility'] !== 'hidden') {
                            ...
                            if (!sf.base.isNullOrUndefined(bounds2)) {
        ...
        for (var o = 0; o < clusterGroup.childElementCount; o++) {
            if (clusterGroup.childNodes[o]['style']['visibility'] !== 'hidden') {
                ...
                bounds1 = tempElement.getBoundingClientRect();
                if (!sf.base.isNullOrUndefined(bounds1) && !(tempElement.id.indexOf('_datalabel_') > -1)) {
                    for (var p = o + 1; p < clusterGroup.childElementCount; p++) {
                        if (clusterGroup.childNodes[p]['style']['visibility'] !== 'hidden') {
                            ...
                            if (!sf.base.isNullOrUndefined(bounds2) && !(tempElement1.id.indexOf('_datalabel_') > -1)) {
                                if (bounds1.left > bounds2.right || bounds1.right < bounds2.left
                                    || bounds1.top > bounds2.bottom || bounds1.bottom < bounds2.top) {
                                }
                                else {
    ...
    if (elementId) {
        for (var index = 0; index < elementId.length; index++) {
            if (element.getAttribute('id') === elementId[index]) {
                if (sf.base.isNullOrUndefined(getElement(elementClass.id)) || index === 0) {
...
function maintainStyleClass(id, idClass, fill, opacity, borderColor, borderWidth, maps) {
    if (!getElement(id)) {
        ...
        styleClass = sf.base.createElement('style', {
...
function checkShapeDataFields(dataSource, properties, dataPath, propertyPath, layer) {
    if (!(sf.base.isNullOrUndefined(properties))) {
        for (var i = 0; i < dataSource.length; i++) {
            ...
            var shapeDataPathValue = !sf.base.isNullOrUndefined(shapeDataPath) && isNaN(properties[shapePath])
                ? shapeDataPath.toLowerCase() : shapeDataPath;
            var propertiesShapePathValue = !sf.base.isNullOrUndefined(properties[shapePath]) && isNaN(properties[shapePath])
...
function checkPropertyPath(shapeData, shapePropertyPath, shape) {
    if (!sf.base.isNullOrUndefined(shapeData) && !sf.base.isNullOrUndefined(shape)) {
        if (!sf.base.isNullOrUndefined(shapePropertyPath)) {
            var properties = (Object.prototype.toString.call(shapePropertyPath) === '[object Array]' ?
                shapePropertyPath : [shapePropertyPath]);
            for (var i = 0; i < properties.length; i++) {
                var shapeDataValue = !sf.base.isNullOrUndefined(shapeData) ? shapeData.toLowerCase() : shapeData;
                var shapePropertiesValue = !sf.base.isNullOrUndefined(shape[properties[i]])
...
function isCustomPath(layerData) {
    ...
    if (Object.prototype.toString.call(layerData) === '[object Array]') {
        Array.prototype.forEach.call(layerData, function (layer, index) {
            if (!sf.base.isNullOrUndefined(layer['geometry']) && layer['geometry']['type'] === 'Path') {
                customPath = true;
            }
        });
    }
...
function getTranslate(mapObject, layer, animate) {
    ...
    var checkMethodeZoom = !sf.base.isNullOrUndefined(mapObject.centerLatOfGivenLocation) &&
        !sf.base.isNullOrUndefined(mapObject.centerLongOfGivenLocation) && mapObject.zoomNotApplied;
    if (sf.base.isNullOrUndefined(mapObject.mapScaleValue)) {
    ...
    if (mapObject.zoomSettings.shouldZoomInitially && mapObject.zoomSettings.enable) {
        mapObject.mapScaleValue = scaleFactor = zoomFactorValue = ((mapObject.zoomSettings.shouldZoomInitially || mapObject.enablePersistence) && mapObject.scale == 1)
            ? mapObject.scale : (sf.base.isNullOrUndefined(mapObject.markerZoomFactor)) ? 1 : mapObject.markerZoomFactor;
        ...
        if (!sf.base.isNullOrUndefined(mapObject.markerCenterLatitude) && !sf.base.isNullOrUndefined(mapObject.markerCenterLongitude)) {
    ...
    if (sf.base.isNullOrUndefined(mapObject.currentShapeDataLength)) {
        mapObject.currentShapeDataLength = !sf.base.isNullOrUndefined(layer.shapeData["features"])
    ...
    if ((!sf.base.isNullOrUndefined(centerLongitude) && !sf.base.isNullOrUndefined(centerLatitude)) || checkMethodeZoom) {
        ...
        if (sf.base.isNullOrUndefined(mapObject.previousProjection) || mapObject.previousProjection !== mapObject.projectionType) {
        ...
        if (sf.base.isNullOrUndefined(mapObject.previousProjection) || mapObject.previousProjection !== mapObject.projectionType) {
            ...
            if (!mapObject.zoomSettings.shouldZoomInitially && mapObject.markerZoomFactor === 1 && mapObject.mapScaleValue === 1) {
                ...
                if ((mapObject.currentShapeDataLength !== (!sf.base.isNullOrUndefined(layer.shapeData["features"])
                    ...
                    if (!sf.base.isNullOrUndefined(mapObject.previousProjection) && mapObject.mapScaleValue === 1 && !mapObject.zoomModule.isDragZoom) {
                        scaleFactor = parseFloat(Math.min(size.width / mapWidth, size.height / mapHeight).toFixed(2));
                        mapWidth *= scaleFactor;
                        x = size.x + ((-(min['x'])) + ((size.width / 2) - (mapWidth / 2)));
                        mapHeight *= scaleFactor;
    ...
    if (!sf.base.isNullOrUndefined(mapObject.translatePoint)) {
...
function getZoomTranslate(mapObject, layer, animate) {
    ...
    var checkZoomMethod = !sf.base.isNullOrUndefined(mapObject.centerLongOfGivenLocation) &&
        !sf.base.isNullOrUndefined(mapObject.centerLatOfGivenLocation) && mapObject.zoomNotApplied;
    if (sf.base.isNullOrUndefined(mapObject.previousCenterLatitude) &&
        sf.base.isNullOrUndefined(mapObject.previousCenterLongitude)) {
    ...
    if (sf.base.isNullOrUndefined(mapObject.mapScaleValue) || (zoomFactorValue > mapObject.mapScaleValue)) {
    ...
    if (mapObject.zoomSettings.shouldZoomInitially) {
        mapObject.mapScaleValue = zoomFactorValue = scaleFactor = ((mapObject.enablePersistence || mapObject.zoomSettings.shouldZoomInitially) && mapObject.scale == 1)
            ? mapObject.scale : (sf.base.isNullOrUndefined(mapObject.markerZoomFactor)) ? mapObject.mapScaleValue : mapObject.markerZoomFactor;
    ...
    var mapWidth = Math.abs(max['x'] - min['x']);
    var mapHeight = Math.abs(min['y'] - max['y']);
    if ((!sf.base.isNullOrUndefined(longitude) && !sf.base.isNullOrUndefined(latitude)) || checkZoomMethod) {
        ...
        if ((!sf.base.isNullOrUndefined(mapObject.zoomTranslatePoint) || !sf.base.isNullOrUndefined(mapObject.previousProjection)) && !mapObject.zoomNotApplied) {
        ...
        if (!sf.base.isNullOrUndefined(mapObject.zoomTranslatePoint) || !sf.base.isNullOrUndefined(mapObject.previousProjection)) {
            if (mapObject.previousProjection !== mapObject.projectionType) {
...
function showTooltip(text, size, x, y, areaWidth, areaHeight, id, element, isTouch) {
    ...
    if (!sf.base.isNullOrUndefined(location)) {
        x = location.x;
        y = location.y;
    }
...
function compareZoomFactor(scaleFactor, maps) {
    var previous = sf.base.isNullOrUndefined(maps.shouldZoomPreviousFactor) ?
        null : maps.shouldZoomPreviousFactor;
    var current = sf.base.isNullOrUndefined(maps.shouldZoomCurrentFactor) ?
    ...
    else if (!sf.base.isNullOrUndefined(previous)
        && sf.base.isNullOrUndefined(current)
...
var Font = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Property('Roboto, Noto, Sans-serif')
    ], Font.prototype, "fontFamily", void 0);
    __decorate$1([
        sf.base.Property('Medium')
...
var MarkerClusterSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Property(new Point(0, 0))
...
var NavigationLineSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Complex({ showArrow: false, position: 'Start', size: 5, color: 'black' }, Arrow)
...
var BubbleSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Collection([], ColorMappingSettings)
...
var SubTitleSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Complex({ size: Theme.mapsSubTitleFont.size }, Font)
...
var TitleSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Complex({ size: Theme.mapsTitleFont.size }, Font)
...
var ZoomSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Property(['ZoomIn', 'ZoomOut', 'Reset'])
...
var ToggleLegendSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Complex({ color: '', width: 0 }, Border)
...
var LegendSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Complex({}, ToggleLegendSettings)
...
var DataLabelSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Complex({ width: 0, color: 'transparent' }, Border)
...
var ShapeSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Complex({ width: 0, color: '#000000' }, Border)
...
var LayerSettings = /** @class */ (function (_super) {
    ...
    __decorate$1([
        sf.base.Collection([], InitialShapeSelectionSettings)
...
var Marker = /** @class */ (function () {
        ...
        currentLayer.markerSettings.map(function (markerSettings, markerIndex) {
            ...
            Array.prototype.forEach.call(markerData, function (data, dataIndex) {
                ...
                _this.maps.trigger('markerRendering', eventArgs, function (MarkerArgs) {
                    ...
                    if (!eventArgs.cancel && markerSettings.visible && !sf.base.isNullOrUndefined(lng) && !sf.base.isNullOrUndefined(lat)) {
                        ...
                        var animate$$1 = currentLayer.animationDuration !== 0 || sf.base.isNullOrUndefined(_this.maps.zoomModule);
                        var translate = (_this.maps.isTileMap) ? (currentLayer.type === "SubLayer" && sf.base.isNullOrUndefined(_this.maps.zoomModule)) ? location_1 = convertTileLatLongToPoint(new MapLocation(lng, lat), _this.maps.tileZoomLevel, _this.maps.tileTranslatePoint, true) : new Object() :
                            !sf.base.isNullOrUndefined(_this.maps.zoomModule) && _this.maps.zoomSettings.zoomFactor > 1 ?
                    ...
                    _this.maps.markerNullCount = (sf.base.isNullOrUndefined(lng) || sf.base.isNullOrUndefined(lat)) ? _this.maps.markerNullCount + 1 : _this.maps.markerNullCount;
    ...
    Marker.prototype.calculateZoomCenterPositionAndFactor = function (layersCollection) {
        if (this.maps.zoomSettings.shouldZoomInitially && this.maps.markerModule) {
            ...
            this.maps.markerZoomedState = this.maps.markerZoomedState ? this.maps.markerZoomedState : sf.base.isNullOrUndefined(this.maps.markerZoomFactor) ?
                    ...
                    Array.prototype.forEach.call(currentLayer.markerSettings, function (markerSetting, markerIndex) {
                        ...
                        Array.prototype.forEach.call(markerData, function (data, dataIndex) {
                            ...
                            maxLat_1 = sf.base.isNullOrUndefined(maxLat_1) && dataIndex === 0 ?
                                latitude : maxLat_1;
                            minLat_1 = sf.base.isNullOrUndefined(minLat_1) && dataIndex === 0 ?
                                latitude : minLat_1;
                            maxLong_1 = sf.base.isNullOrUndefined(maxLong_1) && dataIndex === 0 ?
                                longitude : maxLong_1;
            ...
            if (!sf.base.isNullOrUndefined(minLat_1) && !sf.base.isNullOrUndefined(minLong_1) &&
                !sf.base.isNullOrUndefined(maxLong_1) && !sf.base.isNullOrUndefined(maxLat_1)) {
                ...
                if (sf.base.isNullOrUndefined(this.maps.markerZoomCenterPoint) || this.maps.markerZoomedState) {
                ...
                if (this.maps.isTileMap || this.maps.baseMapRectBounds['min']['x'] === 0) {
                    ...
                    if (this.maps.isTileMap) {
                        markerFactor = sf.base.isNullOrUndefined(this.maps.markerZoomFactor) ?
                            zoomLevel : sf.base.isNullOrUndefined(this.maps.mapScaleValue) ?
                            zoomLevel : this.maps.mapScaleValue > 1 && this.maps.markerZoomFactor !== 1 ?
                            this.maps.mapScaleValue : zoomLevel;
                    }
                    else {
                        markerFactor = sf.base.isNullOrUndefined(this.maps.mapScaleValue) ? zoomLevel :
                            (Math.floor(this.maps.scale) !== 1 &&
                                this.maps.mapScaleValue !== zoomLevel)
                                &&
                                    (sf.base.isNullOrUndefined(this.maps.shouldZoomCurrentFactor))
                                ? this.maps.mapScaleValue : zoomLevel;
...
var ColorMapping = /** @class */ (function () {
    ...
    ColorMapping.prototype.getShapeColorMapping = function (shapeSettings, layerData, color) {
        ...
        var equalValue = (!sf.base.isNullOrUndefined(colorValuePath)) ? ((colorValuePath.indexOf('.') > -1) ?
    ...
    ColorMapping.prototype.getColorByValue = function (colorMapping, colorValue, equalValue) {
        ...
        for (var _i = 0, colorMapping_1 = colorMapping; _i < colorMapping_1.length; _i++) {
            ...
            if ((!sf.base.isNullOrUndefined(colorMap.from) && !sf.base.isNullOrUndefined(colorMap.to)
                && (colorValue >= colorMap.from && colorValue <= colorMap.to)) ||
                (colorMap.value === equalValue)) {
                if (Object.prototype.toString.call(colorMap.color) === '[object Array]') {
                    if (!sf.base.isNullOrUndefined(colorMap.value)) {
            ...
            if (((colorValue >= colorMap.from && colorValue <= colorMap.to) || (colorMap.value === equalValue))
                && (!sf.base.isNullOrUndefined(colorMap.minOpacity) && !sf.base.isNullOrUndefined(colorMap.maxOpacity) && fill)) {
                opacity = this.deSaturationColor(colorMap, fill, colorValue, equalValue);
            }
            if ((fill === '' || sf.base.isNullOrUndefined(fill)) && sf.base.isNullOrUndefined(colorMap.from) && sf.base.isNullOrUndefined(colorMap.to)
                && sf.base.isNullOrUndefined(colorMap.minOpacity) && sf.base.isNullOrUndefined(colorMap.maxOpacity) && sf.base.isNullOrUndefined(colorMap.value)) {
...
var LayerPanel = /** @class */ (function () {
    function LayerPanel(map) {
        ...
        this.mapObject = map;
        this.ajaxModule = new sf.base.Ajax();
        this.ajaxResponse = [];
    }
    /* tslint:disable:no-string-literal */
    LayerPanel.prototype.measureLayerPanel = function () {
        ...
        if (this.mapObject.isTileMap && secondaryEle) {
            ...
            var parentElement = sf.base.createElement('div', {
    ...
    LayerPanel.prototype.renderTileLayer = function (panel, layer, layerIndex, bing) {
        ...
        panel.mapObject.defaultState = ((panel.mapObject.zoomSettings.zoomFactor !== 1) &&
            (!sf.base.isNullOrUndefined(panel.mapObject.tileZoomLevel) && panel.mapObject.tileZoomLevel !== 1)) ?
            false : true;
        if (sf.base.isNullOrUndefined(panel.mapObject.previousCenterLatitude) &&
            sf.base.isNullOrUndefined(panel.mapObject.previousCenterLongitude)) {
        ...
        if ((this.mapObject.isTileMap && panel.mapObject.markerModule) && panel.mapObject.zoomSettings.enable) {
            ...
            if (!sf.base.isNullOrUndefined(this.mapObject.markerCenterLatitude) && !sf.base.isNullOrUndefined(this.mapObject.markerCenterLongitude)) {
        ...
        if (!panel.mapObject.zoomSettings.shouldZoomInitially && panel.mapObject.centerPosition.longitude
            ...
            && !sf.base.isNullOrUndefined(panel.mapObject.markerZoomCenterPoint)) {
        ...
        var zoomFactorValue = panel.mapObject.zoomSettings.shouldZoomInitially ?
            sf.base.isNullOrUndefined(panel.mapObject.markerZoomFactor) ? 1 :
                panel.mapObject.markerZoomFactor : panel.mapObject.zoomSettings.zoomFactor;
        zoomFactorValue = (panel.mapObject.enablePersistence) ? ((sf.base.isNullOrUndefined(panel.mapObject.mapScaleValue))
            ? (sf.base.isNullOrUndefined(panel.mapObject.markerZoomFactor) ? panel.mapObject.zoomSettings.zoomFactor :
        ...
        if (sf.base.isNullOrUndefined(panel.mapObject.tileZoomLevel)) {
            ...
            if (!sf.base.isNullOrUndefined(panel.mapObject.tileTranslatePoint) &&
                (panel.mapObject.markerZoomFactor !== panel.mapObject.mapScaleValue
                    || (sf.base.isNullOrUndefined(panel.mapObject.markerZoomFactor)
                        && sf.base.isNullOrUndefined(panel.mapObject.mapScaleValue)))
        ...
        if (zoomFactorValue <= 1 && !sf.base.isNullOrUndefined(panel.mapObject.height) && !panel.mapObject.zoomSettings.shouldZoomInitially
        ...
        if (!sf.base.isNullOrUndefined(panel.mapObject.centerLatOfGivenLocation) && !sf.base.isNullOrUndefined(panel.mapObject.centerLongOfGivenLocation) &&
        ...
        if (this.mapObject.zoomSettings.resetToInitial && this.mapObject.initialCheck && !sf.base.isNullOrUndefined(panel.mapObject.height)
        ...
        if (!sf.base.isNullOrUndefined(panel.mapObject.previousZoomFactor)
            && panel.mapObject.previousZoomFactor !== panel.mapObject.zoomSettings.zoomFactor) {
            panel.mapObject.previousZoomFactor = panel.mapObject.zoomSettings.zoomFactor;
        }
    ...
    LayerPanel.prototype.processLayers = function (layer, layerIndex) {
        ...
        if (!this.mapObject.enablePersistence) {
            ...
            if (navigator.userAgent.indexOf('Edge') === -1) {
                if (!sf.base.isNullOrUndefined(window.localStorage) && window.localStorage.getItem(itemName)) {
        ...
        this.mapObject.trigger('layerRendering', eventArgs, function (observedArgs) {
            if (!eventArgs.cancel && eventArgs.visible) {
                if (layer.layerType !== 'Geometry') {
                    ...
                    if (!sf.base.isNullOrUndefined(layer.shapeData) && (!sf.base.isNullOrUndefined(layer.shapeData['geometries']) ||
                        !sf.base.isNullOrUndefined(layer.shapeData['features']))) {
                        var featureData = (!sf.base.isNullOrUndefined(layer.shapeData['geometries']) &&
                            layer.shapeData['geometries'].length > 0 ? layer.shapeData['geometries'] :
                            layer.shapeData['features']);
                        layer.layerData = [];
                        var bbox = layer.shapeData['bbox'];
                        if (!sf.base.isNullOrUndefined(bbox) && layer.isBaseLayer) {
                            _this.mapObject.baseMapBounds = new GeoLocation({ min: bbox[0][1], max: bbox[1][1] }, { min: bbox[0][0], max: bbox[1][0] });
                        }
                        else if (sf.base.isNullOrUndefined(_this.mapObject.baseMapBounds) && !isCustomPath(featureData)) {
    ...
    LayerPanel.prototype.calculatePathCollection = function (layerIndex, renderData) {
        ...
        Array.prototype.forEach.call(renderData, function (geometryData, index) {
            if (!sf.base.isNullOrUndefined(geometryData['geometry']) || !sf.base.isNullOrUndefined(geometryData['coordinates'])) {
                var type = !sf.base.isNullOrUndefined(geometryData['geometry']) ? geometryData['geometry']['type'] : geometryData['type'];
                var coords = !sf.base.isNullOrUndefined(geometryData['geometry']) ? geometryData['geometry']['coordinates'] :
        ...
        if (sf.base.isNullOrUndefined(this.mapObject.baseMapRectBounds) && this.currentLayer.isBaseLayer) {
        ...
        if (this.currentLayer.layerData.length !== 0) {
            var _loop_1 = function (i) {
                ...
                if (shapeSettings.colorValuePath !== null && !sf.base.isNullOrUndefined(currentShapeData['property'])) {
                    ...
                    fill = !sf.base.isNullOrUndefined(fill) ? fill : shapeSettings.fill;
                }
                ...
                var shapeRenderingSuccess = function (eventArgs) {
                    var drawingType = !sf.base.isNullOrUndefined(currentShapeData['_isMultiPolygon'])
                        ? 'MultiPolygon' : sf.base.isNullOrUndefined(currentShapeData['type']) ? currentShapeData[0]['type'] : currentShapeData['type'];
                    ...
                    if (!sf.base.isNullOrUndefined(pathEle)) {
                        ...
                        for (var j = 0; j < property.length; j++) {
                            if (!sf.base.isNullOrUndefined(currentShapeData['property'])) {
                                properties = property[j];
                                break;
                            }
                        }
                        pathEle.setAttribute('aria-label', ((!sf.base.isNullOrUndefined(currentShapeData['property'])) ?
    ...
    LayerPanel.prototype.renderBubble = function (layer, bubbleData, color, range, bubbleIndex, dataIndex, group, layerIndex, bubbleSettings) {
        if (sf.base.isNullOrUndefined(this.mapObject.bubbleModule) || !bubbleSettings.visible) {
            return null;
        }
    ...
    LayerPanel.prototype.getShapeColorMapping = function (layer, shape, color) {
        ...
        if (layer.shapeSettings.colorMapping.length === 0 && sf.base.isNullOrUndefined(layer.dataSource)) {
            return color;
        }
    ...
    LayerPanel.prototype.translateLayerElements = function (layerElement, index) {
        ...
        if (!sf.base.isNullOrUndefined(this.mapObject.baseMapRectBounds)) {
            ...
            var animate$$1 = duration !== 0 || sf.base.isNullOrUndefined(this.mapObject.zoomModule);
            this.mapObject.baseTranslatePoint = this.mapObject.zoomTranslatePoint;
            var translate = void 0;
            if (this.mapObject.zoomSettings.zoomFactor > 1 && !sf.base.isNullOrUndefined(this.mapObject.zoomModule)) {
            ...
            for (var i = 0; i < layerElement.childElementCount; i++) {
                ...
                if (!(childNode.id.indexOf('_Markers_Group') > -1) &&
                    ...
                    if (duration > 0 && !sf.base.isNullOrUndefined(this.mapObject.zoomModule)) {
        ...
        else if (this.mapObject.isTileMap && !sf.base.isNullOrUndefined(this.mapObject.scale)) {
    ...
    LayerPanel.prototype.calculateRectBounds = function (layerData) {
        ...
        Array.prototype.forEach.call(layerData, function (obj, index) {
            if (!sf.base.isNullOrUndefined(obj['geometry']) || !sf.base.isNullOrUndefined(obj['coordinates'])) {
                var type = !sf.base.isNullOrUndefined(obj['geometry']) ? obj['geometry']['type'] : obj['type'];
                var coordinates = !sf.base.isNullOrUndefined(obj['geometry']) ? obj['geometry']['coordinates'] : obj['coordinates'];
                switch (type.toLowerCase()) {
                    case 'polygon':
    ...
    LayerPanel.prototype.calculatePolygonBox = function (coordinates, data, properties) {
        ...
        coordinates.map(function (currentPoint, index) {
            ...
            if ((longitude >= bounds.longitude.min && longitude <= bounds.longitude.max)
                ...
                var point = convertGeoToPoint(latitude, longitude, _this.currentFactor, _this.currentLayer, _this.mapObject);
                if (sf.base.isNullOrUndefined(_this.rectBounds)) {
    ...
    LayerPanel.prototype.calculateRectBox = function (coordinates) {
        ...
        Array.prototype.forEach.call(coordinates, function (currentCoords) {
            if (sf.base.isNullOrUndefined(_this.mapObject.baseMapBounds)) {
    ...
    LayerPanel.prototype.generateTiles = function (zoomLevel, tileTranslatePoint, zoomType, bing, position) {
        ...
        if (!sf.base.isNullOrUndefined(zoomType)) {
        ...
        var proxTiles = sf.base.extend([], this.tiles, [], true);
        for (var _i = 0, _a = this.mapObject.layers; _i < _a.length; _i++) {
            ...
            if (layer.layerType === 'OSM' || layer.layerType === 'Bing') {
                for (var _b = 0, proxTiles_1 = proxTiles; _b < proxTiles_1.length; _b++) {
                    ...
                    var subtile = sf.base.extend(baseTile, {}, {}, true);
    ...
    LayerPanel.prototype.arrangeTiles = function (type, x, y) {
        ...
        var timeOut;
        if (!sf.base.isNullOrUndefined(type) && type !== 'Pan' && type !== 'Reset' && type.indexOf('ZoomOut') === -1) {
    ...
    LayerPanel.prototype.tileAnimation = function (zoomType, translateX, translateY) {
        ...
        if (!sf.base.isNullOrUndefined(element1)) {
    ...
    LayerPanel.prototype.renderGoogleMap = function (apikey, zoom) {
        ...
        var mapWidth;
        var mapHeight;
        if (sf.base.isNullOrUndefined(parseInt(map.width, 10))) {
            mapWidth = parseInt(map.width, 10) - 22;
        }
        else {
            mapWidth = Math.round(map.mapAreaRect.width);
        }
        if (sf.base.isNullOrUndefined(parseInt(map.height, 10))) {
    ...
    LayerPanel.prototype.panTileMap = function (factorX, factorY, centerPosition) {
        ...
        if ((sf.base.isNullOrUndefined(this.mapObject.tileTranslatePoint) || (this.mapObject.tileTranslatePoint.y === 0 &&
            this.mapObject.tileTranslatePoint.x === 0)) || (sf.base.isNullOrUndefined(this.mapObject.previousTileWidth) ||
            sf.base.isNullOrUndefined(this.mapObject.previousTileHeight))) {
            this.mapObject.previousTileWidth = factorX;
            this.mapObject.previousTileHeight = factorY;
        }
        if (!sf.base.isNullOrUndefined(this.mapObject.tileTranslatePoint) && (sf.base.isNullOrUndefined(centerPosition.x)) &&
        ...
        if (!sf.base.isNullOrUndefined(this.mapObject.tileTranslatePoint) && !this.mapObject.zoomNotApplied) {
...
var Annotations = /** @class */ (function () {
    ...
    Annotations.prototype.renderAnnotationElements = function () {
        ...
        var annotationGroup = sf.base.createElement('div', { id: this.map.element.id + '_Annotations_Group' });
        ...
        if (annotationGroup.childElementCount > 0 && !(sf.base.isNullOrUndefined(getElementByID(secondaryID)))) {
            getElementByID(secondaryID).appendChild(annotationGroup);
            for (var i = 0; i < this.map.annotations.length; i++) {
                sf.base.updateBlazorTemplate(this.map.element.id + '_ContentTemplate_' + i, 'ContentTemplate', this.map.annotations[i]);
            }
        }
    };
    /**
    ...
    ExportUtils.prototype.export = function (type, fileName, exportDownload, orientation) {
        ...
        var promise = new Promise(function (resolve, reject) {
            ...
            if (type === 'SVG') {
                ...
                var pdfDocument_1 = new sf.pdfexport.PdfDocument();
                ...
                if (!_this.control.isTileMap) {
                    image_1.onload = (function () {
                        ...
                        if (type === 'PDF') {
                            ...
                            pdfDocument_1.pages.add().graphics.drawImage(new sf.pdfexport.PdfBitmap(imageString), 0, 0, (_this.control.availableSize.width - 60), _this.control.availableSize.height);
                            if (exportDownload) {
                        ...
                        tileImg.onload = (function () {
                            ...
                            if (i === tileLength_1 + 1) {
                                if (type === 'PDF') {
                                    ...
                                    pdfDocument_1.pages.add().graphics.drawImage(new sf.pdfexport.PdfBitmap(x), 0, 0, (_this.control.availableSize.width - 60), _this.control.availableSize.height);
                                    if (exportDownload) {
                                        pdfDocument_1.save(fileName + '.pdf');
                                        pdfDocument_1.destroy();
                                    }
...
var Maps = /** @class */ (function (_super) {
    ...
    Maps.prototype.mergePersistMapsData = function () {
        var data;
        if (!sf.base.isNullOrUndefined(window.localStorage)) {
            data = window.localStorage.getItem(this.getModuleName() + this.element.id);
        }
        if (!(sf.base.isNullOrUndefined(data) || (data === ''))) {
    ...
    Maps.prototype.processResponseJsonData = function (processType, data, layer, dataType) {
        ...
        else if (sf.base.isNullOrUndefined(processType)) {
            this.renderMap();
        }
    };
    Maps.prototype.renderMap = function () {
        ...
        if (!sf.base.isNullOrUndefined(this.dataLabelModule)) {
        ...
        if (!sf.base.isNullOrUndefined(document.getElementById(this.element.id + '_tile_parent'))) {
        ...
        for (var i = 0; i < this.layers.length; i++) {
            ...
            if (markerLength >= 0) {
                if (this.layers[i].dataLabelSettings.visible || this.layers[i].markerSettings[markerLength].template) {
                    sf.base.updateBlazorTemplate(this.element.id + '_LabelTemplate', 'LabelTemplate', this.layers[i].dataLabelSettings);
                    for (var j = 0; j < this.layers[i].markerSettings.length; j++) {
                        sf.base.updateBlazorTemplate(this.element.id + '_MarkerTemplate' + j, 'MarkerTemplate', this.layers[i].markerSettings[j]);
                    }
                }
            }
        }
    ...
    Maps.prototype.zoomingChange = function () {
        ...
        if (this.isTileMap && getElementByID(this.element.id + '_Tile_SVG') && getElementByID(this.element.id + '_tile_parent')) {
            ...
            if (!sf.base.isNullOrUndefined(markerTemplateElements) && markerTemplateElements.length > 0) {
        ...
        if (this.zoomSettings.zoomFactor >= 1) {
            ...
            if (!sf.base.isNullOrUndefined(elements) && elements.childElementCount > 0) {
    ...
    Maps.prototype.arrangeTemplate = function () {
        ...
        if (!sf.base.isNullOrUndefined(templateElements) && templateElements.length > 0 &&
            ...
            for (var i = 0; i < templateElements.length; i++) {
                ...
                if (!sf.base.isNullOrUndefined(templateGroupEle) && templateGroupEle.childElementCount > 0) {
    ...
    Maps.prototype.renderBorder = function () {
        ...
        if ((width > 0 || (this.background || this.themeStyle.backgroundColor)) && sf.base.isNullOrUndefined(borderElement)) {
    ...
    Maps.prototype.createSVG = function () {
        ...
        for (var i = 0; i < this.layers.length; i++) {
            for (var j = 0; j < this.layers[i].markerSettings.length; j++) {
                sf.base.resetBlazorTemplate(this.element.id + '_MarkerTemplate' + j, 'MarkerTemplate');
            }
        }
        this.removeSvg();
        createSvg(this);
    ...
    Maps.prototype.removeSvg = function () {
        for (var i = 0; i < this.annotations.length; i++) {
            sf.base.resetBlazorTemplate(this.element.id + '_ContentTemplate_' + i, 'ContentTemplate');
        }
        removeElement(this.element.id + '_Secondary_Element');
        removeElement(this.element.id + '_tile_parent');
        removeElement(this.element.id + '_tiles');
        if (document.getElementsByClassName('e-tooltip-wrap')[0]) {
            sf.base.remove(document.getElementsByClassName('e-tooltip-wrap')[0]);
        }
        if (this.svgObject) {
    ...
    Maps.prototype.wireEVents = function () {
        ...
        sf.base.EventHandler.add(this.element, 'click', this.mapsOnClick, this);
        // EventHandler.add(this.element, 'contextmenu', this.mapsOnRightClick, this);
        sf.base.EventHandler.add(this.element, 'dblclick', this.mapsOnDoubleClick, this);
        sf.base.EventHandler.add(this.element, sf.base.Browser.touchStartEvent, this.mouseDownOnMap, this);
        sf.base.EventHandler.add(this.element, sf.base.Browser.touchMoveEvent, this.mouseMoveOnMap, this);
        sf.base.EventHandler.add(this.element, sf.base.Browser.touchEndEvent, this.mouseEndOnMap, this);
        sf.base.EventHandler.add(this.element, 'pointerleave mouseleave', this.mouseLeaveOnMap, this);
    ...
    Maps.prototype.unWireEVents = function () {
        //let cancelEvent: string = Browser.isPointer ? 'pointerleave' : 'mouseleave';
        sf.base.EventHandler.remove(this.element, 'click', this.mapsOnClick);
        // EventHandler.remove(this.element, 'contextmenu', this.mapsOnRightClick);
        sf.base.EventHandler.remove(this.element, 'dblclick', this.mapsOnDoubleClick);
        sf.base.EventHandler.remove(this.element, sf.base.Browser.touchStartEvent, this.mouseDownOnMap);
        sf.base.EventHandler.remove(this.element, sf.base.Browser.touchMoveEvent, this.mouseMoveOnMap);
        sf.base.EventHandler.remove(this.element, sf.base.Browser.touchEndEvent, this.mouseEndOnMap);
        sf.base.EventHandler.remove(this.element, 'pointerleave mouseleave', this.mouseLeaveOnMap);
    ...
    Maps.prototype.mouseEndOnMap = function (e) {
        ...
        if (this.isTouch) {
            ...
            if (!sf.base.isNullOrUndefined(this.legendModule)) {
    ...
    Maps.prototype.zoomByPosition = function (centerPosition, zoomFactor) {
        ...
        if (!this.isTileMap && this.zoomModule) {
            if (!sf.base.isNullOrUndefined(centerPosition)) {
    ...
    Maps.prototype.zoomToCoordinates = function (minLatitude, minLongitude, maxLatitude, maxLongitude) {
        ...
        if (sf.base.isNullOrUndefined(maxLatitude) && sf.base.isNullOrUndefined(maxLongitude)
            || sf.base.isNullOrUndefined(minLatitude) && sf.base.isNullOrUndefined(minLongitude)) {
            minLatitude = sf.base.isNullOrUndefined(minLatitude) ? 0 : minLatitude;
            minLongitude = sf.base.isNullOrUndefined(minLatitude) ? 0 : minLongitude;
            maxLatitude = sf.base.isNullOrUndefined(maxLatitude) ? minLatitude : maxLatitude;
            maxLongitude = sf.base.isNullOrUndefined(maxLongitude) ? minLongitude : maxLongitude;
                ...
                case 'baseLayerIndex':
                    if (prop === 'layers') {
                        ...
                        for (var x = 0; x < layerPropLength; x++) {
                            if (!sf.base.isNullOrUndefined(newProp.layers[x])) {
    ...
    Maps.prototype.export = function (type, fileName, orientation, isDownload) {
        ...
        if (sf.base.isNullOrUndefined(isDownload) || isDownload) {
    ...
    __decorate([
        sf.base.Complex({ latitude: null, longitude: null }, CenterPosition)
    ], Maps.prototype, "centerPosition", void 0);
    __decorate([
        sf.base.Complex({}, MapsAreaSettings)
...
var Bubble = /** @class */ (function () {
    ...
    Bubble.prototype.renderBubble = function (bubbleSettings, shapeData, color, range, bubbleIndex, dataIndex, layerIndex, layer, group, bubbleID) {
        ...
        var colorValue = (!sf.base.isNullOrUndefined(colorValuePath)) ? ((colorValuePath.indexOf('.') > -1) ?
        ...
        var bubbleValue = (!sf.base.isNullOrUndefined(bubbleSettings.valuePath)) ? ((bubbleSettings.valuePath.indexOf('.') > -1) ?
        ...
        var opacity;
        var bubbleColor;
        if (isNaN(bubbleValue) && isNaN(colorValue) && sf.base.isNullOrUndefined(equalValue)) {
            return null;
        }
        ...
        bubbleColor = (Object.prototype.toString.call(shapeColor) === '[object Object]' &&
            !sf.base.isNullOrUndefined(shapeColor['fill'])) ? shapeColor['fill'] : color;
        opacity = (Object.prototype.toString.call(shapeColor) === '[object Object]' &&
            !sf.base.isNullOrUndefined(shapeColor['opacity'])) ? shapeColor['opacity'] : bubbleSettings.opacity;
        var shapePoints = [[]];
        ...
        for (var i = 0, len = layerData.length; i < len; i++) {
            ...
            var shapeDataLayerPathValue = !sf.base.isNullOrUndefined(shapeData[layer.shapeDataPath]) &&
                isNaN(shapeData[layer.shapeDataPath]) ? shapeData[layer.shapeDataPath].toLowerCase() : shapeData[layer.shapeDataPath];
            var shapePathValue = !sf.base.isNullOrUndefined(shape[shapePath]) && isNaN(shape[shapePath])
        ...
        if (bubbleSettings.visible) {
            if (!sf.base.isNullOrUndefined(center)) {
            ...
            this.maps.trigger('bubbleRendering', eventArgs, function (bubbleArgs) {
                ...
                var translate;
                var animate$$1 = layer.animationDuration !== 0 || sf.base.isNullOrUndefined(_this.maps.zoomModule);
                if (_this.maps.zoomSettings.zoomFactor > 1 && !sf.base.isNullOrUndefined(_this.maps.zoomModule)) {
    ...
    Bubble.prototype.getPoints = function (shape, points) {
        if (sf.base.isNullOrUndefined(shape.map)) {
            points = shape['point'];
        }
        else {
...
var DataLabel = /** @class */ (function () {
    ...
    DataLabel.prototype.getDataLabel = function (dataSource, labelPath, shapeName, shapeDataPath) {
        ...
        for (var i = 0; i < dataSource.length; i++) {
            ...
            dataShapePathValue = !sf.base.isNullOrUndefined(data[shapeDataPath]) && isNaN(data[shapeDataPath]) ?
                data[shapeDataPath].toLowerCase() : data[shapeDataPath];
            shapeName = !sf.base.isNullOrUndefined(shapeName) ? shapeName.toString() : shapeName;
            shapeNameValue = !sf.base.isNullOrUndefined(shapeName) ? shapeName.toLowerCase() : shapeName;
    ...
    DataLabel.prototype.renderLabel = function (layer, layerIndex, shape, layerData, group, labelTemplateElement, index, intersect) {
        ...
        var propertyPath;
        var isPoint = false;
        var animate$$1 = layer.animationDuration !== 0 || sf.base.isNullOrUndefined(this.maps.zoomModule);
        var translate = (this.maps.isTileMap) ? new Object() : ((this.maps.zoomSettings.zoomFactor > 1 &&
            !sf.base.isNullOrUndefined(this.maps.zoomModule)) ? getZoomTranslate(this.maps, layer, animate$$1) :
        ...
        var shapeWidth;
        var scaleZoomValue = !sf.base.isNullOrUndefined(this.maps.scale) ? Math.floor(this.maps.scale) : 1;
        var zoomLabelsPosition = this.maps.zoomSettings.enable ? !sf.base.isNullOrUndefined(this.maps.zoomShapeCollection) &&
        ...
        datasrcObj = this.getDataLabel(layer.dataSource, labelpath, shapeData['properties'][propertyPath], layer.shapeDataPath);
        if (!sf.base.isNullOrUndefined(shapes['property']) && ((shapeProperties[labelpath]) || datasrcObj)) {
            shapePoint = [[]];
        ...
        text = (!sf.base.isNullOrUndefined(datasrcObj)) ? !sf.base.isNullOrUndefined(datasrcObj[labelpath]) ?
        ...
        if (!sf.base.isNullOrUndefined(text) && !sf.base.isNullOrUndefined(location)) {
            ...
            if (!sf.base.isNullOrUndefined(this.maps.format) && !isNaN(parseFloat(text))) {
                if (this.maps.useGroupingSeparator) {
                    text = Internalize(this.maps, parseFloat(text));
                    if (!sf.base.isNullOrUndefined(datasrcObj)) {
            ...
            this.maps.trigger('dataLabelRendering', eventargs_1, function (labelArgs) {
                ...
                if (!sf.base.isNullOrUndefined(_this.maps.dataLabelShape)) {
                ...
                if (eventargs_1.template !== '') {
                    ...
                    var templateElement = templateFn ? templateFn(!sf.base.isNullOrUndefined(datasrcObj) ?
                    ...
                    labelElement = convertElementFromLabel(templateElement, labelId, !sf.base.isNullOrUndefined(datasrcObj) ? datasrcObj : shapeData['properties'], index, _this.maps);
                    ...
                    if (dataLabelSettings.intersectionAction === 'Hide') {
                        for (var i = 0; i < intersect.length; i++) {
                            if (!sf.base.isNullOrUndefined(intersect[i])) {
                    ...
                    if (dataLabelSettings.intersectionAction === 'Trim') {
                        for (var j = 0; j < intersect.length; j++) {
                            if (!sf.base.isNullOrUndefined(intersect[j])) {
                ...
                _this.dataLabelCollections.push({
                    ...
                    element: sf.base.isNullOrUndefined(labelElement) ? element : labelElement,
                    layerIndex: layerIndex,
...
var NavigationLine = /** @class */ (function () {
    ...
    NavigationLine.prototype.renderNavigation = function (layer, factor, layerIndex) {
        ...
        for (var i = 0; i < navigation.length; i++) {
            ...
            showArrow = (sf.base.isNullOrUndefined(arrowSettings)) ? false : arrowSettings['properties']['showArrow'];
            ...
            for (var j = 0; j < point['length'] - 1; j++) {
                ...
                if (point[j]['x'] !== point[j + 1]['x']) {
                    if (showArrow) {
                        ...
                        offSet = (sf.base.isNullOrUndefined(offSet)) ? 0 : offSet;
                    ...
                    navigationEle = this.maps.renderer.drawPath(pathOption);
                    if (!sf.base.isNullOrUndefined(arrowPosition)) {
...
var Legend = /** @class */ (function () {
    ...
    Legend.prototype.calculateLegendBounds = function () {
        ...
        Array.prototype.forEach.call(map.layersCollection, function (layer, layerIndex) {
            if (!sf.base.isNullOrUndefined(layer.shapeData)) {
        ...
        if (this.legendCollection.length > 0) {
            ...
            if (legendMode === 'Interactive') {
                ...
                rectWidth = (arrangement === 'Horizontal') ? (sf.base.isNullOrUndefined(legendWidth)) ? (map.mapAreaRect.width / legendLength) :
                    (legendWidth / legendLength) : (sf.base.isNullOrUndefined(legendWidth)) ? defaultSize : legendWidth;
                rectHeight = (arrangement === 'Horizontal') ? (sf.base.isNullOrUndefined(legendHeight)) ? defaultSize : legendHeight :
                    (sf.base.isNullOrUndefined(legendHeight)) ? (map.mapAreaRect.height / legendLength) : (legendHeight / legendLength);
                startX = 0;
                ...
                legendWidth = (sf.base.isNullOrUndefined(legendWidth)) ? map.mapAreaRect.width : legendWidth;
                legendHeight = (sf.base.isNullOrUndefined(legendHeight)) ? map.mapAreaRect.height : legendHeight;
                var j = 0;
                this.page = 0;
                for (var i = 0; i < this.legendCollection.length; i++) {
                    var legendItem = this.legendCollection[i];
    ...
    Legend.prototype.drawLegendItem = function (page) {
        ...
        if (page >= 0 && page < this.totalPages.length) {
            if (querySelector(this.legendGroup.id, this.maps.element.id)) {
                sf.base.remove(querySelector(this.legendGroup.id, this.maps.element.id));
            }
            for (var i = 0; i < this.totalPages[page]['Collection'].length; i++) {
                ...
                var shape = ((legend.type === 'Markers') ? ((sf.base.isNullOrUndefined(collection['ImageSrc'])) ?
                    legend.shape : 'Image') : collection['legendShape']);
                var strokeColor = (legend.shape === 'HorizontalLine' || legend.shape === 'VerticalLine'
                    || legend.shape === 'Cross') ? sf.base.isNullOrUndefined(legend.fill) ? '#000000' : legend.fill : shapeBorder.color;
                ...
                var imageUrl = ((sf.base.isNullOrUndefined(collection['ImageSrc'])) ? legend.shape : collection['ImageSrc']);
        ...
        if (interactProcess) {
            for (var i = 0; i < collection.length; i++) {
                ...
                if (textEle.textContent === collection[i]['text'] && collection[i]['data'].length > 0
                    ...
                    var module = void 0;
                    var data = void 0;
                    if (!sf.base.isNullOrUndefined(layer)) {
                        ...
                        for (var j = 0; j < data.length; j++) {
                            ...
                            if (shapeEle !== null) {
                                ...
                                if (value === 'highlight' && shapeMatch) {
                                    ...
                                    this.setColor(shapeEle, !sf.base.isNullOrUndefined(module.fill) ? module.fill : shapeOldColor, module.opacity.toString(), module.border.color, module.border.width.toString(), 'highlight');
                                    this.setColor(targetElement, !sf.base.isNullOrUndefined(module.fill) ? module.fill : legendHighlightColor, module.opacity.toString(), module.border.color, module.border.width.toString(), 'highlight');
                                }
                                    ...
                                    this.setColor(targetElement, !sf.base.isNullOrUndefined(module.fill) ? module.fill : legendSelectionColor, module.opacity.toString(), module.border.color, module.border.width.toString(), 'selection');
                                    this.setColor(shapeEle, !sf.base.isNullOrUndefined(module.fill) ? module.fill : legendSelectionColor, module.opacity.toString(), module.border.color, module.border.width.toString(), 'selection');
                ...
                if (getValue === 'highlight' && shapeMatched) {
                    ...
                    if (length_1 > 0) {
                        for (var j = 0; j < length_1; j++) {
                            if (shapeElement['LegendEle'] === this.maps.legendSelectionCollection[j]['legendElement']) {
                                ...
                                this.removeShapeHighlightCollection();
                                this.setColor(legendShape, !sf.base.isNullOrUndefined(module.fill) ? module.fill : legendShape.getAttribute('fill'), module.opacity.toString(), module.border.color, module.border.width.toString(), 'highlight');
                            }
                        }
                    }
                    else {
                    ...
                    if (selectionEle && (selectionEle['IsSelected'] && targetElement.getAttribute('class') === 'ShapeselectionMapStyle')) {
                        ...
                        if (multiSelection <= 1 && (!module.enableMultiSelect ?
                            ...
                            if (!sf.base.isNullOrUndefined(shapeElement['LegendEle'])) {
                        ...
                        if ((selectionEle === undefined || (selectionEle && !selectionEle['IsSelected'])) &&
                            !sf.base.isNullOrUndefined(legendShape)) {
                        ...
                        if (!sf.base.isNullOrUndefined(legendShape)) {
                            this.setColor(legendShape, !sf.base.isNullOrUndefined(module.fill) ? module.fill : legendShape.getAttribute('fill'), module.opacity.toString(), module.border.color, module.border.width.toString(), 'selection');
                            var legendSelectionIndex = this.getIndexofLegend(this.maps.legendSelectionCollection, legendShape);
    ...
    Legend.prototype.isTargetSelected = function (target, collection) {
        var selectEle;
        for (var i = 0; i < collection.length; i++) {
            if (!sf.base.isNullOrUndefined(target['LegendEle'].getAttribute('id')) &&
    ...
    Legend.prototype.removeAllSelections = function () {
        ...
        for (var j = 0; j < this.maps.selectedLegendElementId.length; j++) {
            ...
            if (!sf.base.isNullOrUndefined(legendElement)) {
    ...
    Legend.prototype.legendIndexOnShape = function (data, index) {
        ...
        if (legendType === 'Default' && !sf.base.isNullOrUndefined(this.maps.legendModule.totalPages)) {
    ...
    Legend.prototype.shapeDataOnLegend = function (targetElement) {
        ...
        for (var i = 0; i < collection.length; i++) {
            ...
            if (process) {
                if (sf.base.isNullOrUndefined(currentElement['LegendEle'])) {
    ...
    Legend.prototype.shapesOfLegend = function (targetElement) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            var shapeElement = document.getElementById(this.maps.element.id + '_LayerIndex_' +
                layerIndex + '_shapeIndex_' + shapeIndex + '_dataIndex_' + dataIndex);
            if (!sf.base.isNullOrUndefined(shapeElement)) {
    ...
    Legend.prototype.legendToggle = function () {
        ...
        if (this.maps.selectedLegendElementId) {
            ...
            for (var j = 0; j < this.maps.selectedLegendElementId.length; j++) {
                ...
                if (!sf.base.isNullOrUndefined(selectedElement)) {
                    var fill = !sf.base.isNullOrUndefined(this.maps.legendSelectionClass.fill) ?
        ...
        if (this.maps.toggledLegendId) {
            for (var j = 0; j < this.maps.toggledLegendId.length; j++) {
                ...
                if (!sf.base.isNullOrUndefined(textElement)) {
    ...
    Legend.prototype.getLegendAlignment = function (map, width, height, legend) {
        ...
        var spacing = 10;
        var totalRect;
        totalRect = sf.base.extend({}, map.mapAreaRect, totalRect, true);
    ...
    Legend.prototype.getMarkersLegendCollections = function (layerIndex, markers) {
        ...
        Array.prototype.forEach.call(markers, function (marker$$1, markerIndex) {
            ...
            Array.prototype.forEach.call(dataSource, function (data, dataIndex) {
                var imageSrc = null;
                var showLegend = sf.base.isNullOrUndefined(data[_this.maps.legendSettings.showLegendPath]) ? true :
                    data[_this.maps.legendSettings.showLegendPath];
                if (marker$$1.visible && showLegend && (!sf.base.isNullOrUndefined(data['latitude'])) && (!sf.base.isNullOrUndefined(data['longitude']))) {
                    if (marker$$1.template) {
                        templateFn = getTemplateFunction(marker$$1.template);
                        var templateElement = templateFn(_this.maps);
                        var markerEle = sf.base.isNullOrUndefined(templateElement.childElementCount) ? templateElement[0] :
                            templateElement;
                        imageSrc = markerEle.querySelector('img').src;
                    }
                    var text = sf.base.isNullOrUndefined(data[field]) ? '' : data[field];
    ...
    Legend.prototype.getRangeLegendCollection = function (layerIndex, layerData, colorMapping, dataSource, dataPath, colorValuePath, propertyPath) {
        ...
        var _loop_1 = function (colorMap) {
            if (!sf.base.isNullOrUndefined(colorMap.from) && !sf.base.isNullOrUndefined(colorMap.to)) {
                legendText = !sf.base.isNullOrUndefined(colorMap.label) ? colorMap.label : colorMap.from + ' - ' + colorMap.to;
                rangeData = [];
                ...
                var legendFill = (sf.base.isNullOrUndefined(fill)) ? Object.prototype.toString.call(colorMap.color) === '[object Array]' ?
                    !sf.base.isNullOrUndefined(colorMap.value) ? colorMap.color[0] : this_1.legendGradientColor(colorMap, legendIndex) :
                    colorMap.color : fill;
                legendIndex++;
    ...
    Legend.prototype.getEqualLegendCollection = function (layerIndex, layerData, colorMapping, dataSource, dataPath, colorValuePath, propertyPath) {
        ...
        var _loop_2 = function (colorMap) {
            if (!sf.base.isNullOrUndefined(colorMap.value)) {
                legendText = !sf.base.isNullOrUndefined(colorMap.label) ? colorMap.label : colorMap.value;
                equalData = [];
            ...
            else if (sf.base.isNullOrUndefined(colorMap.minOpacity) && sf.base.isNullOrUndefined(colorMap.maxOpacity) && sf.base.isNullOrUndefined(colorMap.value)
                && sf.base.isNullOrUndefined(colorMap.from) && sf.base.isNullOrUndefined(colorMap.to) && !sf.base.isNullOrUndefined(colorMap.color)) {
                Array.prototype.forEach.call(dataSource, function (data, dataIndex) {
                    var equalValue = ((colorValuePath.indexOf(".") > -1) ? (getValueFromObject(data, colorValuePath)) :
                        (data[colorValuePath]));
                ...
                legendText = !sf.base.isNullOrUndefined(colorMap.label) ? colorMap.label : 'Others';
    ...
    Legend.prototype.getDataLegendCollection = function (layerIndex, layerData, colorMapping, dataSource, dataPath, colorValuePath, propertyPath) {
        ...
        if (!sf.base.isNullOrUndefined(colorValuePath) && !sf.base.isNullOrUndefined(dataSource)) {
            Array.prototype.forEach.call(dataSource, function (data, dataIndex) {
                var showLegend = sf.base.isNullOrUndefined(_this.maps.legendSettings.showLegendPath) ?
                    true : sf.base.isNullOrUndefined(data[_this.maps.legendSettings.showLegendPath]) ?
                ...
                var dataValue = ((colorValuePath.indexOf(".") > -1) ? (getValueFromObject(data, colorValuePath)) :
                    (data[colorValuePath]));
                var newData = [];
                var legendFill = (sf.base.isNullOrUndefined(fill)) ? dataValue : fill;
                if (!sf.base.isNullOrUndefined(dataValue) && colorMapping.length === 0) {
                    legendText = !sf.base.isNullOrUndefined(data[valuePath]) ? ((valuePath.indexOf(".") > -1) ?
    ...
    Legend.prototype.interactiveHandler = function (e) {
        ...
        if (target.id.indexOf(hoverId) > 1) {
            ...
            if (!(sf.base.isNullOrUndefined(querySelector(id, this.maps.element.id)))) {
                sf.base.remove(querySelector(id, this.maps.element.id));
            }
            ...
            if (legend.visible && this.legendRenderingCollections.length > 0
                ...
                for (var i = 0; i < this.legendCollection.length; i++) {
                    ...
                    if (!sf.base.isNullOrUndefined(currentData['data'])) {
    ...
    Legend.prototype.addEventListener = function () {
        ...
        this.maps.on(sf.base.Browser.touchMoveEvent, this.interactiveHandler, this);
        this.maps.on(sf.base.Browser.touchEndEvent, this.interactiveHandler, this);
    ...
    Legend.prototype.legendClick = function (targetEle) {
        ...
        else if (!sf.base.isNullOrUndefined(targetEle.id) && (targetEle.id.indexOf(this.maps.element.id + '_Legend_Shape_Index') > -1 ||
    ...
    Legend.prototype.removeEventListener = function () {
        ...
        this.maps.off(sf.base.Browser.touchMoveEvent, this.interactiveHandler);
        this.maps.off(sf.base.Browser.touchEndEvent, this.interactiveHandler);
    ...
    Legend.prototype.getLegendData = function (layerIndex, dataIndex, data, dataPath, layerData, shapePropertyPath, value) {
        ...
        if (Object.prototype.toString.call(layerData) === '[object Array]') {
            for (var i = 0; i < layerData.length; i++) {
                ...
                var dataPathValueCase = !sf.base.isNullOrUndefined(dataPathValue)
                    ? dataPathValue.toLowerCase() : dataPathValue;
                var shapeDataValueCase = !sf.base.isNullOrUndefined(shapeData['properties'][shapePath])
    ...
    Legend.prototype.legendGradientColor = function (colorMap, legendIndex) {
        ...
        if (!sf.base.isNullOrUndefined(colorMap.color) && typeof (colorMap.color) === 'object') {
...
var Highlight = /** @class */ (function () {
    ...
    Highlight.prototype.addEventListener = function () {
        ...
        this.maps.on(sf.base.Browser.touchMoveEvent, this.mouseMove, this);
        this.maps.on(sf.base.Browser.touchStartEvent, this.mouseMove, this);
    };
    /**
    ...
    Highlight.prototype.removeEventListener = function () {
        ...
        this.maps.off(sf.base.Browser.touchMoveEvent, this.mouseMove);
        this.maps.off(sf.base.Browser.touchStartEvent, this.mouseMove);
    };
    /**
    ...
    Highlight.prototype.mouseMove = function (e) {
        ...
        if ((targetEle.id.indexOf('LayerIndex') !== -1 || targetEle.id.indexOf('NavigationIndex') > -1) &&
            ...
            if (targetEle.id.indexOf('shapeIndex') > -1) {
                ...
                dataIndex = parseInt(targetEle.id.split('_dataIndex_')[1].split('_')[0], 10);
                data = sf.base.isNullOrUndefined(dataIndex) ? null : this.maps.layers[layerIndex].dataSource[dataIndex];
    ...
    Highlight.prototype.mapHighlight = function (targetEle, shapeData, data) {
        ...
        var eventArgs = {
            ...
            fill: targetEle.id.indexOf('NavigationIndex') === -1 ? !sf.base.isNullOrUndefined(this.highlightSettings.fill)
...
var Selection = /** @class */ (function () {
    ...
    Selection.prototype.addEventListener = function () {
        if (!this.maps.isDestroyed) {
            ...
            this.maps.on(sf.base.Browser.touchEndEvent, this.mouseClick, this);
        }
    };
    /**
    ...
    Selection.prototype.removeEventListener = function () {
        ...
        this.maps.off(sf.base.Browser.touchEndEvent, this.mouseClick);
    };
    Selection.prototype.mouseClick = function (targetElement) {
        if (!sf.base.isNullOrUndefined(targetElement['type']) && targetElement['type'].indexOf('touch') !== -1 &&
            sf.base.isNullOrUndefined(targetElement.id)) {
            targetElement = targetElement['target'];
        }
        if (!sf.base.isNullOrUndefined(targetElement.id) && (targetElement.id.indexOf('LayerIndex') > -1 ||
            ...
            !sf.base.isNullOrUndefined(targetElement.id) && targetElement.id.indexOf('_Text') === -1 &&
...
var MapsTooltip = /** @class */ (function () {
    ...
    MapsTooltip.prototype.renderTooltip = function (e) {
        ...
        if (istooltipRender) {
            if (targetId.indexOf('_shapeIndex_') > -1) {
                ...
                if (sf.base.isNullOrUndefined(layer.layerData) || sf.base.isNullOrUndefined(layer.layerData[shape])) {
                    return;
                }
                ...
                if (!sf.base.isNullOrUndefined(properties)) {
                    for (var k = 0; k < properties.length; k++) {
                        for (var i = 0; i < layer['dataSource']['length']; i++) {
                            ...
                            var dataPathValue = !sf.base.isNullOrUndefined(dataPath) && isNaN(data[layer.shapeDataPath])
                                ? dataPath.toLowerCase() : dataPath;
                            var propertyValue = !sf.base.isNullOrUndefined(value[properties[k]])
                ...
                if (option.visible && ((!sf.base.isNullOrUndefined(index) && !isNaN(index)) || (!sf.base.isNullOrUndefined(value)))) {
                    if (layer.tooltipSettings.format) {
                        ...
                        currentData = (!sf.base.isNullOrUndefined(layer.dataSource) && !sf.base.isNullOrUndefined(index)) ?
                        ...
                        if (sf.base.isNullOrUndefined(currentData)) {
            ...
            if (this.maps.isBlazor) {
                ...
                this.maps.trigger(tooltipRender, blazorArgs_1, function (args) {
                    if (!blazorArgs_1.cancel && option.visible && !sf.base.isNullOrUndefined(currentData) &&
                        ...
                        if (blazorArgs_1.cancel) {
                            _this.svgTooltip = new sf.svgbase.Tooltip({
                                enable: true,
                                header: '',
                ...
                this.maps.trigger(tooltipRender, tooltipArgs, function (args) {
                    if (!tooltipArgs.cancel && option.visible && !sf.base.isNullOrUndefined(currentData) &&
                        (targetId.indexOf('_cluster_') === -1 && targetId.indexOf('_dataLabel_') === -1)) {
                        _this.maps['isProtectedOnChange'] = true;
    ...
    MapsTooltip.prototype.setTooltipContent = function (options, templateData) {
        var localData = sf.base.extend({}, templateData, null, true);
    ...
    MapsTooltip.prototype.removeTooltip = function () {
        if (document.getElementsByClassName('EJ2-maps-Tooltip').length > 0) {
            sf.base.remove(document.getElementsByClassName('EJ2-maps-Tooltip')[0]);
        }
    };
    /**
    ...
    MapsTooltip.prototype.addEventListener = function () {
        ...
        if (this.maps.tooltipDisplayMode === 'DoubleClick') {
            ...
            this.maps.on(sf.base.Browser.touchEndEvent, this.mouseUpHandler, this);
        }
        else {
            this.maps.on(sf.base.Browser.touchMoveEvent, this.renderTooltip, this);
        }
        this.maps.on(sf.base.Browser.touchCancelEvent, this.removeTooltip, this);
    };
    MapsTooltip.prototype.removeEventListener = function () {
        ...
        if (this.maps.tooltipDisplayMode === 'DoubleClick') {
            ...
            this.maps.off(sf.base.Browser.touchEndEvent, this.mouseUpHandler);
        }
        else {
            this.maps.off(sf.base.Browser.touchMoveEvent, this.renderTooltip);
        }
        this.maps.off(sf.base.Browser.touchCancelEvent, this.removeTooltip);
    };
    /**
     * Get module name.
     */
...
var Zoom = /** @class */ (function () {
    function Zoom(maps) {
        ...
        this.isPointer = sf.base.Browser.isPointer;
        this.handled = false;
        this.pinchFactor = 1;
        this.startTouches = [];
    ...
    Zoom.prototype.applyTransform = function (animate$$1) {
        ...
        if (this.layerCollectionEle) {
                        ...
                        if (!(currentEle.id.indexOf('_Markers_Group') > -1) && (!(currentEle.id.indexOf('_bubble_Group') > -1))
                            ...
                            if (!sf.base.isNullOrUndefined(currentEle)) {
                                ...
                                if (document.getElementById(this.maps.element.id + '_mapsTooltip') && this.maps.mapsTooltipModule.tooltipTargetID.indexOf('_MarkerIndex_')
                                    ...
                                    if (!sf.base.isNullOrUndefined(tooltipElement)) {
            ...
            if (!sf.base.isNullOrUndefined(this.currentLayer)) {
    ...
    Zoom.prototype.markerTranslates = function (element, factor, x, y, scale, type, layerElement, animate$$1) {
        ...
        var markerTemplateElements = sf.base.createElement('div', {
            id: this.maps.element.id + '_LayerIndex_' + layerIndex + '_Markers_Template_Group',
            className: 'template',
            styles: 'overflow: hidden; position: absolute;pointer-events: none;' +
            ...
            Array.prototype.forEach.call(markerDatas, function (data, dataIndex) {
                ...
                _this.maps.trigger('markerRendering', eventArgs, function (MarkerArgs) {
                    ...
                    if (!eventArgs.cancel && markerSettings.visible && !sf.base.isNullOrUndefined(long) && !sf.base.isNullOrUndefined(lati)) {
                        ...
                        var animate_1 = currentLayers.animationDuration !== 0 || sf.base.isNullOrUndefined(_this.maps.zoomModule);
                    ...
                    _this.maps.markerNullCount = (!sf.base.isNullOrUndefined(lati) || !sf.base.isNullOrUndefined(long))
    ...
    Zoom.prototype.processTemplate = function (x, y, scale, maps) {
        for (var i = 0; i < this.templateCount; i++) {
            ...
            if ((!sf.base.isNullOrUndefined(markerTemplateElement)) && markerTemplateElement.childElementCount > 0) {
            ...
            if ((!sf.base.isNullOrUndefined(datalabelTemplateElemement)) && datalabelTemplateElemement.childElementCount > 0) {
                ...
                if (type === 'Template') {
                    ...
                    if (this.maps.layers[this.index].dataLabelSettings.intersectionAction === 'Hide') {
                        for (var m = 0; m < this.intersect.length; m++) {
                            if (!sf.base.isNullOrUndefined(this.intersect[m])) {
                                if (textLocations['leftWidth'] > this.intersect[m]['rightWidth']
                                    ...
                                    text = !sf.base.isNullOrUndefined(text) ? text : zoomtext;
                                    element.innerHTML = text;
                                }
                                else {
                                    text = '';
                    ...
                    if (this.maps.layers[this.index].dataLabelSettings.intersectionAction === 'Trim') {
                        for (var j = 0; j < this.intersect.length; j++) {
                            if (!sf.base.isNullOrUndefined(this.intersect[j])) {
                                if (textLocations['rightWidth'] < this.intersect[j]['leftWidth']
                                    ...
                                    trimmedLable = !sf.base.isNullOrUndefined(text) ? text : zoomtext;
                                    if (scale > 1) {
                        ...
                        this.intersect.push(textLocations);
                        if (sf.base.isNullOrUndefined(trimmedLable)) {
    ...
    Zoom.prototype.panning = function (direction, xDifference, yDifference, mouseLocation) {
        ...
        var x;
        var y;
        xDifference = !sf.base.isNullOrUndefined(xDifference) ? xDifference : (down.x - move.x);
        yDifference = !sf.base.isNullOrUndefined(yDifference) ? yDifference : (down.y - move.y);
        if (!map.isTileMap) {
    ...
    Zoom.prototype.toAlignSublayer = function () {
        this.maps.translatePoint.x = !sf.base.isNullOrUndefined(this.distanceX) ? (this.maps.translatePoint.x -
            (this.distanceX / this.maps.scale)) : this.maps.translatePoint.x;
        this.maps.translatePoint.y = !sf.base.isNullOrUndefined(this.distanceY) ? this.maps.translatePoint.y -
    ...
    Zoom.prototype.removeTooltip = function () {
        if (getElementByID('EJ2_Map_Toolbar_Tip')) {
            sf.base.remove(getElementByID('EJ2_Map_Toolbar_Tip'));
        }
    };
    Zoom.prototype.alignToolBar = function () {
        var map = this.maps;
        var padding = 10;
        var element = sf.base.createElement('div', { id: map.element.id + '_ToolBar', styles: 'position:absolute;z-index:2' });
        var rectSVGObject = map.renderer.createSvg({
    ...
    Zoom.prototype.wireEvents = function (element, process) {
        sf.base.EventHandler.add(element, sf.base.Browser.touchStartEvent, process, this);
        sf.base.EventHandler.add(element, 'mouseover', this.showTooltip, this);
        sf.base.EventHandler.add(element, 'mouseout', this.removeTooltip, this);
    };
    ...
    Zoom.prototype.mouseMoveHandler = function (e) {
        ...
        var targetId = e.target['id'];
        var targetEle = e.target;
        if (zoom.enable && this.isPanning && ((sf.base.Browser.isDevice && touches.length > 1) || !sf.base.Browser.isDevice)) {
            e.preventDefault();
    ...
    Zoom.prototype.mouseUpHandler = function (e) {
        ...
        if ((!sf.base.isNullOrUndefined(this.distanceX) || !sf.base.isNullOrUndefined(this.distanceY)) && (!sf.base.isNullOrUndefined(this.currentLayer) && this.currentLayer.type === 'SubLayer')) {
    ...
    Zoom.prototype.addEventListener = function () {
        ...
        sf.base.EventHandler.add(this.maps.element, this.wheelEvent, this.mapMouseWheel, this);
        sf.base.EventHandler.add(this.maps.element, 'click', this.click, this);
        sf.base.EventHandler.add(this.maps.element, 'dblclick', this.doubleClick, this);
        this.maps.on(sf.base.Browser.touchMoveEvent, this.mouseMoveHandler, this);
        this.maps.on(sf.base.Browser.touchStartEvent, this.mouseDownHandler, this);
        this.maps.on(sf.base.Browser.touchEndEvent, this.mouseUpHandler, this);
        sf.base.EventHandler.add(this.maps.element, this.cancelEvent, this.mouseCancelHandler, this);
    };
    Zoom.prototype.removeEventListener = function () {
        if (this.maps.isDestroyed) {
            return;
        }
        sf.base.EventHandler.remove(this.maps.element, this.wheelEvent, this.mapMouseWheel);
        sf.base.EventHandler.remove(this.maps.element, 'click', this.click);
        sf.base.EventHandler.remove(this.maps.element, 'dblclick', this.doubleClick);
        this.maps.off(sf.base.Browser.touchMoveEvent, this.mouseMoveHandler);
        this.maps.off(sf.base.Browser.touchStartEvent, this.mouseDownHandler);
        this.maps.off(sf.base.Browser.touchEndEvent, this.mouseUpHandler);
...
Maps.Inject(Bubble, Legend, Marker, Highlight, Selection, MapsTooltip, Zoom, DataLabel, NavigationLine, Annotations);
...
exports.shapeSelected = shapeSelected;
exports.shapeHighlight = shapeHighlight;
exports.mousemove = mousemove;
exports.mouseup = mouseup;
exports.mousedown = mousedown;
exports.layerRendering = layerRendering;
exports.shapeRendering = shapeRendering;
exports.markerRendering = markerRendering;
exports.markerClusterRendering = markerClusterRendering;
exports.markerClick = markerClick;
exports.markerClusterClick = markerClusterClick;
exports.markerMouseMove = markerMouseMove;
exports.markerClusterMouseMove = markerClusterMouseMove;
exports.dataLabelRendering = dataLabelRendering;
exports.bubbleRendering = bubbleRendering;
exports.bubbleClick = bubbleClick;
exports.bubbleMouseMove = bubbleMouseMove;
exports.animationComplete = animationComplete;
exports.legendRendering = legendRendering;
exports.annotationRendering = annotationRendering;
...
exports.ConnectorLineSettings = ConnectorLineSettings;
...
exports.NavigationLineSettings = NavigationLineSettings;
exports.BubbleSettings = BubbleSettings;
...
exports.ToggleLegendSettings = ToggleLegendSettings;
...
exports.Tile = Tile;
exports.MapsAreaSettings = MapsAreaSettings;
exports.Size = Size;
...
exports.convertGeoToPoint = convertGeoToPoint;
exports.convertTileLatLongToPoint = convertTileLatLongToPoint;
exports.xToCoordinate = xToCoordinate;
exports.yToCoordinate = yToCoordinate;
exports.aitoff = aitoff;
exports.roundTo = roundTo;
exports.sinci = sinci;
exports.acos = acos;
exports.calculateBound = calculateBound;
exports.Point = Point;
...
exports.PolylineOption = PolylineOption;
exports.LineOption = LineOption;
exports.Line = Line;
exports.MapLocation = MapLocation;
exports.Rect = Rect;
exports.PatternOptions = PatternOptions;
...
exports.markerColorChoose = markerColorChoose;
exports.markerShapeChoose = markerShapeChoose;
exports.clusterTemplate = clusterTemplate;
exports.mergeSeparateCluster = mergeSeparateCluster;
exports.clusterSeparate = clusterSeparate;
exports.marker = marker;
exports.markerTemplate = markerTemplate;
exports.maintainSelection = maintainSelection;
exports.maintainStyleClass = maintainStyleClass;
...
exports.drawBalloon = drawBalloon;
exports.drawPattern = drawPattern;
exports.getFieldData = getFieldData;
exports.checkShapeDataFields = checkShapeDataFields;
exports.checkPropertyPath = checkPropertyPath;
exports.filter = filter;
exports.getRatioOfBubble = getRatioOfBubble;
exports.findMidPointOfPolygon = findMidPointOfPolygon;
exports.isCustomPath = isCustomPath;
exports.textTrim = textTrim;
exports.findPosition = findPosition;
exports.removeElement = removeElement;
exports.calculateCenterFromPixel = calculateCenterFromPixel;
exports.getTranslate = getTranslate;
exports.getZoomTranslate = getZoomTranslate;
exports.fixInitialScaleForTile = fixInitialScaleForTile;
exports.getElementByID = getElementByID;
exports.Internalize = Internalize;
exports.getTemplateFunction = getTemplateFunction;
exports.getElement = getElement;
exports.getShapeData = getShapeData;
exports.triggerShapeEvent = triggerShapeEvent;
exports.getElementsByClassName = getElementsByClassName;
exports.querySelector = querySelector;
exports.getTargetElement = getTargetElement;
exports.createStyle = createStyle;
exports.customizeStyle = customizeStyle;
exports.triggerItemSelectionEvent = triggerItemSelectionEvent;
exports.removeClass = removeClass;
exports.elementAnimate = elementAnimate;
...
exports.changeBorderWidth = changeBorderWidth;
exports.changeNavaigationLineWidth = changeNavaigationLineWidth;
exports.targetTouches = targetTouches;
exports.calculateScale = calculateScale;
exports.getDistance = getDistance;
exports.getTouches = getTouches;
exports.getTouchCenter = getTouchCenter;
exports.sum = sum;
exports.zoomAnimate = zoomAnimate;
exports.animate = animate;
exports.MapAjax = MapAjax;
