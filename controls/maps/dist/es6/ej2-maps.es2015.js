...
function getTranslate(mapObject, layer, animate) {
    let zoomFactor = animate ? 1 : mapObject.zoomSettings.zoomFactor;
...
class LayerPanel {
    ...
    translateLayerElements(layerElement, index) {
        ...
        if (!isNullOrUndefined(this.mapObject.baseMapRectBounds)) {
            ...
            let translate = getTranslate(this.mapObject, this.currentLayer, animate$$1);
...
let Maps = class Maps extends Component {
    ...
    setBackgroundValue(theme) {
        let color = theme === 'Highcontrast' ? '#000000' : '#FFFFFF';
    ...
    createTile() {
        ...
        if (mainLayer.isBaseLayer && (mainLayer.layerType === 'OSM' || mainLayer.layerType === 'Bing')) {
            ...
            let left = Math.abs(elementRect.left - parentRect.left);
            let top = Math.abs(elementRect.top - parentRect.top);
