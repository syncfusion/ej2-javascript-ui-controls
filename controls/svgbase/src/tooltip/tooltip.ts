            ...
            for (let i: number = 0, len: number = textCollection.length; i < len; i++) { // string value of unicode for LTR is \u200E
                ...
                for (let j: number = 0, len: number = lines.length; j < len; j++) {
                    ...
                    if ((!isColumn && line === ' ') || (line.replace(/<b>/g, '').replace(/<\/b>/g, '').trim() !== '')) {
                        ...
                        tspanElement.style.fontWeight = (this.header === this.formattedText[k] && (this.header.indexOf('<b>') || this.header.indexOf('</b>')) === -1) ? (this.textStyle.fontWeight || '600') : line.indexOf('<b>') > -1 || line.indexOf('</b>') > -1 ? this.theme.indexOf('Bootstrap5') > -1 ? this.textStyle.fontWeight || '600' : 'bold' : ((line.indexOf('<b>') === -1 || line.indexOf('</b>') === -1) && this.theme.indexOf('Bootstrap5') > -1) ? this.textStyle.fontWeight || '600' : (this.textStyle.fontWeight || font.fontWeight);
