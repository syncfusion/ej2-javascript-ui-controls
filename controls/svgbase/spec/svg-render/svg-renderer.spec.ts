...
} from '../../src/svg-render/svg-canvas-interface';
...
describe('SVG Rendering', () => {
    ...
    describe('DrawClipPath', () => {
        it('DrawClipPath method test', function (): void {
            ...
            let element: HTMLElement = this.render.drawClipPath(options);
            let result: boolean = checkAttributes(element.childNodes[0].childNodes[0] as Element, options as SVGCanvasAttributes);
            expect((element.childNodes[0].childNodes[0] as Element).getAttribute('id').indexOf('Rect') > -1).toBe(true);
            expect(result).toBe(true);
        });
    });
