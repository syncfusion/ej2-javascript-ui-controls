...
import { BACKSPACE_EVENT_INIT, BASIC_CONTEXT_MENU_EVENT_INIT, BASIC_MOUSE_EVENT_INIT, ENTERKEY_EVENT_INIT, TOOLBAR_FOCUS_SHORTCUT_EVENT_INIT } from '../../constant.spec';
...
describe('Link Module', () => {
    ...
    describe('1014884: Add Keyboard Shortcut Support to Open the Link Quick Toolbar in Rich Text Editor', () => {
        let rteObj: RichTextEditor;
        beforeAll(() => {
            rteObj = renderRTE({
                value: '<p>This is a <a href="https://www.google.com">link</a> in editor</p>',
                quickToolbarSettings: {
                    showOnRightClick: true,
        ...
        it('Open the Link Quick Toolbar using Keyboard Shortcut', (done: Function) => {
            ...
            setTimeout(() => {
                expect(document.querySelectorAll('.e-rte-quick-popup')[0].id.indexOf('Link_Quick_Popup') >= 0).toBe(true);
                expect(document.activeElement === document.querySelector('.e-rte-quick-popup .e-toolbar-item .e-btn')).toBe(true);
                done();
            }, 500);
        });
    });

    describe('1020750: enableAutoUrl Does Not Prepend HTTPS for www Links Inserted via RTE Link Dialog', () => {
        let rteObj: RichTextEditor;
        beforeEach(() => {
            rteObj = renderRTE({
                enableAutoUrl: true
        ...
        it('should prepend https:// to www.google.com when enableAutoUrl is true', (done) => {
            rteObj.focusIn();
            const createLinkButton: HTMLElement = rteObj.element.querySelector('#' + rteObj.element.id + '_toolbar_CreateLink');
            createLinkButton.click();
            const dialogEle = (<any>rteObj).linkModule.dialogObj.element;
            const urlInput = dialogEle.querySelector('.e-rte-linkurl');
            urlInput.value = 'www.google.com';
            const insertBtn = dialogEle.querySelector('.e-insertLink');
            insertBtn.click();
            setTimeout(() => {
                const anchor = rteObj.contentModule.getEditPanel().querySelector('a');
