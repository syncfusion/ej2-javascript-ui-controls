        ...
        if (selectedNodes.length > 1) {
            ...
            if (!this.isBlockNode(selectedNodes[0] as HTMLElement)) {
                ...
                if (!isNOU(selectedNodes[0].nextSibling) && (selectedNodes[0].nextSibling.nodeName === 'BR')) {
                    if (selectedNodes[0].textContent === '') {
                        selectedNodes[0].appendChild(selectedNodes[0].nextSibling);
                    } else {
            ...
            for (let j: number = 0; j < ranges.length; j++) {
                ...
                if (parentNode && collectionNodes.indexOf(parentNode) < 0) {
                    if (!isNOU(action) && action) {
                        const tableCellNodeNames: string[] = ['TD', 'TH'];
                        const nodesToCheck: Node[] = [range.commonAncestorContainer, parentNode, endParentNode];
                        if (nodesToCheck.some((node: Node) => tableCellNodeNames.indexOf(node.nodeName) !== -1)) {
