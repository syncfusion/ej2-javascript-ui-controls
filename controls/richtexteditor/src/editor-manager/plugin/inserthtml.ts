...
export class InsertHtml {
    ...
    private static formattingInlineNodes: string[] = ['span', 'strong', 'b', 'em', 'i', 'u', 's', 'strike', 'code',
        'sub', 'sup'];
    ...
    */
    ...
    private static handleCursorInlineContent(
        ...
        if (this.shouldInsertInAnchor(range)) {
            ...
            if (insideInline) {
                const inlineFragment: DocumentFragment = document.createDocumentFragment();
    ...
    private static processBlockContent(
        ...
        let insideInline: boolean = false;
        let inlineCandidate: boolean = false;
        let foundListOrTable: boolean = false;
        while (currentNode && currentNode !== editNode) {
            const tagName: string = currentNode.nodeName.toLowerCase();
        ...
        if (insideInline) {
            const inlineWrapper: HTMLElement | null = insertedFragment.fragment.querySelector('#pasteContent_rte') as HTMLElement | null;
            if (inlineWrapper && inlineWrapper.firstChild) {
                const inlineFragment: DocumentFragment = document.createDocumentFragment();
                let lastInlineChild: Node = null;
                ...
                const currentDocument: Document = editNode.ownerDocument;
                const selection: Selection | null = currentDocument.getSelection();
                if (selection) {
