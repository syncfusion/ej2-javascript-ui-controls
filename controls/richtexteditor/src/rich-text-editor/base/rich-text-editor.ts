...
export class RichTextEditor extends Component<HTMLElement> implements INotifyPropertyChanged {
    ...
    public keyDown(e: KeyboardEvent): void {
        ...
        const isDeleteAction: boolean = this.editorMode === 'HTML' && !isNOU(e) && !isNOU(e.code) && (e.code === 'Backspace' || e.code === 'Delete');
    ...
    private updateFocusState(target: HTMLElement): void {
    ...
    private setCloneValue(): void {
        if (this.editorMode === 'HTML') {
            const rteContent: string = this.inputElement.innerHTML;
            ...
            this.cloneValue = (this.inputElement as HTMLTextAreaElement).value === '' ? null : (this.inputElement as HTMLTextAreaElement).value;
        }
    }

    private handleInternalFocus(target: HTMLElement): void {
        const active: Element = document.activeElement;
        let isRTEFocusedOnTBClick: boolean;
        const toolbarElement: HTMLElement = this.getToolbarElement() as HTMLElement;
        const isIframePanel: boolean = this.iframeSettings.enable && active === this.contentModule.getPanel();
        ...
        if (active === this.element || active === toolbarElement || active === this.contentModule.getEditPanel() ||
            ...
            if (!isNOU(toolbarElement)) {
                ...
                const items: NodeList = toolbarElement.querySelectorAll('[tabindex="0"]');
    ...
    private resetToolbarOnFocus(e: FocusEvent): void {
        if (!this.readonly) {
