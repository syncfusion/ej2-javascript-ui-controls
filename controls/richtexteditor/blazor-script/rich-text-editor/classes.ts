/**
 * Css class constants
 */
export const CLS_RTE: string = 'e-richtexteditor';
export const CLS_RTL: string = 'e-rtl';
export const CLS_CONTENT: string = 'e-content';
export const CLS_DISABLED: string = 'e-disabled';
export const CLS_SCRIPT_SHEET: string = 'rte-iframe-script-sheet';
export const CLS_STYLE_SHEET: string = 'rte-iframe-style-sheet';
export const CLS_TOOLBAR: string = 'e-rte-toolbar';
export const CLS_TB_FIXED: string = 'e-rte-tb-fixed';
export const CLS_TB_FLOAT: string = 'e-rte-tb-float';
export const CLS_TB_ABS_FLOAT: string = 'e-rte-tb-abs-float';
export const CLS_INLINE: string = 'e-rte-inline';
export const CLS_TB_INLINE: string = 'e-rte-tb-inline';
export const CLS_RTE_EXPAND_TB: string = 'e-rte-tb-expand';
export const CLS_FULL_SCREEN: string = 'e-rte-full-screen';
export const CLS_QUICK_TB: string = 'e-rte-quick-toolbar';
export const CLS_QUICK_DROPDOWN: string = 'e-quick-dropdown';
export const CLS_IMAGE_POP: string = 'e-rte-image-popup';
export const CLS_TEXT_POP: string = 'e-rte-text-popup';
export const CLS_INLINE_DROPDOWN: string = 'e-rte-inline-dropdown';
export const CLS_DROPDOWN_POPUP: string = 'e-rte-dropdown-popup';
export const CLS_DROPDOWN_ICONS: string = 'e-rte-dropdown-icons';
export const CLS_DROPDOWN_ITEMS: string = 'e-rte-dropdown-items';
export const CLS_DROPDOWN_BTN: string = 'e-rte-dropdown-btn';
export const CLS_RTE_CONTENT: string = 'e-rte-content';
export const CLS_TB_ITEM: string = 'e-toolbar-item';
export const CLS_TB_EXTENDED: string = 'e-toolbar-extended';
export const CLS_TB_WRAP: string = 'e-toolbar-wrapper';
export const CLS_POPUP: string = 'e-popup';
export const CLS_SEPARATOR: string = 'e-separator';
export const CLS_MINIMIZE: string = 'e-minimize';
export const CLS_MAXIMIZE: string = 'e-maximize';
export const CLS_BACK: string = 'e-back';
export const CLS_SHOW: string = 'e-show';
export const CLS_HIDE: string = 'e-hide';
export const CLS_VISIBLE: string = 'e-visible';
export const CLS_FOCUS: string = 'e-focused';
export const CLS_RM_WHITE_SPACE: string = 'e-remove-white-space';
export const CLS_IMGRIGHT: string = 'e-imgright';
export const CLS_IMGLEFT: string = 'e-imgleft';
export const CLS_IMGCENTER: string = 'e-imgcenter';
export const CLS_IMGBREAK: string = 'e-imgbreak';
export const CLS_AUDBREAK: string = 'e-audio-break';
export const CLS_VIDBREAK: string = 'e-video-break';
export const CLS_CAPTION: string = 'e-img-caption';
export const CLS_RTE_CAPTION: string = 'e-rte-img-caption';
export const CLS_CAPINLINE: string = 'e-caption-inline';
export const CLS_IMGINLINE: string = 'e-imginline';
export const CLS_AUDINLINE: string = 'e-audio-inline';
export const CLS_VIDINLINE: string = 'e-video-inline';
export const CLS_COUNT: string = 'e-rte-character-count';
export const CLS_WARNING: string = 'e-warning';
export const CLS_ERROR: string = 'e-error';
export const CLS_ICONS: string = 'e-icons';
export const CLS_ACTIVE: string = 'e-active';
export const CLS_EXPAND_OPEN: string = 'e-expand-open';
export const CLS_RTE_ELEMENTS: string = 'e-rte-elements';
export const CLS_TB_BTN: string = 'e-tbar-btn';
export const CLS_HR_SEPARATOR: string = 'e-rte-horizontal-separator';
export const CLS_TB_IOS_FIX: string = 'e-tbar-ios-fixed';
export const CLS_TB_STATIC: string = 'e-tb-static';
export const CLS_FORMATS_TB_BTN: string = 'e-formats-tbar-btn';
export const CLS_FONT_NAME_TB_BTN: string = 'e-font-name-tbar-btn';
export const CLS_FONT_SIZE_TB_BTN: string = 'e-font-size-tbar-btn';
export const CLS_FONT_COLOR_TARGET: string = 'e-rte-fontcolor-element';
export const CLS_BACKGROUND_COLOR_TARGET: string = 'e-rte-backgroundcolor-element';
export const CLS_COLOR_CONTENT: string = 'e-rte-color-content';
export const CLS_FONT_COLOR_DROPDOWN: string = 'e-rte-fontcolor-dropdown';
export const CLS_BACKGROUND_COLOR_DROPDOWN: string = 'e-rte-backgroundcolor-dropdown';
export const CLS_COLOR_PALETTE: string = 'e-rte-square-palette';
export const CLS_FONT_COLOR_PICKER: string = 'e-rte-fontcolor-colorpicker';
export const CLS_BACKGROUND_COLOR_PICKER: string = 'e-rte-backgroundcolor-colorpicker';
export const CLS_RTE_READONLY: string = 'e-rte-readonly';
export const CLS_TABLE_SEL: string = 'e-cell-select';
export const CLS_TABLE_SEL_END: string = 'e-cell-select-end';
export const CLS_TABLE_MULTI_CELL: string = 'e-multi-cells-select';
export const CLS_TB_DASH_BOR: string = 'e-dashed-border';
export const CLS_TB_ALT_BOR: string = 'e-alternate-border';
export const CLS_TB_COL_RES: string = 'e-column-resize';
export const CLS_TB_ROW_RES: string = 'e-row-resize';
export const CLS_TB_BOX_RES: string = 'e-table-box';
export const CLS_RTE_HIDDEN: string = 'e-rte-hidden';
export const CLS_RTE_PASTE_KEEP_FORMAT: string = 'e-rte-keepformat';
export const CLS_RTE_PASTE_REMOVE_FORMAT: string = 'e-rte-removeformat';
export const CLS_RTE_PASTE_PLAIN_FORMAT: string = 'e-rte-plainformat';
export const CLS_RTE_PASTE_OK: string = 'e-rte-pasteok';
export const CLS_RTE_PASTE_CANCEL: string = 'e-rte-pastecancel';
export const CLS_RTE_DIALOG_MIN_HEIGHT: string = 'e-rte-dialog-minheight';
export const CLS_RTE_RES_HANDLE: string = 'e-resize-handle';
export const CLS_RTE_RES_EAST: string = 'e-south-east';
export const CLS_RTE_RES_WEST: string = 'e-south-west';
export const CLS_RTE_IMAGE: string = 'e-rte-image';
export const CLS_RESIZE: string = 'e-resize';
export const CLS_IMG_FOCUS: string = 'e-img-focus';
export const CLS_AUD_FOCUS: string = 'e-audio-focus';
export const CLS_VID_FOCUS: string = 'e-video-focus';
export const CLS_RTE_DRAG_IMAGE: string = 'e-rte-drag-image';
export const CLS_RTE_UPLOAD_POPUP: string = 'e-rte-upload-popup';
export const CLS_POPUP_OPEN: string = 'e-popup-open';
export const CLS_IMG_RESIZE: string = 'e-img-resize';
export const CLS_VID_RESIZE: string = 'e-vid-resize';
export const CLS_DROPAREA: string = 'e-droparea';
export const CLS_IMG_INNER: string = 'e-img-inner';
export const CLS_UPLOAD_FILES: string = 'e-upload-files';
export const CLS_RTE_DIALOG_UPLOAD: string = 'e-rte-dialog-upload';
export const CLS_RTE_RES_CNT: string = 'e-rte-resize';
export const CLS_CUSTOM_TILE: string = 'e-custom-tile';
export const CLS_NOCOLOR_ITEM: string = 'e-nocolor-item';
export const CLS_TABLE: string = 'e-rte-table';
export const CLS_TABLE_BORDER: string = 'e-rte-table-border';
export const CLS_RTE_TABLE_RESIZE: string = 'e-rte-table-resize';
export const CLS_RTE_FIXED_TB_EXPAND: string = 'e-rte-fixed-tb-expand';
export const CLS_RTE_QUICK_POPUP_HIDE: string = 'e-rte-quick-popup-hide';
export const CLS_RTE_OVERFLOW: string = 'e-rte-overflow';
export const CLS_PASTED_CONTENT_IMG: string = 'pasteContent_Img';
export const CLS_FILE_SELECT_WRAP: string = 'e-file-select-wrap';
export const CLS_RTE_PASTE_CONTENT: string = 'pasteContent_RTE';
export const CLS_POPUP_CLOSE: string = 'e-popup-close';
export const CLS_CLICKELEM: string = 'e-clickelem';
export const CLS_VID_CLICK_ELEM: string = 'e-video-clickelem';
export const CLS_AUDIOWRAP: string = 'e-audio-wrap';
export const CLS_VIDEOWRAP: string = 'e-video-wrap';
export const CLS_VIDEOINLINE: string = 'e-video-inline';
export const CLS_AUDIOINLINE: string = 'e-audio-inline';
export const CLS_EXTENDED_TOOLBAR: string = 'e-extended-toolbar';
export const CLS_EXPENDED_NAV: string = 'e-expended-nav';
export const CLS_INSERT_TABLE_BTN: string = 'e-insert-table-btn';
export const CLS_DROP_DOWN_BTN: string = 'e-dropdown-btn';
export const CLS_RTE_IMG_BOX_MARK: string = 'e-rte-imageboxmark';
export const CLS_RTE_VID_BOX_MARK: string = 'e-rte-videoboxmark';
export const CLS_RTE_CUSTOM_TB: string = 'e-rte-custom-toolbar';
export const CLS_RTE_SOURCE_CODE_TXTAREA: string = 'e-rte-srctextarea';
export const CLS_RTE_SRC_CONTENT: string = 'e-source-content';
export const ALLOWED_ACTIONKEYS: string[] = ['Enter', 'ArrowRight', 'ArrowLeft'];
export const CLS_QUICK_POP: string = 'e-rte-quick-popup';
export const CLS_INLINE_POP: string = 'e-rte-inline-popup';
export const CLS_QUICK_TBAR_TIP_POINTER: string = 'e-rte-tip-pointer';
export const CLS_RTE_IFRAME_CONTENT: string = 'e-rte-iframe-content';
