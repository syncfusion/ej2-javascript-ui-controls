...
import { detach, Browser, isNullOrUndefined as isNOU, createElement, closest } from '../../../../base'; /*externalscript*/
    ...
    */
    ...
    private static handleCursorInlineContent(
        ...
        if (this.shouldInsertInAnchor(range, nodes)) {
            ...
            && !(insertedNode as HTMLElement).querySelector('a') && range.endOffset === range.startContainer.textContent.length) {
    ...
    private static pasteLI(insertedNode: Node, parentLi: Node, mergeNode: Node,
        ...
        if (parentLi.contains(mergeNode) && mergeNode.previousSibling) {
