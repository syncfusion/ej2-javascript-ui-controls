...
class BaseQuickToolbar {
    ...
    removeEleFromDOM() {
        ...
        if (this.isDOMElement) {
            ...
            detach(element);
            let args = isBlazor() ? { element: this.popupObj.element } : this.popupObj;
...
class Formatter {
    ...
    process(self, args, event, value) {
        ...
        if (isNullOrUndefined(args)) {
            ...
            let isTableModule = isNullOrUndefined(self.tableModule) ? true : self.tableModule ?
        ...
        if (start.nodeType === 3) {
            ...
            if (start.tagName === 'TABLE') {
                ...
                if (start.textContent === '') {
                    let tdNode = start.querySelectorAll('td');
...
class Formats {
    ...
    removeCodeContent(range) {
        let regEx = new RegExp(String.fromCharCode(65279), 'g');
        if (!isNullOrUndefined(range.endContainer.textContent.match(regEx))) {
            let pointer = range.endContainer.textContent.charCodeAt(range.endOffset - 1) === 65279 ?
    ...
    isNotEndCursor(preElem, range) {
        ...
        let isEnd = range.startOffset === preElem.lastChild.textContent.length &&
        ...
        if (preElem.textContent.indexOf(range.startContainer.textContent) === 0 &&
            ...
            //Cursor at middle
            let cloneNode = nodeCutter.SplitNode(range, preElem, true);
            this.insertMarker(preElem, range);
            let previousSib = preElem.previousElementSibling;
    ...
    setCursorPosition(isEnd, preElem) {
        let isEmpty = false;
        let markerElem = this.parent.editableElement.querySelector('.tempSpan');
        let mrkParentElem = markerElem.parentElement;
    ...
    applyFormats(e) {
        ...
        for (let i = 0; i < formatsNodes.length; i++) {
            ...
            let replaceNode = (e.subCommand.toLowerCase() === 'pre' && parentNode.tagName.toLowerCase() === 'pre') ?
                'p' : e.subCommand;
            let replaceTag = this.parent.domNode.createTagString(replaceNode, parentNode, replaceHTML.replace(/>\s+</g, '><'));
    ...
    preFormatMerge() {
        let preNodes = this.parent.editableElement.querySelectorAll('PRE');
        if (!isNullOrUndefined(preNodes)) {
            for (let i = 0; i < preNodes.length; i++) {
                let previousSib = preNodes[i].previousElementSibling;
...
class InsertHtml {
    ...
    static imageFocus(node, nodeSelection, docElement) {
        let focusNode = document.createTextNode(' ');
...
class LinkCommand {
    ...
    createLink(e) {
        ...
        if (!isNullOrUndefined(closestAnchor) && closestAnchor.tagName === 'A') {
            ...
            if (linkText === e.item.text) {
                ...
                let startIndex = e.item.action === 'Paste' ? anchorEle.childNodes[0].textContent.length : 0;
            ...
            let text = isNullOrUndefined(e.item.text) ? true : e.item.text.replace(/ /g, '').localeCompare(range.toString()
                .replace(/\n/g, ' ').replace(/ /g, '')) < 0;
            if (e.event && e.event.type === 'keydown' && (e.event.keyCode === 32
                || e.event.keyCode === 13) || e.item.action === 'Paste' || range.collapsed || text) {
                let anchor = this.createAchorNode(e);
    ...
    createLinkNode(e) {
        ...
        let nodeCutter = new NodeCutter();
        let range = domSelection.getRange(this.parent.currentDocument);
        let nodes = this.getSelectionNodes(domSelection.getNodeCollection(range));
        let save = domSelection.save(range, this.parent.currentDocument);
        let txtArray = [];
        let inlineNodes = [];
        let currentNode;
        let removeNodes = [];
        let anchorNodes = [];
        let finalinlineNodes = [];
        let cloneNode;
        for (let index = 0; index < nodes.length; index++) {
        ...
        for (let i = 0; i < finalinlineNodes.length; i++) {
            ...
            if (i < finalinlineNodes.length - 1) {
                if (finalinlineNodes[i].parentNode === finalinlineNodes[i + 1].parentNode) {
                    let cln = finalinlineNodes[i + 1].cloneNode(true);
    ...
    removeLink(e) {
        let blockNodes = this.parent.domNode.blockNodes();
        if (blockNodes.length < 2) {
            ...
            for (let i = 0; i < blockNodes.length; i++) {
                let linkNode = blockNodes[i].querySelectorAll('a');
...
class TableCommand {
    ...
    deleteColumn(e) {
        ...
        if (e.callBack) {
            let sContainer = this.parent.nodeSelection.getRange(this.parent.currentDocument).startContainer;
            if (sContainer.nodeName !== 'TD') {
                let startChildLength = this.parent.nodeSelection.getRange(this.parent.currentDocument).startOffset;
                let focusNode = sContainer.children[startChildLength];
    ...
    deleteRow(e) {
        let selectedCell = e.item.selection.range.startContainer;
        selectedCell = (selectedCell.nodeType === 3) ? selectedCell.parentNode : selectedCell;
        let selectedRowIndex = closest(selectedCell, 'tr').rowIndex;
        let parentTable = closest(selectedCell, 'table');
...
class SelectionCommands {
    ...
    static removeFormat(nodes, index, formatNode, isCursor, isFormat, isFontStyle, range, nodeCutter, format, value, domSelection) {
        ...
        let formatNodeStyles = formatNode.getAttribute('style');
        let formatNodeTagName = formatNode.tagName;
    ...
    static clear(docElement, endNode, selector) {
        ...
        if (!isCollapsed) {
            ...
            if (nodes.length === 1) {
                ...
                let lasNode = nodeCutter.GetSpliceNode(range, lastText);
                nodeSelection.setSelectionText(docElement, preNode, lasNode, 0, (lasNode.nodeType === 3) ?
                    lasNode.textContent.length : lasNode.childNodes.length);
                range = nodeSelection.getRange(docElement);
            }
...
class MsWordPaste {
    ...
    wordCleanup(e) {
        ...
        let rtfData = e.args.clipboardData.getData('text/rtf');
    ...
    imageConversion(elm, rtfData) {
        ...
        if (imgElem.length > 0) {
            ...
            let hexValue = this.hexConversion(rtfData);
    ...
    checkVShape(elm) {
        let allNodes = elm.querySelectorAll('*');
            ...
            switch (allNodes[i].nodeName) {
                ...
                case 'V:SHAPE':
                    if (allNodes[i].firstElementChild.nodeName === 'V:IMAGEDATA') {
                        let src = allNodes[i].firstElementChild.getAttribute('src');
                        let imgElement = createElement('img');
    ...
    convertToBase64(hexValue) {
        let base64;
        let byteArr = this.conHexStringToBytes(hexValue.hex);
        let base64String = this.conBytesToBase64(byteArr);
    ...
    conBytesToBase64(byteArr) {
        ...
        for (let i = 0; i < byteArrLen; i += 3) {
            let array3 = byteArr.slice(i, i + 3);
    ...
    hexConversion(rtfData) {
        let picHead = /\{\\pict[\s\S]+?\\bliptag\-?\d+(\\blipupi\-?\d+)?(\{\\\*\\blipuid\s?[\da-fA-F]+)?[\s\}]*?/;
        let pic = new RegExp('(?:(' + picHead.source + '))([\\da-fA-F\\s]+)\\}', 'g');
...
class XhtmlValidation {
    ...
    ;
    removeTags() {
        let removeAttribute = [['br', 'ul'], ['br', 'ol'], ['table', 'span'], ['div', 'span'], ['p', 'span']];
    ...
    RemoveUnsupported() {
        ...
        let attrArray = ['language', 'role', 'target', 'contenteditable', 'cellspacing',
...
class PasteCleanup {
    ...
    pasteClean(e) {
        ...
        if (e.args && value !== null && this.parent.editorMode === 'HTML') {
            ...
            if (this.parent.pasteCleanupSettings.prompt) {
                e.args.preventDefault();
                let tempDivElem = this.parent.createElement('div');
    ...
    getBlob(allImgElm) {
        for (let i = 0; i < allImgElm.length; i++) {
            if (!isNullOrUndefined(allImgElm[i].getAttribute('src')) &&
                ...
                let blopUrl = URL.createObjectURL(convertToBlob(allImgElm[i].getAttribute('src')));
                allImgElm[i].setAttribute('src', blopUrl);
            }
        }
    }
    uploadMethod(fileList, imgElem) {
        let uploadEle = document.createElement('div');
    ...
    refreshPopup(imageElement, popupObj) {
        let imgPosition = this.parent.iframeSettings.enable ? this.parent.element.offsetTop +
            imageElement.offsetTop : imageElement.offsetTop;
        let rtePosition = this.parent.element.offsetTop + this.parent.element.offsetHeight;
    ...
    base64ToFile(base64, filename) {
        ...
        if (Browser.isIE || navigator.appVersion.indexOf('Edge') > -1) {
            let blob = new Blob([decodeArr], { type: extension });
    ...
    getTextContent(clipBoardElem) {
        for (let i = 0; i < this.blockNode.length; i++) {
            let inElem = clipBoardElem.querySelectorAll(this.blockNode[i]);
            for (let j = 0; j < inElem.length; j++) {
                ...
                for (let k = 0, l = 0, preNode; k < inElem[j].childNodes.length; k++, l++) {
                    if (inElem[j].childNodes[k].nodeName === 'DIV' || inElem[j].childNodes[k].nodeName === 'P' ||
                        ...
                        parElem = inElem[j].childNodes[k].parentElement;
                        if (preNode === 'text') {
                            let previousElem = parElem.previousElementSibling;
                            previousElem.appendChild(inElem[j].childNodes[k]);
                        }
                        else {
                            let divElement = this.parent.createElement('div', { id: 'newDiv' });
        ...
        let allElems = clipBoardElem.querySelectorAll('*');
        for (let i = 0; i < allElems.length; i++) {
            let allAtr = allElems[i].attributes;
    ...
    detachInlineElements(clipBoardElem) {
        for (let i = 0; i < this.inlineNode.length; i++) {
            let inElem = clipBoardElem.querySelectorAll(this.inlineNode[i]);
            for (let j = 0; j < inElem.length; j++) {
                let parElem;
...
class Resize {
    ...
    performResize(e) {
        ...
        let boundRect = this.parent.element.getBoundingClientRect();
...
class Link {
    ...
    editLink(e) {
        let selectedNode = this.getAnchorNode(e.selectNode[0]);
        let selectParentEle = this.getAnchorNode(e.selectParent[0]);
...
class Image {
    ...
    dragDrop(args) {
        this.parent.trigger(beforeImageDrop, args, (e) => {
            ...
            let isImgOrFileDrop = (imgElement && imgElement.tagName === 'IMG') || e.dataTransfer.files.length > 0;
     ...
     */
    getDropRange(x, y) {
        let startRange = this.contentModule.getDocument().createRange();
        this.parent.formatter.editorManager.nodeSelection.setRange(this.contentModule.getDocument(), startRange);
        let elem = this.contentModule.getDocument().elementFromPoint(x, y);
    ...
    insertDragImage(e) {
        e.preventDefault();
        let activePopupElement = this.parent.element.querySelector('' + CLS_POPUP_OPEN);
        ...
        if (e.dataTransfer.files.length > 0) { //For external image drag and drop
            ...
            let fileName = imgFiles[0].name;
            let imgType = fileName.substring(fileName.lastIndexOf('.'));
            let allowedTypes = this.parent.insertImageSettings.allowedTypes;
            for (let i = 0; i < allowedTypes.length; i++) {
                if (imgType.toLocaleLowerCase() === allowedTypes[i].toLowerCase()) {
                    if (this.parent.insertImageSettings.saveUrl) {
                        ...
                        let args = { args: e, text: '', file: imgFiles[0] };
                        e.preventDefault();
    ...
    onSelect(args) {
        let proxy = this;
        let range = this.parent.formatter.editorManager.nodeSelection.getRange(this.parent.contentModule.getDocument());
        let parentElement = this.parent.createElement('ul', { className: CLS_UPLOAD_FILES });
        ...
        let imageTag = this.parent.createElement('IMG');
        ...
        reader.addEventListener('load', () => {
            let url = URL.createObjectURL(convertToBlob(reader.result));
    ...
    showPopupToolBar(e, url) {
        let imageSrc = 'img[src="' + url.url + '"]';
        let imageElement = this.parent.inputElement.querySelector(imageSrc);
        ...
        if (imageElement) {
            setTimeout(() => { this.showImageQuickToolbar(args); this.resizeStart(e.args, imageElement); }, 0);
        }
    }
    /**
     * Destroys the ToolBar.
...
class Table {
    ...
    verticalAlign(args, e) {
        let tdEle = closest(args.selectParent[0], 'td') || closest(args.selectParent[0], 'th');
        if (tdEle) {
...
let RichTextEditor = class RichTextEditor extends Component {
    ...
    keyDown(e) {
        ...
        if (this.editorMode === 'HTML' && ((e.which === 8 && e.code === 'Backspace') || (e.which === 46 && e.code === 'Delete'))) {
            let range = this.getRange();
            let startNode = range.startContainer.nodeName === '#text' ? range.startContainer.parentElement :
                range.startContainer;
            if (closest(startNode, 'pre') &&
                ...
                let regEx = new RegExp(String.fromCharCode(8203), 'g');
                let pointer = e.which === 8 ? range.startOffset - 1 : range.startOffset;
    ...
    setContentHeight(target, isExpand) {
        ...
        if (this.toolbarSettings.enableFloating && this.getToolbar() && !this.inlineMode.enable) {
            let tbWrapHeight = (isExpand ? (tbHeight + expandPopHeight) : tbHeight) + 'px';
    ...
    restrict(e) {
        if (this.maxLength >= 0) {
            let element = this.editorMode === 'Markdown' ? this.contentModule.getText() :
