                ...
                if (typeof data[key] === 'boolean') {
                    ...
                    switch (key) {
                        ...
                        case 'fontname':
                            ...
                            let name = (isNullOrUndefined(result) ? 'Segoe UI' : result);
                ...
                switch (item) {
                    case 'formats':
                        ...
                        this.formatDropDown = this.toolbarRenderer.renderDropDownButton({
                            ...
                            content: this.dropdownContent(this.parent.format.width, type, ((type === 'quick') ? '' : getDropDownValue(formatItem, this.parent.format.default, 'text', 'text'))),
                    ...
                    case 'fontname':
                        ...
                        this.fontNameDropDown = this.toolbarRenderer.renderDropDownButton({
                            ...
                            content: this.dropdownContent(this.parent.fontFamily.width, type, ((type === 'quick') ? '' : getDropDownValue(fontItem, this.parent.fontFamily.default, 'text', 'text'))),
...
class Count {
    ...
    appendCount() {
        let htmlText = this.editPanel.textContent.trim();
 ...
 */
class InsertHtml {
    static Insert(docElement, insertNode) {
        let node;
        ...
        let closestPrantNode = (node.nodeName.toLowerCase() === 'table') ? this.closestEle(nodes[0].parentNode, 'p') : nodes[0];
        if (!isCollapsed || (node.nodeName.toLowerCase() === 'table' && closestPrantNode)) {
            let preNode = nodeCutter.GetSpliceNode(range, closestPrantNode);
    ...
    static closestEle(element, selector) {
        let el = element;
    ...
    static searchEle(element, selector) {
...
class TableCommand {
    ...
    insertRow(e) {
        ...
        let newRow = closest(selectedCell, 'tr').cloneNode(true);
...
class Link {
    ...
    insertlink(e) {
        ...
        let target = (this.selfLink.checkBoxObj.checked) ? '_blank' : '';
        if (linkUrl === '') {
...
class Image {
    ...
    imagDialog(e) {
        ...
        let selection = e.selection;
        let selectObj = { selfImage: this, selection: e.selection, args: e.args };
        this.dialogObj = new Dialog({
            header: imgHeader,
            cssClass: CLS_RTE_ELEMENTS,
            enableRtl: this.parent.enableRtl,
...
class Table {
    ...
    createDialog(args) {
        ...
        let tableDialog = this.parent.createElement('div', { className: 'e-rte-edit-table', id: this.rteID + '_image' });
