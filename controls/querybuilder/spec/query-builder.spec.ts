...
describe('QueryBuilder', () => {
    ...
    describe('Scroll Event Handler - Tooltip Visibility', () => {
        beforeEach((): void => {
            document.body.appendChild(createElement('div', { id: 'querybuilder' }));
        });
        afterEach(() => {
        ...
        it('should display validation tooltip when operator popup scrolls inside the popup bounds', (done) => {
            queryBuilder = new QueryBuilder({
                columns: [
                    { field: 'FirstName', label: 'First Name', type: 'string' }
                ],
            ...
            setTimeout(() => {
                // Get the operator dropdown element
                const operatorDropdown = document.querySelector('.e-rule-operator .e-control') as any;
                if (operatorDropdown && operatorDropdown.ej2_instances) {
                    const ddlObj = operatorDropdown.ej2_instances[0];
                    // Open the popup
                    ddlObj.showPopup();

                    setTimeout(() => {
                        // Get popup element
                        const popupElement = document.querySelector('.e-ddl.e-popup');
                        if (popupElement) {
                            // Trigger scroll event inside popup
                            const scrollEvent = new Event('scroll', { bubbles: true });
                            popupElement.dispatchEvent(scrollEvent);

                            // Check if tooltip is visible
                            const tooltip = document.querySelector('.e-tooltip-wrap');
                            expect(tooltip).toBeDefined();
                            expect(popupElement.classList.contains('e-popup')).toBeTruthy();
                        }
                        done();
                    }, 100);
                } else {
        ...
        it('should hide validation tooltip when scrolling outside operator popup bounds', (done) => {
            queryBuilder = new QueryBuilder({
                columns: [
                    { field: 'FirstName', label: 'First Name', type: 'string' }
                ],
            ...
            setTimeout(() => {
                ...
                if (operatorDropdown && operatorDropdown.ej2_instances) {
                    ...
                    setTimeout(() => {
                        // Trigger scroll event outside popup (on body)
                        const scrollEvent = new Event('scroll', { bubbles: true });
                        document.body.dispatchEvent(scrollEvent);

                        // Check if tooltip is hidden
                        const tooltip = document.querySelector('.e-tooltip-wrap');
                        // Tooltip should either not exist or be hidden
                        if (tooltip) {
                            expect(tooltip.classList.contains('e-tooltip-wrap')).toBeTruthy();
                        }
                        done();
                    }, 100);
                } else {
        ...
        it('should maintain tooltip visibility during internal popup scroll', (done) => {
            queryBuilder = new QueryBuilder({
                columns: [
                    { field: 'FirstName', label: 'First Name', type: 'string' },
                    { field: 'Age', label: 'Age', type: 'number' }
            ...
            setTimeout(() => {
                ...
                if (operatorDropdown && operatorDropdown.ej2_instances) {
                    ...
                    setTimeout(() => {
                        const popupElement = document.querySelector('.e-ddl.e-popup');
                        if (popupElement) {
                            // Simulate scrolling with multiple events
                            for (let i = 0; i < 3; i++) {
        ...
        it('should handle validation tooltip with "in" operator on string field', (done) => {
            queryBuilder = new QueryBuilder({
                columns: [
                    { field: 'FirstName', label: 'First Name', type: 'string', validation: { isRequired: true } }
                ]
            }, '#querybuilder');

            setTimeout(() => {
                // Add rule with "in" operator
                const rule: RuleModel = {
                    condition: 'and',
                    rules: [{
                        field: 'FirstName',
                ...
                setTimeout(() => {
                    ...
                    if (operatorDropdown && operatorDropdown.ej2_instances) {
                        ...
                        setTimeout(() => {
                            ...
                            // Trigger scroll to validate tooltip behavior
                            if (popupElement) {
                                const scrollEvent = new Event('scroll', { bubbles: true });
                                popupElement.dispatchEvent(scrollEvent);
                            }
        ...
        it('should show tooltip when scrolling within operator popup and hide when scrolling outside', (done) => {
            queryBuilder = new QueryBuilder({
                columns: [
                    { field: 'FirstName', label: 'First Name', type: 'string' }
                ],
            ...
            setTimeout(() => {
                ...
                if (operatorDropdown && operatorDropdown.ej2_instances) {
                    ...
                    setTimeout(() => {
                        ...
                        // Step 1: Scroll inside popup - tooltip should be visible
                        if (popupElement) {
                            const insideScrollEvent = new Event('scroll', { bubbles: false });
                            popupElement.dispatchEvent(insideScrollEvent);
                            
                            // Step 2: Simulate scroll outside popup
                            const outsideScrollEvent = new Event('scroll', { bubbles: true });
                            document.body.dispatchEvent(outsideScrollEvent);
                            
                            // Verify popup is still tracked
                            expect(popupElement.classList.contains('e-popup')).toBeTruthy();
                        }
                        done();
                    }, 100);
        ...
        it('should validate tooltip behavior with multiple operator popups', (done) => {
            queryBuilder = new QueryBuilder({
                columns: [
                    { field: 'FirstName', label: 'First Name', type: 'string' },
                    { field: 'Age', label: 'Age', type: 'number' }
            ...
            setTimeout(() => {
                // Get first operator dropdown
                const operatorDropdowns = document.querySelectorAll('.e-rule-operator .e-control') as any;
                if (operatorDropdowns && operatorDropdowns.length >= 1) {
                    const firstDropdown = operatorDropdowns[0].ej2_instances ? operatorDropdowns[0].ej2_instances[0] : null;
                    
                    if (firstDropdown) {
                        firstDropdown.showPopup();

                        setTimeout(() => {
                            const firstPopup = document.querySelector('.e-ddl.e-popup');
                            if (firstPopup) {
                                // Trigger scroll on first popup
                                const scrollEvent1 = new Event('scroll', { bubbles: false });
                                firstPopup.dispatchEvent(scrollEvent1);
                            }

                            // Open second operator dropdown
                            if (operatorDropdowns.length >= 2 && operatorDropdowns[1].ej2_instances) {
                                const secondDropdown = operatorDropdowns[1].ej2_instances[0];
                                secondDropdown.showPopup();

                                setTimeout(() => {
                                    const secondPopup = document.querySelector('.e-ddl.e-popup');
                                    if (secondPopup) {
                                        const scrollEvent2 = new Event('scroll', { bubbles: false });
                                        secondPopup.dispatchEvent(scrollEvent2);
                                    }
                                    done();
                                }, 100);
                            } else {
        ...
        it('should persist tooltip across popup scroll with validation error state', (done) => {
            queryBuilder = new QueryBuilder({
                columns: [
                    { field: 'FirstName', label: 'First Name', type: 'string', validation: { isRequired: true, min: 3 } }
                ],
                rule: {
                    condition: 'and',
                    rules: [{
            ...
            setTimeout(() => {
                ...
                if (operatorDropdown && operatorDropdown.ej2_instances) {
                    ...
                    setTimeout(() => {
                        ...
                        // Multiple scroll events to simulate continuous scrolling
                        if (popupElement) {
                            for (let i = 0; i < 5; i++) {
                                const scrollEvent = new Event('scroll', { bubbles: false });
                                popupElement.dispatchEvent(scrollEvent);
                        ...
                        // Verify that the component is still in valid state
        ...
        it('should properly cleanup tooltip references after popup close', (done) => {
            queryBuilder = new QueryBuilder({
                columns: [
                    { field: 'FirstName', label: 'First Name', type: 'string' }
                ],
                    ...
                    setTimeout(() => {
                        ...
                        if (popupElement) {
                            ...
                            setTimeout(() => {
                                ...
                                const closedPopup = document.querySelector('.e-ddl.e-popup.e-popup-close');
                                expect(closedPopup === null || closedPopup === undefined).toBeTruthy();
                                done();
                            }, 100);
                        } else {
                            done();
