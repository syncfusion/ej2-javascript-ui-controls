window.sf = window.sf || {};
window.sf.querybuilder = (function (exports) {
'use strict';
...
var Columns = /** @class */ (function (_super) {
    ...
    __decorate([
        sf.base.Property({ isRequired: true, min: 0, max: Number.MAX_VALUE })
...
var QueryBuilder = /** @class */ (function (_super) {
    ...
    QueryBuilder.prototype.clickEventHandler = function (event) {
        ...
        if (target.className.indexOf('e-collapse-rule') > -1) {
            var animation = new sf.base.Animation({ duration: 1000, delay: 0 });
    ...
    QueryBuilder.prototype.addRuleSuccessCallBack = function (args, target, rule) {
        if (!args.cancel) {
            ...
            if (this.element.className.indexOf('e-device') > -1 || this.displayMode === 'Vertical') {
                ...
                var tooltip = new sf.popups.Tooltip({ content: this.l10n.getConstant('DeleteRule') });
    ...
    QueryBuilder.prototype.renderToolTip = function (element) {
        var tooltip = new sf.popups.Tooltip({ content: this.l10n.getConstant('ValidationMessage'),
        ...
        if (this.allowValidation) {
            ...
            for (i = 0, len = ruleElemCln.length; i < len; i++) {
                ...
                if (validateRule && validateRule.isRequired) {
                    ...
                    if (sf.base.isNullOrUndefined(rule.rules[index].value) || rule.rules[index].value === ''
    ...
    QueryBuilder.prototype.addGroupSuccess = function (args, isGroup, eventTarget, condition, isBtnClick) {
        if (!args.cancel && (this.element.querySelectorAll('.e-group-container').length <= this.maxGroupCount)) {
            ...
            if (isGroup) {
                ...
                var button = new sf.buttons.Button({ iconCss: 'e-icons e-delete-icon', cssClass: 'e-small e-round' });
                button.appendTo(dltGroupBtn);
                var tooltip = new sf.popups.Tooltip({ content: this.l10n.getConstant('DeleteGroup') });
                tooltip.appendTo(dltGroupBtn);
                sf.base.rippleEffect(dltGroupBtn, { selector: '.deletegroup' });
    ...
    QueryBuilder.prototype.changeValue = function (i, args) {
        ...
        if (element.className.indexOf('e-radio') > -1) {
            ...
            dropDownObj =
                sf.base.getComponent(sf.base.closest(element, '.e-rule-container').querySelector('.e-filter-input'), 'dropdownlist');
            this.selectedColumn = dropDownObj.getDataByValue(dropDownObj.value);
    ...
    QueryBuilder.prototype.destroyControls = function (target) {
        ...
        for (var i = 0, len = inputElement.length; i < len; i++) {
            if (inputElement[i].classList.contains('e-textbox')) {
                ...
                sf.base.detach(target.nextElementSibling.querySelector('input#' + inputElement[i].id));
            }
    ...
    QueryBuilder.prototype.createSpinner = function (element) {
        ...
        sf.popups.createSpinner({ target: spinnerElem, width: sf.base.Browser.isDevice ? '16px' : '14px' });
    };
                        ...
                        case 'date':
                            {
                                ...
                                if (format) {
                                    ...
                                    if (formatObj.skeleton) {
                                        datepick = new sf.calendars.DatePicker({ locale: this.getLocale(), value: selectedValue,
                                            placeholder: place, format: formatObj, change: this.changeValue.bind(this, i) });
                                    }
                                    else {
                                        datepick = new sf.calendars.DatePicker({ value: selectedValue, locale: this.getLocale(),
                                            placeholder: place, format: formatObj.format, change: this.changeValue.bind(this, i) });
                                    }
                                }
                                else {
                                    datepick = new sf.calendars.DatePicker({ locale: this.getLocale(),
    ...
    QueryBuilder.prototype.initWrapper = function () {
        ...
        if (this.summaryView) {
            ...
            for (var i = 0; i < buttons.length; i++) {
                ...
                sf.base.rippleEffect(button, { selector: '.e-btn' });
            }
        }
    };
    ...
    QueryBuilder.prototype.getLocale = function () {
        ...
        var cultureObj = sf.base.getValue('main.' + '' + this.locale + gregorianFormat, sf.base.cldrData);
        if (!cultureObj) {
            localeString = 'en';
        }
        return localeString;
    ...
    __decorate([
        sf.base.Property({ ruleDelete: true, groupInsert: true, groupDelete: true })
    ...
    __decorate([
        sf.base.Complex({ condition: 'and', rules: [] }, Rule)
...
sfBlazor.loadDependencies(sfBlazor.dependencyJson.querybuilder, () => {
    sf.querybuilder = sf.querybuilder({});
});
