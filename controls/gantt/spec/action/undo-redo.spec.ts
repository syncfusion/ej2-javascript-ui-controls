...
describe('Coverage for undo redo', () => {
    Gantt.Inject(Sort,UndoRedo,Edit,Toolbar, Selection );
    let ganttObj: Gantt;
    beforeAll((done: Function) => {
        ganttObj = createGantt(
    ...
    it('preparePreviousUndoActions', () => {
        const updateAction: any = {
            action: 'update',
            modifiedRecords: [
            ]
        }
        updateAction.modifiedRecords.push(ganttObj.flatData[1]);
        const previousActions = {};
        ganttObj.undoRedoModule['preparePreviousUndoActions'](updateAction, previousActions);
        expect(ganttObj.flatData.length).toBe(5);
    });
    it('preparePreviousRedoActions', () => {
        const updateAction: any = {
            action: 'update',
            modifiedRecords: [
            ]
        ...
        updateAction.modifiedRecords.push(ganttObj.flatData[1]);
        const previousActions = {};
        ganttObj.undoRedoModule['preparePreviousRedoActions'](updateAction, previousActions);
        expect(ganttObj.flatData.length).toBe(5);
    });
    it('updateModifiedAndConnectedRecords', () => {
        const updateAction = {
            action: 'update',
            modifiedRecords: [ganttObj.flatData[1]],
            connectedRecords: [ganttObj.flatData[2], ganttObj.flatData[3]]
        };
        ganttObj.undoRedoModule['getUndoCollection'] = [
            {
                modifiedRecords: [ganttObj.flatData[0]]
            }
        ];
        ganttObj.undoRedoModule['updateModifiedAndConnectedRecords'](updateAction);
        expect(ganttObj.flatData.length).toBe(5);
    });
});