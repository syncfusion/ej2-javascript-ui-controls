...
describe('CR-924980:Work calculation not working properly while changing endDates in grid columns', () => {
    let ganttObj: Gantt;
    let modeData: object[] = [
        {
            TaskID: 1,
            TaskName: 'Identify site location ZZZ',
            StartDate: new Date('03/29/2019'),
            Duration: 1,
            Progress: 30,
            Work: 8,
    ...
    it('Checking the work value, after enddate celledit action', () => {
        ...
        expect(ganttObj.currentViewData[0].ganttProperties.work).toBe(40);
...
describe('CR-924999:Work field column value is inconsistent during cell editing', () => {
    let ganttObj: Gantt;
    let data: object[] = [
        {
            TaskID: 1,
    ...
    beforeAll((done: Function) => {
        ganttObj = createGantt(
            ...
            { field: 'resources', headerText: 'Resources', width: '160' },
            { field: 'Work', width: '110' },
            { field: 'Progress', width: '110' },
            { field: 'Duration', width: '100' },
            { field: 'taskType', headerText: 'Task Type', width: '110' },
            ],
        }, done);
    });
    it('Checking the work value, after cell editing to leave blank value', () => {
        expect(ganttObj.currentViewData[1].ganttProperties.work).toBe(12);
        let work: HTMLElement = ganttObj.element.querySelector('#treeGrid' + ganttObj.element.id + '_gridcontrol_content_table > tbody > tr:nth-child(2) > td:nth-child(6)') as HTMLElement;
        triggerMouseEvent(work, 'dblclick');
        let input: any = (document.querySelector('#treeGrid' + ganttObj.element.id + '_gridcontrolWork') as any).ej2_instances[0];
        input.value = '';
        input.dataBind();
        let element: HTMLElement = ganttObj.element.querySelector('#treeGrid' + ganttObj.element.id + '_gridcontrol_content_table > tbody > tr:nth-child(3) > td:nth-child(2)') as HTMLElement;
        triggerMouseEvent(element, 'click');
