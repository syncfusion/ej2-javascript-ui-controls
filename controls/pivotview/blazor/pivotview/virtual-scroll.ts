...
export class VirtualScroll {
        ...
        if (this.parent.pageSettings) {
            if (this.direction === 'vertical') {
                ...
                if (this.parent.options.dataType === 'pivot') {
                    ...
                    let pos: number = exactSize * exactpage - (this.parent.scrollPageInfo.rowFirstLvl * rowValues * this.parent.gridSettings.rowHeight);
                    this.parent.scrollPosObject.verticalSection = pos;
                }
            }
            else {
                let colValues: number = this.parent.dataSourceSettings.valueAxis === 'column' ? this.parent.dataSourceSettings.values.length : 1;
                ...
                if (this.parent.options.dataType === 'pivot') {
                    ...
                    let exactPage: number = Math.ceil(colStartPos / (this.parent.pageSettings.columnSize * colValues));
                    let pos: number = exactSize * exactPage - (this.parent.scrollPageInfo.colFirstLvl * colValues * this.parent.gridSettings.columnWidth);
    ...
    private onHorizondalScroll(mHdr: HTMLElement, mCont: HTMLElement, fCont: HTMLElement): Function {
        ...
        return (e: Event) => {
            ...
            if (this.parent.scrollPosObject.horizontalSection > left ? true : (excessMove > 1 && notLastPage)) {
                ...
                let vWidth: number = (this.parent.gridSettings.columnWidth * this.parent.scrollPageInfo.columnCount
                    - ((this.parent.internalGrid.columnModel[0] as Column).width as number));
    ...
    private onVerticalScroll(fCont: HTMLElement, mCont: HTMLElement): Function {
        ...
        return (e: Event) => {
            ...
            if (mCont.scrollTop < this.parent.scrollerBrowserLimit) {
                setStyleAttribute(fCont.querySelector('.' + cls.TABLE) as HTMLElement, {
            ...
            let excessMove: number = this.parent.scrollPosObject.verticalSection > top ?
                -(this.parent.scrollPosObject.verticalSection - top) : ((top + fCont.clientHeight) -
                    (this.parent.scrollPosObject.verticalSection + (fCont.querySelector('.' + cls.TABLE) as HTMLElement).offsetHeight));
            ...
            if (this.parent.scrollPosObject.verticalSection > top ? true : (excessMove > 1 && notLastPage)) {
                ...
                let movableTable: HTMLElement =
                    this.parent.element.querySelector('.' + cls.MOVABLECONTENT_DIV).querySelector('.' + cls.TABLE) as HTMLElement;
                vertiOffset = -((top - (this.parent.scrollPosObject.verticalSection + excessMove) - mCont.scrollTop));
                let vHeight: number = (this.parent.gridSettings.rowHeight * this.parent.scrollPageInfo.rowCount + 0.1
    ...
    public updateScrollInfo(element: HTMLElement, columnCount: number, rowCount: number): void {
        if (this.parent.options.enableVirtualization) {
            let virtualHeaderDiv: HTMLElement;
            let virtualDiv: HTMLElement
            if (element.querySelector('.' + cls.MOVABLECONTENT_DIV) &&
                ...
                virtualDiv = createElement('div', { className: cls.VIRTUALTRACK_DIV }) as HTMLElement;
            ...
            if (element.querySelector('.' + cls.MOVABLEHEADER_DIV) &&
                ...
                virtualHeaderDiv = createElement('div', { className: cls.VIRTUALTRACK_DIV }) as HTMLElement;
                element.querySelector('.' + cls.MOVABLEHEADER_DIV).appendChild(virtualHeaderDiv);
            } else {
                virtualHeaderDiv = element.querySelector('.' + cls.MOVABLEHEADER_DIV).querySelector('.' + cls.VIRTUALTRACK_DIV) as HTMLElement;
            }
            let movableTable: HTMLElement =
                element.querySelector('.' + cls.MOVABLECONTENT_DIV).querySelector('.' + cls.TABLE) as HTMLElement;
            let vHeight: number = (this.parent.gridSettings.rowHeight * rowCount + 0.1 - movableTable.clientHeight);
            if (vHeight > this.parent.scrollerBrowserLimit) {
                this.parent.verticalScrollScale = vHeight / this.parent.scrollerBrowserLimit;
                vHeight = this.parent.scrollerBrowserLimit;
            }
            let vWidth: number = (this.parent.gridSettings.columnWidth * columnCount
                - ((this.parent.internalGrid.columnModel[0] as Column).width as number));
            if (vWidth > this.parent.scrollerBrowserLimit) {
                this.parent.horizontalScrollScale = vWidth / this.parent.scrollerBrowserLimit;
                vWidth = this.parent.scrollerBrowserLimit;
            ...
            let mCnt: HTMLElement = element.querySelector('.' + cls.MOVABLECONTENT_DIV) as HTMLElement;
            let fCnt: HTMLElement = element.querySelector('.' + cls.FROZENCONTENT_DIV) as HTMLElement;
            let mHdr: HTMLElement = element.querySelector('.' + cls.MOVABLEHEADER_DIV) as HTMLElement;
            let verOffset: string = (mCnt.scrollTop > this.parent.scrollerBrowserLimit) ?
            ...
            let horiOffset: string = (mCnt.scrollLeft > this.parent.scrollerBrowserLimit) ?
            ...
            setStyleAttribute(fCnt.querySelector('.' + cls.TABLE) as HTMLElement, {
