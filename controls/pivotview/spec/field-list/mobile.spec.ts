...
describe('Field List rendering on mobile device', () => {
    ...
    describe('922652: Need to restrict the "Add to Value" text from the values menu panel button in mobile mode', () => {
        let pivotGridObj: PivotView;
        let elem: HTMLElement = createElement('div', { id: 'PivotGrid', styles: 'height:200px; width:500px' });
        afterAll(() => {
            if (pivotGridObj) {
        ...
        it('context menu', (done: Function) => {
            setTimeout(() => {
                document.querySelectorAll('.e-pivot-button.Measures')[0].dispatchEvent(mousedown);
                expect(document.querySelectorAll('.e-pivot-button').length === 7).toBeTruthy();
                done();
            }, 1000);
        });
        it('context menu select', (done: Function) => {
            setTimeout(() => {
                document.querySelectorAll('.e-pivot-button.Measures')[0].dispatchEvent(mouseup);
                expect(document.querySelectorAll('#PivotGrid_PivotContextMenu').length > 0).toBeTruthy();
                done();
            }, 1000);
        });
