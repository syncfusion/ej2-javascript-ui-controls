...
export class Grouping implements IAction {
    ...
    private validateSettings(
        ...
        if (groups[groupOrders[2]] && groupType === 'custom') {
            ...
            for (const field of Object.keys(customFields)) {
                ...
                orderedGroups = this.reOrderSettings(customFields[field as string], fields, orderedGroups, field, fieldName);
            }
            ...
            if (groupType === 'date') {
                ...
                if (groups[groupOrders[0]].length === 0) {
                    delete this.parent.engineModule.groupingFieldsInfo[fieldName as string];
                    for (const key in this.parent.engineModule.groupingFieldsInfo) {
                        if (groupingTypes.some((dateTypes: string) => key === `${fieldName}${dateTypes}`)) {
                            delete this.parent.engineModule.groupingFieldsInfo[key as string];
                        }
                    }
                    for (const key in this.parent.engineModule.fieldList) {
                        if (groupingTypes.some((dateTypes: string) => key === `${fieldName}${dateTypes}`)) {
                            delete this.parent.engineModule.fieldList[key as string];
                        }
                    }
                    const updatedFields: string[] = this.parent.engineModule.fields.filter((item: string) =>
                        !groupingTypes.some((dateTypes: string) => item === `${fieldName}${dateTypes}`)
                    );
    ...
    private reOrderSettings(
        customGroups: IGroupSettings[], fields: string[], orderedSettings: IGroupSettings[], fieldName: string,
        groupedFieldName?: string): IGroupSettings[] {
        ...
        } else if (index > -1 && customGroups[index as number].customGroups && customGroups[index as number].customGroups.length === 0 &&
                  !isNullOrUndefined(groupedFieldName)) {
            delete this.parent.engineModule.fieldList[groupedFieldName as string];
            delete this.parent.engineModule.groupingFieldsInfo[groupedFieldName as string];
            const updatedFields: string[] = this.parent.engineModule.fields.filter((value: string) => value !== groupedFieldName);
            this.parent.engineModule.fields = updatedFields;
        }
