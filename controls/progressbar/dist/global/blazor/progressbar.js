...
var ProgressAnimation = /** @class */ (function () {
    ...
    ProgressAnimation.prototype.doCircularAnimation = function (x, y, radius, progressEnd, totalEnd, element, progress, thickness, delay, startValue, previousTotal, active) {
        ...
        progressEnd += (progress.cornerRadius === 'Round' && totalEnd !== completeAngle && totalEnd !== 0) ?
            ((progress.enableRtl) ? -(lineCapRadius / 2) * thickness : (lineCapRadius / 2) * thickness) : 0;
        startPos = (!sf.base.isNullOrUndefined(startValue)) ? startValue : start;
    ...
    ProgressAnimation.prototype.doLabelAnimation = function (labelPath, start, end, progress, delay, textSize) {
        var animation = new sf.base.Animation({});
        var label = new sf.base.Animation({});
        var startPos;
        var endPos;
    ...
    ProgressAnimation.prototype.doAnnotationAnimation = function (circularPath, progress, previousEnd, previousTotal) {
        ...
        startValue = (!sf.base.isNullOrUndefined(previousEnd)) ? previousEnd : start;
        endValue = (!sf.base.isNullOrUndefined(previousEnd)) ? totalEnd - previousTotal : totalEnd;
...
var Segment = /** @class */ (function () {
    ...
    Segment.prototype.createLinearRange = function (totalWidth, progress) {
        ...
        for (var i = 0; i < range.length; i++) {
            ...
            option = new sf.svgbase.PathOption(progress.element.id + '_LinearRange_' + i, 'none', thickness, color, opacity, '0', 'M' + ' ' + startX + ' ' + startY + ' ' + 'L' + endX + ' ' + startY);
    ...
    Segment.prototype.createCircularRange = function (centerX, centerY, radius, progress) {
        ...
        for (var i = 0; i < range.length; i++) {
            ...
            option = new sf.svgbase.PathOption(progress.element.id + '_CircularRange_' + i, 'none', thickness, color, opacity, '0', getPathArc(centerX, centerY, radius, startAngle, endAngle, progress.enableRtl));
            rangePath = progress.renderer.drawPath(option);
            rangeGroup.appendChild(rangePath);
            if (progress.isGradient) {
                if (range.length - 1 === i) {
...
var Linear = /** @class */ (function () {
    ...
    Linear.prototype.renderLinearProgress = function (refresh, previousWidth) {
        ...
        if (!refresh) {
            ...
            linearProgressGroup = sf.svgbase.getElement(progress.element.id + '_LinearProgressGroup');
        }
...
var Circular = /** @class */ (function () {
    ...
    Circular.prototype.renderCircularProgress = function (previousEnd, previousTotalEnd, refresh) {
        ...
        if (!refresh) {
            ...
            circularProgressGroup = sf.svgbase.getElement(progress.element.id + '_CircularProgressGroup');
        }
        ...
        option = new sf.svgbase.PathOption(progress.element.id + '_Circularprogress', fill, strokeWidth, stroke, progress.themeStyle.progressOpacity, '0', circularPath);
        progress.progressWidth = progress.renderer.drawPath(option).getTotalLength();
    ...
    Circular.prototype.renderActiveState = function (progressGroup, radius, strokeWidth, circularPath, endAngle, totalEnd, refresh) {
        ...
        if (!refresh) {
            ...
            circularActive = sf.svgbase.getElement(progress.element.id + '_CircularActiveProgress');
...
var ProgressBar = /** @class */ (function (_super) {
            ...
            switch (prop) {
                ...
                case 'value':
                    ...
                    if (this.type === 'Circular') {
                        ...
                        this.linear.renderLinearProgress(!sf.base.isNullOrUndefined(oldProp.value), this.previousWidth);
                    }
                    break;
                case 'animation':
                    this.createSVG();
    ...
    ProgressBar.prototype.show = function () {
        if (!sf.base.isNullOrUndefined(this.svgObject)) {
