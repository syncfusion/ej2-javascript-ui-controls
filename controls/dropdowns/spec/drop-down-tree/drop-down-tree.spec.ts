...
describe('Dropdown Tree with Filter', () => {
    ...
    afterEach((): void => {
        if (ddtreeObj)
            if (ddtreeObj.filterTimer) {
                clearTimeout(ddtreeObj.filterTimer);
                ddtreeObj.filterTimer = null;
            }
            ddtreeObj.destroy();
            ddtreeObj = undefined;
        ele.remove();
    ...
    it('checked nodes persist after filtering', (done) => {
        ddtreeObj = new DropDownTree({
            fields: { dataSource: listData, value: "id", text: "name", parentValue: "pid", hasChildren: "hasChild" },
            allowFiltering: true,
            showCheckBox: true
        ...
        ddtreeObj.treeObj.touchClickObj.tap(tapEvent);
        mouseEventArgs.target = li[9].querySelector('.e-list-text');
        ddtreeObj.treeObj.touchClickObj.tap(tapEvent);
        expect(ddtreeObj.treeObj.checkedNodes.length).toBe(3);
        expect(ddtreeObj.treeObj.checkedNodes).toContain('1');
        expect(ddtreeObj.treeObj.checkedNodes).toContain('7');
        expect(ddtreeObj.treeObj.checkedNodes).toContain('11');
        let filterEle = ddtreeObj.popupObj.element.querySelector('#' + ddtreeObj.element.id + "_filter");
        let filterObj = filterEle.ej2_instances[0];
        filterEle.value = 'Au';
        filterObj.value = 'Au';
        filterObj.input({ value: 'Au', container: filterEle });
        setTimeout(() => {
            expect(ddtreeObj.treeObj.checkedNodes.length).toBe(3);
            expect(ddtreeObj.treeObj.checkedNodes).toContain('1');
            expect(ddtreeObj.treeObj.checkedNodes).toContain('7');
