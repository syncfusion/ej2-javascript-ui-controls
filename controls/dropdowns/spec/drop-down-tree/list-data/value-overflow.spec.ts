...
describe('DropDown Tree control List datasource', () => {
    ...
    describe('valueOverFlow -', () => {
        ...
        it('For allowMultiSelection with selected field', () => {
            ddtreeObj = new DropDownTree({
                fields: { dataSource: listData, value: "id", text: "name", parentValue: "pid", selected: 'isSelected', hasChildren: "hasChild", expanded: 'expanded' },
                allowMultiSelection: true,
               text: 'Australia, China, France, India',
                width: '150px'
            }, '#ddtree');
        ...
        it('allowMultiSelection enabled dynamically', () => {
            ...
            mouseEventArgs.target = li[2].querySelector('.e-list-text');
            (ddtreeObj as any).treeObj.touchClickObj.tap(tapEvent);
            let ochipElement = ddtreeObj.element.parentElement.firstElementChild.nextElementSibling;
            expect(ochipElement.classList.contains('e-chips-wrapper')).toBe(false);
            ddtreeObj.allowMultiSelection = true;
            ddtreeObj.dataBind();
            mouseEventArgs.ctrlKey = true;
            let nli: any = ddtreeObj.treeObj.element.querySelectorAll('li');
            mouseEventArgs.target = nli[0].querySelector('.e-list-text');
            (ddtreeObj as any).treeObj.touchClickObj.tap(tapEvent);
