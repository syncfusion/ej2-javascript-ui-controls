...
export function Search(
    ...
    if (inputVal && inputVal.length && items) {
        ...
        for (let i: number = 0, itemsData: Element[] = listItems; i < itemsData.length; i++) {
            ...
            let text: string = dataSource && filterValue ? (ignoreCase ? filterValue.toString().toLocaleLowerCase() : filterValue).replace(/^\s+|\s+$/g, '') : (ignoreCase ? item.textContent.toLocaleLowerCase() : item.textContent).replace(/^\s+|\s+$/g, '');
