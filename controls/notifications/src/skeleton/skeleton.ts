...
export class Skeleton extends Component<HTMLElement> implements INotifyPropertyChanged {
    ...
    public destroy(): void {
        super.destroy();
        let attrs: string[] = ['role', 'aria-live', 'aria-busy', 'aria-label'];
        let cssClass: string[] = [];
    ...
    private updateShape(): void {
        let shapeCss: string = cssClassName[this.shape.toUpperCase() + 'SHAPE'];
        let removeCss: string[] = this.element.classList.value.match(/(e-skeleton-[^\s]+)/g);
        this.updateDimension();
        if (removeCss)
    ...
    private updateDimension(): void {
        let width: string = (!this.width && (['Text', 'Rectangle'].indexOf(this.shape) > -1)) ? '100%' : formatUnit(this.width);
        let height: string = ['Circle', 'Square'].indexOf(this.shape) > -1 ? width : formatUnit(this.height);
    ...
    private updateEffect(): void {
        let removeCss: string[] = this.element.classList.value.match(/(e-shimmer-[^\s]+)/g);
        if (removeCss)
            removeClass([this.element], removeCss);
    ...
    private updateVisibility(): void {
        this.visible ? removeClass([this.element], [cssClassName.VISIBLENONE]) : addClass([this.element], [cssClassName.VISIBLENONE]);
    }
