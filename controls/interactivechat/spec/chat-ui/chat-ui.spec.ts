...
describe('ChatUI Component', () => {
    ...
    describe('Methods - ', () => {
        ...
        it('prependMessages method checking with string array as parameter', () => {
            chatUI = new ChatUI({
                user: { id: 'user1', user: 'John Doe' }
            });
            chatUI.appendTo('#chatUI');
            chatUI.prependMessages(['Hello, World!', 'Bye World!']);
            const messageElems = chatUIElem.querySelectorAll('.e-message-item');
            expect(messageElems).not.toBeNull(); 
            expect(messageElems.length).toBe(2);
            expect(messageElems[0].querySelector('.e-text').textContent).toBe('Hello, World!');
            expect(messageElems[1].querySelector('.e-text').textContent).toBe('Bye World!');
            expect(chatUI.messages[0].text).toBe('Hello, World!');
            expect(chatUI.messages[1].text).toBe('Bye World!');
            expect(messageElems[0].id).toBe('chatUI-message-1');
            expect(messageElems[1].id).toBe('chatUI-message-2');
        });
    
        it('prependMessages method checking with MessageModel array as parameter', () => {
            const messages: MessageModel[] = [
                {
                    id: 'msg1',
                    text: 'Test message 1',
            ...
            const messageElems = chatUIElem.querySelectorAll('.e-message-item');
            expect(messageElems).not.toBeNull();
            expect(messageElems[0].querySelector('.e-text').textContent).toBe('Test message 1');
            expect(chatUIElem.querySelector('.e-message-header').textContent).toBe('Jane Smith');
            expect(messageElems.length).toBe(2);
            expect(chatUI.messages.length).toBe(2);
            expect(chatUI.messages[0].text).toBe('Test message 1');
            chatUI.prependMessages([newMessage]);
            const newmessageElems = chatUIElem.querySelectorAll('.e-message-item');
            expect(newmessageElems).not.toBeNull();
            expect(newmessageElems[0].querySelector('.e-text').textContent).toBe('Test message 3');
            expect(chatUIElem.querySelector('.e-message-header').textContent).toBe('Jane Smith');
            expect(newmessageElems.length).toBe(3);
        ...
        it('prependMessages method checking with MessageModel array as parameter without any property', () => {
            let currentUserModel: UserModel = {
                id: "user1",
                user: "Albert"
            };
    ...
    describe('Attachment Support', () => {
        ...
        it('should render a document attachment using prependMessages method', function () {
            chatUI = new ChatUI({
                user: { id: 'user1', user: 'Albert' },
                enableAttachments: true,
                attachmentSettings: {
            ...
            const newMessages: MessageModel[] = [
                ...
                {
                    id: 'msg2',
                    text: 'Second attachment message',
                    author: { id: 'user3', user: 'FileUser', statusIconCss: 'e-icons e-user-away' },
                    timeStamp: new Date(),
                    attachedFile: {
                        id: 'file1',
            ...
            chatUI.prependMessages(newMessages);
            const messageElems = chatUIElem.querySelectorAll('.e-message-item');
            expect(messageElems).not.toBeNull();
            expect(messageElems.length).toBe(2);
            const fileName = messageElems[0].querySelector('.e-chat-file-name') as HTMLElement;
            expect(fileName.textContent).toBe('report.pdf');
            expect(chatUI.messages[0].id).toBe('msg1');
            expect(chatUI.messages[1].id).toBe('msg2');
             expect(messageElems[0].id).toBe('msg1');
            expect(messageElems[1].id).toBe('msg2');
        });

        it('should render an image attachment using prependMessages method', function () {
            chatUI = new ChatUI({
                user: { id: 'user1', user: 'Albert' },
                enableAttachments: true,
                attachmentSettings: {
        ...
        it('should render a video attachment using prependMessages method', function () {
            chatUI = new ChatUI({
                user: { id: 'user1', user: 'Albert' },
                enableAttachments: true,
                attachmentSettings: {
